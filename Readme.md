...
PhiPsi is a computational solid mechanics program, which involves the eXtended Finite Element Method (XFEM) as well as the Finite Element Method (FEM). The program is written in Fortran. The program is named PhiPsi because $\phi$ and $\psi$ are the basic variables for the level set method which is widely used in the XFEM. On the other hand, $\phi$ and $\psi$ denote the internal friction angle and the dilation angle in plasticity theory.

PhiPsi was initially developed to solve problems involving defects such as cracks, voids, and inclusions. PhiPsi includes the following functions:
...
* Supported analysis types: 2D and 3D static analysis, 2D and 3D hydraulic fracturing analysis.
* Supports up to 1000 fractures, voids, and inclusions.
* Random generation of initial fractures, voids, and inclusions.
* Handles 2D/3D fracture intersections, as well as fracture-void and fracture-inclusion intersections (2D).
* Employs the penalty function method to determine contact status.
* Uses an optimized Newton-Raphson scheme to solve nonlinear systems.
* Utilizes sparse matrix storage for the global stiffness matrix K.
* Supports coupling of DOFs (degrees of freedom).
...
cd $phipsi_root
# Step 2: Modify the keyword "Work_Directory" in the keywords file 2d_fem.kpp.
# 
...
# Step 1: change directory to PhiPsi.
cd $phipsi_root
# Step 2: Modify the keyword "Work_Directory" in the keywords file 2d_xfem.kpp.
# 
# Step 3: Perform the simulation.
...
# Step 1: change directory to PhiPsi.
cd $phipsi_root
# Step 2: Modify the keyword "Work_Directory" in the keywords file 2d_xfem_hydraulic_fracturing.kpp.
# 
# Step 3: Perform the simulation.
...
# Step 1: change directory to PhiPsi.
cd $phipsi_root
# Step 2: Modify the keyword "Work_Directory" in the keywords file 3d_xfem_block_tension.kpp.
# 
# Step 3: Perform the simulation.
...
# Step 1: change directory to PhiPsi.
cd $phipsi_root
# Step 2: Modify the keyword "Work_Directory" in the keywords file 3d_xfem_hydraulic_fracturing.kpp.
# 
# Step 3: Perform the simulation.
...
# 
# Step 3: Perform the simulation.
./phipsi -i examples/3d_xfem_hydraulic_fracturing.kpp -n 4
~~~
...
### 2.3.8 Run PhiPsi examples

#### 2.3.8.1 Example 1: 2d_fem.kpp
~~~bash
...
./phipsi.exe -i "X:\PhiPsi_Project\GitHub_Prep\examples\2d_fem_MSYS2.kpp" -n 2
~~~

#### 2.3.8.2 Example 2: 2d_xfem.kpp
~~~bash
# Step 1: change directory to PhiPsi.
cd $phipsi_root
# Step 2: Modify the keyword "Work_Directory" in the keywords file 2d_xfem.kpp.
# 
# Step 3: Perform the simulation.
./phipsi.exe -i "X:\PhiPsi_Project\GitHub_Prep\examples\2d_xfem_MSYS2.kpp" -n 2
~~~
...
The input files of PhiPsi include a keywords file and other data files containing the node coordinates ( *.node), element-node information ( *.elem), boundary conditions ( *.boux and *.bouy), and external forces ( *.focx and *.focy). The keywords file ( *.kpp) defines information such as the work directory, analysis type, and coordinates of initial cracks. The data files can be generated automatically by running the macro file called Ansys2PhiPsi_2D.mac for 2D problems (or Ansys2PhiPsi_3D.mac for 3D problems) once the model is created in ANSYS. Alternatively, you can define the data files by using other software such as Abaqus (A useful tool to convert Abaqus inp model file to PhiPsi input files can be found at http://www.phipsi.top/source/Abaqus2PhiPsi_Matlab.rar) or just generate it manually.
    
Once PhiPsi starts, it will check all the input files first. After the analysis, the output files will be saved to the working directory. A MATLAB-based program (all the source codes are available for download, see "The source codes of the Post-Processor written in MATLAB" on page http://phipsi.top/downloads.html) is offered for post-processing. A tutorial (a pdf file) is available on the downloads page (see "The tutorial" on page http://phipsi.top/downloads.html).

This document describes only OPTION 1.

### 3.1.2 OPTION 2 -- Use PPView (recommended)
Alternatively, you can also perform an analysis using PPView (A Visualization Tool for PhiPsi). It can be used to import Abaqus inp file, view the model defined in the PhiPsi keywords file (*.kpp), edit PhiPsi keywords file, perform a PhiPsi simulation, and view the simulation result files generated by PhiPsi. The PPView Help Manual can be found at [here (PPView Help Manual)](http://www.phipsi.top/PPView_Help.html).
You can download PPView for Windows, macOS, or Linux from [here](https://sourceforge.net/projects/phipsi/).
	  

## 3.2 How to run PhiPsi with specified keywords file (*.kpp)?

Open terminal, use cd command to change directory to the folder which contains the compiled PhiPsi, then you can run PhiPsi by typing a command line as follows:
~~~bash
...
~~~
where -i specifies the file path of the keywords file (*.kpp), and -n specifies the number of threads for the OpenMP parallel simulation.

<a id="section-4"></a>
# 4. File formats
## 4.1 *.kpp file
Keywords file of PhiPsi, composed of keywords that begin with the character '*', see Section 5 for details on [PhiPsi Keywords manual](http://www.phipsi.top/phipsi_keywords_manual.html). The lines beginning with character "%" are comment lines. For example:
...
......
~~~                                
For 3D problems, this file defines the 8 nodes of each hexahedral element, and the format is ($node_1$, $node_2$, $node_3$, $node_4$, $node_5$, $node_6$, $node_7$, $node_8$, Material_Number). For example:
...
These three files contain information about boundary conditions and define nodes whose degrees of freedom are constrained in $x$, $y$, and $z$ directions, respectively. For example, the following lines in the *.bouy file indicate that $y$ degrees of freedom of nodes 50, 92, 97, 98, 99 are constrained:
~~~bash
	50
	92
	97
...
These three files contain information about external forces and define nodes with force applied in $x$, $y$, and $z$ directions, respectively. For example, the following lines in the *.focx file indicate that nodes 487 to 489 are under the action of force at 4.5 KN in the $x$ direction:
~~~bash
	487   0.45000000E+04
	488   0.45000000E+04
	489   0.45000000E+04
...
These three files contain information about non-zero boundary conditions and define nodes whose degrees of freedom are constrained at a non-zero value in $x$, $y$, and $z$ directions, respectively. For example, the following lines in the *.buxn file indicate that degrees of freedom in the $x$ direction of nodes 1, 2, 3 are constrained at 0.01:
~~~bash
  1    0.10000000E-01
  2    0.10000000E-01
  3    0.10000000E-01
...
### 4.2.7 *.fbqn
This file is necessary for field problems and defines flux of specified nodes. This file should be presented in the format (node_number, flux), for example
~~~bash
1    1.00000000E+00
2    1.00000000E+00
3    1.00000000E+00
...
## 4.3 Output files
The following output files are generated by PhiPsi after performing the analysis, and are stored in the folder defined by the keyword *Work_Directory.
...
~~~
### 4.3.2 *.disn_i
This file stores displacements of nodes generated at load step $i$. For 2D problems, the file format is (node_number, $u_x$, $u_y$), for example:
~~~bash
......
...
~~~	  
### 4.3.3 *.disg_i
This file stores displacements of Gauss points generated at load step $i$. For 2D problems, the file format is (Gauss_point_number, $u_x$, $u_y$), for example:
~~~bash
......
...
~~~
### 4.3.5 *.sifs_i
This file stores stress intensity factors of each tip of each crack at load step $i$. File format is ($K_I$ of Tip 1 of Crack $i$, $K_{II}$ of Tip 1 of Crack $i$, $K_I$ of Tip 2 of Crack $i$, $K_{II}$ of Tip 2 of Crack $i$). For example, the following lines present stress intensity factors of 4 cracks:
~~~bash
...
~~~		    
### 4.3.8 *.gcor_i
This file stores coordinates of Gauss points generated at load step $i$. File format for 2D problems is (Gauss_point_number, $x$, $y$), for example:
~~~bash
...
These files store $x$, $y$, and $z$ coordinates of points of cracks generated at load step $i$. File format is (coordinate of point 1 of crack $i$, coordinate of point 2 of crack $i$, coordinate of point 3 of crack $i$, ...). In the following cray_i example, line 3 indicates that crack 3 has 4 points, and their $y$ coordinates are 0.883063441879E-02, 0.915860471085E-02, 0.939531592630E-02, and 0.97E-02, respectively.
~~~bash
...
~~~		  
### 4.3.10 *.hftm
This file stores information about load step, substep, and step time, for example:
~~~bash
