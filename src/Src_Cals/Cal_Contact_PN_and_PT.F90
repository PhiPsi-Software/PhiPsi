!-----------------------------------------------------------
! Brief: Compute normal and tangential contact tractions at 2D Gauss points.
!
! Parameters:
...
!           Kt_Gauss          - tangential penalty stiffness at Gauss points
!           CT_State_Gauss    - contact state at each Gauss point
!           c_Elem_Conta_Sta  - element contact status flags
!           PC_Gauss_x/y      - accumulated contact forces at Gauss points
!
! Notes:   Transforms local contact forces to global coordinates, detects
!          contact/separation/slip, and updates penalty tractions.
!-----------------------------------------------------------

subroutine Cal_Contact_PN_and_PT(iter,ifra,Counter_Iter,i_NR_P, c_Total_Freedom,c_num_freeDOF, &
Kn,Kn_Gauss,Kt_Gauss,fric_mu,c_DISP, delta_u_a, CT_State_Gauss,c_Elem_Conta_Sta, PC_Gauss_x,PC_Gauss_y)
...
! it in CT_State_Gauss, and
! element contact status for post-processing, stored in c_Elem_Conta_Sta
...
integer,intent(inout):: CT_State_Gauss(num_Crack,Max_Num_Cr_CalP-1,2)
integer,intent(inout):: c_Elem_Conta_Sta(Num_Elem,num_Crack)
real(kind=FT),intent(inout):: PC_Gauss_x(num_Crack,Max_Num_Cr_CalP-1,2), PC_Gauss_y(num_Crack,Max_Num_Cr_CalP-1,2)
integer i_E,i_C
...
do i_C=1,num_Crack
    ...
    do i_CT_Elem = 1,Num_Div_Points - 1
        ...
        do i_CT_GAUSS  = 1,num_CT_Gauss
            ...
            call Cal_Point_Aperture_and_Tangent_disp(i_C, [CT_GAUSS_x,CT_GAUSS_y], c_DISP,ori_CT_elem,c_Aperture, c_Tangent_disp)
            ...
            if(Key_Propped_Width==1)then
                c_Yes_Proppant = 1
                tem_Aper_Tol = (Cracks_CalP_wpnp(i_C,i_CT_Elem) + Cracks_CalP_wpnp(i_C,i_CT_Elem+1))/TWO
                Aper_Tol = tem_Aper_Tol
                call Cal_C1_C2_for_Width_Propped_Frac(Aper_Tol, Prop_C1,Prop_C2)
            endif
#endif
#ifndef github
            if(Key_Analysis_Type==17 .and. Key_Proppant_Active==1)  then
                c_Yes_Proppant=1
                tem_Aper_Tol = (Cracks_CalP_wpnp(i_C,i_CT_Elem) + Cracks_CalP_wpnp(i_C,i_CT_Elem+1))/TWO
                Aper_Tol = tem_Aper_Tol
                call Cal_C1_C2_for_Width_Propped_Frac(Aper_Tol, Prop_C1,Prop_C2)
            ...
            if(c_Aperture <= Aper_Tol)then
                ...
                elseif(c_Yes_Proppant==1)then
                    ...
                    if(Aper_Tol > 0.01D-3) then
                        ...
                        call Cal_Closure_P_Given_C1_C2_Wp_Secant( Prop_C0,Prop_C1,Prop_C2,P_closure, Curve_Slope)

                        c_PN = -P_closure
                        c_PT = ZR

                        Cracks_CalP_wdeform(i_C,i_CT_Elem) = 1.04D0*tem_Aper_Tol* (P_closure**(TWO/THR))* &
                        ((ONE-Prop_Poss)/Prop_Elas)**(TWO/THR)

                        Cracks_CalP_wdeform(i_C,i_CT_Elem+1) = Cracks_CalP_wdeform(i_C,i_CT_Elem)
                        c_Kn  = Curve_Slope
                ...
                if(i_NR_P>=2)then
                    ...
                    if(old_Elem_Conta_Sta(CT_GAUSS_Elem,i_C)/=2) then
                        c_Elem_Conta_Sta(CT_GAUSS_Elem,i_C) = 1
                    else
                        c_Elem_Conta_Sta(CT_GAUSS_Elem,i_C) = 2
                    endif
                endif
                if((Key_Propped_Width==0) .or. (Key_Propped_Width==1 .and. Aper_Tol <= 0.01D-3)) then
                    if(abs(c_PT)-fric_mu*abs(c_PN) >ZR)then
                        call Cal_Point_Aperture_and_Tangent_disp(i_C, [CT_GAUSS_x,CT_GAUSS_y], delta_u_a,ori_CT_elem, &
                        delta_Aperture,delta_Tangent_disp)
                        c_PT = fric_mu*abs(c_PN)* delta_Tangent_disp/abs(delta_Tangent_disp)
                ...
                PC_Gauss_x(i_C,i_CT_Elem,i_CT_GAUSS) = c_PT*cos(ori_CT_elem)-c_PN*sin(ori_CT_elem)
                PC_Gauss_y(i_C,i_CT_Elem,i_CT_GAUSS) = c_PT*sin(ori_CT_elem)+c_PN*cos(ori_CT_elem)
...
do i_C=1,num_Crack
    ...
    do i_CT_Elem = 1,Num_Div_Points - 1
        do i_CT_GAUSS  = 1,num_CT_Gauss         
            Cracks_CalP_Contact_Force_x(i_C,i_CT_Elem) = sum(PC_Gauss_x(i_C,i_CT_Elem,1:2))
            Cracks_CalP_Contact_Force_y(i_C,i_CT_Elem) = sum(PC_Gauss_y(i_C,i_CT_Elem,1:2))
        enddo
    enddo
enddo
        ...
        do i_C=1,num_Crack
            ...
            if (c_Adj_Ele/= 0) then
                if(c_Elem_Conta_Sta(c_Adj_Ele,i_C) /= 0 )then 
                    c_Elem_Conta_Sta(i_E,i_C)= c_Elem_Conta_Sta(c_Adj_Ele,i_C)
                endif
            end if
        enddo
    end do
