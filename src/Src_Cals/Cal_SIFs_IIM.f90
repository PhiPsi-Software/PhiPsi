!-----------------------------------------------------------
! Brief: Compute 2D SIFs by the interaction integral method.
!
! Parameters:
!   Input:  iter        - current load step / iteration index
!           Display_info - flag to print progress to screen
!           c_DISP      - global displacement vector
!
! Notes:   For each crack tip, builds the J-domain (elements
!          inside ~2.1 Ave_Elem_L radius), assembles the IIM
!          contributions with auxiliary asymptotic fields, and
!          extracts KI, KII. Supports HF pressure contributions.
!-----------------------------------------------------------

subroutine Cal_SIFs_IIM(iter,Display_info,c_DISP)    
! Calculation of stress intensity factor using the interaction integral method.
! After the calculation is completed, pass it to the public variables KI and KII.
!
...
offset_delta,Ux,Uy,Dx,Dy,Disp_U(2),Disp_D(2), delta_Disp_x,delta_Disp_y,delta_Disp_x_CRACK,delta_Disp_y_CRACK
...
real(kind=FT) N(2,8),dNdkesi(4,2),dNdx(4,2),Coor_AB(2,2), c_Q(4),Q_Grad(2),QT(2,2),omega,Stress_Gauss(3), &
Q_Grad_Local(2),H_Local(2,2),CalStress(2,2)
real(kind=FT) tem_loc_Stress(2,2),Local_Gauss_coor(2), Global_Gauss(2),Tip_Coor(2),r,theta,K1,K2
real(kind=FT) SQR,CosT,SinT,CT2,ST2,C3T2,S3T2
real(kind=FT) AuxStress(2,2),AuxGradDisp(2,2),FAC_Disp1, FAC_Disp2,FAC_Stress1,FAC_Stress2, &
drdx,drdy,dtdx,dtdy,AuxStrain(2,2),du1dr, du1dt,du2dr,du2dt,I1,I2,StrainEnergy,I(2), Kcalc(2),Eeff
integer Num_CalP,i_CalP,Ele_CalP,i_S
real(kind=FT) x_CalP,y_CalP,P_CalP,Distance,Global_CalP(2), Ori_CalP,Local_CalP_coor(2),N_4(4),Q_CalP, &
c_Pres_x,c_Pres_y,I_Press,L_HF_Elem,x_CalP_temp, y_CalP_temp,crack_p1(2),crack_p2(2)
...
real(kind=FT) a_dot,beta1,beta2,DD,f1_val,f2_val
real(kind=FT) c_s_Tip,c_d_Tip,Lambda_Tip,mui_Tip
real(kind=FT) c_Velc(2), v_loc(2), Vel_dof(80), Hv(2,2), Hv_Local(2,2)
real(kind=FT) AuxGGD(2), hf(2), hfp(2), hfpp(2), Cdisp
real(kind=FT) ur2(2), urr2(2), ut2(2), urt2(2), utt2(2), dGdr2(2), dGdt2(2)
real(kind=FT) auxvel(2), auxvelg(2), I_KE, I_d2, I_d3
...
r_JDomain  = IIM_Radius_Factor*Ave_Elem_L_Enrich
...
LOOP_i_C: do i_C =1,num_Crack
    ...
    LOOP_i_Tip: do i_Tip = 1,2
        ...
        LOOP_i_JE : do i_JE = 1,num_J_Elem
            ...
            do j_C =1,num_Crack 
                call Location_Element_Stiff_Matrix(c_Ele,j_C,c_POS(:,j_C), Location_ESM_C_Crack,num_Loc_ESM_C_Crack, &
            ...
            LOOP_i_G : do i_G = 1,c_Num_Gauss_Point
                ...
                if (Material_Type(c_mat_num) .eq. 1) then
                    ...
                    if ((Key_Analysis_Type==2 .or. Key_Analysis_Type==6) .and. &
                         DY_SIFs_IIM_Enable_Velocity_Coupling==1) then
                        a_dot = Crack_Prop_Velocity(i_C,i_Tip)
                        call Cal_Any_Point_Disp_KesiYita(c_Ele,c_Kesi,c_Yita,i_G,Dy_VELC,c_Velc)
                        v_loc = MATMUL(QT,c_Velc)
                        Vel_dof(1:num_Loc_ESM) = Dy_VELC(Location_ESM(1:num_Loc_ESM))
                        Hv(1,1)=DOT_PRODUCT(B(1,1:num_B:2),Vel_dof(1:num_B:2))
                        Hv(1,2)=DOT_PRODUCT(B(2,2:num_B:2),Vel_dof(1:num_B:2))
                        Hv(2,1)=DOT_PRODUCT(B(1,1:num_B:2),Vel_dof(2:num_B:2))
                        Hv(2,2)=DOT_PRODUCT(B(2,2:num_B:2),Vel_dof(2:num_B:2))
                        Hv_Local = MATMUL(MATMUL(QT,Hv),transpose(QT))
                    end if
                    ...
                    LOOP_i_Mode:do i_Mode=1,2
                        ...
                        I1 = (CalStress(1,1)*AuxGradDisp(1,1)+ CalStress(2,1)*AuxGradDisp(2,1))* &
                        Q_Grad_Local(1)+ (CalStress(1,2)*AuxGradDisp(1,1)+ CalStress(2,2)*AuxGradDisp(2,1))*Q_Grad_Local(2)
                        I2 = (AuxStress(1,1)*H_Local(1,1)+ AuxStress(2,1)*H_Local(2,1))*Q_Grad_Local(1)+ &
                        ...
                        if ((Key_Analysis_Type==2 .or. Key_Analysis_Type==6)) then
                            ...
                            N_4(1:4) =  [N(1,1),N(1,3),N(1,5),N(1,7)]
                            c_Q_Gauss = DOT_PRODUCT(c_Q,N_4)       
                            call Cal_Any_Point_Disp_KesiYita(c_Ele,c_Kesi,c_Yita,i_G,Dy_ACCL,c_Accel)
                            c_Accel   = MATMUL(QT,c_Accel)
                            ...
                            if (DY_SIFs_IIM_Enable_Velocity_Coupling==1 .and. a_dot > Tol_10) then
                                Cdisp = FAC_Disp1
                                if (i_Mode==1) then
                                    hf(1)  =  CT2*(k-CosT)
                                    hfp(1) = -HLF*ST2*(k-CosT) + CT2*SinT
                                    hfpp(1)= -0.25D0*CT2*(k-CosT) - ST2*SinT + CT2*CosT
                                    hf(2)  =  ST2*(k-CosT)
                                    hfp(2) =  HLF*CT2*(k-CosT) + ST2*SinT
                                    hfpp(2)= -0.25D0*ST2*(k-CosT) + CT2*SinT + ST2*CosT
                                else
                                    hf(1)  =  ST2*(k+TWO+CosT)
                                    hfp(1) =  HLF*CT2*(k+TWO+CosT) - ST2*SinT
                                    hfpp(1)= -0.25D0*ST2*(k+TWO+CosT) - CT2*SinT - ST2*CosT
                                    hf(2)  = -CT2*(k-TWO+CosT)
                                    hfp(2) =  HLF*ST2*(k-TWO+CosT) + CT2*SinT
                                    hfpp(2)=  0.25D0*CT2*(k-TWO+CosT) - ST2*SinT + CT2*CosT
                                end if

                                do ii=1,2
                                    ur2(ii)  =  Cdisp*HLF*hf(ii)/SQR
                                    urr2(ii) = -Cdisp*0.25D0*hf(ii)/(SQR*r)
                                    ut2(ii)  =  Cdisp*SQR*hfp(ii)
                                    urt2(ii) =  Cdisp*HLF*hfp(ii)/SQR
                                    utt2(ii) =  Cdisp*SQR*hfpp(ii)
                                    dGdr2(ii)= urr2(ii)*CosT - urt2(ii)*SinT/r + ut2(ii)*SinT/(r*r)
                                    dGdt2(ii)= urt2(ii)*CosT - ur2(ii)*SinT - utt2(ii)*SinT/r - ut2(ii)*CosT/r
                                    AuxGGD(ii) = dGdr2(ii)*CosT - dGdt2(ii)*SinT/r
                                end do

                                auxvel(1)  = -a_dot*AuxGradDisp(1,1)
                                auxvel(2)  = -a_dot*AuxGradDisp(2,1)
                                auxvelg(1) = -a_dot*AuxGGD(1)
                                auxvelg(2) = -a_dot*AuxGGD(2)

                                I_KE = c_density*(v_loc(1)*auxvel(1)+v_loc(2)*auxvel(2))*Q_Grad_Local(1)
                                I_d2 = c_density*(auxvel(1)*Hv_Local(1,1)+auxvel(2)*Hv_Local(2,1))*c_Q_Gauss
                                I_d3 = c_density*(v_loc(1)*auxvelg(1)+v_loc(2)*auxvelg(2))*c_Q_Gauss

                                I(i_Mode) = I(i_Mode) + (I_KE + I_d2 + I_d3)*detJ*weight(i_G)
                            end if
        ...
        if (((Key_Analysis_Type==3 .or. Key_Analysis_Type==4 .or.Key_Analysis_Type==5) &
        .and.(Cracks_HF_State(i_C)==1))     .or. (Key_Analysis_Type==1 .and. &
             Key_Crack_Inner_Pressure==1 .and. Crack_Pressure(i_C) >Tol_10)    ) then
            do i_S = 1,Each_Cr_Poi_Num(i_C)-1
                ...
                call Tool_Intersection_Line_and_Circle(Tip_x, Tip_y,r_JDomain,crack_p1,crack_p2,num_Inter,State,Inter)
            ...
            do i_SS=1,num_Calp_inJ
                ...
                c_Pres_x = -sin(SS_Ori)*SS_P
                c_Pres_y =  cos(SS_Ori)*SS_P
                ...
                do i_Mode=1,2
                    ...
                    I_Press = TWO*(abs(c_Pres_x)*abs(AuxGradDisp(1,1)) + abs(c_Pres_y)*abs(AuxGradDisp(2,1)))*Q_CalP*L_HF_Elem
        ...
        if (Material_Type(c_mat_num) .eq. 1) then
            ...
            if ((Key_Analysis_Type==2 .or. Key_Analysis_Type==6) .and. &
                DY_SIFs_IIM_Enable_f1_f2 == 1) then
                a_dot = Crack_Prop_Velocity(i_C,i_Tip)
                mui_Tip = c_G_Tip
                Lambda_Tip = c_E_Tip*c_v_Tip/(ONE+c_v_Tip)/(ONE-TWO*c_v_Tip)
                c_s_Tip = sqrt(mui_Tip/density(mat_num_Tip))
                c_d_Tip = sqrt((Lambda_Tip+TWO*mui_Tip)/density(mat_num_Tip))
                
                if (a_dot <= Tol_10 .or. DD <= Tol_20 .or. a_dot >= 0.92D0*c_s_Tip) then
                    f1_val = ONE; f2_val = ONE    
                else
                    beta1 = sqrt(ONE - (a_dot/c_d_Tip)**2)
                    beta2 = sqrt(ONE - (a_dot/c_s_Tip)**2)
                    DD = 4.0D0*beta1*beta2 - (ONE + beta2**2)**2
                    if (abs(DD) <= Tol_20) then
                        f1_val = ONE; f2_val = ONE
                    else
                        f1_val = 4.0D0*beta1*(ONE - beta2**2) / ((k_Tip + ONE)*DD)
                        f2_val = 4.0D0*beta2*(ONE - beta2**2) / ((k_Tip + ONE)*DD)
                    end if
                end if
                Kcalc(1) = Kcalc(1) / f1_val
                Kcalc(2) = Kcalc(2) / f2_val
            end if
