!-----------------------------------------------------------
! Brief: Compute the 2D XFEM displacement at a point in an element.
!
! Parameters:
!   Input:  elem    - element number
!           Kesi    - local isoparametric coordinate
!           Yita    - local isoparametric coordinate
!           i_G     - Gauss point index (for enriched DoF lookup)
!           c_DISP  - global displacement vector
!   Output: P_Disp  - displacement vector (2 components)
!
! Notes:   Supports cracks, inclusions, holes, kinked tips and
!          cross-shaped cracks via the full 2D enrichment set.
!-----------------------------------------------------------

subroutine Cal_Any_Point_Disp_KesiYita(elem,Kesi,Yita,i_G,c_DISP,P_Disp)
! Calculate the displacement of a point, given the element number and local coordinates.
...
real(kind=FT) X_NODES(4),Y_NODES(4), detJ,dNdkesi(4,2),J(2,2),N(2,8), Coor_AB(2,2),Point(2)
integer c_Elem,i_C,i_N,NODES_iE(4),Num_Enri_Node,ref_elem
real(kind=FT) c_Segment(2), omega, Coor_Tip(2),Trans_Matrix(2,2), tem_N(4),coor_A(2),coor_B(2), &
coor_C(2),len_AB,line_AB(2,2), Tri_BCD(3,2),Closed_Tri_BCD(4,2), tip_x,tip_y,xp_Gauss(2),r_Gauss, &
Line_BC(2,2),thete_Doc,Global_coor_Gauss_Doc(2), len_BGauss,Line_BGauss(2,2),angle_AB_BGauss, &
...
logical Yes_in_KAB
real(kind=FT) Coor_CD(2,2),x0(2), distance_Node_M,distance_Gauss_M, distance_Node_sm,distance_Gauss_sm, &
distance_x0,H_gp_M,H_gp_sm,H_i_M,H_i_sm, H_x0, J_gp, J_i
real(kind=FT) distance_Vertex, Tri_KAB(3,2),Closed_Tri_KAB(4,2),H_i, H_Vertex,distance_Gauss,H_gp,distance_Node, &
...
real(kind=FT) vector_e1(2),element_kesi,Le,x_in(2),x_out(2)
real(kind=FT) extended_Line_AB(2,2),new_Point(2)
integer N1,N2,N3,N4,N5,NN_L(5),i_Side,n_node_1,n_node_2
real(kind=FT) point_side_1(2),point_side_2(2),c_X,c_Y,lambda
logical c_Yes_Cross
real(kind=FT) Gauss_kesi,Node_kesi,Gauss_R,Node_R
real(kind=FT) Gauss_R_x,Gauss_R_y,Node_R_x,Node_R_y
real(kind=FT) tem_vector_1(2),tem_vector_2(2),tem_vector_3(2)
...
if(Num_Crack /=0)then
    do i_C=1,Num_Crack
        ...
        do i_N = 1,4
            ...
            if (Enriched_Node_Type(NODES_iE(i_N),i_C) ==2) then
                ...
                if ((Elem_Type(c_Elem,i_C) == 2) .or.  (Elem_Type(c_Elem,i_C) == 3)) then
                    ...
                    if  (Elem_Type(c_Elem,i_C) .eq. 3) then
                        tem_vector_3(1:2) = Coors_Vertex(ref_elem,1:2)
                        call Cal_Signed_Distance(Coor_AB,tem_vector_3,distance_Vertex)
                        ...
                        if (H_Vertex*H_gp <= 0) then
                            ...
                            Tri_KAB(1,:) =Coor_AB(1,1:2)
                            Tri_KAB(2,:) =Coor_AB(2,1:2)
                            Tri_KAB(3,:) =Coors_Vertex(ref_elem,1:2)
            ...
            elseif (Enriched_Node_Type(NODES_iE(i_N),i_C) ==1  .and. (Key_TipEnrich/=5)) then
                ref_elem = 0
                ...
                if(Key_Analysis_Type==6 .and. ref_elem==0)then
                    ref_elem = Ref_Elem_Maping_Tip(NODES_iE(i_N))
                endif
            ...
            elseif (Enriched_Node_Type(NODES_iE(i_N),i_C).eq.1 .and. (Key_TipEnrich==5))then
                ref_elem = 0
                Num_Enri_Node = c_POS(NODES_iE(i_N),i_C)
                if (Elem_Type(c_Elem,i_C) .eq. 1) then
                    ref_elem = c_Elem
                ...
                if (Elem_Type(c_Elem,i_C) ==1) then
                    vector_e1 = [cos(omega),sin(omega)]
                    x_in  = Coor_AB(1,1:2)
                    call Tool_Shorten_or_Extend_Line(Coor_AB(1:2,1:2),FIV*Ave_Elem_L,'B',extended_Line_AB,new_Point)
                    extended_Line_AB(1,1:2) = Coor_AB(2,1:2)
                    extended_Line_AB(2,1:2) = new_Point

                    tem_vector_1(1:2) = extended_Line_AB(1,1:2)
                    tem_vector_2(1:2) = extended_Line_AB(2,1:2)

                    N1  = Elem_Node(ref_elem,1); N2  = Elem_Node(ref_elem,2)                                             
                    N3  = Elem_Node(ref_elem,3); N4  = Elem_Node(ref_elem,4)                                            
                    NN_L= [N1,N2,N3,N4,N1]
                    c_Yes_Cross = .False.
                    do i_Side = 1,4
                        ...
                        call Tool_Intersection(tem_vector_1,tem_vector_2(1:2), point_side_1,point_side_2,c_X,c_Y,c_Yes_Cross)
                        if (c_Yes_Cross.eqv..True.) then
                            x_out =[c_X,c_Y]; exit
                        end if
                    end do  
                    if (c_Yes_Cross .eqv. .False.) then
                        print *, '    ERROR-2026052702 :: Cannot find intersections!'
                        print *, '                        ERROR in Cal_Any_Point_Disp_KesiYita.f90!'
                        call Warning_Message('S',Keywords_Blank)
                    endif
                    Le = dot_product(x_out-x_in, vector_e1)
                    lambda = dot_product(Coor_Tip - x_in, vector_e1)/Le
                    Gauss_kesi = dot_product(Point - x_in, vector_e1)/Le

                    Node_kesi  = dot_product([X_NODES(i_N),Y_NODES(i_N)] - x_in, vector_e1)/Le


                    call Cal_R_and_R_x_R_y(lambda,Le,cos(omega),sin(omega),Gauss_kesi,Gauss_R,Gauss_R_x,Gauss_R_y)
                    ...
                    Disp_x = Disp_x+(H_gp-H_i)*Gauss_R*tem_N(i_N)*c_DISP(Num_Enri_Node*2-1)
                    Disp_y = Disp_y+(H_gp-H_i)*Gauss_R*tem_N(i_N)*c_DISP(Num_Enri_Node*2)
...
if(num_Poly_Incl /=0)then
    do i_Incl =1,num_Poly_Incl
        ...
        do i_N = 1,4
            c_NODE  = Elem_Node(c_Elem,i_N)
            call Tool_Dis_Point_to_Poly(Coor(c_NODE,1),Coor(c_NODE,2), Poly_Incl_Coor_x_Cl(i_Incl,1:n_Vertex+1), &
