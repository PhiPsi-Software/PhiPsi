!-----------------------------------------------------------
! Brief: Compute flow velocity and flow rate magnitude at
!        each hydraulic-fracture calculation point.
!
! Parameters:
!   Input:  ifra        - current fracture step
!           iter        - current Picard iteration
!           Counter     - global step counter
!           total_time  - current simulation time
!
! Notes:   Updates global Cracks_CalP_Velo and
!          Cracks_CalP_Quan. Uses cubic-spline Poiseuille
!          law with adaptive forward/backward differencing.
!-----------------------------------------------------------

subroutine Cal_HF_Flow_Quan(ifra,iter,Counter,total_time) 
! Calculate the flow velocity at a point (including magnitude) and the flow rate (magnitude only,
! not including direction)
! The direction can be determined by the water pressure on the left and right sides.
...
do i_C = 1,num_Crack
    if (Cracks_HF_State(i_C) == 1) then
        ...
        do i_CalP=2,Num_Div_Points-1
            ...
            elseif(Key_Visco_Type==2)then
                ...
                Real_Visco = Viscosity* (ONE-c_CalP_c/Max_c)**(-Viscosity_Par_m)
            endif
            if(Real_Visco>Visco_Zoom_Factor*Viscosity)then
                Real_Visco=Visco_Zoom_Factor*Viscosity
            endif
            delta_x_l = sqrt((l_CalP_x-c_CalP_x)**2 + (l_CalP_y-c_CalP_y)**2)
            delta_x_n = sqrt((n_CalP_x-c_CalP_x)**2 + (n_CalP_y-c_CalP_y)**2)
    ...
    Max_Aperture = maxval(Cracks_CalP_Aper(i_C,1:Num_Div_Points))
    if (Cracks_CalP_Aper(i_C,1)<=ZPZ1*max_aperture)then
        ...
        if(i_C == Inject_Crack_Num.and. CalP_num_InjP_Local==1)then
            Cracks_CalP_Quan(i_C,1)=Inject_Q
            Cracks_CalP_Velo(i_C,1)=Inject_Q/c_CalP_w
            Cracks_CalP_Pgra(i_C,1) = -12.0D0*Real_Visco*Inject_Q/c_CalP_w**3
        else
            if(Num_Div_Points>=4)then
                ...
                delta_1_2 = sqrt((CalP_2_x-CalP_1_x)**2 + (CalP_2_y-CalP_1_y)**2)
                delta_2_3 = sqrt((CalP_3_x-CalP_2_x)**2 + (CalP_3_y-CalP_2_y)**2)
                delta_3_4 = sqrt((CalP_4_x-CalP_3_x)**2 + (CalP_4_y-CalP_3_y)**2)
                Pa_1_x = 0   
                Pa_2_x = delta_1_2
                ...
                call Tool_abc_of_Parabola(Point_1, Point_2,Point_3,a,b,c)
                Q_1 = a*Pa_1_x**2 + b*Pa_1_x + c
                Cracks_CalP_Quan(i_C,1) = Q_1
                Cracks_CalP_Velo(i_C,1) = Q_1/c_CalP_w
                Cracks_CalP_Pgra(i_C,1) = -12.0D0*Real_Visco*Q_1/c_CalP_w**3
            else  
    ...
    if(i_C == Inject_Crack_Num.and. CalP_num_InjP_Local==Num_Div_Points)then
        Cracks_CalP_Quan(i_C,Num_Div_Points)=Inject_Q
        Cracks_CalP_Velo(i_C,Num_Div_Points)= Inject_Q/c_CalP_w
        Cracks_CalP_Pgra(i_C,Num_Div_Points) = -12.0D0*Real_Visco*Inject_Q/c_CalP_w**3
    else
        if(Num_Div_Points>=4)then
            CalP_1_x = Cracks_CalP_Coors(i_C,Num_Div_Points-3,1)
            CalP_1_y = Cracks_CalP_Coors(i_C,Num_Div_Points-3,2)
            CalP_2_x = Cracks_CalP_Coors(i_C,Num_Div_Points-2,1)
            CalP_2_y = Cracks_CalP_Coors(i_C,Num_Div_Points-2,2)
            CalP_3_x = Cracks_CalP_Coors(i_C,Num_Div_Points-1,1)
            CalP_3_y = Cracks_CalP_Coors(i_C,Num_Div_Points-1,2)
            CalP_4_x = Cracks_CalP_Coors(i_C,Num_Div_Points,1)
            CalP_4_y = Cracks_CalP_Coors(i_C,Num_Div_Points,2)
            delta_1_2 = sqrt((CalP_2_x-CalP_1_x)**2 + (CalP_2_y-CalP_1_y)**2)
            delta_2_3 = sqrt((CalP_3_x-CalP_2_x)**2 + (CalP_3_y-CalP_2_y)**2)
            delta_3_4 = sqrt((CalP_4_x-CalP_3_x)**2 + (CalP_4_y-CalP_3_y)**2)
            Pa_1_x = 0   
            ...
            Pa_4_x = delta_1_2 + delta_2_3 + delta_3_4
            Point_1(1) = Pa_1_x
            Point_1(2) = Cracks_CalP_Quan(i_C,Num_Div_Points-3)
            Point_2(1) = Pa_2_x
            Point_2(2) = Cracks_CalP_Quan(i_C,Num_Div_Points-2)
            Point_3(1) = Pa_3_x
            Point_3(2) = Cracks_CalP_Quan(i_C,Num_Div_Points-1)
            call Tool_abc_of_Parabola(Point_1, Point_2,Point_3,a,b,c)
            Q_1 = a*Pa_4_x**2 + b*Pa_4_x + c
            Cracks_CalP_Quan(i_C,Num_Div_Points) = Q_1
            Cracks_CalP_Velo(i_C,Num_Div_Points) = Q_1/c_CalP_w
            Cracks_CalP_Pgra(i_C,Num_Div_Points) = -12.0D0*Real_Visco*Q_1/c_CalP_w**3
        else   
            Cracks_CalP_Pgra(i_C,Num_Div_Points)= Cracks_CalP_Pgra(i_C,Num_Div_Points-1)
            Cracks_CalP_Velo(i_C,Num_Div_Points)= Cracks_CalP_Velo(i_C,Num_Div_Points-1)
            Cracks_CalP_Quan(i_C,Num_Div_Points)= Cracks_CalP_Quan(i_C,Num_Div_Points-1)
            print *,'    WARNING::less than 4 fluids nodes!'
        endif
    end if
end if
