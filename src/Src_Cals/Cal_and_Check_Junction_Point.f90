!-----------------------------------------------------------
! Brief: Validate and adjust a crack junction point on an element.
!
! Parameters:
!   Input:  ifra, iter          - fracture / global iteration counters
!           Point_B, Point_C, Point_D - geometric anchors (2D)
!   In/Out: Inter_x, Inter_y   - junction coordinates, adjusted if invalid
!
! Notes:   If the candidate junction would intersect the host
!          element on more than one side, the point is shifted
!          along (J->C) and (J->D) up to 50 times until the
!          single-crossing constraint is satisfied.
!-----------------------------------------------------------

subroutine Cal_and_Check_Junction_Point(ifra,iter,Point_B, Point_C,Point_D, Inter_x,Inter_y)

!     Check the newly generated junction points to ensure that each 
!     intersection requires only one junction enhancement element.
...
do i_Side = 1,4
    ...
    call Tool_Intersection(Point_C,Point_D, point_side_1,point_side_2,c_X,c_Y,c_Yes_Cross_1)

    if (c_Yes_Cross_1.eqv..True.) then
        num_Cross_Side(i_Side)= num_Cross_Side(i_Side)+1
    end if
    call Tool_Intersection(point_side_1,point_side_2, Point_B,Point_J,c_X,c_Y,c_Yes_Cross_2)
...
do i_Try = 1,50
    ...
    call Tool_Shorten_or_Extend_Line(line_J_C,incre_L,'A', shorted_Line_AB,Point_J_L)
    call Tool_Shorten_or_Extend_Line(line_J_D,incre_L,'A', shorted_Line_AB,Point_J_R)
    ...
    do i_Side = 1,4
        ...
        call Tool_Intersection(point_side_1,point_side_2, Point_B,Point_J_L,c_X,c_Y,c_Yes_Cross_2)
        if (c_Yes_Cross_2.eqv..True.) then
            num_Cross_Side(i_Side)= num_Cross_Side(i_Side)+1
        end if
    end do  
    ...
    do i_Side = 1,4
        ...
        call Tool_Intersection( point_side_1,point_side_2, Point_B,Point_J_R, c_X,c_Y,c_Yes_Cross_2)
        if (c_Yes_Cross_2.eqv..True.) then
            num_Cross_Side(i_Side)= num_Cross_Side(i_Side)+1
        end if
    end do  
