!-----------------------------------------------------------
! Brief: Compute the global (x,y) coordinates of all Gauss points in the model.
!
! Parameters:
!   Input:  isub          - sub-domain / sub-mesh index (0 for full model)
!           Total_Num_G_P - total number of Gauss points to populate
!   Output: c_G_CoorX     - x-coordinates of the Gauss points
!           c_G_CoorY     - y-coordinates of the Gauss points
!
! Notes:   Honors Key_Integral_Sol (2 = standard, 3 = sub-quadrilateral) and the
!          XFEM flag to choose between enriched and standard Gauss schemes; OpenMP
!          parallelized over elements.
!-----------------------------------------------------------

subroutine Cal_Gauss_Coors(isub,Total_Num_G_P, c_G_CoorX,c_G_CoorY)
!     Calculate the Gaussian point coordinates for use in post-processing (Note: Actually, during the assembly of the stiffness matrix)
...
real(kind=FT),intent(out)::c_G_CoorX(Total_Num_G_P), c_G_CoorY(Total_Num_G_P)
...
if (Yes_XFEM.eqv..False.) then
    ...
    do i_E = 1,Num_Elem
        ...
        do i_G = 1,Num_Gauss_P_FEM
            ...
            call Cal_Coor_by_KesiYita(kesi(i_G),yita(i_G), c_X_NODES,c_Y_NODES, Out_x,Out_y)
