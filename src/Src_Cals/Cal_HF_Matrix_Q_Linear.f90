!-----------------------------------------------------------
! Brief: Assemble the coupled matrix Q for 2D linear HF elements.
!
! Parameters:
!   Input:  Counter       - current load/iteration step index
!           c_Total_FD    - total number of displacement DOFs
!   Output: Coupled_Q     - coupled matrix linking solid and fluid DOFs
!
! Notes:   Loops over HF crack calculation points, integrating the
!          enrichment functions weighted by HF element shape
!          functions with Gauss quadrature.
!-----------------------------------------------------------

subroutine Cal_HF_Matrix_Q_Linear(Counter,Coupled_Q,c_Total_FD)
!This subroutine is used to calculate the matrix Q.
...
real(kind=FT),intent(out):: Coupled_Q(c_Total_FD,num_Tol_CalP_Water)
integer i_N,i_C,i_HF_Elem,i_HF_GAUSS    
real(kind=FT) Kesi,Yita,Orient1,Orient2,x1,y1,x2,y2
integer c_NN(4),Num_Div_Points 
real(kind=FT) Div_Points(Max_Num_Cr_CalP,2),N(2,8), tem_n(4),Coor_Tip(2), r,Coor_AB(2,2),Orient_Tip
...
real(kind=FT) tem_Q(60,2),tem(60,2),c_Sign,ori_HF_elem, ele_Sign,tem_val1,tem_val2,tem_val3,tem_val4
...
do i_C=1,num_Crack
    if (Cracks_HF_State(i_C) == 1) then  
        ...
        Div_Points(1:Num_Div_Points,1:2) = Cracks_CalP_Coors(i_C,1:Num_Div_Points,1:2)
        if(i_C>1)then
        ...
        do i_HF_Elem = 1,Num_Div_Points - 1
            ...
            detJ   = HF_Length/TWO
            ori_n = [cos(pi/TWO+ori_HF_elem), sin(pi/TWO+ori_HF_elem)]
            do i_HF_GAUSS  = 1,num_HF_Gauss
                call Cal_HF_Coor_by_Kesi(kesi_HF(i_HF_GAUSS), x1,y1,x2,y2, HF_GAUSS_x,HF_GAUSS_y)
                call Cal_Ele_Num_by_Coors(HF_GAUSS_x,HF_GAUSS_y, HF_GAUSS_Elem)
                call Cal_KesiYita_by_Coor([HF_GAUSS_x,HF_GAUSS_y], HF_GAUSS_Elem,Kesi,Yita)
                ...
                do i_N = 1,4
                    ...
                    elseif(Enriched_Node_Type(c_NN(i_N),i_C) ==3)then
                        ...
                        Orient_Tip = atan2(Coor_Tip(2)-HF_GAUSS_y, Coor_Tip(1)-HF_GAUSS_x)
                    ...
                    elseif(Enriched_Node_Type(c_NN(i_N),i_C) ==1)then
                        ...
                        r=sqrt((HF_GAUSS_x-Coor_Tip(1))**2 + (HF_GAUSS_y-Coor_Tip(2))**2)
                        Orient_Tip = atan2(Coor_Tip(2)-HF_GAUSS_y, Coor_Tip(1)-HF_GAUSS_x)
                        num_Local_W = num_Local_W+1
                        Local_W(2*num_Local_W-1)= 2*(c_POS(c_NN(i_N),i_C)) - 1
                        Local_W(2*num_Local_W)  = 2*(c_POS(c_NN(i_N),i_C))
                ...
                call Vectors_Multi(MATMUL(transpose(N_W(1:2,1:2*num_Local_W)),ori_n), 2*num_Local_W,N_HF,2,tem(1:2*num_Local_W,1:2))

                tem_Q(1:2*num_Local_W,1:2) = tem_Q(1:2*num_Local_W,1:2)  + tem(1:2*num_Local_W,1:2)* Weight_HF(i_HF_GAUSS)*detJ
            end do
            Coupled_Q(Local_W(1:2*num_Local_W),Local_P(1:2)) = Coupled_Q(Local_W(1:2*num_Local_W),Local_P(1:2)) + &
