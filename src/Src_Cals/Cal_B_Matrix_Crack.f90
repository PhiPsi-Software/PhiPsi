!-----------------------------------------------------------
! Brief: Assemble the strain-displacement B matrix for 2D crack XFEM.
!
! Parameters:
!   Input:  kesi, yita        - local coordinates of the GP
!           i_C, i_E, i_G     - crack, element, GP indices
!           c_NN              - element node IDs
!           c_X_NODES, c_Y_NODES - element nodal coordinates
!   Output: tem_B, num_tem_B  - B matrix and active column count
!
! Notes:   Supports Heaviside, tip-F and crack-tip asymptotic
!          enrichments, kinked tips and cross-shaped cracks.
!-----------------------------------------------------------
...
real(kind=FT) Coor_CD(2,2),x0(2),distance_Node_M,distance_Gauss_M, distance_Node_sm,distance_Gauss_sm,distance_x0, &
...
angle_AB_BC,Line_BC(2,2),Line_BGauss(2,2),angle_AB_BGauss, len_BGauss,thete_Doc
real(kind=FT) Global_coor_Gauss_Doc(2),xp_Gauss(2),r_Gauss, theta_Gauss,xp_Node(2),r_Node,theta_Node
...
elseif(maxval(Enriched_Node_Type(c_NN,i_C)).gt.0 .or. minval(Enriched_Node_Type(c_NN,i_C)).gt.0) then
    B_XFEM(1:3,1:60) = ZR
    num_B_XFEM = 0
    do i_N = 1,4
        ...
        elseif (Enriched_Node_Type(c_NN(i_N),i_C).eq.6 .and. num_Ellip_Hole>=1)then 
            ...
            call Tool_Signed_Distance_Point_to_Ellipse(x0_Hole,y0_Hole,a_Hole,b_Hole,theta_Hole, Global_coor_Gauss,distance_Gauss_M)
        ...
        elseif (Enriched_Node_Type(c_NN(i_N),i_C).eq.1 .and. (Key_TipEnrich/=5))then
            ref_elem = 0
            if (Elem_Type(i_E,i_C) .eq. 1) then
                ...
                if(Key_Analysis_Type==6 .and. ref_elem==0)then
                    ref_elem = Ref_Elem_Maping_Tip(c_NN(i_N))
                endif
                if (ref_elem ==0) then  
            ...
            if ((Flag_Check_Kink_Tip .eqv. .True.) .and. (num_points_crack > 2)) then
                if ((Elem_Type(i_E,i_C) .eq. 2) .or.(Elem_Type(i_E,i_C) .eq. 3))then
                    ...
                    elseif((Crack_Coor(i_C,num_points_crack,1).eq.tip_x).and.(Crack_Coor(i_C,num_points_crack,2) .eq. tip_y)) then
                    ...
                    elseif(coor_A(1).eq. Crack_Coor(i_C,num_points_crack,1).and.coor_A(2).eq.Crack_Coor(i_C,num_points_crack,2))then
        ...
        elseif (Enriched_Node_Type(c_NN(i_N),i_C).eq.1 .and. (Key_TipEnrich==5))then
            ...
            Coor_AB(1,1:2) = [Coors_Element_Crack(ref_elem,i_C,1),Coors_Element_Crack(ref_elem,i_C,2)]
            Coor_AB(2,1:2) = [Coors_Element_Crack(ref_elem,i_C,3),Coors_Element_Crack(ref_elem,i_C,4)]  
            c_Segment = Coor_AB(2,1:2) - Coor_AB(1,1:2)    
            ...
            if (Elem_Type(i_E,i_C) .eq. 1) then
                ...
                do i_Side = 1,4
                    ...
                    call Tool_Intersection(tem_vector_1,tem_vector_2, point_side_1,point_side_2,c_X,c_Y,c_Yes_Cross)
                    if (c_Yes_Cross.eqv..True.) then
                        x_out =[c_X,c_Y]; exit
                    end if
                end do  
                if (c_Yes_Cross .eqv. .False.) then
                    print *, '    ERROR-2026052701 :: Cannot find intersections!'
                    print *, '                        ERROR in Cal_B_Matrix_Crack.f90!'
                ...
                Gauss_kesi = dot_product(Global_coor_Gauss - x_in, vector_e1)/Le
                ...
                BI_enr(1,:) = [dNdx(i_N,1)*(H_gp-H_i)*Gauss_R,                             ZR]
                BI_enr(2,:) = [ZR,                             dNdx(i_N,2)*(H_gp-H_i)*Gauss_R]
                BI_enr(3,:) = [dNdx(i_N,2)*(H_gp-H_i)*Gauss_R, dNdx(i_N,1)*(H_gp-H_i)*Gauss_R]





            elseif (Elem_Type(i_E,i_C) .eq. 2 .or. Elem_Type(i_E,i_C) .eq. 3) then
                call Cal_Signed_Distance(Coor_AB,Global_coor_Gauss,distance_Gauss)
                call Cal_Sign(distance_Gauss,H_gp)
                call Cal_Signed_Distance(Coor_AB,[c_X_NODES(i_N),c_Y_NODES(i_N)],distance_Node)
                call Cal_Sign(distance_Node,H_i)
