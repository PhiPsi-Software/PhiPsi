!-----------------------------------------------------------
! Brief: Assemble the stiffness matrix of an 8-node 3D hexahedral element.
!
! Parameters:
!   Input:  i_E        - element index
!           num_Gauss  - number of Gauss points per element
...
!           zeta       - parametric coordinates of Gauss points
!           weight     - Gauss integration weights
!   Output: localK     - element stiffness matrix (24x24)
!
! Notes:   Builds K via Gauss quadrature of B^T D B over the element volume.
!-----------------------------------------------------------

subroutine Cal_Ele_Stiffness_Matrix_3D_8nodes(i_E,num_Gauss,X_NODES,Y_NODES,Z_NODES, c_D,kesi,yita,zeta,weight,localK)
! Each 3D element has 8 Gauss points
...
do i_G = 1,num_Gauss      
    ...
    (one_p_yita)*(one_m_zeta),-(one_p_yita)*(one_m_zeta), -(one_m_yita)*(one_p_zeta), (one_m_yita)*(one_p_zeta), &
    (one_p_yita)*(one_p_zeta),-(one_p_yita)*(one_p_zeta)]
    temp(2,1:8)=[-(one_m_kesi)*(one_m_zeta),-(one_p_kesi)*(one_m_zeta), &
    (one_p_kesi)*(one_m_zeta), (one_m_kesi)*(one_m_zeta), -(one_m_kesi)*(one_p_zeta),-(one_p_kesi)*(one_p_zeta), &
    (one_p_kesi)*(one_p_zeta), (one_m_kesi)*(one_p_zeta)]
    temp(3,1:8)=[-(one_m_kesi)*(one_m_yita),-(one_p_kesi)*(one_m_yita), &
    -(one_p_kesi)*(one_p_yita),-(one_m_kesi)*(one_p_yita), (one_m_kesi)*(one_m_yita), (one_p_kesi)*(one_m_yita), &
