!-----------------------------------------------------------
! Brief: Set the initial fluid pressure at HF calculation
!        points for the current fracture step.
!
! Parameters:
!   Input:  iter, ifra, Counter - step / iteration counters
!           Yes_Growth          - per-tip growth flags
!           Last_Cracks_CalP_Num_ifra - previous counts
!           Last_Cr_CalP_Pres_ifra    - previous pressures
!   Output: Initial_Cr_CalP_Pres     - starting pressures
!
! Notes:   First step uses the analytical HF pressure; later
!          steps map from the previous step via crack growth.
!-----------------------------------------------------------
...
real(kind=FT) Inj_x,Inj_y,c_iCalp_Dis,c_iCalp_x,c_iCalp_y, l_Crack,Kesi,tem,tem1,tem2,tem3,cc_time, A0,A1,B1,E_1,u_1
...
if (ifra==1) then
    do i_C = 1,num_Crack
        if (Cracks_HF_State(i_C) == 1) then      
            ...
            call Tool_Length_of_Crack(Inject_Crack_Num, Inject_Crack_L)
            if(Key_Symm_HF==0)then
                Inject_Crack_L=Inject_Crack_L*HLF
            end if
            call Cal_HF_Theoretic_Pressure(iter,Counter, Inject_Crack_L,HF_Theor_Time,HF_Ini_Pressure, HF_Theor_Aper)
            Num_Div_Points = Cracks_CalP_Num(i_C)

            Initial_Cr_CalP_Pres(i_C,1:Num_Div_Points)= HF_Ini_Pressure
