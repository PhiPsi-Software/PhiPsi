!-----------------------------------------------------------
! Brief: Weighted-average stress at a point over a search radius.
!
! Parameters:
!   Input:  c_Key_Ave_Stress - weighting scheme (1 polynomial, 2 Gaussian)
!           Point(2)        - query point coordinates
!           Search_R        - search radius
!           a_Weight        - exponent for scheme 1 weighting
!           l_Ordina        - smoothing length for scheme 2
!   Output: WA_S_xx, WA_S_yy, WA_S_xy - weighted average stress comps
!
! Notes:   Gathers Gauss-point stresses from neighboring elements,
!          weights them by distance to Point, and normalizes.
!-----------------------------------------------------------

subroutine Cal_Weighted_Ave_Stress_of_Point(c_Key_Ave_Stress, Point,Search_R,a_Weight,l_Ordina, WA_S_xx,WA_S_yy,WA_S_xy)
!     Calculate the weighted average stress of the point.
!     Stresses of all the Gauss points inside the circle of radius Search_R
!     are calculated.
!     For SHI Fang's Formation, see page 25 of my doctoral dissertation for details.
!     For ordinary Formation, See p498 of book:Extended Finite Element Method: Theory and Applications  
...
real(kind=FT),intent(in)::Point(2),Search_R,a_Weight, l_Ordina
