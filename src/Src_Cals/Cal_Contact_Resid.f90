!-----------------------------------------------------------
! Brief: Compute the full contact residual vector for the 2D penalty method.
!
! Parameters:
...
!           c_Total_FD      - total free DOFs
!           c_num_freeDOF   - number of free DOFs
!           F_U             - external force vector
!           c_DISP          - displacement vector
!           freeDOF         - free-DOF map
!           PC_Gauss_x/y    - contact forces at Gauss points
!   Output: R_PSI           - contact residual vector
!
! Notes:   Assembles contact internal forces into the residual over all
!          elements using the current Gauss-point contact forces and shape
!          functions.
!-----------------------------------------------------------

subroutine Cal_Contact_Resid( iter,ifra,Counter_Iter,i_NR_P, c_Total_FD,c_num_freeDOF,F_U, &
c_DISP,freeDOF,PC_Gauss_x,PC_Gauss_y,R_PSI)
! Calculation of residuals in the contact iteration process
...
do i_E = 1,Num_Elem
    ...
    do i_G = 1, c_Num_Gauss_Point  
        ...
        c_Inter_Force(1:num_Loc_ESM) = MATMUL(transpose(B(1:3,1:num_Loc_ESM)),c_Stress)
        R_PSI(Location_ESM(1:num_Loc_ESM)) = R_PSI(Location_ESM(1:num_Loc_ESM))  + &
...
do i_C=1,num_Crack
    ...
    do i_CT_Elem = 1,Num_Div_Points - 1
        ...
        do i_CT_GAUSS  = 1,num_CT_Gauss
            ...
            enddo
            R_PSI(Local_W(1:2*num_Local_W)) = R_PSI(Local_W(1:2*num_Local_W))  + MATMUL(transpose(N_W(1:2,1:2*num_Local_W)), &
