!-----------------------------------------------------------
! Brief: Compute the in-situ stress field with holes/inclusions.
!
! Parameters: (none)
!
! Notes:   Initial cracks are excluded from the in-situ solve
!          but holes and inclusions are honored. Saves the 4
!          Gauss-point in-situ stresses per element for later
!          superposition onto the analysis solution.
!-----------------------------------------------------------

subroutine Cal_InSitu_Stress_for_Hole_and_Inclusion
!     Supports the calculation of initial stress fields with holes and inclusions.
!     Note: (1) Only save the initial stress of the 4 Gauss points.
!           (2) Initial cracks do not participate in the calculation of the initial stress field, but initial voids and inclusions do.
...
call Boundary_Cond_output_Fixed_Freedom(Total_FD,1, freeDOF_InSitu,num_FreeD_InSitu, fixedDOF_InSitu,num_FixedD_InSitu)
...
print *,'    Assembling K......'
call Assemble_Stiffness_Matrix_XFEM(1,globalK_InSitu, Total_FD,c_Total_Num_G_P)
...
call Matrix_Solve_LSOE(0,1,Key_SLOE, globalK_InSitu(freeDOF_InSitu(1:num_FreeD_InSitu), &
freeDOF_InSitu(1:num_FreeD_InSitu)), F_InSitu(freeDOF_InSitu(1:num_FreeD_InSitu)), tem_DISP_InSitu,num_FreeD_InSitu)
...
call Get_Node_Stress_XFEM_IN_OUT(Yes_Add_Insitu,1,DISP_InSitu, Str_xx_InSitu,Str_yy_InSitu,Str_xy_InSitu,Str_vm_InSitu)
