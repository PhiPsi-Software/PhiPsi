!-----------------------------------------------------------
! Brief: Locate the 3D hex element that contains a given (x,y,z) point.
!
! Parameters:
!   Input:  x              - x-coordinate of the query point
!           y              - y-coordinate of the query point
!           z              - z-coordinate of the query point
!           Ele_Num_Cache  - last successful element index (used as cache)
!   Output: OUT_Elem       - index of the element containing the point (0 if none)
!   In/Out: Ele_Num_Cache  - updated to the matched element index
!
! Notes:   Uses the cached element as the first candidate, falls back to brute-
!          force bounding-box scan and point-in-hexahedron test.
!-----------------------------------------------------------

subroutine Cal_Ele_Num_by_Coors_3D(x,y,z,Ele_Num_Cache,OUT_Elem)
! Calculate the element number based on coordinates, in 3D.
! Ele_Num_Cache !Cache, stores the most recently obtained element numbers via coordinates.
! 2022-09-24.
...
call Tool_Yes_Point_in_3D_Hexahedron([x,y,z], Point_A,Point_B,Point_C,Point_D,Point_E,Point_F,Point_G,Point_H, &
c_Yes_in,c_Yes_on)
...
if(c_count>1 .and. OUT_Elem==0)then
    ...
    do i =1,c_count
        ...
        Point_G = Coor(c_NN(7),1:3)
        Point_H = Coor(c_NN(8),1:3)
        call Tool_Yes_Point_in_3D_Hexahedron_with_Tol([x,y,z], Point_A,Point_B,Point_C,Point_D,Point_E,Point_F,Point_G,Point_H, &
