!-----------------------------------------------------------
! Brief: Build Gauss points and weights for a 3D element
!        subdivided into many sub-cubes (subdivision rule).
!
! Parameters:
!   Input:  Num_Sub_Cubes       - number of sub-cubes
!           Num_Gau_Points_Cube - Gauss points per sub-cube
!   Output: kesi_Cubes          - local x coordinates
!           yita_Cubes          - local y coordinates
!           zeta_Cubes          - local z coordinates
!           weight_Cubes        - integration weights
!
! Notes:   Subdivision integration scheme: element is split
!          into Num_Sub_Cubes equal sub-cubes, each integrated
!          with Num_Gau_Points_Cube points.
!-----------------------------------------------------------
...
real(kind=FT),intent(out)::kesi_Cubes(Num_Sub_Cubes*Num_Gau_Points_Cube), yita_Cubes(Num_Sub_Cubes*Num_Gau_Points_Cube), &
zeta_Cubes(Num_Sub_Cubes*Num_Gau_Points_Cube), weight_Cubes(Num_Sub_Cubes*Num_Gau_Points_Cube)
integer i,j,k
