!-----------------------------------------------------------
! Brief: Assemble the strain-displacement B matrix for 3D crack XFEM.
!
! Parameters:
!   Input:  kesi, yita, zeta  - local coordinates of the GP
!           i_C, i_E, i_G     - crack, element, GP indices
!           c_NN              - element node IDs
!           c_X_NODES, c_Y_NODES, c_Z_NODES - nodal coordinates
!   Output: tem_B, num_tem_B  - B matrix and active column count
!
! Notes:   Uses 3D signed distance to the crack mesh and the local
!          front-aligned basis for tip-enrichment derivatives.
!-----------------------------------------------------------

subroutine Cal_B_Matrix_Crack_3D(kesi,yita,zeta,i_C,i_E,i_G, c_NN,c_X_NODES,c_Y_NODES,c_Z_NODES,tem_B,num_tem_B)
! Calculate the B-matrix for an XFEM problem (3D).
...
if(c_maxval_Enriched_Node_Type .gt. 0 .or. c_minval_Enriched_Node_Type .gt. 0)then 
    ...
    if(Key_InPlane_Growth== 0 ) then
        call D3_Get_Signed_Dis_to_Crack_Mesh(Global_coor_Gauss,i_C, Check_Ball_R,c_Distance,c_Signed_Dis_v2, &
        c_Yes_Node_PER_in_FS,c_PER_Node_to_FS(1:3), Yes_Found_Min_Signed_Dis,c_n_Vector)
    elseif(Key_InPlane_Growth== 1) then
        call D3_Get_Signed_Dis_to_Crack_Mesh_for_InPlane_Growth(Global_coor_Gauss,i_C, Check_Ball_R,c_Distance, &
        c_Yes_Node_PER_in_FS,c_PER_Node_to_FS(1:3), Yes_Found_Min_Signed_Dis,c_n_Vector)
    endif
    ...
    do i_N = 1,8
        ...
        elseif (Enriched_Node_Type_3D(c_NN(i_N),i_C).eq.3)then
            ...
            elseif(Key_InPlane_Growth== 1) then
                call D3_Get_Signed_Dis_to_Crack_Mesh_for_InPlane_Growth(Global_coor_Gauss,neg_C, Check_Ball_R,c_Distance, &
                c_Yes_Node_PER_in_FS,c_PER_Node_to_FS(1:3), Yes_Found_Min_Signed_Dis,c_n_Vector)
            endif                       
            ...
            if(Key_InPlane_Growth== 0 ) then
                ...
                c_Yes_Node_PER_in_FS,c_PER_Node_to_FS(1:3), Yes_Found_Min_Signed_Dis_x0,c_n_Vector)
            elseif(Key_InPlane_Growth== 1) then
                call D3_Get_Signed_Dis_to_Crack_Mesh_for_InPlane_Growth(x0,neg_C, Check_Ball_R,c_Distance, &
                c_Yes_Node_PER_in_FS,c_PER_Node_to_FS(1:3), Yes_Found_Min_Signed_Dis,c_n_Vector)
            endif                         
