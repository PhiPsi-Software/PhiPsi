!-----------------------------------------------------------
! Brief: Compute N, dN/dxi, Jacobian and detJ for 2D quad.
!
! Parameters:
!   Input:  kesi, yita  - parent coords of point
!           X_NODES     - x-coords of 4 corner nodes
!           Y_NODES     - y-coords of 4 corner nodes
!   Output: detJ        - determinant of Jacobian
!           J           - 2x2 Jacobian matrix
!           N           - 2x8 shape function matrix
!           dNdkesi     - 4x2 derivatives wrt parent coords
!
! Notes:   Standard 4-node isoparametric quad mapping used
!          by element-level integration routines.
!-----------------------------------------------------------

subroutine Cal_N_dNdkesi_J_detJ(kesi,yita, X_NODES,Y_NODES, detJ,J,N,dNdkesi)
...
temp(1,1:4) =[(yita-ONE)/FOU,(-yita+ONE)/FOU, (yita+ONE)/FOU,(-yita-ONE)/FOU]
temp(2,1:4) =[(kesi-ONE)/FOU,(-kesi-ONE)/FOU, (kesi+ONE)/FOU,(-kesi+ONE)/FOU]
