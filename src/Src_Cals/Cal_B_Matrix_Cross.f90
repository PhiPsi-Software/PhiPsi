!-----------------------------------------------------------
! Brief: Assemble the B matrix for elements cut by a cross-shaped crack.
!
! Parameters:
!   Input:  kesi, yita        - local coordinates of the GP
!           i_Cross, i_E, i_G - cross, element, GP indices
...
!
! Notes:   Adds cross-intersection enrichment rows on top of the
...
if(maxval(Enriched_Node_Type_Cross(c_NN,i_Cross)).eq.0 .and. minval(Enriched_Node_Type_Cross(c_NN,i_Cross)).eq.0) then
