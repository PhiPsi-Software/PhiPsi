!-----------------------------------------------------------
! Brief: Assemble the global Biot coupling matrix for the FEM framework.
!
! Parameters:
!   Output: (no arguments; populates global Biot_c_MAT module array)
!
! Notes:   Pure-FEM coupling between displacement (2*nodes) and
!          pore-pressure (nodes) DoFs, ignoring XFEM enrichment.
!-----------------------------------------------------------
...
do i_E = 1,Num_Elem
    ...
    local_U=[c_NN(1)*2-1,c_NN(1)*2,c_NN(2)*2-1,c_NN(2)*2, c_NN(3)*2-1,c_NN(3)*2,c_NN(4)*2-1,c_NN(4)*2]
    ...
    do i_G = 1,Num_Gauss_P_FEM  
        ...
        Biot_c_MAT(local_U,local_P) = Biot_c_MAT(local_U,local_P)  + c_temp(1:8,1:4)*c_thick*weight(i_G)*detJ
    end do
enddo
