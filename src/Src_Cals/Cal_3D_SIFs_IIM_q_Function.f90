!-----------------------------------------------------------
! Brief: Evaluate the 3D IIM weight function and its gradient.
!
! Parameters:
!   Input:  Elem_ID    - element containing the Gauss point
!           q_nodal    - nodal q values on the element
!           N, dN_d*   - shape functions and their derivatives
!           GP_coords  - global coordinates of the Gauss point
!           Tip_Coords - crack-front tip coordinates
!           e1, e2, e3 - local basis at the tip
!           R_in, R_out, L_f - domain dimensions
!   Output: q_val      - weight function value at GP
!           grad_q     - gradient of the weight function at GP
!
! Notes:   Uses direct analytical evaluation in local frame for
!          accuracy rather than nodal interpolation.
!-----------------------------------------------------------
