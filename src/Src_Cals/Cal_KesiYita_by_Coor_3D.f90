!-----------------------------------------------------------
! Brief: Invert the HEX8 isoparametric map: (x,y,z) -> (xi,eta,zeta).
!
! Parameters:
!   Input:  Point(3)   - global point coordinates
!           c_elem     - HEX8 element index
!   Output: Kesi       - local isoparametric xi coordinate
!           Yita       - local isoparametric eta coordinate
!           Zeta       - local isoparametric zeta coordinate
!
! Notes:   Solves the trilinear HEX8 mapping by the gradient
!          (steepest-descent) method; initial guess is (0,0,0).
!-----------------------------------------------------------

subroutine Cal_KesiYita_by_Coor_3D(Point,c_elem,Kesi,Yita,Zeta)
!     Calculate the local coordinate system (3D) based on coordinates; the derivation can be found in my notes V3-177.
...
call Slove_NOLE_Gradient_Method_3D(3,EPS,X,Y,FS_3D,L, a1,a2,a3,a4,a5,a6,a7,a8, b1,b2,b3,b4,b5,b6,b7,b8, &
...
subroutine Slove_NOLE_Gradient_Method_3D(N,EPS,X,Y,FS_3D,L, a1,a2,a3,a4,a5,a6,a7,a8, b1,b2,b3,b4,b5,b6,b7,b8, &
c1,c2,c3,c4,c5,c6,c7,c8, Point)
use Global_Float_Type
implicit none    
dimension X(N),Y(N)
...
L=1000
5 call fs_3d(x,n,f,y,a1,a2,a3,a4,a5,a6,a7,a8, b1,b2,b3,b4,b5,b6,b7,b8, c1,c2,c3,c4,c5,c6,c7,c8,point)
if (F>=EPS) then
    L=L-1
    if (L==0) return
    D=ZR
...
subroutine FS_3D(X,N,F,Y, a1,a2,a3,a4,a5,a6,a7,a8, b1,b2,b3,b4,b5,b6,b7,b8, c1,c2,c3,c4,c5,c6,c7,c8, Point)
...
F1= a1*X(1)*X(2)*X(3)+a2*X(1)*X(2)+a3*X(1)*X(3)+a4*X(2)*X(3) +a5*X(1) + a6*X(2) + a7*X(3) + a8 -Point(1)
F2= b1*X(1)*X(2)*X(3)+b2*X(1)*X(2)+b3*X(1)*X(3)+b4*X(2)*X(3) +b5*X(1) + b6*X(2) + b7*X(3) + b8 -Point(2)
F3= c1*X(1)*X(2)*X(3)+c2*X(1)*X(2)+c3*X(1)*X(3)+c4*X(2)*X(3) +c5*X(1) + c6*X(2) + c7*X(3) + c8 -Point(3)
