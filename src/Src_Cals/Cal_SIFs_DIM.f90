!-----------------------------------------------------------
! Brief: Compute 2D SIFs (KI, KII) by the displacement method.
!
! Parameters:
!   Input:  iter    - current load step / iteration index
!           c_DISP  - global displacement vector
!
! Notes:   Samples the displacement jump at three radii behind
!          each crack tip and solves the 3x3 DIM linear system
!          to extract KI, KII. Results are written to the
!          public KI/KII module-level variables.
!-----------------------------------------------------------

subroutine Cal_SIFs_DIM(iter,c_DISP)
!Calculating stress intensity factors using the displacement interpolation method.
...
real(kind=FT) cr_Tip(2,2),omega_Tip(2),delta_L, c_E,c_v,c_G,k,offset_delta,Ux,Uy,Dx,Dy, &
Disp_U(2),Disp_D(2),delta_Disp_x,delta_Disp_y, delta_Disp_x_CRACK,delta_Disp_y_CRACK
integer c_Elem
real(kind=FT) c_Kesi,c_Yita
real(kind=FT) c_Ave_Length
real(kind=FT) x,y

real(kind=FT) delta_Disp_x1,delta_Disp_y1,delta_Disp_x1_CRACK,delta_Disp_y1_CRACK
real(kind=FT) delta_Disp_x2,delta_Disp_y2,delta_Disp_x2_CRACK,delta_Disp_y2_CRACK
real(kind=FT) delta_Disp_x3,delta_Disp_y3,delta_Disp_x3_CRACK,delta_Disp_y3_CRACK   
...
do i_C =1,num_Crack
    ...
    do i_Tip = 1,2
        ...
        call Cal_Length_of_ELe_by_Coors_Ave(cr_Tip(i_Tip,1),cr_Tip(i_Tip,2),c_Ave_Length)
        ...
        call Cal_KesiYita_by_Coor([Ux1,Uy1],c_Elem,c_Kesi,c_Yita)
        call Cal_Any_Point_Disp_KesiYita(c_Elem,c_Kesi,c_Yita,0,c_DISP,Disp_U1)
        call Cal_Ele_Num_by_Coors(Dx1,Dy1,c_Elem)
        call Cal_KesiYita_by_Coor([Dx1,Dy1],c_Elem,c_Kesi,c_Yita)
        call Cal_Any_Point_Disp_KesiYita(c_Elem,c_Kesi,c_Yita,0,c_DISP,Disp_D1)

        call Cal_Ele_Num_by_Coors(Ux2,Uy2,c_Elem)
        call Cal_KesiYita_by_Coor([Ux2,Uy2],c_Elem,c_Kesi,c_Yita)
        call Cal_Any_Point_Disp_KesiYita(c_Elem,c_Kesi,c_Yita,0,c_DISP,Disp_U2)
        call Cal_Ele_Num_by_Coors(Dx2,Dy2,c_Elem)
        call Cal_KesiYita_by_Coor([Dx2,Dy2],c_Elem,c_Kesi,c_Yita)
        call Cal_Any_Point_Disp_KesiYita(c_Elem,c_Kesi,c_Yita,0,c_DISP,Disp_D2)
        call Cal_Ele_Num_by_Coors(Ux3,Uy3,c_Elem)
        call Cal_KesiYita_by_Coor([Ux3,Uy3],c_Elem,c_Kesi,c_Yita)
        call Cal_Any_Point_Disp_KesiYita(c_Elem,c_Kesi,c_Yita,0,c_DISP,Disp_U3)
        call Cal_Ele_Num_by_Coors(Dx3,Dy3,c_Elem)
        call Cal_KesiYita_by_Coor([Dx3,Dy3],c_Elem,c_Kesi,c_Yita)
        call Cal_Any_Point_Disp_KesiYita(c_Elem,c_Kesi,c_Yita,0,c_DISP,Disp_D3)
        delta_Disp_x1 = Disp_U1(1)-Disp_D1(1)
        delta_Disp_y1 = Disp_U1(2)-Disp_D1(2)

        delta_Disp_x1_CRACK =  delta_Disp_x1*cos(omega_Tip(i_Tip)) + delta_Disp_y1*sin(omega_Tip(i_Tip))
        delta_Disp_y1_CRACK = -delta_Disp_x1*sin(omega_Tip(i_Tip)) + delta_Disp_y1*cos(omega_Tip(i_Tip))
        KI_r1   = sqrt(2*pi)*c_G/(1+k)*delta_Disp_y1_CRACK/sqrt(r_1)
        KII_r1  = sqrt(2*pi)*c_G/(1+k)*delta_Disp_x1_CRACK/sqrt(r_1)  

        delta_Disp_x2 = Disp_U2(1)-Disp_D2(1)
        delta_Disp_y2 = Disp_U2(2)-Disp_D2(2)

        delta_Disp_x2_CRACK =  delta_Disp_x2*cos(omega_Tip(i_Tip)) + delta_Disp_y2*sin(omega_Tip(i_Tip))
        delta_Disp_y2_CRACK = -delta_Disp_x2*sin(omega_Tip(i_Tip)) + delta_Disp_y2*cos(omega_Tip(i_Tip))
        KI_r2   = sqrt(2*pi)*c_G/(1+k)*delta_Disp_y2_CRACK/sqrt(r_2)

        KII_r2  = sqrt(2*pi)*c_G/(1+k)*delta_Disp_x2_CRACK/sqrt(r_2)  
        delta_Disp_x3 = Disp_U3(1)-Disp_D3(1)
        delta_Disp_y3 = Disp_U3(2)-Disp_D3(2)

        delta_Disp_x3_CRACK =  delta_Disp_x3*cos(omega_Tip(i_Tip)) + delta_Disp_y3*sin(omega_Tip(i_Tip))
        delta_Disp_y3_CRACK = -delta_Disp_x3*sin(omega_Tip(i_Tip)) + delta_Disp_y3*cos(omega_Tip(i_Tip))
        KI_r3   = sqrt(2*pi)*c_G/(1+k)*delta_Disp_y3_CRACK/sqrt(r_3)
        KII_r3  = sqrt(2*pi)*c_G/(1+k)*delta_Disp_x3_CRACK/sqrt(r_3) 
