!-----------------------------------------------------------
! Brief: Compute 2D XFEM stress at a point in a quad element.
!
! Parameters:
!   Input:  c_Elem       - element number
!           in_Kesi      - local coordinate (xi)
!           in_Yita      - local coordinate (eta)
!           i_G          - Gauss point index
!           c_DISP       - global displacement vector
!   Output: Sxx_GP, Syy_GP, Sxy_GP - 2D stress components
!
! Notes:   XFEM-aware; for pure FEM the result is zero. Supports
!          Weibull elastic-modulus distribution and composite mats.
!-----------------------------------------------------------

subroutine Cal_Any_Point_Stress_KesiYita(c_Elem,in_Kesi, in_Yita,i_G, c_DISP, Sxx_GP, Syy_GP, Sxy_GP)
! Calculate the stress at a certain point (usually a Gauss point), given the element number and
! local coordinates, among other information.
! Note: Only used for cases with XFEM enhancement. For FEM analysis, the results of this subroutine
! are 0!
...
real(kind=FT) c_X_NODES(4),c_Y_NODES(4), c_Stress(3),B(3,80), tem_B(3,80)
...
if(num_Crack/=0)then
    do i_C =1,num_Crack 
        call Location_Element_Stiff_Matrix(c_Elem,i_C, c_POS(:,i_C), Location_ESM_C_Crack, num_Loc_ESM_C_Crack, &
...
if(num_Hole/=0)then
    do i_H =1,num_Hole
        call Location_Element_Stiff_Matrix_Hl(c_Elem,i_H, c_POS_Hl(:,i_H), Location_ESM_C_Crack, num_Loc_ESM_C_Crack, &
        Location_ESM_C_Cr_NoFEM, num_Loc_ESM_C_Cr_NoFEM)
        Location_ESM(num_Loc_ESM+1: num_Loc_ESM+num_Loc_ESM_C_Crack) = Location_ESM_C_Crack(1:num_Loc_ESM_C_Crack)
        num_Loc_ESM  =  num_Loc_ESM + num_Loc_ESM_C_Crack
    end do
...
if(num_Cross/=0)then
    do i_Cross =1,num_Cross
        call Location_Element_Stiff_Matrix_Cross(c_Elem,i_Cross, c_POS_Cross(:,i_Cross), Location_ESM_C_Crack, &
...
if(num_Inclusion/=0)then
    do i_Incl =1,num_Inclusion
        call Location_Element_Stiff_Matrix_Incl(c_Elem,i_Incl, c_POS_Incl(:,i_Incl), Location_ESM_C_Crack, num_Loc_ESM_C_Crack, &
        Location_ESM_C_Cr_NoFEM, num_Loc_ESM_C_Cr_NoFEM)
        Location_ESM(num_Loc_ESM+1: num_Loc_ESM+num_Loc_ESM_C_Crack) = Location_ESM_C_Crack(1:num_Loc_ESM_C_Crack)
        num_Loc_ESM  =  num_Loc_ESM + num_Loc_ESM_C_Crack
    end do
...
if(num_Crack/=0)then
    do i_C =1,num_Crack 
        call Cal_B_Matrix_Crack(in_Kesi,in_Yita,i_C,c_Elem,i_G, c_NN,c_X_NODES,c_Y_NODES, tem_B,num_tem_B)
...
if(num_Hole/=0)then
    do i_H =1,num_Hole 
        call Cal_B_Matrix_Hl(in_Kesi,in_Yita, i_H,c_Elem,i_G, c_NN,c_X_NODES,c_Y_NODES, tem_B,num_tem_B)
        B(1:3,num_B+1:num_B+num_tem_B) = tem_B(1:3,1:num_tem_B)
        num_B = num_B + num_tem_B
    end do
endif
if(num_Cross/=0)then
    do i_Cross =1,num_Cross
        call Cal_B_Matrix_Cross(in_Kesi,in_Yita, i_Cross,c_Elem,i_G, c_NN,c_X_NODES,c_Y_NODES, tem_B,num_tem_B)
        B(1:3,num_B+1:num_B+num_tem_B) = tem_B(1:3,1:num_tem_B)
        num_B = num_B + num_tem_B
    end do
endif
if(num_Circ_Incl/=0)then
    do i_Incl =1,num_Circ_Incl 
        call Cal_B_Matrix_Circ_Incl(in_Kesi,in_Yita, i_Incl,c_Elem,i_G, c_NN,c_X_NODES,c_Y_NODES, tem_B,num_tem_B)
        B(1:3,num_B+1:num_B+num_tem_B) = tem_B(1:3,1:num_tem_B)
        num_B = num_B + num_tem_B
    end do
...
if(num_Poly_Incl/=0)then
    do i_Incl =1,num_Poly_Incl 
        call Cal_B_Matrix_Poly_Incl(in_Kesi,in_Yita, i_Incl,c_Elem,i_G, c_NN,c_X_NODES,c_Y_NODES, tem_B,num_tem_B)
        B(1:3,num_B+1:num_B+num_tem_B) = tem_B(1:3,1:num_tem_B)
        num_B = num_B + num_tem_B
    end do
    call Tool_Yes_Point_in_Inclusions(c_G_x,c_G_y, Yes_Gauss_in_Incl,c_Incl_Num)
...
if(Key_InSitu_Strategy==2)then
    c_Stress(1)=c_Stress(1) + sum(InSitu_Strs_Gaus_xx(c_Elem,1:Num_Gauss_P_FEM))/ Num_Gauss_P_FEM
    c_Stress(2)=c_Stress(2) + sum(InSitu_Strs_Gaus_yy(c_Elem,1:Num_Gauss_P_FEM))/ Num_Gauss_P_FEM
    c_Stress(3)=c_Stress(3) + sum(InSitu_Strs_Gaus_xy(c_Elem,1:Num_Gauss_P_FEM))/ Num_Gauss_P_FEM
endif


if(Key_Thermal_Stress==1)then
    c_T_Alpha = T_Alpha(Elem_Mat(c_Elem))
    c_TStress = c_T_Alpha*Thermal_Str_Temper(Elem_Mat(c_Elem))* MATMUL(c_D,[ONE,ONE,ZR])
