!-----------------------------------------------------------
! Brief: Compute the 3D XFEM displacement at a point in a hex element.
!
! Parameters:
!   Input:  in_Elem    - element number
!           c_Kesi     - local coordinate (xi)
!           c_Yita     - local coordinate (eta)
!           c_Zeta     - local coordinate (zeta)
!           c_DISP     - global displacement vector
!   Output: P_Disp     - displacement vector (3 components)
!
! Notes:   Supports 3D crack enrichments with signed-distance
!          computations and material-type-aware stiffness handling.
!-----------------------------------------------------------
...
real(kind=FT) c_X_NODES(8),c_Y_NODES(8),c_Z_NODES(8), detJ,dNdkesi(8,3),J(3,3),N(3,24),c_N(8),Gauss_P(3)
...
if(Num_Crack /=0)then
    do i_C=1,Num_Crack
        ...
        do i_Crack_Ele=1,Crack3D_Meshed_Ele_num(i_C)
            ...
            (Gauss_P,Point1,Point2,Point3, Tri123_Distance,Tri123_PER,Tri123_Yes_PER_in,Tri123_Yes_PER_on)
        ...
        do i_N = 1,8
            ...
            elseif (Enriched_Node_Type_3D(NODES_iE(i_N),i_C).eq.3)then
                ...
                call D3_Get_Signed_Dis_to_Crack_Mesh(Gauss_P,neg_C,Check_Ball_R, c_Distance,c_Signed_Dis_v2,c_Yes_Node_PER_in_FS, &
                ...
                call D3_Get_Signed_Dis_to_Crack_Mesh(Gauss_P,i_C,Check_Ball_R, c_Distance,c_Signed_Dis_v2,c_Yes_Node_PER_in_FS, &
                c_PER_Node_to_FS(1:3),Yes_Found_Min_Signed_Dis,c_n_Vector)
                distance_Gauss_sm = c_Distance
                call D3_Get_Signed_Dis_to_Crack_Mesh(x0,neg_C,Check_Ball_R, c_Distance,c_Signed_Dis_v2,c_Yes_Node_PER_in_FS, &
                c_PER_Node_to_FS(1:3),Yes_Found_Min_Signed_Dis,c_n_Vector)
