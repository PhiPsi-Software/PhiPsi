!-----------------------------------------------------------
! Brief: Build Gauss points and weights for a 2D element
!        subdivided into many sub-quadrilaterals.
!
! Parameters:
!   Input:  Num_QUAD - number of sub-quadrilaterals
!   Output: kesi     - local xi coordinates
!           yita     - local eta coordinates
!           weight   - integration weights
!
! Notes:   Subdivision scheme: parent element is split
!          into sqrt(Num_QUAD) x sqrt(Num_QUAD) squares,
!          each integrated with four 2x2 Gauss points.
!-----------------------------------------------------------

subroutine Cal_Gauss_Points_QUAD_for_SUBQUAD(Num_QUAD, kesi,yita,weight)
!     Integration Scheme 3: Local Coordinates and Weight Coefficients of Gaussian Points for Quadrilateral Subdivision Rules
!     Theoretical References: Extended Finite Element Method: Theory and Applications_2015 New XFEM Book_P58_Figure 2.14

use Global_Float_Type      
implicit none
integer,intent(in)::Num_QUAD
real(kind=FT),intent(out)::kesi(Num_QUAD*4), yita(Num_QUAD*4), weight(Num_QUAD*4)
integer i,j
