!-----------------------------------------------------------
! Brief: Compute the 3D aperture vector at an arbitrary point on a crack.
!
! Parameters:
!   Input:  c_DISP                 - global displacement vector
!           Crack_Number           - crack index
!           Crack_Point            - 3D point coordinates
!           Fluid_element_number   - optional hint: containing fluid element
!           Fluid_node_number      - optional hint: containing fluid node
!           Crack_mesh_ele_number  - optional hint: meshed element
!           Crack_mesh_node_number - optional hint: meshed node
!   Output: Crack_Point_Aperture   - 3D aperture vector in global frame
!
! Notes:   Locates the point on the discretized crack surface, projects
!          above/below along the local normal, retrieves displacements,
!          and returns the relative opening vector. Hint parameters can
!          be 0 when the location is unknown (will be searched).
!-----------------------------------------------------------
...
real(kind=FT) c_mesh_element_points_1(3),c_mesh_element_points_2(3),c_mesh_element_points_3(3)
...
enddo
call Tool_Get_Nearest_Point_on_3D_Polygon (Num_CrMesh_Outlines,c_Polygon(1:Num_CrMesh_Outlines,1:3), &
...
call Tool_ThetaX_ThetaY_ThetaZ_3D_rotation(1, Tip_vector_x, Tip_vector_y, Tip_vector_z, &
Vector_Crack_x(1:3),Vector_Crack_y(1:3),Vector_Crack_z(1:3), ThetaX,ThetaY,ThetaZ,c_T_Matrx(1:3,1:3))
...
if(Crack_mesh_ele_number==0) then
    do i_Crack_Ele = 1,Crack3D_Meshed_Ele_num(Crack_Number) 
        ...
        c_mesh_element_points_1(1:3) = Crack3D_Meshed_Node(Crack_Number)%row(c_mesh_node(1),1:3)
        c_mesh_element_points_2(1:3) = Crack3D_Meshed_Node(Crack_Number)%row(c_mesh_node(2),1:3)
        c_mesh_element_points_3(1:3) = Crack3D_Meshed_Node(Crack_Number)%row(c_mesh_node(3),1:3)
        call Tool_Yes_Point_on_3D_Triangle_with_Tol(Crack_Point, &
        c_mesh_element_points_1,c_mesh_element_points_2,c_mesh_element_points_3, Yes_on,Yes_Point_on_3D_Triangle_Tol)
        if(Yes_on) then
