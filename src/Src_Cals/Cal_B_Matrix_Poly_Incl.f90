!-----------------------------------------------------------
! Brief: Assemble the B matrix for elements cut by a polygonal inclusion.
!
! Parameters:
!   Input:  kesi, yita        - local coordinates of the GP
!           i_Incl, i_E, i_G  - inclusion, element, GP indices
...
!
! Notes:   Adds polygonal-inclusion enrichment rows on top of the
!          standard FEM B-matrix; tracks per-GP assembly state.
!-----------------------------------------------------------

subroutine Cal_B_Matrix_Poly_Incl(kesi,yita, i_Incl,i_E,i_G, c_NN,c_X_NODES,c_Y_NODES, tem_B,num_tem_B)

!     Calculate the B matrix (for polygonal inclusions).
...
call Cal_Coor_by_KesiYita(kesi,yita,c_X_NODES,c_Y_NODES, c_G_x,c_G_y)
...
if(maxval(Enriched_Node_Type_Incl(c_NN,i_Incl)).eq.0 .and. minval(Enriched_Node_Type_Incl(c_NN,i_Incl)).eq.0) then
    if (i_Incl.eq.1) then
        tem_B(1:3,1:8) = B_FEM
        num_tem_B = 8
    else
...
elseif(maxval(Enriched_Node_Type_Incl(c_NN,i_Incl)).gt.0 .or. minval(Enriched_Node_Type_Incl(c_NN,i_Incl)).gt.0) then
    B_XFEM(1:3,1:60) = ZR
    num_B_XFEM = 0
    ...
    do i_N = 1,4 
        if (Enriched_Node_Type_Incl(c_NN(i_N),i_Incl).eq.1)then
            if (Elem_Type_Incl(i_E,i_Incl).eq.1) then
                Incl_gp = Za-abs(Zm)                 
                Ex =     dNdx(1,1)*Zeta_Node_abs(1)+ dNdx(2,1)*Zeta_Node_abs(2)+ &
                dNdx(3,1)*Zeta_Node_abs(3)+ dNdx(4,1)*Zeta_Node_abs(4)- &
                Zmn*(dNdx(1,1)*Zeta_Node(1)+ dNdx(2,1)*Zeta_Node(2)+ dNdx(3,1)*Zeta_Node(3)+ dNdx(4,1)*Zeta_Node(4))
                Ey =     dNdx(1,2)*Zeta_Node_abs(1)+ dNdx(2,2)*Zeta_Node_abs(2)+ &
                dNdx(3,2)*Zeta_Node_abs(3)+ dNdx(4,2)*Zeta_Node_abs(4)- &
                Zmn*(dNdx(1,2)*Zeta_Node(1)+ dNdx(2,2)*Zeta_Node(2)+ dNdx(3,2)*Zeta_Node(3)+ dNdx(4,2)*Zeta_Node(4))
                BI_enr(1,:)=[dNdx(i_N,1)*Incl_gp+c_N(i_N)*Ex,ZR]
                BI_enr(2,:)=[ZR,dNdx(i_N,2)*Incl_gp+c_N(i_N)*Ey]
                BI_enr(3,:)=[dNdx(i_N,2)*Incl_gp+c_N(i_N)*Ey, dNdx(i_N,1)*Incl_gp+c_N(i_N)*Ex]
            else
                BI_enr(1:3,1:2)=ZR
            end if
...
return
end subroutine Cal_B_Matrix_Poly_Incl
