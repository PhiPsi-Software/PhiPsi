!-----------------------------------------------------------
! Brief: Perform backtracking line search along the NR
!        search direction for the coupled HF iteration.
!
! Parameters:
!   Input:  ifra, iter, Counter_Iter - step / iteration IDs
!           num_ALlDOF, c_Total_FD, num_FreeD, num_free_CalP
!           Total_freeDOF, freeDOF_HF, Local_freeDOF_HF
!           NR_Deri, delta_x, c_R, Initial_DISP, c_DISP
!           c_freeDOF, c_CalP_Pres, c_globalK, Coupled_Q, F_U
!           c_Temp_Cr_CalP_Aper, c_Temp_total_Time
!   In/Out: Num_Line_Search, total_Time
!   Output: x - accepted step (search direction scaled)
!
! Notes:   Standard Armijo backtracking with ALF and TOLX
!          line-search parameters; updates total_Time as the
!          trial step is accepted.
!-----------------------------------------------------------

subroutine Cal_HF_Line_Searching(ifra,iter,Counter_Iter, num_ALlDOF,c_Total_FD,num_FreeD,num_free_CalP,Total_freeDOF, &
c_num_Tol_CalP_Water,num_Total_FD,freeDOF_HF, Local_freeDOF_HF,NR_Deri, delta_x, c_R,Initial_DISP, &
c_DISP,c_freeDOF,c_CalP_Pres,c_globalK,Coupled_Q,F_U, c_Temp_Cr_CalP_Aper,c_Temp_total_Time, &
Num_Line_Search,total_Time,x)
! Hydraulic Fracturing Newton-Raphson Iterative Line Search Process
!num_ALlDOF = c_Total_FD + num_Tol_CalP_Water
!     ----------------------------
...
print *,'    Starting Line searches ' // 'and backtracking...'
print *,'      ~~~~Prepare data for Line searches ' // 'and backtracking...'
...
x_old(num_FreeD+1:num_FreeD+num_free_CalP) = c_CalP_Pres(Local_freeDOF_HF(1:num_free_CalP))
do i=1,num_Total_FD
    c_Sum = ZR
    do j=1,num_Total_FD
        c_i = Total_freeDOF(i)
...
if (slope > ZR) then
    print *,'          Wrong slope value in line ' //'searching process!'
...
Try_CalP_Pres(Local_freeDOF_HF(1:num_free_CalP))= x(num_FreeD+1:num_Total_FD)
call Cal_Crack_Aperture(Counter_Iter,Try_DISP)
call Cal_HF_delta_Time_Linear(Counter_Iter, c_Temp_Cr_CalP_Aper,total_Time,delta_Time)
...
if(iter==1)then
    ...
    call Read_Disp_to(Counter_Iter-1,Last_DISP, c_Total_FD)
endif
call Cal_HF_Resid(ifra,2,Counter_Iter,c_R_new, c_Total_FD,num_FreeD,num_free_CalP,c_globalK, &
Coupled_Q,F_U,Try_DISP,Last_DISP, c_freeDOF,freeDOF_HF,Local_freeDOF_HF, Try_CalP_Pres,delta_Time,H,c_S)
...
if(F_c < F_old + ALF*alam*slope) then
    ...
    print *,'              value of F is too big,' //' goto the next backtracking'
