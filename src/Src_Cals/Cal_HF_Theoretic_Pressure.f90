!-----------------------------------------------------------
! Brief: Compute theoretical pressure/aperture for HF (KGD).
!
! Parameters:
!   Input:  iter           - current iteration counter
!           Counter_Iter   - outer counter
!           Inject_Crack_L - half-length of injection crack
!   Output: Theor_Time     - elapsed time at this crack length
!           Theor_Press    - theoretical fluid pressure
!           Theor_Aper     - theoretical maximum aperture
!
! Notes:   Viscosity-dominated (KGD) analytical solution. Prints
!          the theoretical pressure in MPa for monitoring.
!-----------------------------------------------------------

subroutine Cal_HF_Theoretic_Pressure(iter,Counter_Iter, Inject_Crack_L,Theor_Time,Theor_Press,Theor_Aper)
...
Theor_Time=((Inject_Crack_L/0.616D0)**SIX*u_1/E_1/(Q_0**THR)) **(ONE/FOU)
...
Theor_Aper = ((u_1/E_1/Theor_Time)**(ONE/THR))* Inject_Crack_L*tem
