!-----------------------------------------------------------
! Brief: Compute 3D stress or strain at a point in a hex element.
!
! Parameters:
!   Input:  c_Elem       - element number
!           i_N          - node number (or 0 for Gauss-based)
!           Key_to_cal   - 1 for stress, 2 for strain
!           Key_CoorSys  - 1 Cartesian, 2 cylindrical
!           in_Kesi, in_Yita, in_Zeta - local coordinates
!           i_G          - Gauss point index
!           c_DISP       - global displacement vector
!   Output: Sxx_GP, Syy_GP, Szz_GP - normal components
!           Sxy_GP, Syz_GP, Sxz_GP - shear components
!
! Notes:   XFEM-aware; for pure FEM analyses the result is zero.
!-----------------------------------------------------------

subroutine Cal_Any_Point_Str_KesiYita_3D(c_Elem,i_N,Key_to_cal, Key_CoorSys, in_Kesi, in_Yita,in_Zeta,i_G,c_DISP, &
Sxx_GP,Syy_GP,Szz_GP, Sxy_GP,Syz_GP,Sxz_GP)
! Calculate the stress or strain at a certain point (usually a Gauss point, but can also calculate
! at a node), given the element number and local coordinates, among other information.
! 3D.
! Consider XFEM.
...
real(kind=FT) c_X_NODES(8),c_Y_NODES(8),c_Z_NODES(8), c_Str(6),B(6,MDOF_3D), tem_B(6,MDOF_3D)
...
if(num_Crack/=0)then
    do i_C =1,num_Crack
        ...
        call Location_Element_Stiff_Matrix_3D(c_Elem,i_C, c_POS_3D_c_Ele(1:8), Location_ESM_C_Crack, num_Loc_ESM_C_Crack, &
...
if(num_Crack/=0)then
    do i_C =1,num_Crack 
        call Cal_B_Matrix_Crack_3D(in_Kesi,in_Yita,in_Zeta, i_C,c_Elem,i_G, c_NN,c_X_NODES,c_Y_NODES,c_Z_NODES, tem_B,num_tem_B)
...
if(Key_to_cal==1)then
    if (Key_CoorSys==1) then
        c_Str=MATMUL(MATMUL(c_D,B(1:6,1:num_Loc_ESM)), U(1:num_Loc_ESM))
...
elseif(Key_to_cal==2)then
    ...
    elseif (Key_CoorSys==2) then            
        Tem_Str=MATMUL(MATMUL(c_D,B(1:6,1:num_Loc_ESM)), U(1:num_Loc_ESM))
        c_Theta     =  Theta_Cartesian_to_Cylinder(c_Elem,i_N)
        sTheta      =  sin(c_Theta)
        cTheta      =  cos(c_Theta)
...
if(Key_Thermal_Stress==1 .and. Key_to_cal==1)then
    ...
    c_TStress = c_T_Alpha*Elem_T_for_Stress(c_Elem)* MATMUL(c_D,[ONE,ONE,ONE,ZR,ZR,ZR])
...
if(Key_InSitu_Strategy==4  .and. Key_to_cal==1)then
    c_Stress = MATMUL(c_D,[ InSitu_Strain_Gaus_xx(c_Elem,1), InSitu_Strain_Gaus_yy(c_Elem,1), &
    InSitu_Strain_Gaus_zz(c_Elem,1), InSitu_Strain_Gaus_xy(c_Elem,1), InSitu_Strain_Gaus_yz(c_Elem,1), &
...
if(Key_InSitu_Strategy==2 .and. Key_to_cal==1)then
    c_Str(1)=c_Str(1) + sum(InSitu_Strs_Gaus_xx(c_Elem,1:Num_Gauss_P_FEM_3D))/ Num_Gauss_P_FEM_3D
    c_Str(2)=c_Str(2) + sum(InSitu_Strs_Gaus_yy(c_Elem,1:Num_Gauss_P_FEM_3D))/ Num_Gauss_P_FEM_3D
    c_Str(3)=c_Str(3) + sum(InSitu_Strs_Gaus_zz(c_Elem,1:Num_Gauss_P_FEM_3D))/ Num_Gauss_P_FEM_3D
    c_Str(4)=c_Str(4) + sum(InSitu_Strs_Gaus_xy(c_Elem,1:Num_Gauss_P_FEM_3D))/ Num_Gauss_P_FEM_3D
    c_Str(5)=c_Str(5) + sum(InSitu_Strs_Gaus_yz(c_Elem,1:Num_Gauss_P_FEM_3D))/ Num_Gauss_P_FEM_3D
    c_Str(6)=c_Str(6) + sum(InSitu_Strs_Gaus_xz(c_Elem,1:Num_Gauss_P_FEM_3D))/ Num_Gauss_P_FEM_3D
endif
