!-----------------------------------------------------------
! Brief: Assemble the contact Jacobian matrix for 2D penalty contact.
!
! Parameters:
...
!           c_Total_FD      - total free degrees of freedom
...
!   In/Out: Kn_Gauss        - normal penalty stiffness at contact Gauss points
!           Kt_Gauss        - tangential penalty stiffness at contact Gauss points
!   Output: CT_Jacobian     - assembled contact Jacobian matrix
!
! Notes:   Part of the penalty-function contact algorithm. Loops over
!          contact Gauss points and adds the contact contribution to the
!          stiffness matrix based on the current contact state.
!-----------------------------------------------------------

subroutine Cal_Contact_Jacobian( iter,ifra,Counter_Iter,i_NR_P, c_Total_FD,c_num_freeDOF,ori_globalK, &
...
real(kind=FT),intent(in):: ori_globalK(c_Total_FD,c_Total_FD),Kn
    ...
    do i_CT_Elem = 1,Num_Div_Points - 1
        ...
        do i_CT_GAUSS  = 1,num_CT_Gauss
            if(CT_State_Gauss(i_C,i_CT_Elem,i_CT_GAUSS) /= 0)then
                ...
                enddo
                CT_Jacobian(Local_W(1:2*num_Local_W), Local_W(1:2*num_Local_W)) = CT_Jacobian(Local_W(1:2*num_Local_W), &
                Local_W(1:2*num_Local_W)) + MATMUL(MATMUL(transpose(N_W(1:2,1:2*num_Local_W)), D_ep), N_W(1:2,1:2*num_Local_W)) &
