!-----------------------------------------------------------
! Brief: Compute N and detJ for a 3D hex (no Jacobian export).
!
! Parameters:
!   Input:  kesi, yita, zeta - parent coords of point
!           X/Y/Z_NODES      - coordinates of 8 hex nodes
!   Output: detJ             - determinant of Jacobian
!           N                - 3x24 shape function matrix
!
! Notes:   Lighter variant of Cal_N_dNdkesi_J_detJ_3D for
!          callers that only need N and the Jacobian measure.
!-----------------------------------------------------------

subroutine Cal_N_detJ_3D(kesi,yita,zeta, X_NODES,Y_NODES,Z_NODES, detJ,N)

!     This function calculates N and the determinant of Jacobian matrix.
...
temp(1,1:8)=[ -(ONE - yita)*(ONE - zeta)/EIG, (ONE - yita)*(ONE - zeta)/EIG, (ONE + yita)*(ONE - zeta)/EIG, &
-(ONE + yita)*(ONE - zeta)/EIG, -(ONE - yita)*(ONE + zeta)/EIG, (ONE - yita)*(ONE + zeta)/EIG, &
(ONE + yita)*(ONE + zeta)/EIG, -(ONE + yita)*(ONE + zeta)/EIG]

temp(2,1:8)=[ -(ONE - kesi)*(ONE - zeta)/EIG, -(ONE + kesi)*(ONE - zeta)/EIG, (ONE + kesi)*(ONE - zeta)/EIG, &
(ONE - kesi)*(ONE - zeta)/EIG, -(ONE - kesi)*(ONE + zeta)/EIG, -(ONE + kesi)*(ONE + zeta)/EIG, &
(ONE + kesi)*(ONE + zeta)/EIG, (ONE - kesi)*(ONE + zeta)/EIG]

temp(3,1:8)=[ -(ONE - kesi)*(ONE - yita)/EIG, -(ONE + kesi)*(ONE - yita)/EIG, -(ONE + kesi)*(ONE + yita)/EIG, &
-(ONE - kesi)*(ONE + yita)/EIG, (ONE - kesi)*(ONE - yita)/EIG, (ONE + kesi)*(ONE - yita)/EIG, &
(ONE + kesi)*(ONE + yita)/EIG, (ONE - kesi)*(ONE + yita)/EIG]
