!-----------------------------------------------------------
! Brief: Classify contact state at each contact-element Gauss point.
!
! Parameters:
!   Input:  iter, ifra, Counter_Iter, i_NR_P - iteration counters
!           c_DISP  - global displacement vector
!   Output: Yes_Contact - whether any contact is detected
!           c_Elem_Conta_Sta - per-element contact status
!           CT_State_Gauss   - per-Gauss-point contact state
!                              (0 separate, 1 bonded, 2 sliding)
!
! Notes:   Part of the penalty-function contact algorithm. Uses
!          a crack-opening tolerance scaled by average element length.
!-----------------------------------------------------------

subroutine Cal_Contact_Contact_State_Gauss( iter,ifra,Counter_Iter,i_NR_P, c_DISP,Yes_Contact,c_Elem_Conta_Sta, &
CT_State_Gauss)
!     Obtain the contact state of each Gauss point in the contact element and store it in CT_State_Gauss for the penalty function contact detection algorithm.
!     Also save the element contact status (c_Elem_Conta_Sta) for post-processing

!     integer CT_State_Gauss(num_Crack, Max_Num_Cr_CalP-1, 2)        !Contact status at two Gauss points of each contact element for each crack
! = 0, Separation; = 1, Bonding; = 2, Sliding
!     Yes_Contact indicates whether there is contact
...
do i_C=1,num_Crack
    ...
    do i_CT_Elem = 1,Num_Div_Points - 1
        ...
        do i_CT_GAUSS  = 1,num_CT_Gauss
            ...
            call Cal_Point_Aperture(i_C,[CT_GAUSS_x,CT_GAUSS_y], c_DISP,ori_CT_elem,c_Aperture)
