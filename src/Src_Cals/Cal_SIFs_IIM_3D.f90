...
!===============================================
! INTERACTION INTEGRAL METHOD FOR 3D CRACK SIFs
!===============================================
! **MODIFICATION 2026-01-13**: Enhanced integration domain with 3D q-function
! Added proper local coordinate system handling
!===============================================================================
...
use module_Cal_3D_SIFs_IIM_CrackFace_IntegralPoint_1_Point
use module_Cal_3D_SIFs_IIM_CrackFace_IntegralPoint_3_Points
use module_Cal_3D_SIFs_IIM_CrackFace_IntegralPoint_7_Points
...
real(kind=FT) c_density
real(kind=FT) :: Acl_elem(MDOF_3D)
real(kind=FT) :: Acl_gp_global(3), Acl_gp_local(3)
real(kind=FT) :: Integrand_inertia
...
do i_Crack = 1, num_Crack
    ...
    !$OMP PRIVATE(i_Vertex, Vertex_Node_ID, Tip_Coords,                  &
    !$OMP         e1_growth, e2_normal, e3_tangent,                       &
    !$OMP         Rotation_Global_to_Local,                               &
    !$OMP         Num_Domain_Elems, Domain_Elem_List, q_nodal,            &
    !$OMP         Elem_at_Tip, Material_ID,                               &
    !$OMP         Young_Modulus, Poisson_Ratio, Shear_Modulus,            &
    !$OMP         E_prime, Kappa_3D,                                      &
    !$OMP         M_integral, M_integral_Elements,                        &
    !$OMP         Element_Search_Cache,                                   &
    !$OMP         i_Elem, i_GP, i_Mode, i_C, i, j, jj, i_GP_face,        &
    !$OMP         Current_Elem, Node_IDs, Node_X, Node_Y, Node_Z,         &
    !$OMP         Constitutive_Matrix, c_density,                         &
    !$OMP         Node_i, DOF_indices, Element_DOF_values, dof_index,     &
    ...
    !$OMP         U_elem, Acl_elem, Acl_gp_global, Acl_gp_local,          &
    !$OMP         Integrand_inertia,                                       &
    !$OMP         Shape_Functions, dN_dxi, dN_deta, dN_dzeta,             &
    !$OMP         Jacobian_Matrix, Det_Jacobian, Inv_Jacobian,             &
    !$OMP         dN_dx, dN_dy, dN_dz,                                    &
    !$OMP         GP_coords_global, GP_coords_local,                      &
    !$OMP         q_value_at_GP, grad_q_global, grad_q_local,             &
    !$OMP         Strain_Voigt, Stress_Voigt, Temp_Stress_Voigt,          &
    !$OMP         Stress_Tensor, Strain_Tensor, Grad_u_Tensor,             &
    !$OMP         Stress_Local, Strain_Local, Grad_u_Local,               &
    !$OMP         Stress_from_pressure,                                    &
    !$OMP         r_polar, theta_polar,                                   &
    !$OMP         Aux_Stress_Local, Aux_Strain_Local, Aux_Grad_u_Local,   &
    !$OMP         dV, Wint, A, Integrand_value, Term_W,                   &
    !$OMP         B, tem_B, num_B, num_tem_B,                             &
    !$OMP         Integrand_value1, Integrand_value2,                     &
    ...
    !$OMP         Aux_ddu1_dX3dX1, Aux_ddu2_dX3dX1, Aux_ddu3_dX3dX1,    &
    !$OMP         fac_two_faces, nGP_face, n_unit, t_global, t_local,    &
    !$OMP         GP_face_global, GP_face_w, GP_face_n_global,            &
    !$OMP         q_face, Integrand_face, Normal_Stress,                  &
    !$OMP         q_line_integral, L_eff, nrm_n,                          &
    !$OMP         temp, temp1, temp2, c_File_name_test)
    do i_Vertex = 1, num_Front_Vertices
        ...
        if(Key_Save_Nothing /= 1) then
            if (Key_Save_3D_SIFs_IIM_Integral_cylinders == 1) then
                ...
                c_File_name_test = trim(Full_Pathname)//'.integral_cylinder_' &
                    //ADJUSTL(temp)//'_'//ADJUSTL(temp1)//'_'//ADJUSTL(temp2)
                call Tool_chrpak_s_blank_delete(c_File_name_test)
                !$OMP CRITICAL (crit_file_cylinder)
                open(101,file=c_File_name_test,status='unknown')
                write(101, '(15E20.12)') Tip_Coords,e1_growth,e2_normal,e3_tangent, &
                                         L_front,R_inner,R_outer
                close(101)
                !$OMP END CRITICAL (crit_file_cylinder)
            endif
        endif
        ...
        call Cal_3D_SIFs_IIM_Integration_Domain(Tip_Coords, e1_growth, e2_normal, e3_tangent, &
             R_inner, R_outer, L_front, Num_Domain_Elems, Domain_Elem_List, q_nodal)
        ...
        do i_Elem = 1, Num_Domain_Elems
            ...
            c_density = Material_Para(Material_ID,3)
            ...
            if(num_Crack/=0)then
                do i_C = 1, num_Crack
                    ...
                    call Location_Element_Stiff_Matrix_3D(Current_Elem,i_C, c_POS_3D_c_Ele(1:8), &
            ...
            if (Key_Analysis_Type==6) then
                Acl_elem(1:MDOF_3D) = ZR
                Acl_elem(1:num_Loc_ESM) = Dy_ACCL(Location_ESM(1:num_Loc_ESM))
            endif
            ...
            do i_GP = 1, nGP
                ...
                call Cal_3D_SIFs_IIM_q_Function(Current_Elem, q_nodal, Shape_Functions, &
                     dN_dx, dN_dy, dN_dz, GP_coords_global, Tip_Coords, &
                     e1_growth, e2_normal, e3_tangent, R_inner, R_outer, L_front, &
                ...
                if(num_Crack/=0)then
                    do i_C = 1, num_Crack
                        call Cal_B_Matrix_Crack_3D(GP_xi(i_GP), GP_eta(i_GP), GP_zeta(i_GP), &
                             i_C, Current_Elem, i_GP, Node_IDs, Node_X, Node_Y, Node_Z, &
                             tem_B, num_tem_B)
                ...
                Stress_Local  = matmul(matmul(Rotation_Global_to_Local, Stress_Tensor), &
                                       transpose(Rotation_Global_to_Local))
                Strain_Local  = matmul(matmul(Rotation_Global_to_Local, Strain_Tensor), &
                                       transpose(Rotation_Global_to_Local))
                Grad_u_Local  = matmul(matmul(Rotation_Global_to_Local, Grad_u_Tensor), &
                                       transpose(Rotation_Global_to_Local))
                ...
                do i_Mode = 1, 3
                    ...
                    Integrand_value1 = &
                        Stress_Local(1,1)*(Aux_ddu1_dX1dX1 - Aux_dEps11_dX1) + &
                    ...
                    Integrand_value2 = &
                        (Aux_dSig11_dX1+Aux_dSig12_dX2+Aux_dSig13_dX3)*Grad_u_Local(1,1) + &
                ...
                if (Key_Analysis_Type == 6) then
                    Acl_gp_global(1:3) = ZR

                    call Cal_Any_Point_Disp_KesiYita_3D(Current_Elem, &
                         GP_xi(i_GP), GP_eta(i_GP), GP_zeta(i_GP), Dy_ACCL, Acl_gp_global)

                    Acl_gp_local = matmul(Rotation_Global_to_Local, Acl_gp_global)
                    ...
                    do i_Mode = 1, 3
                        call Cal_3D_SIFs_IIM_Auxiliary_Fields_Part1(i_Mode, r_polar, theta_polar, &
                             Shear_Modulus, Poisson_Ratio, Kappa_3D, &
                             Aux_Stress_Local, Aux_Strain_Local, Aux_Grad_u_Local)
                        Integrand_inertia = c_density * ( &
                            Acl_gp_local(1) * Aux_Grad_u_Local(1,1) + &
                            Acl_gp_local(2) * Aux_Grad_u_Local(2,1) + &
                            Acl_gp_local(3) * Aux_Grad_u_Local(3,1) )
                        M_integral_Elements(i_Elem,i_Mode) = M_integral_Elements(i_Elem,i_Mode) &
                            + (Integrand_inertia * q_value_at_GP) * dV
                    enddo
                endif
            ...
            if (Has_Fluid_Pressure .and. abs(p_fluid) > 1.0D-16) then
                fac_two_faces = TWO
                call Cal_3D_SIFs_IIM_CrackFace_IntegralPoint_1_Point(i_Crack, Current_Elem, &
                     nGP_face, GP_face_global, GP_face_w, GP_face_n_global)
                ...
                if (nGP_face > 0) then
                    do i_GP_face = 1, nGP_face
                        ...
                        call Cal_3D_SIFs_IIM_q_Function(Current_Elem, q_nodal, Shape_Functions, &
                             dN_dx, dN_dy, dN_dz, GP_face_global(i_GP_face,1:3), &
                             Tip_Coords, e1_growth, e2_normal, e3_tangent, &
                             R_inner, R_outer, L_front, q_face, grad_q_global)

                        if (abs(q_face) < 1.0D-15) cycle

                        GP_coords_local = matmul(Rotation_Global_to_Local, &
                        ...
                        do i_Mode = 1, 3
                            ...
                            M_integral_Elements(i_Elem,i_Mode) = M_integral_Elements(i_Elem,i_Mode) &
                                + fac_two_faces * s_trac * Integrand_face * q_face &
                                * GP_face_w(i_GP_face)
                        enddo
                    enddo
                endif
            ...
            if (Key_Contact==1 .and. allocated(Contact_Stress)) then
                fac_two_faces = 2.0D0
                call Cal_3D_SIFs_IIM_CrackFace_IntegralPoint_1_Point(i_Crack, Current_Elem, &
                     nGP_face, GP_face_global, GP_face_w, GP_face_n_global)
                ...
                if (nGP_face > 0) then
                    do i_GP_face = 1, nGP_face
                        n_unit(1:3) = GP_face_n_global(i_GP_face,1:3)
                        call Get_Contact_Stress_from_Crack_Surface(Current_Elem, &
                             GP_face_global(i_GP_face,1:3), n_unit(1:3), t_global(1:3))
                        t_local(1:3) = matmul(Rotation_Global_to_Local, t_global)
        ...
        if (Key_3D_SIFs_Print==1) then
            !$OMP CRITICAL (crit_sif_print)
            write(*,2001) i_Vertex, i_Crack, &
                KI_3D(i_Crack)%row(i_Vertex)/1.0D6, &
                KII_3D(i_Crack)%row(i_Vertex)/1.0D6, &
                KIII_3D(i_Crack)%row(i_Vertex)/1.0D6
            !$OMP END CRITICAL (crit_sif_print)
        endif
...
if (Key_Denoise_Vertex_Value>=1)then
    ...
    do i_C = 1,num_Crack
        ...
        call Tool_Denoise_Data(KI_3D(i_C)%row(1:num_Cr_Edges),   num_Cr_Edges, &
             Key_Denoise_Vertex_Value, n_Sigma, logical_close)
        call Tool_Denoise_Data(KII_3D(i_C)%row(1:num_Cr_Edges),  num_Cr_Edges, &
             Key_Denoise_Vertex_Value, n_Sigma, logical_close)
        call Tool_Denoise_Data(KIII_3D(i_C)%row(1:num_Cr_Edges), num_Cr_Edges, &
             Key_Denoise_Vertex_Value, n_Sigma, logical_close)
    enddo
    !$OMP END PARALLEL DO
endif
...
do i_C = 1,num_Crack
    ...
    do i_Out_Node = 1,Num_CrMesh_Outlines
        ...
        if(c_OUT_Elem == 0) cycle

        Ld_Cr(i_Out_Node) = Tool_Function_2Point_Dis_3D( &
            Crack3D_Meshed_Node(i_C)%row(c_Mesh_Node,1:3), &
        ...
        do i_Check_Theta = 1, num_of_check+1
            ...
            Check_Theta(i_Check_Theta) = -Schollm_Max_Theta_in_pi &
                + TWO*Schollm_Max_Theta_in_pi/num_of_check*(i_Check_Theta-1)
            tem_part1 = THR*cos(Check_Theta(i_Check_Theta)/TWO) &
                      + cos(THR*Check_Theta(i_Check_Theta)/TWO)
            tem_part2 = THR*sin(Check_Theta(i_Check_Theta)/TWO) &
                      + THR*sin(THR*Check_Theta(i_Check_Theta)/TWO)
            Stress_Theta = c_KI_3D/FOU/tem_root_2pir*tem_part1 &
                         - c_KII_3D/FOU/tem_root_2pir*tem_part2
            Tao_Theta = c_KIII_3D*cos(Check_Theta(i_Check_Theta)/TWO)/tem_root_2pir
            Spe_Pri_Stress(i_Check_Theta) = Stress_Theta/TWO &
                + ZP5*sqrt(Stress_Theta**2 + FOU*Tao_Theta**2)
        enddo
