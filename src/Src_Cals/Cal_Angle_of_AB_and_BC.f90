!-----------------------------------------------------------
! Brief: Compute the included angle between two connected 2D segments.
!
! Parameters:
!   Input:  Line_AB     - first segment endpoints (2x2 array)
!           Line_BC     - second segment endpoints (2x2 array)
!   Output: angle_AB_BC - signed included angle (0 to 2*pi)
!
! Notes:   Verifies the segments share endpoint B; uses the cross
!          product z-component to determine orientation (CW/CCW).
!-----------------------------------------------------------
...
if  ((Line_AB(2,1) .ne. Line_BC(1,1)) .or. (Line_AB(2,2) .ne. Line_BC(1,2))) then
    print *,'    ********************************' //'*********************************'
    print *,'    Attention should be paid here!'
    print *,'    This situation rarely' //' appears in subroutine Cal_Angle_of_AB_and_BC!'
    print *,'    ********************************' //'*********************************'
end if     
a_x = Line_AB(1,1)
a_y = Line_AB(1,2)
