!-----------------------------------------------------------
! Brief: Find HF element node locations along a crack
!        segment by walking a step size and crossing element
!        boundaries.
!
! Parameters:
!   Input:  A, B            - segment endpoints (x,y)
!   Output: Div_Points     - node coordinates (incl. ends)
!           Num_Div_Points  - number of detected nodes
!
! Notes:   Step size is 0.0099 * Ave_Elem_L_Enrich. Each new
!          element crossing produces a node at the crossing.
!-----------------------------------------------------------

subroutine Cal_HF_HFNode_Location(A,B, Div_Points,Num_Div_Points)
...
integer Ele_l,Ele_r,num_Inter,N1,N2,N3,N4,NN_L(5),n_node_1, n_node_2
...
do i = 1,number_Check
    ...
    if(Ele_l /= Ele_r .and. c_L<=L_Seg)then
        ...
        do i_Side = 1,4
            ...
            call Tool_Intersection( [c_x_l,c_y_l],[c_x_r,c_y_r], point_side_1,point_side_2, c_Inter_x,c_Inter_y,c_Yes_Cross)
            if (c_Yes_Cross.eqv..True.) then
                call Vector_belongs_Matrix_Is_Dou(5,2, c_Intersec,[c_Inter_x,c_Inter_y], cc_Location,Yes_Already_has)
        ...
        if(num_Inter/=0)then
            Div_Points(Num_Div_Points+1: Num_Div_Points+num_Inter,:) = c_Intersec(1:num_Inter,:)
