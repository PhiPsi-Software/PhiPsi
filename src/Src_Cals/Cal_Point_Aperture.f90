!-----------------------------------------------------------
! Brief: Compute crack opening (aperture) at a 2D point on a crack.
!
! Parameters:
!   Input:  c_Crack - crack index
!           Point   - 2D coordinates of the query point
!           c_DISP  - global displacement vector
!           Cr_Omega - crack orientation angle
!   Output: Aperture - normal opening at the point
!
! Notes:   Offsets up/down by delta_L along the normal, looks
!          up the host element, and projects displacements
!          onto the crack normal to obtain the opening width.
!-----------------------------------------------------------

subroutine Cal_Point_Aperture(c_Crack,Point, c_DISP,Cr_Omega,Aperture)
...
real(kind=FT) Up_Point(2),Low_Point(2),Cr_p1(2),Cr_p2(2), Up_Disp(2),Low_Disp(2),delta_L, &
Delta_Disp(2),cos_angle,x1,y1,x2,y2
real(kind=FT) Vector_a_b(2),Vector_ap_bp(2),sign_Aper, Magni_Aper,Vector_n_Up(2),Vector_n_Low(2), &
...
if (c_Elem_Up<=0) then
    ...
    if (c_Elem_Up<=0) then
        Aperture = -TEN_15
        print *, '    Warning:  illegal element number found' //' in Cal_Point_Aperture.f!'
        return 
    endif
endif
...
call Cal_Any_Point_Disp_KesiYita(c_Elem_Up,c_Kesi_Up, c_Yita_Up,0,c_DISP,Up_Disp)
...
call Cal_KesiYita_by_Coor(Low_Point,c_Elem_Low, c_Kesi_Low,c_Yita_Low)
call Cal_Any_Point_Disp_KesiYita(c_Elem_Low,c_Kesi_Low,c_Yita_Low, 0,c_DISP,Low_Disp)
