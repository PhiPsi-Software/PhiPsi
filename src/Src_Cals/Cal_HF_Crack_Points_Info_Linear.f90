!-----------------------------------------------------------
! Brief: Populate calculation-point arrays for linear HF
!        elements: coordinates, orientation, segment, element.
!
! Parameters:
!   Input:  isub - current load/sub-step index
!
! Notes:   Fills the global Cracks_CalP_* arrays (coordinates,
!          segment, owning element, type, far-field stress,
!          contact stress, etc.) and HF element lengths.
!-----------------------------------------------------------
...
! real(kind=FT) Cracks_CalP_Coors(Max_Num_Cr, Max_Num_Cr_CalP, 2) !Coordinates of calculation points for each crack
! real(kind=FT) Cracks_CalP_Orient(Max_Num_Cr, Max_Num_Cr_CalP)   !Orientation of each crack corresponding to its calculation points
...
real(kind=FT) Line_AB(2,2),delta_L, new_Point(2),new_Line_AB(2,2)
integer :: i_HF_Elem
...
real(kind=FT) Inj_Poin_x,Inj_Poin_y,Min_Distance,c_Distance,c_CalP_x,c_CalP_y
integer :: i_tt
logical :: Yes_Has
integer :: Location
...
do i_C = 1,num_Crack
    ...
    do i_S = 1,Each_Cr_Poi_Num(i_C)-1
        ...
        Cr_p1 = [Edge_Disposed_Crack(i_C,i_S,1), Edge_Disposed_Crack(i_C,i_S,2)]
        Cr_p2 = [Edge_Disposed_Crack(i_C,i_S+1,1), Edge_Disposed_Crack(i_C,i_S+1,2)]
        ...
        do i_Jun=1,n_Crs
            ...
            call Tool_Yes_On_Line(x_Jun,y_Jun,Cr_p1,Cr_p2, Yes_Jun_OnSeg)
        ...
        if (Yes_SegHasJun .eqv. .False.) then          
            call Cal_HF_HFNode_Location(Cr_p1,Cr_p2, S_Div_Points,Num_S_Div_Points)
            if (i_S  < Each_Cr_Poi_Num(i_C)-1) then
                Num_S_Div_Points = Num_S_Div_Points-1
            end if
            do i_tt = 1,Num_S_Div_Points
                temp_vector_1(1:2) = S_Div_Points(i_tt,1:2)
                call Vector_belongs_Matrix_Is_Dou(Num_Div_Points,2,Div_Points(1:Num_Div_Points,1:2),temp_vector_1,Location,Yes_Has)
                if(Yes_Has.eqv..False.)then
                    Num_Div_Points = Num_Div_Points+1
                    Div_Points(Num_Div_Points,1:2) = S_Div_Points(i_tt,1:2)
        ...
        elseif(Yes_SegHasJun .eqv. .True.) then
            do i_new_Seg =1,num_Jun+1
                ...
                if (i_new_Seg==1)then     
                    Num_Div_Points  = Num_Div_Points +1
                    Div_Points(Num_Div_Points,1:2) = New_Seg_withJun(1,1:2)
                    Div_Points_Seg_Num(Num_Div_Points) = i_S
                end if
                call Cal_HF_HFNode_Location(New_Seg_P1,New_Seg_P2,New_S_Div_Points,New_Num_S_Div_Points)

                do i_tt = 1,New_Num_S_Div_Points
                    call Vector_belongs_Matrix_Is_Dou(Num_Div_Points,2, Div_Points(1:Num_Div_Points,1:2), &
                    New_S_Div_Points(i_tt,1:2),Location,Yes_Has)
                    if(Yes_Has.eqv..False.)then
                        Num_Div_Points = Num_Div_Points+1
                        Div_Points(Num_Div_Points,1:2) = New_S_Div_Points(i_tt,1:2)
                        Div_Points_Seg_Num(Num_Div_Points)=i_S
                        ...
                        if(x_Jun==c_D_P_x.and.y_Jun==c_D_P_y)then
                            ...
                            Cracks_MidJunCalpNum(i_C,c_Jun_Num)  = Num_Div_Points
                            ...
                            do j_Jun = 1,num_Jun_Crack
                                Proba_Ele=Cracks_Cone_MidJuEle(i_C, j_Jun) 
                                if((abs(Cracks_Cone_MidJuCor(i_C, j_Jun,1)- x_Jun)<=Tiny_L) .and. &
                                (abs(Cracks_Cone_MidJuCor(i_C,j_Jun,2)- y_Jun)<=Tiny_L)) then
                                Cracks_CalP_Type(i_C,Num_Div_Points,2) = Cracks_Cone_MidCrNum(i_C,j_Jun)
                                exit
                            end if
                        end do         
                    end if
...
if (Crack_Tip_Type(i_C,1).eq.1) then 
    Line_AB(1,:) =[Edge_Disposed_Crack(i_C,1,1), Edge_Disposed_Crack(i_C,1,2)]
    Line_AB(2,:) =[Edge_Disposed_Crack(i_C,2,1), Edge_Disposed_Crack(i_C,2,2)]
    call Tool_Shorten_or_Extend_Line(Line_AB,-delta_L,'A', new_Line_AB,new_Point)
    Div_Points(1,:) = new_Point
end if
if (Crack_Tip_Type(i_C,2).eq.1) then   
    Line_AB(1,:) =[Edge_Disposed_Crack(i_C,Each_Cr_Poi_Num(i_C)-1,1), Edge_Disposed_Crack(i_C,Each_Cr_Poi_Num(i_C)-1,2)]
    Line_AB(2,:) =[Edge_Disposed_Crack(i_C,Each_Cr_Poi_Num(i_C),1), Edge_Disposed_Crack(i_C,Each_Cr_Poi_Num(i_C),2)]
    call Tool_Shorten_or_Extend_Line(Line_AB,-delta_L,'B',new_Line_AB,new_Point)
    Div_Points(Num_Div_Points,:) = new_Point 
end if

if (Key_Kink_Point==0)then
    Old_Div_Points(1:Max_Num_Cr_CalP,1:2)= Div_Points(1:Max_Num_Cr_CalP,1:2)
        ...
        if(i_Cr_P==1 .or. i_Cr_P == Old_Num_Div_Points)then
            ...
            if(Each_Cr_Poi_Num(i_C)>=3)then
                do i_Coor = 2,Each_Cr_Poi_Num(i_C)-1
                    ...
                    if((abs(c_Cr_Point(1)-c_Div_P(1))<c_Tiny).and. (abs(c_Cr_Point(2)-c_Div_P(2))<c_Tiny)) then
                        Yes_Overlap = .True.
                        exit
                    endif
                enddo
...
if (Crack_Tip_Type(i_C,2).eq.1) then
    if (Cracks_Cone_TipCrNum(i_C,2).ne.0) then
        ...
        Cracks_CalP_Type(i_C,Num_Div_Points,2) =  Cracks_Cone_TipCrNum(i_C,2)
        Cracks_TipJunCalpNum(i_C,2) = Num_Div_Points
    end if
end if

Cracks_CalP_Coors(i_C,1:Num_Div_Points,1:2) = Div_Points(1:Num_Div_Points,1:2)
Cracks_CalP_Seg(i_C,1:Num_Div_Points)       = Div_Points_Seg_Num(1:Num_Div_Points)
Cracks_CalP_Num(i_C)  = Num_Div_Points

do i_P = 1,Num_Div_Points
    call Cal_Ele_Num_by_Coors(Div_Points(i_P,1), Div_Points(i_P,2), Cracks_CalP_Elem(i_C,i_P))
    if(i_P==1)then
        Cr_p1 = [Cracks_CalP_Coors(i_C,1,1), Cracks_CalP_Coors(i_C,1,2)]
        Cr_p2 = [Cracks_CalP_Coors(i_C,2,1), Cracks_CalP_Coors(i_C,2,2)]
        Cracks_CalP_Orient(i_C,i_P) = atan2(Cr_p2(2)-Cr_p1(2),Cr_p2(1)-Cr_p1(1))
    elseif(i_P==Num_Div_Points)then
        Cr_p1 = [Cracks_CalP_Coors(i_C,Num_Div_Points-1,1), Cracks_CalP_Coors(i_C,Num_Div_Points-1,2)]
        Cr_p2 = [Cracks_CalP_Coors(i_C,Num_Div_Points,1), Cracks_CalP_Coors(i_C,Num_Div_Points,2)]
        Cracks_CalP_Orient(i_C,i_P) = atan2(Cr_p2(2)-Cr_p1(2),Cr_p2(1)-Cr_p1(1))
    else
        Cr_p1 = [Cracks_CalP_Coors(i_C,i_P-1,1), Cracks_CalP_Coors(i_C,i_P-1,2)]
        Cr_p2 = [Cracks_CalP_Coors(i_C,i_P,1),  Cracks_CalP_Coors(i_C,i_P,2)]
        Orient_L= atan2(Cr_p2(2)-Cr_p1(2),Cr_p2(1)-Cr_p1(1))
        Cr_p1 = [Cracks_CalP_Coors(i_C,i_P,1), Cracks_CalP_Coors(i_C,i_P,2)]
        Cr_p2 = [Cracks_CalP_Coors(i_C,i_P+1,1), Cracks_CalP_Coors(i_C,i_P+1,2)]
...
do i_C = 1,num_Crack
    do i_CalP = 1,Cracks_CalP_Num(i_C) 
        if ((Cracks_CalP_Type(i_C,i_CalP,1) == 1) .or. (Cracks_CalP_Type(i_C,i_CalP,1) == 2)) then
            Other_Crack = Cracks_CalP_Type(i_C,i_CalP,2)
            if((Cracks_HF_State(i_C) == 1) .and. (Cracks_HF_State(Other_Crack) == 1) )then
                do j_CalP=1,Cracks_CalP_Num(Other_Crack)
                    if((Cracks_CalP_Type(Other_Crack,j_CalP,1)==3).and. (Cracks_CalP_Type(Other_Crack,j_CalP,2)==i_C)) then

                        Num_JunPair = Num_JunPair +1
                        Cracks_JunPair(Num_JunPair,1) = Cracks_GloNumCalP(i_C,i_CalP)
                        Cracks_JunPair(Num_JunPair,2) = Cracks_GloNumCalP(Other_Crack,j_CalP)
                    endif
                end do
            endif
        end if
...
do i_Pair =1,Num_JunPair 
    write(*,5001)i_Pair,Cracks_JunPair(i_Pair,1), Cracks_JunPair(i_Pair,2)
end do   
    ...
    do i_C=1,num_Crack
        if (i_C == Inject_Crack_Num) then
            ...
            do i_CalP = 1,Cracks_CalP_Num(i_C)
                ...
                c_Distance = sqrt((Inj_Poin_x-c_CalP_x)**2 + (Inj_Poin_y-c_CalP_y)**2)
...
do i_C=1,num_Crack
    do i_CalP = 1,Cracks_CalP_Num(i_C)
        ...
        Cracks_CalP_Remo_Strs(i_C,i_CalP) = InSitu_y*cos(c_theta)**2 + InSitu_x*sin(c_theta)**2
    end do
end do 
...
if (Key_InSitu_Strategy == 2) then
    do i_C=1,num_Crack
        do i_CalP = 1,Cracks_CalP_Num(i_C)
            ...
            c_InSitu_x = sum(InSitu_Strs_Gaus_xx(c_Ele,1:Num_Gauss_P_FEM))/ Num_Gauss_P_FEM
            c_InSitu_y = sum(InSitu_Strs_Gaus_yy(c_Ele,1:Num_Gauss_P_FEM))/ Num_Gauss_P_FEM
            c_theta = Cracks_CalP_Orient(i_C,i_CalP) 

            Contact_stress_Normal = c_InSitu_x*sin(c_theta)**2 + c_InSitu_y*cos(c_theta)**2
            Contact_stress_Tan = (c_InSitu_x-c_InSitu_y)*sin(c_theta)*cos(c_theta)

            Cracks_CalP_Contact_Strs(i_C,i_CalP,1) = Contact_stress_Normal
            Cracks_CalP_Contact_Strs(i_C,i_CalP,2) = Contact_stress_Tan
        end do
    end do       
endif
return 
