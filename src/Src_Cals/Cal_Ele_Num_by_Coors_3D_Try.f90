!-----------------------------------------------------------
! Brief: Retry locating a 3D element by perturbing the query point in many directions.
!
! Parameters:
!   Input:  x          - x-coordinate of the original query point
!           y          - y-coordinate of the original query point
!           z          - z-coordinate of the original query point
!           offset_dis - perturbation step length
!   Output: x_out      - x-coordinate of the successful query point
!           y_out      - y-coordinate of the successful query point
!           z_out      - z-coordinate of the successful query point
!           OUT_Elem   - element index containing the perturbed point (0 if none)
!
! Notes:   Used as a fallback when Cal_Ele_Num_by_Coors_3D fails; tries up to 80
!          offset directions with multiple distance scales.
!-----------------------------------------------------------

subroutine Cal_Ele_Num_by_Coors_3D_Try(x,y,z,offset_dis, x_out,y_out,z_out,OUT_Elem)
!     Calculate the element number based on coordinates, in 3D.
!     Cal_Ele_Num_by_Coors_3D cannot find the element number; this subroutine adjusts the position and continues searching.
...
do i_Try = 1,80
    ...
    do i=1,Num_Elem
        ...
        if   ((x_new.le.c_x_max) .and. (x_new.ge.c_x_min).and. (y_new.le.c_y_max) .and. (y_new.ge.c_y_min).and. &
        (z_new.le.c_z_max) .and. (z_new.ge.c_z_min))then
        c_count = c_count +1
...
do i =1,c_count
    ...
    call Tool_Yes_Point_in_3D_Hexahedron([x_new,y_new,z_new], Coor(c_NN(1),1:3),Coor(c_NN(2),1:3), &
    Coor(c_NN(3),1:3),Coor(c_NN(4),1:3), Coor(c_NN(5),1:3),Coor(c_NN(6),1:3), Coor(c_NN(7),1:3),Coor(c_NN(8),1:3), &
    c_Yes_in,c_Yes_on)
    if(c_Yes_in)then
