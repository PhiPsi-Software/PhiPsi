!-----------------------------------------------------------
! Brief: Assemble the H matrix (fluid leak-off / storage)
!        for 2D hydraulic fracturing with linear HF elements.
!
! Parameters:
!   Input:  ifra, Counter         - step / iteration IDs
!           total_time            - current simulation time
!           Last_Cr_CalP_Aper     - previous apertures
!   Output: Matrix_H              - assembled H matrix
!
! Notes:   Two-point Gauss rule per 1D HF element; uses
!          mapped proppant concentration when Key_Propp_Trans=1.
!-----------------------------------------------------------
