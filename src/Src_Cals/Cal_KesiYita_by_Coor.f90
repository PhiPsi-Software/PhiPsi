!-----------------------------------------------------------
! Brief: Invert the Q4 isoparametric map: (x,y) -> (kesi,yita).
!
! Parameters:
!   Input:  Point(2) - global point coordinates
!           c_elem   - element index
!   Output: Kesi     - local isoparametric xi coordinate
!           Yita     - local isoparametric eta coordinate
!
! Notes:   Solves the nonlinear Q4 mapping by the gradient
!          (steepest-descent) method. Initial guess is (0,0).
!-----------------------------------------------------------

subroutine Cal_KesiYita_by_Coor(Point,c_elem,Kesi,Yita)
!     Calculate local coordinate system based on coordinates
...
call Slove_NOLE_Gradient_Method(2,EPS,X,Y,FS,L, a1,a2,a3,a4,b1,b2,b3,b4, Point)
...
subroutine Slove_NOLE_Gradient_Method(N,EPS,X,Y,FS,L, a1,a2,a3,a4,b1,b2,b3,b4, Point)
use Global_Float_Type
implicit none    
dimension X(N),Y(N)
