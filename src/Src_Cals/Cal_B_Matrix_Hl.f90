!-----------------------------------------------------------
! Brief: Assemble the B matrix for elements containing a circular hole.
!
! Parameters:
!   Input:  kesi, yita        - local coordinates of the GP
!           i_H, i_E, i_G     - hole, element, GP indices
...
!
! Notes:   Adds hole-level (signed-distance) enrichment rows on
!          top of the standard FEM B-matrix; tracks per-GP state.
!-----------------------------------------------------------

subroutine Cal_B_Matrix_Hl(kesi,yita, i_H,i_E,i_G, c_NN,c_X_NODES,c_Y_NODES, tem_B,num_tem_B)
...
if(maxval(Enriched_Node_Type_Hl(c_NN,i_H)).eq.0 .and. minval(Enriched_Node_Type_Hl(c_NN,i_H)).eq.0) then
    if (i_H.eq.1) then
        tem_B(1:3,1:8) = B_FEM
        num_tem_B = 8
    else
...
elseif(maxval(Enriched_Node_Type_Hl(c_NN,i_H)).gt.0 .or. minval(Enriched_Node_Type_Hl(c_NN,i_H)).gt.0) then
    B_XFEM(1:3,1:60) = ZR
    num_B_XFEM = 0
    do i_N = 1,4
        if (Enriched_Node_Type_Hl(c_NN(i_N),i_H).eq.1)then
            if (Elem_Type_Hl(i_E,i_H).eq.1) then
                ...
                if (num_Ellip_Hole>=1)then
                    ...
                    Tol = 1.0D-10
                    call Tool_Yes_Point_in_Oblique_Ellipse( [c_G_x,c_G_y], x0_Hole,y0_Hole,a_Hole,b_Hole,theta_Hole, &
                    Return_Statu_G,Tol)
                    call Tool_Yes_Point_in_Oblique_Ellipse( Coor(c_NODE,1:2), x0_Hole,y0_Hole,a_Hole,b_Hole,theta_Hole, &
                    Return_Statu_N,Tol)
                ...
                BI_enr(3,:) = [dNdx(i_N,2)*(Hl_gp-Hl_i), dNdx(i_N,1)*(Hl_gp-Hl_i)]
            else
                BI_enr(1:3,1:2)=ZR
            end if
