...
!     ------------------------------------------------- !
      ...
      subroutine Cal_Any_Point_Disp_KesiYita_NoFEM(elem,Kesi,Yita,i_G,
     &                                       U_e,P_Disp)
      ! Calculate the displacement at a certain point on the crack, given the element number and local
      ! coordinates, etc.
      !-----------
      ...
      real(kind=FT),intent(in)::Kesi,Yita,U_e(Enrich_Freedom)
     ...
     &           Tri_KAB(3,2),Closed_Tri_KAB(4,2),H _i,
     ...
     &           F_Gauss(4),F_Node(4)
      ...
      call Cal_Coor_by_KesiYita(Kesi,Yita,X_NODES,Y_NODES,
     &                                    point(1),point(2)) 
      c_G_x = point(1)
      c_G_y = point(2)
        ...
        do i_C=1,Num_Crack
          ...
          do i_N = 1,4
              ...
              if (Enriched_Node_Type(NODES_iE(i_N),i_C) ==2) then
                  ...
                  Num_Enri_Node = Num_Enri_Node - Num_Node
        if ((Elem_Type(c_Elem,i_C) == 2) .or. 
     &                (Elem_Type(c_Elem,i_C) == 3)) then
                      ref_elem = c_Elem
      	  Coor_AB(1,:)=[Coors_Element_Crack(ref_elem,i_C,1),
     &                              Coors_Element_Crack(ref_elem,i_C,2)]
                      Coor_AB(2,:)=[Coors_Element_Crack(ref_elem,i_C,3),
     ...
     &                                distance_Node)
                      ...
                      Disp_x = Disp_x+(H_gp-H_i)*tem_N(i_N)
     &                                    *U_e(Num_Enri_Node*2-1)
                      Disp_y = Disp_y+(H_gp-H_i)*tem_N(i_N)
     &                                    *U_e(Num_Enri_Node*2)
                  end if
              elseif (Enriched_Node_Type(NODES_iE(i_N),i_C) ==3) then
                  ...
                  Coor_AB(1,:) = [Coors_Element_Crack(Jun_elem,i_C,1),
     &                            Coors_Element_Crack(Jun_elem,i_C,2)]
                  Coor_AB(2,:) = [Coors_Element_Crack(Jun_elem,i_C,3),
     &                            Coors_Element_Crack(Jun_elem,i_C,4)] 
                  Coor_CD(1,:) = [Coors_Junction(Jun_elem,i_C,1),
     &                            Coors_Junction(Jun_elem,i_C,2)]
                  Coor_CD(2,:) = [Coors_Junction(Jun_elem,i_C,3), 
     &                            Coors_Junction(Jun_elem,i_C,4)]
                  x0 = [Coors_Junction(Jun_elem,i_C,1),
     &                  Coors_Junction(Jun_elem,i_C,2)]
                  call Cal_Signed_Distance(Coor_AB,
     &                             [X_NODES(i_N),Y_NODES(i_N)],
     &                                distance_Node_M)    
                  call Cal_Signed_Distance(Coor_AB,
     &                             Point,distance_Gauss_M)                      
                  call Cal_Signed_Distance(Coor_CD,
     &                             [X_NODES(i_N),Y_NODES(i_N)],
     &                                distance_Node_sm)    
                  call Cal_Signed_Distance(Coor_CD,
     &                             Point,distance_Gauss_sm) 
            ...
            if (H_i_M*H_x0 > ZR) then
                  ...
                  Disp_x = Disp_x + (J_gp-J_i)*tem_N(i_N)*
     &                               U_e(Num_Enri_Node*2-1)
                  Disp_y = Disp_y + (J_gp-J_i)*tem_N(i_N)*
     &                               U_e(Num_Enri_Node*2)
              elseif (Enriched_Node_Type(NODES_iE(i_N),i_C) ==6.and. 
     &                num_Circ_Hole>=1)then
                  Jun_elem = Node_Jun_elem(NODES_iE(i_N),i_C)
     ...
     &                  Coors_Junction(Jun_elem,i_C,2)]
                  ...
                  call Tool_Signed_Distance_Point_to_Circle(
     ...
     &                  Coors_Junction(Jun_elem,i_C,2)]
                  ...
                  call Tool_Signed_Distance_Point_to_Ellipse(
     ...
     &                            Coors_Element_Crack(ref_elem,i_C,4)]  
            ...
            Trans_Matrix(2,:) = [ -sin(omega),cos(omega)]
                  ...
                  if ((Flag_Check_Kink_Tip .eqv. .True.)
     &                          .and. (num_points_crack > 2)) then
                if ((Elem_Type(c_Elem,i_C) .eq. 2 )
     &                          .or.(Elem_Type(c_Elem,i_C) .eq. 3))then
                           coor_A = Coor_Tip
                           tip_x = x_cr_tip_nodes(i_C,NODES_iE(i_N))
      	       tip_y = y_cr_tip_nodes(i_C,NODES_iE(i_N))
                           if((Crack_Coor(i_C,1,1).eq. tip_x).and.
     &                        (Crack_Coor(i_C,1,2) .eq. tip_y)) then
                                 coor_B =Crack_Coor(i_C,2,:)
                           elseif((Crack_Coor(i_C,num_points_crack,1)
     &                              .eq. tip_x) .and.
     &                        (Crack_Coor(i_C,num_points_crack,2)
     &                               .eq. tip_y)) then    
                               coor_B =
     &                             Crack_Coor(i_C,num_points_crack-1,:)
                           end if
                           len_AB = 
     &                     sqrt((coor_A(1)-coor_B(1))**2
     &                         +(coor_A(2)-coor_B(2))**2)
      	       if (coor_A(1).eq. Crack_Coor(i_C,1,1).and.
     &                          coor_A(2) .eq.Crack_Coor(i_C,1,2)) then
     ...
     &                              Crack_Coor(i_C,num_points_crack,2)) 
     &                                                            then
      		       coor_C = Crack_Coor(i_C,
     &                                             num_points_crack-2,:)
      	       end if                         
                           Line_AB(1,:) = coor_A
                           Line_AB(2,:) = coor_B
                           call Tool_Shorten_or_Extend_Line(Line_AB,
     &                                      2*len_AB,'B',
     &                                      new_Line_AB,coor_D)    
                           Tri_BCD(1,:) = coor_B
                           Tri_BCD(2,:) = coor_C
                           ...
                           call Tool_Yes_In_Poly
     &                       (Point(1),Point(2),
     &                         Closed_Tri_BCD(:,1),Closed_Tri_BCD(:,2),
     &                         4,Yes_Gauss_in_BCD)        
                           if (Yes_Gauss_in_BCD) then
                               Line_BC(1,:) = coor_B
                               Line_BC(2,:) = coor_C                               
                               call Cal_Angle_of_AB_and_BC(Line_AB,
     &                                           Line_BC,angle_AB_BC)
                               Line_BGauss(1,:) = coor_B
                               Line_BGauss(2,:) = Point
                               call Cal_Angle_of_AB_and_BC(Line_AB,
     &                                     Line_BGauss,angle_AB_BGauss)
                               len_BGauss  = 
     &                               sqrt((coor_B(1)-
     &                               Point(1))**2.0
     &                           +(coor_B(2)-Point(2))**2.0)   
      			       if (angle_AB_BGauss >= angle_AB_BC) then
      		           thete_Doc= HLF*pi*
     &                                    (angle_AB_BGauss-angle_AB_BC)
     &                              /(1.5D0*pi-angle_AB_BC)
      		       else
      		           thete_Doc= pi/TWO*
     &                                    (angle_AB_BGauss-angle_AB_BC)
     &                              /(angle_AB_BC-HLF*pi)
                               end if
                               Global_coor_Gauss_Doc =
     &                             [-len_AB-len_BGauss*cos(thete_Doc),
     &                              -len_BGauss*sin(thete_Doc)]
                          end if
                      end if                   
                  end if
     ...
     &                                   Global_coor_Gauss_Doc(1))
                  ...
                  call Cal_F(r_Gauss,theta_Gauss,omega,
     &                                 c_mat_type,F_Gauss)  
                  xp_Node= MATMUL(Trans_Matrix,
     &                        [X_NODES(i_N),Y_NODES(i_N)]-Coor_Tip)
     ...
     &                                 c_mat_type,F_Node)    
                  if (Key_TipEnrich==1) then   
                      do i_F =1,4
                          ...
                          Disp_x = Disp_x + F*tem_N(i_N)*
     &                              U_e(Num_Enri_Node*2-1)
                          Disp_y = Disp_y + F*tem_N(i_N)*
     &                              U_e(Num_Enri_Node*2)
                      end do
 ...
 1000   continue
          ...
          do i_N = 1,4
              if (Enriched_Node_Type_Hl(NODES_iE(i_N),i_H).eq.1)then
                ...
                if (Elem_Type_Hl(c_Elem,i_H).eq.1) then
                      ...
                      c_Dis_G=Tool_Function_2Point_Dis([c_G_x,c_G_y],
     &                                              [c_Hole_x,c_Hole_y])
     ...
     &                                              [c_Hole_x,c_Hole_y])
                      ...
                      Disp_x = Disp_x+(Hl_gp-Hl_i)*tem_N(i_N)
     &                                    *U_e(Num_Enri_Node*2-1)
                      Disp_y = Disp_y+(Hl_gp-Hl_i)*tem_N(i_N)
     &                                    *U_e(Num_Enri_Node*2)
                  endif
              endif
          end do
2000    continue
      ...
      if(Num_Cross /=0)then
        do i_Cross =1,Num_Cross
          if(maxval(Enriched_Node_Type_Cross(NODES_iE,i_Cross)).eq.0)
     &                                                           then
              goto 2100
          end if
          do i_N = 1,4
              if (Enriched_Node_Type_Cross(NODES_iE(i_N),i_Cross).eq.1)
     &                                                             then
     ...
     &                                distance_Gauss_sm) 
                      if(Key_Heaviside_Value==-1)then
                          call Cal_Sign(
     &                              distance_Gauss_M*distance_Gauss_sm,
     &                              Cross_gp)
                      call Cal_Sign(
     &                              distance_Node_M*distance_Node_sm,
     &                              Cross_i)
                      elseif(Key_Heaviside_Value==0)then
                          call Cal_Sign_1_and_0(
     &                              distance_Gauss_M*distance_Gauss_sm,
     &                              Cross_gp)
                          call Cal_Sign_1_and_0(
     &                              distance_Node_M*distance_Node_sm,
     &                              Cross_i)
                      ...
                      Disp_x = Disp_x+(Cross_gp-Cross_i)*tem_N(i_N)
     &                                    *U_e(Num_Enri_Node*2-1)
                      Disp_y = Disp_y+(Cross_gp-Cross_i)*tem_N(i_N)
     &                                    *U_e(Num_Enri_Node*2)
                  endif
              endif
          end do
          ...
          do i_N = 1,4
              if(Enriched_Node_Type_Incl(NODES_iE(i_N),i_Incl).eq.1)then
                ...
                if (Elem_Type_Incl(c_Elem,i_Incl).eq.1) then                                    
                      Incl_gp = Za-abs(Zm)   
                      Disp_x = Disp_x+(Incl_gp)*tem_N(i_N)
     &                                    *U_e(Num_Enri_Node*2-1)
                      Disp_y = Disp_y+(Incl_gp)*tem_N(i_N)
     &                                    *U_e(Num_Enri_Node*2)
                  endif
              endif
          end do
3000    continue
      ...
      return 
      end SUBROUTINE Cal_Any_Point_Disp_KesiYita_NoFEM            
