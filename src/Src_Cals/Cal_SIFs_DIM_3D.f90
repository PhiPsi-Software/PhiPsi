!-----------------------------------------------------------
! Brief: Compute 3D SIFs (KI, KII, KIII) by the displacement
!        interpolation method on each crack front edge.
!
! Parameters:
!   Input:  iter    - current load step / iteration index
!           c_DISP  - global displacement vector
!           Tab_Num - indentation for log output
!
! Notes:   Walks the discretized crack-front edges, offsets
!          two points on each side, transforms relative
!          displacements into the local crack frame, and
!          extracts the three SIFs from the DIM relations.
!-----------------------------------------------------------
    ...
    do i_V =1,num_Cr_Edges
        ...
        if(Key_Crack_Aperture_Method==1) then
            ...
            do i_R_Point = 1,Num_of_SIFs_R_Points
                ...
                Signed_Dis,Signed_Dis_v2,c_Yes_Node_PER_in_FS, c_PER_Node_to_FS,Yes_Found_Min_Signed_Dis,c_n_Vector)
        ...
        if(Key_Crack_Aperture_Method==2) then    
            do i_R_Point = 1,Num_of_SIFs_R_Points
                ...
                Offseted_Points_Up(i_R_Point,3), Ele_Num_Cache,up_Elem_num(i_R_Point))
                call Cal_Ele_Num_by_Coors_3D(Offseted_Points_Dn(i_R_Point,1),Offseted_Points_Dn(i_R_Point,2), &
                Offseted_Points_Dn(i_R_Point,3), Ele_Num_Cache,dn_Elem_num(i_R_Point))
