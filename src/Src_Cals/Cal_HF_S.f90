!-----------------------------------------------------------
! Brief: Build the HF source vector S for 2D flow coupling.
!
! Parameters:
!   Input:  Counter    - current iteration counter
!           total_time - current accumulated time
!   Output: S          - fluid source/sink vector at all calc pts
!
! Notes:   Injects Inject_Q at the injection calc point (halved
!          for symmetric models) and adds Carter-model leakoff
!          contributions when leakoff is enabled.
!-----------------------------------------------------------

subroutine Cal_HF_S(Counter,S,total_time)
! Calculation of flow-structure interaction S vector
...
real(kind=FT) Theor_Time,Theor_Pres,delta_S,delta_Time, Theor_Aper
...
if(total_time > maxval(Inject_Q_Time))then
    print *,'    ~~~~~~!~~~~~~!~~~~~~~!~~~~~~!~~~~~~!~~~~~~!~~~~'
    print *,'    Error:: time beyonds the range of ' //'Inject_Q_Time!'
    print *,'    ~~~~~~!~~~~~~!~~~~~~~!~~~~~~!~~~~~~!~~~~~~!~~~~'
    ...
    if (Cracks_HF_State(i_C) == 1) then  
        ...
        do i_CalP = 1,Num_Div_Points
            ...
            if(Key_Leakoff==1)then
                call Tool_Length_CalP_to_InjP(i_C, Length_Out,CalP_x,CalP_y,i_CalP)
                call Cal_HF_Theoretic_Pressure(1,Counter, Length_Out,Theor_Time,Theor_Pres,Theor_Aper)
