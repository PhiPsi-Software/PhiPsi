...
!     ------------------------------------------------- !
      ...
      subroutine Cal_Weighted_Ave_Stress_without_Calculated_Gauss_Stress
     &                       (c_Key_Ave_Stress,
     &                       Point,
     &                       Search_R,a_Weight,l_Ordina,
     &                       WA_S_xx,WA_S_yy,WA_S_xy)
c     Calculate the weighted average stress at a certain point without computing the stress at all Gauss points.
c     Therefore, this subroutine needs to calculate the stresses at the involved Gauss points.
     ...
     &                             l_Ordina
      ...
      do i_E = 1,num_Surround_Ele
          ...
          do i_G = 1,num_GP_Elem(c_E)
              ...
              call Cal_KesiYita_by_Coor([x_GP,y_GP],c_E,in_Kesi,in_Yita)
              call Cal_Any_Point_Stress_KesiYita(c_E ,in_Kesi,
     &                         in_Yita,i_G,DISP,Sxx_GP, Syy_GP, Sxy_GP)  
     
     
              if(Key_InSitu_Strategy==2)then
                  Sxx_GP=Sxx_GP
     &              + sum(InSitu_Strs_Gaus_xx(c_E,1:Num_Gauss_P_FEM))/
     &                Num_Gauss_P_FEM
                  Syy_GP=Syy_GP
     &              + sum(InSitu_Strs_Gaus_yy(c_E,1:Num_Gauss_P_FEM))/
     &                Num_Gauss_P_FEM
                  Sxy_GP=Sxy_GP
     &              + sum(InSitu_Strs_Gaus_xy(c_E,1:Num_Gauss_P_FEM))/
     &                Num_Gauss_P_FEM   
              endif              
      ...
      RETURN
      END SUBROUTINE Cal_Weighted_Ave_Stress_without_
     &               Calculated_Gauss_Stress