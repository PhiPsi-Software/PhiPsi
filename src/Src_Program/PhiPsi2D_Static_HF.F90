!-----------------------------------------------------------
! Brief: 2D hydraulic fracturing static analysis driver.
!
! Notes:   Orchestrates the HF simulation: time stepping, load
!   increment, Newton-Raphson with line-search backtracking,
!   contact detection, assembly of global K/Q/H matrices,
!   sparse linear solve, and update of pressure, displacement,
!   and crack state. Supports closure, T-shaped cracks, and
...
use module_Cal_HF_Matrix_H_Linear
...
real(kind=FT) Last_Cr_CalP_Pres(Max_Num_Cr,Max_Num_Cr_CalP)
real(kind=FT) Last_Cr_CalP_Aper_ifra(Max_Num_Cr,Max_Num_Cr_CalP)
real(kind=FT) Last_Cr_CalP_Pres_ifra(Max_Num_Cr, Max_Num_Cr_CalP)
real(kind=FT) Temp_Cr_CalP_Aper(Max_Num_Cr,Max_Num_Cr_CalP)
...
!---------------------------
1001 FORMAT(' >>  MNR iteration ',I3,' of fracturing step',I3,'(',I3,') started:')
...
1131 FORMAT(5X,'KI and KII of crack ',I5,' tip 1 are ', F18.6,' and ',F12.6,' MPa.m^(1/2)')
1132 FORMAT(5X,'KI and KII of crack ',I5,' tip 2 are ', F18.6,' and ',F12.6,' MPa.m^(1/2)')
1201 FORMAT(5X,'Computing counter: ',I6)
5001 FORMAT('     Elapsed CPU time: ',F7.3,' mins')
...
!-------------------------------------------------------------
!Handling natural fractures related: Generate initial natural 
!fractures (if needed), and based on the natural fractures
!Perform relevant data processing based on the type
!-------------------------------------------------------------
call Natural_Fractures_Related_Preparation

!--------------------------------------------------------------------------------------------
!Some logical checks and data corrections:
!(1) Friction cracks cannot contain water (only applicable to third-type natural fractures).
!(2) The current version of the program can only have one initial water-driven fracture.
!--------------------------------------------------------------------------------------------
call HF_Logic_Checking

!-----------------------------------------------------------------------------------------
!Calculate the traditional degree-of-freedom displacement field under 
!the action of in-situ stress (far-field stress), and calculate the in-situ stress level.
!-----------------------------------------------------------------------------------------
...
do ifra = 1,Num_Frac   
    ...
    do iter = 1,Max_MNR_Iter
        ...
        if (iter == 1) then
            ...
            call Boundary_Cond(Total_FD,Counter_Iter,freeDOF,num_FreeD)
            ...
            call Boundary_Cond_HF(Counter_Iter,freeDOF_HF,num_free_CalP,Local_freeDOF_HF)
            ...
            Total_freeDOF(num_FreeD+1:num_Total_FD) =freeDOF_HF(1:num_free_CalP)
            ...
            if(Key_Propp_Trans==1 .and. ifra >1)then
                call Cal_HF_Mapping_ifrac_old2new(ifra,Counter_Iter, Old_total_Time,total_Time,Yes_Growth,L_Cracks_CalP_Conc, &
            ...
            !************************************************************************
            call Force_Vector(Total_FD,Counter_Iter,.False.,Lambda,F_U)
            F_U_Backup = F_U
            ...
            !******************************************************************************
            call Cal_HF_Matrix_Q_Linear(Counter_Iter,Coupled_Q,Total_FD)
            ...
            call Cal_HF_Initial_Pressure(iter,ifra,Counter_Iter, Yes_Growth,Last_Cracks_CalP_Num_ifra, &
            ...
            do i_C = 1,num_Crack
                ...
                if (Cracks_HF_State(i_C) == 1) then
                    do i_CalP=1,Cracks_CalP_Num(i_C)
                        ...
                        Last_CalP_Pres(num_Count) =Last_Cr_CalP_Pres(i_C,i_CalP)
                    end do
                end if
            end do

            ! Assemble the load vector for calculating the initial displacement
            F(freeDOF(1:num_FreeD)) =MATMUL(Coupled_Q(freeDOF(1:num_FreeD),Local_freeDOF_HF(1:num_free_CalP)), &
            thick(1)* &
            Last_CalP_Pres(Local_freeDOF_HF(1:num_free_CalP))) + F_U(freeDOF(1:num_FreeD))
            ...
            call Assemble_Stiffness_Matrix_XFEM(Counter_Iter,globalK,Total_FD,Total_Num_G_P)
            ...
            !************************
            call Matrix_Solve_LSOE(0,1,Key_SLOE, globalK(freeDOF(1:num_FreeD),freeDOF(1:num_FreeD)), &
            F(freeDOF(1:num_FreeD)),delta_x(1:num_FreeD),num_FreeD)
            ! Initialize displacement vector
            DISP(1:Total_FD) = ZR
            ! Get displacement vector
            Initial_DISP = ZR
            Initial_DISP(freeDOF(1:num_FreeD)) =delta_x(1:num_FreeD)
            ...
            call Tool_Get_Current_Time(current_data,date_time,c_CT_time_s)
            if (Key_Contact /= 0) then
                Contact_DISP(1:Total_FD) =ZR
                ! Displacement for contact iteration
                Contact_DISP(freeDOF(1:num_FreeD))=delta_x(1:num_FreeD)
                call Determine_Contact_State_by_Iteration( iter,ifra,Counter_Iter,Contact_DISP,Total_FD, &
            ...
            call Tool_Get_Current_Time(current_data,date_time,c_CT_time_f)
            Total_CT_CPU_time =  Total_CT_CPU_time +(dble(c_CT_time_f)-dble(c_CT_time_s))
            ...
            call Cal_HF_Initial_Aperture(iter,ifra,Counter_Iter,Last_Cracks_CalP_Num_ifra, Last_Cr_CalP_Aper_ifra,Last_Cr_CalP_Aper)

            !***********************************************************
            ! Calculate delta_Time (based on overall mass conservation)
            !***********************************************************
            call Cal_HF_delta_Time_Linear(Counter_Iter,Last_Cr_CalP_Aper,total_Time,delta_Time)


            total_Time = total_Time+delta_Time
            print*,'    Delta time: ',delta_Time,delta_Time/Con_60
            print*,'    Current time: ',total_Time,total_Time/Con_60
            ...
            !************************************************************************
            call Cal_HF_Matrix_H_Linear(ifra,Counter_Iter, H,Cracks_CalP_Aper,total_Time)
            ...
            call Cal_HF_Resid(ifra,iter,Counter_Iter,R, Total_FD,num_FreeD,num_free_CalP,globalK, &
            Coupled_Q,F_U,Last_DISP,Last_Last_DISP, freeDOF,freeDOF_HF,Local_freeDOF_HF, Last_CalP_Pres,delta_Time,H,c_S)


            !*****************************************************
            ! Assemble Newton-Raphson Iterative Derivative Matrix
            !*****************************************************
            call Cal_HF_Jacobian_NR(Counter_Iter,NR_Deri,Total_FD,num_FreeD, num_free_CalP,globalK,Coupled_Q,freeDOF, &
            ...
            if (Num_JunPair > 0) then
                call Cal_HF_Jacobian_PenaltyFunc(Counter_Iter, NR_Deri,Total_FD,num_FreeD,num_free_CalP,freeDOF,freeDOF_HF)
            end if
            ...
            !******************************************************
            ! Timestamp Start
            call Tool_Get_Current_Time(current_data,date_time,c_LS_time_s)

            call Matrix_Solve_LSOE(0,1,Key_SLOE, NR_Deri(Total_freeDOF(1:num_Total_FD),Total_freeDOF(1:num_Total_FD)), &
            R(Total_freeDOF(1:num_Total_FD)),delta_x,num_FreeD+num_free_CalP)
            !********************************************
            ...
            if(Key_HF_LineSearch==1)then
                call Cal_HF_Line_Searching(ifra,iter,Counter_Iter, num_ALlDOF,Total_FD,num_FreeD,num_free_CalP,Total_freeDOF, &
                num_Tol_CalP_Water,num_Total_FD,freeDOF_HF, Local_freeDOF_HF,NR_Deri,delta_x, R,Initial_DISP,DISP,freeDOF, &
                Last_CalP_Pres,globalK,Coupled_Q,F_U, Temp_Cr_CalP_Aper,Temp_total_Time, Num_Line_Search,total_Time,x)
            endif

            ! Timestamp ended
            call Tool_Get_Current_Time(current_data,date_time,c_LS_time_f)
            Total_LS_CPU_time =  Total_LS_CPU_time +(dble(c_LS_time_f)-dble(c_LS_time_s))
            ...
            if(Key_HF_LineSearch==1)then
                ...
                CalP_Pres(Local_freeDOF_HF(1:num_free_CalP)) =x(num_FreeD+1:num_Total_FD)
            elseif(Key_HF_LineSearch==0)then
                DISP(freeDOF(1:num_FreeD)) =DISP(freeDOF(1:num_FreeD)) -delta_x(1:num_FreeD)
                CalP_Pres(Local_freeDOF_HF(1:num_free_CalP)) = Last_CalP_Pres(Local_freeDOF_HF(1:num_free_CalP))- &
                delta_x(num_FreeD+1:num_FreeD+num_free_CalP)
            endif
            ! Water pressure cannot be negative
            if(Key_HF_Del_Ne_Pres==1)then
                call Vector_ZeroOut_Neg_value(CalP_Pres,num_Tol_CalP_Water)
            endif
            !************************************************************************
            ! Transform the pressure vectors of all calculation points to each crack
            !************************************************************************
            call Tool_HF_Pres_Transform(Counter_Iter,CalP_Pres,Cracks_CalP_Pres)
            call Print_to_Screen_Value_Crack1(4,'PRESS')

            !************************************
            ! Calculate the flow rate at a point
            !************************************
            call Cal_HF_Flow_Quan(ifra,iter,Counter_Iter,total_time)
        ...
        elseif (iter >= 2) then
            ...
            if (Key_Contact /= 0) then
                ...
                F_CT(1:Total_FD) = ZR
                F_CT(freeDOF(1:num_FreeD)) = MATMUL(Coupled_Q(freeDOF(1:num_FreeD),Local_freeDOF_HF(1:num_free_CalP)), &
            ...
            elseif (iter >= 3) then
                ...
                call Read_Disp_to(Counter_Iter-2,Last_Last_DISP,Total_FD)
            end if
            ...
            !******************************************************
            print *,'    Calculating residual vector R......'
            call Cal_HF_Resid(ifra,iter,Counter_Iter,R,Total_FD,num_FreeD, num_free_CalP,globalK,Coupled_Q,F_U, &
            Last_DISP,Last_Last_DISP, freeDOF,freeDOF_HF,Local_freeDOF_HF,CalP_Pres, delta_Time,H,c_S)

            !***************************************************************************
            ! Modify the Newton-Raphson iteration derivative matrix (bottom-right part)
            !***************************************************************************
            call Cal_HF_Modify_Jacobian(Counter_Iter,NR_Deri, Total_FD,num_free_CalP,freeDOF_HF, Local_freeDOF_HF,delta_Time,H)
            ...
            if(Key_HF_LineSearch==1)then
                call Cal_HF_Line_Searching(ifra,iter,Counter_Iter,num_ALlDOF, Total_FD,num_FreeD,num_free_CalP,Total_freeDOF, &
                num_Tol_CalP_Water,num_Total_FD,freeDOF_HF, Local_freeDOF_HF,NR_Deri,delta_x, R,Initial_DISP,DISP,freeDOF, &
                CalP_Pres,globalK,Coupled_Q,F_U, Temp_Cr_CalP_Aper,Temp_total_Time, Num_Line_Search,total_Time,x)
            endif

            call Tool_Get_Current_Time(current_data,date_time,c_LS_time_f)
            ifra_LS_CPU_time(ifra) = ifra_LS_CPU_time(ifra)+(dble(c_LS_time_f)-dble(c_LS_time_s))
            ...
            elseif(Key_HF_LineSearch==0)then
                DISP(freeDOF(1:num_FreeD)) =DISP(freeDOF(1:num_FreeD)) -delta_x(1:num_FreeD)
                CalP_Pres(Local_freeDOF_HF(1:num_free_CalP)) = CalP_Pres(Local_freeDOF_HF(1:num_free_CalP))- &
                delta_x(num_FreeD+1:num_FreeD+num_free_CalP)
            endif
            ! Water pressure cannot be negative
            if(Key_HF_Del_Ne_Pres==1)then
            ...
            if(Key_Unit_System==1)then
                print *,'    Max pressure of crack 1(MPa):',maxval(Cracks_CalP_Pres(1,1:Cracks_CalP_Num(1)))/Cn_M
            ...
            elseif(Key_Unit_System==2)then
                print *,'    Max pressure of crack 1(m*2/s):',maxval(Cracks_CalP_Quan(1,1:Cracks_CalP_Num(1)))/Cn_M
            endif
            !*******************
            ! Save displacement
            !*******************
            ...
            !****************************************************
            call Cal_HF_CheckConv(Counter_Iter,Last_Cr_CalP_Aper,Last_Cr_CalP_Pres,Yes_Conv)
            ...
            !******************************************************
            if ((iter ==Max_MNR_Iter).and. (Yes_Conv.eqv..False.)) then
    ...
    if(Key_Propp_Trans==1)then
        ...
        call Cal_HF_Proppant_c(ifra,Counter_Iter,Old_total_Time,total_Time,Yes_Growth)
    ...
    if (Key_Post_S_Dof_F == 1) then
        F(1:Total_FD) = ZR
        F(freeDOF(1:num_FreeD)) = MATMUL(Coupled_Q(freeDOF(1:num_FreeD),Local_freeDOF_HF(1:num_free_CalP)), &
        thick(1)*CalP_Pres(Local_freeDOF_HF(1:num_free_CalP)))  + F_U(freeDOF(1:num_FreeD))
    ...
    if (Key_Post_S_TanDisp==1)then
        ...
        call Cal_Crack_Tan_Relative_Disp(Counter_Iter,DISP,Cracks_CalP_Tan_Aper)
        call Save_Crack_Tan_Relative_Disp(Counter_Iter,Cracks_CalP_Tan_Aper)
    ...
    if (Key_Post_CS_G_Coor==1) then
        ...
        call Cal_Gauss_Coors(Counter_Iter,Total_Num_G_P,Gauss_CoorX,Gauss_CoorY)
        ! Save Gauss point coordinates
        call Save_Gauss_Coors(Counter_Iter,Total_Num_G_P,Gauss_CoorX,Gauss_CoorY)
    end if
    ...
    do i_C=1,num_Crack
        if(Cracks_HF_State(i_C)==1)then
            do i_CalP = 1,Cracks_CalP_Num(i_C)
                ...
                CalP_Pres(t_num_CalP)= CalP_Pres(t_num_CalP)+Cracks_CalP_Remo_Strs(i_C,i_CalP)*ONE
            end do
        endif
    end do
    ...
    F(freeDOF(1:num_FreeD))=MATMUL(Coupled_Q(freeDOF(1:num_FreeD),1:num_Tol_CalP_Water), &
    thick(1)*CalP_Pres(1:num_Tol_CalP_Water))+ F_U_Backup(freeDOF(1:num_FreeD))
    ...
    ! NEWFTU-2025122602.
    !***********************************************************************************
    call Cal_Shaped_Cracks_2D(Counter_Iter,POST_DISP)
    ...
    if (Key_Propagation ==1) then
        ...
        do i_C = 1,num_Crack
            ...
            elseif(Key_Unit_System==2)then
                ...
                WRITE(*,1131)i_C,KI(i_C,1)/trans_factor,KII(i_C,1)/trans_factor
                WRITE(*,1132)i_C,KI(i_C,2)/trans_factor,KII(i_C,2)/trans_factor
            endif
        end do
    ...
    !---------------------------------------------------------------------
    call Tool_Get_Current_Time(current_data,date_time,curent_time)
    ...
    if(Num_Frac>=2) then
        call Write_Lfrac_Vars_to_Memory_HF(ifra,Counter_Iter,Old_total_Time,total_Time,Yes_Growth)
    end if
