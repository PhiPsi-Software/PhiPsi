...
1104      FORMAT(5X,'Total active CPC:         ',I8)
          ...
          if(Key_Propp_Trans==1 .and. ifra >1)then
              call Cal_HF_Mapping_ifrac_old2new(ifra,Counter_Iter, &
                     Old_total_Time,total_Time,Yes_Growth,L_Cracks_CalP_Conc, &
...
1301             FORMAT(5X,'Total num of Gauss Points:     ',I8)
          ...
          call Cal_HF_Initial_Aperture(iter,ifra,Counter_Iter,Last_Cracks_CalP_Num_ifra, &
          ...
          !*****************************************************
          call Cal_HF_Jacobian_NR(Counter_Iter,NR_Deri,Total_FD,num_FreeD,&
          ...
          if (Num_JunPair > 0) then
              call Cal_HF_Jacobian_PenaltyFunc(Counter_Iter,&
                        NR_Deri,Total_FD,num_FreeD,num_free_CalP,freeDOF,freeDOF_HF)
          end if
          ...
          call Matrix_Solve_LSOE(0,1,Key_SLOE, &
                 NR_Deri(Total_freeDOF(1:num_Total_FD),Total_freeDOF(1:num_Total_FD)), &
          ...
          if(Key_HF_LineSearch==1)then
            call Cal_HF_Line_Searching(ifra,iter,Counter_Iter, &
                 num_ALlDOF,Total_FD,num_FreeD,num_free_CalP,Total_freeDOF, &
      ...
      elseif (iter >= 2) then
          ...
          if (Key_Contact /= 0) then
              ...
              F_CT(freeDOF(1:num_FreeD)) =  &
                  MATMUL(Coupled_Q(freeDOF(1:num_FreeD),Local_freeDOF_HF(1:num_free_CalP)), &
...
50             continue
          ...
          !******************************************************
          print *,'    Calculating residual vector R......'
          call Cal_HF_Resid(ifra,iter,Counter_Iter,R,Total_FD,num_FreeD, &
                              num_free_CalP,globalK,Coupled_Q,F_U, &
                              Last_DISP,Last_Last_DISP, &
                              freeDOF,freeDOF_HF,Local_freeDOF_HF,CalP_Pres, &
                              delta_Time,H,c_S)
          ...
          call Tool_Get_Current_Time(current_data,date_time,c_LS_time_s)
          if(Key_HF_LineSearch==1)then
            call Cal_HF_Line_Searching(ifra,iter,Counter_Iter,num_ALlDOF, &
