!-----------------------------------------------------------
! Brief: Parse the .kpp keywords file into module variables.
!
! Notes:   Reads the keyword control file line by line,
!   strips comments, splits instructions from parameter
!   lines, and dispatches each instruction to the correct
!   global module (cracks, holes, materials, dynamic, HF,
!   cohesive, nonlinear, field problem, surface load, etc.).
!   Honors -i and -n command-line flags for input file and
!   OpenMP thread count.
!-----------------------------------------------------------
...
integer Int_Temp_Value_4(4),Int_Temp_Value_3(3)
integer :: ios, num_len
...
!2026-06-22.
DO I_INFO=1,Num_Instruction_line
    if(Instructions(I_INFO)(1:13)=='*MAX_NUM_CR_P' .and. Instructions(I_INFO)(14:14)==' ')THEN
        READ(Instructions(I_INFO+1), *) Max_Num_Cr_P
    endif
    if(Instructions(I_INFO)(1:16)=='*MAX_NUM_CR_CALP' .and. Instructions(I_INFO)(17:17)==' ')THEN
        READ(Instructions(I_INFO+1), *) Max_Num_Cr_CalP
    endif
ENDDO
if (ALLOCATED(Crack_Coor)) deallocate(Crack_Coor)
allocate(Crack_Coor(Max_Num_Cr,Max_Num_Cr_P,2)) 
...
DO I_INFO=1,Num_Instruction_line
    ...
    IF(Instructions(I_INFO)(1:33)=='*KEY_3D_SIFS_IIM_NUM_GAUSS_POINTS' .and. Instructions(I_INFO)(34:34)==' ')THEN
        ...
        if (     Key_3D_Sifs_IIM_num_Gauss_Points/=2**3 .and. Key_3D_Sifs_IIM_num_Gauss_Points/=4**3 &
        .and. Key_3D_Sifs_IIM_num_Gauss_Points/=6**3 .and. Key_3D_Sifs_IIM_num_Gauss_Points/=8**3 &
...
!2026-03-24.
IF(Instructions(I_INFO)(1:33)=='*EDY_DATA_SAVE_INTERVAL_TIMESTEPS' .and. Instructions(I_INFO)(34:34)==' ')THEN
    READ(Instructions(I_INFO+1), *) EDY_Data_Save_Interval_Timesteps
ENDIF
IF(Instructions(I_INFO)(1:33)=='*EDY_INFO_SHOW_INTERVAL_TIMESTEPS' .and. Instructions(I_INFO)(34:34)==' ')THEN
    READ(Instructions(I_INFO+1), *) EDY_Info_Show_Interval_Timesteps
ENDIF
IF(Instructions(I_INFO)(1:15)=='*EDY_TOTAL_TIME' .and. Instructions(I_INFO)(16:16)==' ')THEN
    READ(Instructions(I_INFO+1), *) EDY_Total_Time
ENDIF
IF(Instructions(I_INFO)(1:15)=='*EDY_FORCE_TIME' .and. Instructions(I_INFO)(16:16)==' ')THEN
    READ(Instructions(I_INFO+1), *) EDY_Force_Time
ENDIF

!2026-03-30.
IF(Instructions(I_INFO)(1:27)=='*EDY_KEY_CAL_CRACK_APERTURE' .and. Instructions(I_INFO)(28:28)==' ')THEN
    READ(Instructions(I_INFO+1), *) EDY_Key_Cal_Crack_Aperture
ENDIF

!2026-03-31. NEWFTU-2026033001.
IF(Instructions(I_INFO)(1:19)=='*EDY_KEY_CAL_ENERGY' .and. Instructions(I_INFO)(20:20)==' ')THEN
    READ(Instructions(I_INFO+1), *) EDY_Key_Cal_Energy
ENDIF

!2026-04-06. NEWFTU-2026040601.
IF(Instructions(I_INFO)(1:26)=='*EDY_INTERNAL_FORCE_SCHEME' .and. Instructions(I_INFO)(27:27)==' ')THEN
    READ(Instructions(I_INFO+1), *) EDY_Internal_Force_Scheme
ENDIF

!2026-04-16. 
IF(Instructions(I_INFO)(1:24)=='*EQ_AC_NODES_LIST_METHOD' .and. Instructions(I_INFO)(25:25)==' ')THEN
    READ(Instructions(I_INFO+1), *) EQ_Ac_nodes_list_method
ENDIF

!2026-04-17. 
IF(Instructions(I_INFO)(1:34)=='*KEY_EDY_SAVE_NODAL_VELOCITY_ACCEL' .and. Instructions(I_INFO)(35:35)==' ')THEN
    READ(Instructions(I_INFO+1), *) Key_EDY_Save_Nodal_Velocity_Accel
ENDIF

!2026-04-20. 
IF(Instructions(I_INFO)(1:27)=='*KEY_EDY_LUMPED_MASS_SCHEME' .and. Instructions(I_INFO)(28:28)==' ')THEN
    READ(Instructions(I_INFO+1), *) Key_EDY_Lumped_Mass_Scheme
ENDIF

!2026-04-21. 
IF(Instructions(I_INFO)(1:25)=='*KEY_EDY_SAVE_LUMPED_MASS' .and. Instructions(I_INFO)(26:26)==' ')THEN
    READ(Instructions(I_INFO+1), *) Key_EDY_Save_Lumped_Mass
ENDIF

!2026-04-23.
if(Instructions(I_INFO)(1:41)=='*KEY_EDY_CONTACT_PENALTY_PARAMETER_SCHEME' .and. Instructions(I_INFO)(42:42)==' ')THEN
    READ(Instructions(I_INFO+1), *) Key_EDY_Contact_Penalty_Parameter_Scheme
endif

!2026-04-26.
if(Instructions(I_INFO)(1:30)=='*EDY_NUM_CRACK_GAUSS_POINTS_3D' .and. Instructions(I_INFO)(31:31)==' ')THEN
    READ(Instructions(I_INFO+1), *) EDY_num_Crack_Gauss_Points_3D
endif
if(Instructions(I_INFO)(1:25)=='*EDY_CONTACT_APERTURE_TOL' .and. Instructions(I_INFO)(26:26)==' ')THEN
    READ(Instructions(I_INFO+1), *) EDY_Contact_Aperture_Tol
endif

!2026-05-03.
if(Instructions(I_INFO)(1:33)=='*EDY_SAVE_SIFS_AT_SPECIFIED_POINT' .and. Instructions(I_INFO)(34:34)==' ')THEN
    CALL TOOL_GET_NUM_DATA_FROM_A_STRING(Instructions(I_INFO+1),256,NUM_DATA,YES_EVEN)
    if(NUM_DATA/=3)then
        print *, '    Error :: NUM_DATA should = 3 for *EDY_SAVE_SIFS_AT_SPECIFIED_POINT!'
        call Warning_Message('S',Keywords_Blank)
    else
        READ(Instructions(I_INFO+1),*) EDY_Save_SIFs_at_Specified_Point(1:NUM_DATA)
    endif
endif

!2026-05-05.
if(Instructions(I_INFO)(1:31)=='*USER_SPECIFIED_CRACK_MESH_SIZE' .and. Instructions(I_INFO)(32:32)==' ')THEN
    READ(Instructions(I_INFO+1), *) User_Specified_Crack_Mesh_Size
endif

!2026-05-07.
if(Instructions(I_INFO)(1:37)=='*EDY_SAVE_APERTURE_AT_SPECIFIED_POINT' .and. Instructions(I_INFO)(38:38)==' ')THEN
    CALL TOOL_GET_NUM_DATA_FROM_A_STRING(Instructions(I_INFO+1),256,NUM_DATA,YES_EVEN)
    if(NUM_DATA/=7)then
        print *, '    Error :: NUM_DATA/=7 for *EDY_Save_Aperture_at_Specified_Point!'
        call Warning_Message('S',Keywords_Blank)
    else
        READ(Instructions(I_INFO+1),*) EDY_Save_Aperture_at_Specified_Point(1:NUM_DATA)
    endif
endif

!2026-05-07.
if(Instructions(I_INFO)(1:49)=='*EDY_CRACK_PROPAGATON_AT_SPECIFIED_TIME_AND_SPEED' .and. Instructions(I_INFO)(50:50)==' ')THEN
    CALL TOOL_GET_NUM_DATA_FROM_A_STRING(Instructions(I_INFO+1),256,NUM_DATA,YES_EVEN)
    if(NUM_DATA/=2)then
        print *, '    Error :: NUM_DATA should =2 for *EDY_Crack_Propagaton_at_Specified_Time_and_Speed!'
        call Warning_Message('S',Keywords_Blank)
    else
        READ(Instructions(I_INFO+1),*) EDY_Crack_Propagaton_at_Specified_Time_and_Speed(1:NUM_DATA)
    endif
endif

!2026-05-11.
if(Instructions(I_INFO)(1:33)=='*EDY_SAVE_SIFS_AT_SPECIFIED_PLANE' .and. Instructions(I_INFO)(34:34)==' ')THEN
    CALL TOOL_GET_NUM_DATA_FROM_A_STRING(Instructions(I_INFO+1),256,NUM_DATA,YES_EVEN)
    if(NUM_DATA/=7)then
        print *, '    Error :: NUM_DATA should =7 for *EDY_Save_SIFs_at_Specified_Plane!'
        call Warning_Message('S',Keywords_Blank)
    else
        READ(Instructions(I_INFO+1),*) EDY_Save_SIFs_at_Specified_Plane(1:NUM_DATA)
    endif
endif

!2026-05-05.
if(Instructions(I_INFO)(1:35)=='*KEY_SAVE_OLD_SOLID_EL_TIP_BASELINE' .and. Instructions(I_INFO)(36:36)==' ')THEN
    READ(Instructions(I_INFO+1), *) Key_Save_Old_Solid_El_Tip_BaseLine
endif

!2026-05-20.
if(Instructions(I_INFO)(1:30)=='*DY_SAVE_SIFS_AT_SPECIFIED_TIP' .and. Instructions(I_INFO)(31:31)==' ')THEN
    CALL TOOL_GET_NUM_DATA_FROM_A_STRING(Instructions(I_INFO+1),256,NUM_DATA,YES_EVEN)
    if(NUM_DATA/=2)then
        print *, '    Error :: NUM_DATA should =2 for *DY_Save_SIFs_at_Specified_Tip!'
        call Warning_Message('S',Keywords_Blank)
    else
        READ(Instructions(I_INFO+1),*) DY_Save_SIFs_at_Specified_Tip(1:NUM_DATA)
    endif
endif


!2026-05-23.
if(Instructions(I_INFO)(1:17)=='*KEY_MAP_TIP_TO_H' .and. Instructions(I_INFO)(18:18)==' ')THEN
    READ(Instructions(I_INFO+1), *) Key_Map_Tip_to_H
endif
if(Instructions(I_INFO)(1:27)=='*KEY_MAP_TIP_TO_H_LAST_LAST' .and. Instructions(I_INFO)(28:28)==' ')THEN
    READ(Instructions(I_INFO+1), *) Key_Map_Tip_to_H_Last_Last
endif
if(Key_Map_Tip_to_H==1) then
    do i = 1, 100
        ! Construct pattern string, e.g., '*MAP_TIP_TO_H_1', '*MAP_TIP_TO_H_10'
        write(num_str, '(I0)') i
        pattern = '*MAP_TIP_TO_H_' // trim(adjustl(num_str))
        num_len = len_trim(pattern)
        ! Check if current instruction matches the pattern and the next character is a space
        if (Instructions(I_INFO)(1:num_len) == pattern(1:num_len) .and. Instructions(I_INFO)(num_len+1:num_len+1) == ' ') then

            call TOOL_GET_NUM_DATA_FROM_A_STRING(Instructions(I_INFO+1), 256, NUM_DATA, YES_EVEN)

            if (NUM_DATA /= 4) then
                print *, '    Error :: NUM_DATA should =4 for ', trim(pattern)
                call Warning_Message('S', Keywords_Blank)
            else
                read(Instructions(I_INFO+1), *) Int_Temp_Value_4(1:4)
                ! Note: Temp_Value_4 in the original code should be Int_Temp_Value_4
                Data_Map_Tip_to_H(Int_Temp_Value_4(1), 1:4) = Int_Temp_Value_4(1:4)
            end if
            exit
        end if
    end do
endif
if(Key_Map_Tip_to_H_Last_Last==1) then
    do i = 1, 100
        ! Construct pattern string, e.g., '*MAP_TIP_TO_H_1', '*MAP_TIP_TO_H_10'
        write(num_str, '(I0)') i
        pattern = '*MAP_TIP_TO_H_LAST_LAST_' // trim(adjustl(num_str))
        num_len = len_trim(pattern)
        ! Check if current instruction matches the pattern and the next character is a space
        if (Instructions(I_INFO)(1:num_len) == pattern(1:num_len) .and. Instructions(I_INFO)(num_len+1:num_len+1) == ' ') then
            call TOOL_GET_NUM_DATA_FROM_A_STRING(Instructions(I_INFO+1), 256, NUM_DATA, YES_EVEN)
            if (NUM_DATA /= 4) then
                ...
                read(Instructions(I_INFO+1), *) Int_Temp_Value_4(1:4)
                Data_Map_Tip_to_H_Last_Last(Int_Temp_Value_4(1), 1:4) = Int_Temp_Value_4(1:4)
            end if
            exit
        end if
    end do
...
!2026-05-24.
if(Instructions(I_INFO)(1:28)=='*KEY_TIPENRICH_RADIUS_FACTOR' .and. Instructions(I_INFO)(29:29)==' ')THEN
    READ(Instructions(I_INFO+1), *) Key_TipEnrich_Radius_Factor
endif

!if(instructions(i_info)(1:40)=='*SET_ALL_NODES_AS_BOTH_HEAVISIDE_AND_TIP' .and. Instructions(I_INFO)(41:41)==' ')THEN
!    READ(Instructions(I_INFO+1), *) Set_All_Nodes_as_Both_Heaviside_and_Tip
!endif


!2026-05-27.
if(Instructions(I_INFO)(1:17)=='*NUM_GAUSS_POINTS' .and. Instructions(I_INFO)(18:18)==' ')THEN
    READ(Instructions(I_INFO+1), *) Num_Gauss_Points
endif

!2026-06-03.
if(Instructions(I_INFO)(1:25)=='*DY_KEEP_INITIAL_VELOCITY' .and. Instructions(I_INFO)(26:26)==' ')THEN
    READ(Instructions(I_INFO+1), *) DY_Keep_Initial_Velocity
endif

!2026-06-03.
if(Instructions(I_INFO)(1:15)=='*KEY_PROPA_TYPE' .and. Instructions(I_INFO)(16:16)==' ')THEN
    READ(Instructions(I_INFO+1), *) Key_Propa_Type
endif
if(Instructions(I_INFO)(1:24)=='*DY_SAVE_CRACK_LENGTH_2D' .and. Instructions(I_INFO)(25:25)==' ')THEN
    READ(Instructions(I_INFO+1), *) DY_Save_Crack_Length_2D
endif

!2026-06-11.
if(Instructions(I_INFO)(1:24)=='*DY_PROPAGATION_L_FACTOR' .and. Instructions(I_INFO)(25:25)==' ')THEN
    READ(Instructions(I_INFO+1), *) DY_Propagation_L_Factor
endif

!2026-06-23.
if(Instructions(I_INFO)(1:18)=='*PROP_ANGLE_ALOWED' .and. Instructions(I_INFO)(19:19)==' ')THEN
    READ(Instructions(I_INFO+1), *) Prop_Angle_Alowed
endif
if(Instructions(I_INFO)(1:18)=='*IIM_RADIUS_FACTOR' .and. Instructions(I_INFO)(19:19)==' ')THEN
    READ(Instructions(I_INFO+1), *) IIM_Radius_Factor
endif

!2026-06-24.
if(Instructions(I_INFO)(1:36)=='*DY_TIME_TO_ACTIVE_CRACK_PROPAGATION' .and. Instructions(I_INFO)(37:37)==' ')THEN
    READ(Instructions(I_INFO+1), *) DY_Time_to_Active_Crack_Propagation
endif

!2026-06-25.
if(Instructions(I_INFO)(1:25)=='*DY_SIFS_IIM_ENABLE_F1_F2' .and. Instructions(I_INFO)(26:26)==' ')THEN
    READ(Instructions(I_INFO+1), *) DY_SIFs_IIM_Enable_f1_f2
endif

!2026-06-25.
if(Instructions(I_INFO)(1:43)=='*DY_SAVE_PROPAGATION_SPEED_AT_SPECIFIED_TIP' .and. Instructions(I_INFO)(44:44)==' ')THEN
    CALL TOOL_GET_NUM_DATA_FROM_A_STRING(Instructions(I_INFO+1),256,NUM_DATA,YES_EVEN)
    if(NUM_DATA/=2)then
        print *, '    Error :: NUM_DATA should =2 for *DY_Save_Propagation_Speed_at_Specified_Tip!'
        call Warning_Message('S',Keywords_Blank)
    else
        READ(Instructions(I_INFO+1),*) DY_Save_Propagation_Speed_at_Specified_Tip(1:NUM_DATA)
    endif
endif

!2026-06-26.
if(Instructions(I_INFO)(1:37)=='*DY_SIFS_IIM_ENABLE_VELOCITY_COUPLING' .and. Instructions(I_INFO)(37:37)==' ')THEN
    READ(Instructions(I_INFO+1), *) DY_SIFs_IIM_Enable_Velocity_Coupling
endif

!2026-06-28.
if(Instructions(I_INFO)(1:15)=='*KEY_CAL_ENERGY' .and. Instructions(I_INFO)(16:16)==' ')THEN
    READ(Instructions(I_INFO+1), *) Key_Cal_Energy
endif

!2026-06-28.
if(Instructions(I_INFO)(1:22)=='*HF_PRES_INITIAL_GUESS' .and. Instructions(I_INFO)(23:23)==' ')THEN
    READ(Instructions(I_INFO+1), *) HF_Pres_Initial_Guess
endif
