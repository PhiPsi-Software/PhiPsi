!-----------------------------------------------------------
! Brief: 3D quasi-static FEM/XFEM analysis driver.
!
! Notes:   Main 3D static solver. Allocates global K, fixed
!   and free DOF vectors, F_U and F load vectors, and CSR
!   storage for sparse assembly. Supports XFEM crack growth,
!   contact detection, candidate element selection by S1,
!   and L2 projection of enriched values for new nodes.
!-----------------------------------------------------------
...
integer i_Candidate
integer crack_number,element_number,number_nodes_projected
integer,ALLOCATABLE:: nodes_to_be_projected(:) 
real(kind=FT),ALLOCATABLE:: nodes_new_enriched_value(:)
    ...
    do i_Stage = 1,num_Stages_Wellbores(i_WB) 
        do i_Prop = 1,Num_Substeps
            ...
            if(num_Crack.ne.0)then
                ...
                if(Flag_HF_3D==1)then
                    do i_Dof =1,num_FreeD
                        if(allocated(Coupled_Q_3D(freeDOF(i_Dof))%row))then 
                            ...
                            F(freeDOF(i_Dof)) =  F_U(freeDOF(i_Dof))+ dot_product(Coupled_Q_3D(freeDOF(i_Dof))%row(1:3), &
                ...
                elseif(Key_K_Sparse==1)then
                    ...
                    call Assemble_Stiffness_Matrix_SPARS_XFEM_3D(isub, freeDOF(1:num_FreeD),num_FreeD,  &
                ...
                elseif(Key_K_Sparse==1)then
                    ...
                    K_CSR_ia(1:num_FreeD+1), F(freeDOF(1:num_FreeD)), delta_x(1:num_FreeD),num_FreeD)
                ...
                !NEWFTU-2026020801.
                !*************************************************************************
                print *,'    Cut and save boundary cracks if necessary...'
        ...
        elseif (num_Crack.eq.0)then 
            ...
            elseif(Key_K_Sparse==1)then
                ...
                print *,'    Get max NNZ before assembling K......'
                call Assemble_Stiffness_Matrix_SPARS_FEM_3D_Get_MaxNNZ(isub, freeDOF(1:num_FreeD),num_FreeD,Total_FD,K_CSR_NNZ_Max)
                print *,'    K_CSR_NNZ_Max:',K_CSR_NNZ_Max           
                if (allocated(K_CSR_aa)) DEALLOCATE(K_CSR_aa)
                if (allocated(K_CSR_ja)) DEALLOCATE(K_CSR_ja)
                if (allocated(K_CSR_ia)) DEALLOCATE(K_CSR_ia)
                ...
                call Assemble_Stiffness_Matrix_SPARS_FEM_3D(isub, freeDOF(1:num_FreeD),num_FreeD, K_CSR_aa,K_CSR_ja,K_CSR_ia, &
            ...
            elseif(Key_K_Sparse==1)then
                ...
                !#ifndef github
                call Matrix_Solve_LSOE_Sparse(0,1,Key_SLOE, K_CSR_NNZ, K_CSR_aa(1:K_CSR_NNZ), K_CSR_ja(1:K_CSR_NNZ), &
                K_CSR_ia(1:num_FreeD+1), F(freeDOF(1:num_FreeD)), tem_DISP,num_FreeD)
        ...
        !#################################################################################
        if(Key_Post_CS_N_Stra_Cylindr==1 .and. Key_Save_Nothing==0)then
        ...
        ! Compute Node Stress
        !#####################
        if(Key_Post_CS_N_Strs==1 .and. Key_Save_Nothing==0)then
            ...
            if(Enrich_Freedom==0) then
                call Get_Node_Stress_FEM_IN_OUT_3D(.True.,isub,DISP, Stress_xx_Node,Stress_yy_Node,Stress_zz_Node, &
        ...
        !######################################
        if(Key_Post_CS_N_Stra==1 .and. Key_Save_Nothing==0)then  
        ...
        ! stress (2021-09-11)
        !########################################################################################
        if(Key_Post_CS_N_Strs_Cylindr==1 .and. Key_Save_Nothing==0)then
            ! Calculate the transformation matrix from Cartesian coordinates to cylindrical coordinates
            ALLOCATE(Theta_Cartesian_to_Cylinder(Num_Elem,8))
            ALLOCATE(Theta_Cartesian_to_Cylinder_Node(Num_Node))
            call Get_Theta_Cartesian_to_Cylinder
            ...
            ! No XFEM.
            if(Enrich_Freedom==0) then
                call Get_Node_Stress_FEM_IN_OUT_3D(.True.,isub,DISP, Stress_Crr_Node,Stress_Ctt_Node, &
                Stress_Czz_Node, Stress_Crt_Node,Stress_Ctz_Node, Stress_Crz_Node,Stress_Cvm_Node)
                !XFEM.
            else
        ...
        !for test: 3d_dynamic_Freund_TEST_Static_fine2.kpp

        !Test---Test---Test---Test---Test---Test---Test---Test---Test
        !For test: 3d_dynamic_Freund_TEST_Static_fine2.kpp
        !Test subroutine Local_L2_projection_for_inter_element.
        !CDTEST-2026051401. Version 1.59.18.
        !Test---Test---Test---Test---Test---Test---Test---Test---Test
        if(trim(Name_of_Keywords)=="X:\PhiPsi_Project\Test_Examples\3d_dynamic_Freund_TEST_Static_fine2-1.kpp") then
            if(isub==1) then
                crack_number   = 1
                ...
                num_gp = 14
                call Local_L2_projection_for_inter_element(isub,crack_number,num_gp, DISP(1:Total_FD),number_nodes_projected, &
                nodes_to_be_projected(1:number_nodes_projected), nodes_new_enriched_value(1:number_nodes_projected*3))
            endif
        endif

        !Test---Test---Test---Test---Test---Test---Test---Test---Test
        !For test: 3d_dynamic_Freund_TEST_Static_fine2.kpp
        !Test subroutine Local_L2_projection_for_intra_element.
        !CDTEST-2026051402. Version 1.59.18.
        !Test---Test---Test---Test---Test---Test---Test---Test---Test
        if(trim(Name_of_Keywords)=="X:\PhiPsi_Project\Test_Examples\3d_dynamic_Freund_TEST_Static_fine2-2.kpp") then
            if(isub==2) then
                crack_number   = 1
                ...
                nodes_new_enriched_value(1:number_nodes_projected*3) = ZR
                num_gp = 14
                call Local_L2_projection_for_intra_element(isub,crack_number,num_gp, DISP(1:Total_FD),number_nodes_projected, &
                nodes_to_be_projected(1:number_nodes_projected), nodes_new_enriched_value(1:number_nodes_projected*3))
            endif   
        endif
