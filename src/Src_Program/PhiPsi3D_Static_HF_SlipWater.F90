!-----------------------------------------------------------
! Brief: 3D slickwater hydraulic fracturing driver.
!
! Notes:   3D static HF solver with constant water pressure
!   in a double iterative loop. Uses Newton-Raphson or the
!   bisection method to determine the time step, supports
!   natural and post-pressurized fractures alongside HF,
!   and classifies cracks by Crack_Type_Status_3D columns.
!-----------------------------------------------------------
...
do i_WB = 1,num_Wellbore
    ...
    do i_Stage = 1,num_Stages_Wellbores(i_WB)
        ...
        if(Key_3D_HF_Time_Step_Method ==  1)then 
            ...
            !2026-07-02.
            if (HF_Pres_Initial_Guess>=0) then
                Pres_Initial_Guess = HF_Pres_Initial_Guess
            endif
        ...
        do i_Prop = 1,Max_Prop_Steps
            ...
            if(Key_SLOE==11)then  
                ...
                call Assemble_Stiffness_Matrix_SPARS_XFEM_3D_Get_MaxNNZ(isub, freeDOF(1:num_FreeD),num_FreeD,Total_FD,K_CSR_NNZ_Max)
                ...
                call Assemble_Stiffness_Matrix_SPARS_XFEM_3D(isub, freeDOF(1:num_FreeD), &
                num_FreeD, fixedDOF(1:num_FixedD),num_FixedD, &
            ...
            if(Key_3D_HF_Time_Step_Method==1) then
                do i_Time = 1,SlipWater_Max_Time_Steps_3D
                    ...
                    call D3_HF_Get_Pres_by_NR_Method(i_WB,i_Stage,i_Prop, isub,i_Time,c_Stage_Q, &
                    Applied_Time,SlipWater_Max_Pres_Steps_3D, &
                    num_FreeD,Total_FD,num_Tol_CalP_Water,freeDOF(1:num_FreeD),F_U,F,Lambda, diag_precon_no_invert(0:num_FreeD), &
                    ...
                    call D3_Get_Max_Min_Ave_KIeq_of_HF_Cracks(Max_KI_eq_3D,Min_KI_eq_3D, &
                    Ave_KI_eq_3D,c_Crack_Max_Num,c_Vertex_Max_Num)
                    ...
                    call D3_HF_Get_Pres_by_NR_Method(i_WB,i_Stage,i_Prop, isub, &
                    i_Time,c_Stage_Q,Applied_Time,SlipWater_Max_Pres_Steps_3D, &
                    num_FreeD,Total_FD,num_Tol_CalP_Water,freeDOF(1:num_FreeD),F_U,F,Lambda, diag_precon_no_invert(0:num_FreeD), &
            ...
            elseif(Key_3D_HF_Time_Step_Method==2) then 
                ...
                do i_Bisection=1,Max_Bisection
                    ...
                    if(i_Bisection==1)then
                        ...
                        ! Extract the maximum, minimum, and average equivalent stress intensity factors of the HF fracture.
                        call D3_Get_Max_Min_Ave_KIeq_of_HF_Cracks(Max_KI_eq_3D, &
                        Min_KI_eq_3D,Ave_KI_eq_3D,c_Crack_Max_Num,c_Vertex_Max_Num)
                        !KIc
                        ! Obtain the element number of the vertex at the leading edge of the crack with the maximum
                        ! equivalent stress. 2023-05-16. NEWFTU2023051601.
                        c_Elem_num = Cr3D_Meshed_Node_in_Ele_Num(c_Crack_Max_Num)%row(c_Vertex_Max_Num)
