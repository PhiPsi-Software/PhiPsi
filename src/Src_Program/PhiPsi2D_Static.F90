!-----------------------------------------------------------
! Brief: Main driver for 2D quasi-static FEM/XFEM analysis.
!
! Parameters:
!   Input:  none (reads global state and input files)
!   Output: none
!   In/Out: none
!
! Notes:   Performs load stepping, Newton iterations, sparse K assembly,
!          contact/inclusion/HF handling, crack propagation, SIFs, and
!          writes post-processing outputs. The canonical 2D driver.
!-----------------------------------------------------------
...
if (Key_Rand_Circ_Incl==1) then
    ...
    call Tool_Generate_Circles( Ger_Min_X_Coor,Ger_Max_X_Coor, Ger_Min_Y_Coor,Ger_Max_Y_Coor, &
    Rand_Circ_Incl_R,Rand_Circ_Inc_R_Delta, num_Rand_Circ_Incl, (Rand_Circ_Incl_R+Rand_Circ_Inc_R_Delta)*2.5D0, &
    (Rand_Circ_Incl_R+Rand_Circ_Inc_R_Delta)*0.5D0, Circ_Inclu_Coor(1:num_Rand_Circ_Incl,1:2), &
...
if (Key_Rand_Poly_Incl==1) then
    ! Randomly generate a circle, then generate an inscribed polygon (only within the rectangular area)
    call Tool_Generate_Inscribed_Poly( Ger_Min_X_Coor,Ger_Max_X_Coor, Ger_Min_Y_Coor,Ger_Max_Y_Coor, &
    Rand_Poly_Incl_R,Rand_Poly_Inc_R_Delta, num_Rand_Poly_Incl,num_Vert_Poly_Incl, &
...
if (Key_Rand_Poly_Incl_Irregular ==1) then
    ...
    call Tool_Generate_Inscribed_Poly_Irregular( Ger_Min_X_Coor,Ger_Max_X_Coor, Ger_Min_Y_Coor,Ger_Max_Y_Coor, &
    num_Rand_Poly_Incl, &
    num_Vert_Poly_Incl, num_Rand_Poly_Incl_for_Each_Type(1:10), &
    ...
    1:num_Vert_Poly_Incl), Poly_Incl_Coor_y(1:num_Rand_Poly_Incl, 1:num_Vert_Poly_Incl))
    num_Poly_Incl = num_Rand_Poly_Incl
    Poly_Inclu_Edges_Num(1:num_Rand_Poly_Incl)= num_Vert_Poly_Incl
    Poly_Inclu_Mat_Num(1:num_Rand_Poly_Incl) =2
endif
...
if (Key_Random_Hole==1) then
    call Tool_Generate_Circles( Ger_Min_X_Coor,Ger_Max_X_Coor, Ger_Min_Y_Coor,Ger_Max_Y_Coor, Rand_Hole_R,Rand_Hole_R_Delta, &
    num_Rand_Hole, (Rand_Hole_R+Rand_Hole_R_Delta)*2.5D0, (Rand_Hole_R+Rand_Hole_R_Delta)*0.5D0, &
    Hole_Coor(1:num_Rand_Hole,1:2), Hole_Coor(1:num_Rand_Hole,3))
...
do isub = 1,Num_Substeps
    ...
    if(num_Crack.ne.0 .or. num_Hole.ne.0 .or. num_Inclusion.ne.0 )then
        ...
        if(Key_Crack_Inner_Pressure==1)then
            globalF(freeDOF(1:num_FreeD))=globalF(freeDOF(1:num_FreeD))+ MATMUL(Coupled_Q(freeDOF(1:num_FreeD), &
        ...
        if(Key_SLOE==11)then
            ...
            num_FreeD,freeDOF(1:num_FreeD), globalF(freeDOF(1:num_FreeD)),DISP, Total_Num_G_P, storK,size_local,all_local, &
        ...
        if(Key_Contact==0) then
            ...
            elseif(Key_K_Sparse==1)then
                ...
                call Assemble_Stiffness_Matrix_SPARS_XFEM_Get_MaxNNZ(isub, freeDOF(1:num_FreeD),num_FreeD,Total_FD,K_CSR_NNZ_Max)
                ...
                call Assemble_Stiffness_Matrix_SPARS_XFEM(isub, freeDOF(1:num_FreeD),num_FreeD, fixedDOF(1:num_FixedD),num_FixedD, &
            ...
            elseif(Key_K_Sparse==1)then
#ifdef gfortran
                call Matrix_Solve_LSOE_Sparse(0,1,Key_SLOE, K_CSR_NNZ, K_CSR_aa(1:K_CSR_NNZ), K_CSR_ja(1:K_CSR_NNZ), &
                K_CSR_ia(1:num_FreeD+1), globalF(freeDOF(1:num_FreeD)), tem_DISP,num_FreeD)

                !2026-02-18. NEWFTU-2026021801.          
                if (Key_Save_Stiffness_Matrix==1)then
                    !Save CSR matrix to files csra,csrj,csri,csrn. 
                    call Save_Stiffness_Matrix_CSR(isub,K_CSR_NNZ, num_FreeD, Usual_Freedom-num_FixedD+1, &
        ...
        elseif(Key_Contact /= 0) then
            ...
            elseif(Key_K_Sparse==1)then
                ...
                call Assemble_Stiffness_Matrix_SPARS_XFEM(isub, freeDOF(1:num_FreeD),num_FreeD, fixedDOF(1:num_FixedD),num_FixedD, &
                K_CSR_aa,K_CSR_ja,K_CSR_ia, K_CSR_NNZ_Max, &
            ...
            if(Key_K_Sparse==0)then
                call Determine_Contact_State_by_Iteration( isub,isub,isub, &
        ...
        if(Key_SLOE==11 .and. Key_Contact /= 0) then
            call EBE_Determine_Contact_State_by_Iteration(isub, cg_tol,max_cg, &
            num_FreeD,freeDOF(1:num_FreeD), globalF,DISP, storK,size_local,all_local, &
            diag_precon(0:num_FreeD))
        endif
        ...
        if(num_Crack.ne.0)then
            ...
            print *,'    Min aperture of crack 1 (mm):', minval(Cracks_CalP_Aper(1,1:Cracks_CalP_Num(1)))*1000.0D0
        endif
        !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        !%                                                                 %
        !%                                                                 %
    ...
    elseif (num_Crack.eq.0 .and. num_Hole.eq.0 .and. num_Inclusion.eq.0 )then
        ...
        elseif(Key_K_Sparse==1)then
            ...
            print *,'    Get max NNZ before assembling K......'
            call Assemble_Stiffness_Matrix_SPARS_FEM_Get_MaxNNZ(isub, freeDOF(1:num_FreeD),num_FreeD,Total_FD,K_CSR_NNZ_Max)
            print *,'    K_CSR_NNZ_Max:',K_CSR_NNZ_Max    
            ...
            print *,'    Assembling the sparse K......'
            call Assemble_Stiffness_Matrix_SPARS_FEM(isub, freeDOF(1:num_FreeD),num_FreeD, K_CSR_aa,K_CSR_ja,K_CSR_ia, &
    ...
    if(Key_Element_Break==1) then
        if(Key_Element_Break_Rule ==1)then
            ...
            if (Candidate_Elements_S1_num <=Key_Element_Break_num_Elements_Per_Step) then
                ...
                Key_Element_Break_num_Elements_Per_Step, Real_Candidate_Elements(1:Key_Element_Break_num_Elements_Per_Step))
    ...
    if (Key_Cal_Energy==1) then
        ...
        call Cal_Energy_2D(isub, DISP, globalF)
        ...
        call Save_Energy(isub,Elastic_Strain_Energy,Fracture_Energy,External_Work,Residual_Energy,Normalized_Residual_Energy)
    endif
    ...
    if(num_Crack.ne.0)then
        if((Yes_Last_Growth .eqv. .False.) .and. (Yes_Generated .eqv. .False.)   .and. (New_Crack_Flag .eqv. .False.)  .and. &
