!-----------------------------------------------------------
! Brief: Saves hydraulic-fracturing injection pressure and time per step.
!
! Parameters:
!   Input:  ifra   - current fracture step index
!   Input:  c_Time - current simulation time
!
! Notes:   Writes to <Full_Pathname>.injp.
!          Appends a header at the first step and clears any prior file.
!          Net pressure is obtained by adding the far-field stress term.
!-----------------------------------------------------------

subroutine Save_HF_Injection_Pressure(ifra,c_Time)
!     Save water pressure & time at injection points (per step) to injp file.
