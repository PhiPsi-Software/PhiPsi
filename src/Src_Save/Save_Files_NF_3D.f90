!-----------------------------------------------------------
! Brief: Save 3D natural-fracture vertex coordinates for post-processing.
!
! Parameters:
!   Input:  isub      - current sub-step index used in the output filenames
!   Input:  Save_Type - selects output: 1 = vertex coordinates, 2 = reserved
!
! Notes:   Writes the .nfcx/.nfcy/.nfcz coordinate files for every random natural
!          fracture; only quadrilateral/polygonal fractures are supported here.
!-----------------------------------------------------------
