!-----------------------------------------------------------
! Brief: Logs wellbore/stage/proppant pressure versus time to .wbpt.
!
! Parameters:
!   Input: i_WB    - wellbore index
!   Input: i_Stage - stage index within the wellbore
!   Input: i_Prop  - proppant slug index
!   Input: c_Pres  - pressure value (Pa)
!   Input: c_Time  - time value (s)
!
! Notes:   Appends a row each call; rewrites header at the first record.
!-----------------------------------------------------------
...
if(i_WB ==1 .and. i_Stage==1 .and. i_Prop==1)then
    write(301,*) '    i_WB   |   i_Stage   | i_Prop   ' //'|    Time (s)   |     Pressure (Pa)'
endif   
write(301, '(3I10,E18.5,5X,E18.5)') i_WB,i_Stage,i_Prop, c_Time,c_Pres
close(301)    
