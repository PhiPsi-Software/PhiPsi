!-----------------------------------------------------------
! Brief: Writes the list of element-death (killed) IDs for the current substep.
!
! Parameters:
!   Input: isub - current substep number used in the file suffix
!
! Notes:   Output file is <Full_Pathname>.kiel_<isub>; includes prior steps
!          and any element flagged as broken in the current global state.
!-----------------------------------------------------------
...
c_File_name_1   =  trim(Full_Pathname)//'.' //'kiel'//'_'//ADJUSTL(temp)
