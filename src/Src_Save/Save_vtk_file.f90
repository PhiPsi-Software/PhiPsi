!-----------------------------------------------------------
! Brief: Writes the current solution snapshot to a legacy ASCII VTK file.
!
! Parameters:
!   Input: isub - current substep number used in the file suffix
!
! Notes:   Outputs an UNSTRUCTURED_GRID dataset with nodes, elements, and
!          results; returns early when Key_Save_vtk /= 1 or output is disabled.
!-----------------------------------------------------------

SUBROUTINE Save_vtk_file(isub)
!Save vtk files.
!Ref: Github > PFEMFort > writervtk.F
!Written by Fang Shi, 2021-07-16.
...
ELSEIF(Key_Dimension == 3) then
    ...
    ELSE IF(npElem == 8) THEN
        ...
        DO i_E=1,Num_Elem
            ...
            Elem_Node(i_E,4)-1,Elem_Node(i_E,5)-1,Elem_Node(i_E,6)-1, Elem_Node(i_E,7)-1,Elem_Node(i_E,8)-1
        END DO          
    END IF
END IF
