!-----------------------------------------------------------
! Brief: Dumps the assembled XFEM stiffness matrix in CSR form to disk.
!
! Parameters:
!   Input: isub           - substep number used in the file suffix
!   Input: K_CSR_NNZ      - number of nonzeros in the CSR matrix
!   Input: num_FreeD      - number of free DOFs (matrix dimension)
!   Input: XFEM_Start_DOF - first enriched DOF index
!   Input: K_CSR_aa       - CSR value array
!   Input: K_CSR_ja       - CSR column index array
!   Input: K_CSR_ia       - CSR row pointer array
!
! Notes:   Produces four .csr*_ files (nnz, values, ja, ia) for external use.
!-----------------------------------------------------------

subroutine Save_Stiffness_Matrix_CSR(isub, K_CSR_NNZ, num_FreeD, XFEM_Start_DOF, K_CSR_aa, K_CSR_ja, K_CSR_ia)
