!-----------------------------------------------------------
! Brief: Save circular/elliptical hole coordinates and hole XFEM enrichment data.
!
! Parameters:
!   Input:  isub - current sub-step index used in the output filenames
!
! Notes:   Writes .hlcr/.ehcr for hole geometries plus .ennh/.elth/.posh files
!          describing node enrichment, element types, and signed-distance signs.
!-----------------------------------------------------------

subroutine Save_Files_Holes(isub)
!     Save the coordinates and radius of the Hole.
...
case(1:2)
    ...
    if (num_Circ_Hole>=1)then
        c_File_name_1   =  trim(Full_Pathname)//'.' //'hlcr'
    ...
    if (num_Ellip_Hole>=1)then 
        c_File_name_1   =  trim(Full_Pathname)//'.' //'ehcr'
    ...
    c_File_name_1   =  trim(Full_Pathname)//'.' //'ennh'//'_'//ADJUSTL(temp)
    open(103,file=c_File_name_1,status='unknown')         
    do i=1,Num_Node
        write(103, '(200I10)') (Enriched_Node_Type_Hl(i, j),j=1,num_Hole)
    end do
    close(103)           
    print *,'    Saving elth file of hole...'
    c_File_name_1   =  trim(Full_Pathname)//'.' //'elth'//'_'//ADJUSTL(temp)
    ...
    c_File_name_1   =  trim(Full_Pathname)//'.' //'posh'//'_'//ADJUSTL(temp)
