!-----------------------------------------------------------
! Brief: Writes wellbore fracturing path coordinates to a .wbfp file.
!
! Parameters:
!
! Notes:   Walks each wellbore and subdivides it into stage segments.
!          Emits start, intermediate stage, and end points for plotting.
!-----------------------------------------------------------

subroutine Save_HF_wbfp_File
!     Save wellbore coordinate data for HF analysis. Version: NEWFTU2022110901.
!     2022-11-09.
...
do c_i_WB = 1,num_Wellbore
    ...
    do c_i_Stage = 1,num_Stages_Wellbores(c_i_WB)-1
        call Tool_Offset_Point_A_to_Point_B_3D(Start_Point, End_Point, L_Stage*c_i_Stage,c_Point(1:3))
