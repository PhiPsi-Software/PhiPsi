!-----------------------------------------------------------
! Brief: Decomposes a 3D surface pressure load vector into x/y/z nodal files.
!
! Parameters:
!   Input: isub              - current substep number
!   Input: i_SL              - surface-load index
!   Input: F                 - assembled load vector over all solid DOFs
!   Input: all_Solid_Freedom - number of solid DOFs (3 per node)
!
! Notes:   Produces .fxsl_ / .fysl_ / .fzsl_ files; returns early when simple
!          post-processing is active or output is globally disabled.
!-----------------------------------------------------------
