!-----------------------------------------------------------
! Brief: Compute and store 2D XFEM element stiffness matrices and preconditioner.
!
! Parameters:
...
!   Output: Total_Num_G_P         - total number of Gauss points
...
!
! Notes:   Loops over enriched and non-enriched elements, integrates B^T D B
!          at Gauss points (crack-aware and FEM-only).
!-----------------------------------------------------------

SUBROUTINE EBE_XFEM_PCG_Get_K(isub,Lambda, num_FreeD,freeDOF, Total_Num_G_P, storK,size_local,all_local, &
...
Total_Num_G_P = 0
print *, "    EBE-PCG: prepare data..."
diag_precon= ZR

print *, "    EBE-PCG: get and store element stiffness matrix..."
...
do i_E=1,Num_Elem
    ...
    do i_G = 1,c_Num_Gauss_Point
        ...
        localK_FEM_or_XFEM(1:num_B,1:num_B) = localK_FEM_or_XFEM(1:num_B,1:num_B) + c_thick*weight(i_G)*detJ* &
...
RETURN
END SUBROUTINE EBE_XFEM_PCG_Get_K
