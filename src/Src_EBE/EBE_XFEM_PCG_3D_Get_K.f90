!-----------------------------------------------------------
! Brief: Build EBE XFEM stiffness and diagonal preconditioner (3D).
!
! Parameters:
!   Input:  isub, num_FreeD  - substep index and free DOF count
!           freeDOF           - DOF map
!   Output: diag_precon_no_invert - diagonal precon (no inverse)
!
! Notes:   Element-by-element XFEM assembly for 3D; runs the
!          diagonally-preconditioned CG with enriched DOFs.
!-----------------------------------------------------------
...
!Modified on 2022-07-01.
!Modified on 2023-08-27.
...
integer c_Elem
integer ndof
real(kind=FT),ALLOCATABLE::max_storK_XFEM(:)
...
if (isub==1) then
    ...
    !$OMP            Rot_c_D_Comp,Location_ESM,k,c_loca,local,localK,c_i,c_j)                                &
    !$OMP            SCHEDULE(static)                                     
    do i_E = 1,num_FEM_Elem
        ...
        G_X_NODES(1:8,c_Elem),G_Y_NODES(1:8,c_Elem),G_Z_NODES(1:8,c_Elem), c_D,kesi,yita,zeta,weight,localK(1:ndof,1:ndof))

        if(Key_EBE_Sym_Storage_K==0)then
            storK_FEM(1:ndof,1:ndof,Elem_Location(c_Elem,2))=localK(1:ndof,1:ndof)        
        elseif(Key_EBE_Sym_Storage_K==1)then
            do c_i=1,ndof
                do c_j=c_i,ndof
                    storK_FEM_Sym((c_i-1)*24 -(c_i-1)*c_i/2 +c_j,Elem_Location(c_Elem,2))  = localK(c_i,c_j)
                enddo
            enddo
        endif
    enddo
...
!$OMP            num_Loc_ESM_C_Cr_NoFEM,i_G,k,c_loca,localK,num_B,tem_B,num_tem_B,detJ,local,                   &
...
do i_E = 1,num_XFEM_Elem
    ...
    elseif (Need_XFEM_Assem_Flag .eqv. .False.) then
        ...
        localK(1:num_B,1:num_B) = storK_XFEM_Old(c_Rec)%row(1:num_B,1:num_B)
    ...
    storK_XFEM(cEle_Loc)%row(1:num_B,1:num_B) = localK(1:num_B,1:num_B)
enddo
!$omp end parallel do   
...
101 FORMAT(8X,'Element  ',I6,' condition number:',E12.5)  
if (Key_EBE_Condition_Number == 1 )then
    ...
    do i_E=1,Num_Elem
        if (Elem_XFEM_Flag(i_E) ==1)then  
            ...
            write(*,101) i_E,EBE_Condition_Number(i_E)
        endif
    enddo       
endif      
