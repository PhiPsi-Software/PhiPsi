!-----------------------------------------------------------
! Brief: EBE-based Newton iteration driver for 3D contact state.
!
! Parameters:
...
!           diag_precon_no_invert - baseline precon
!           Num_Indent       - indentation display flag
!   In/Out: DISP             - displacement (in/out)
!
! Notes:   3D analog of the 2D contact-state iteration driver;
!          supports penalty, reduced-penalty, and two-step schemes.
!-----------------------------------------------------------
...
!                   6: Two-step calculation. If the opening is negative, the element is marked 
!                      as a compressive-shear element, and the stiffness matrix is corrected 
...
case(1)
    do i_NR_P = 1,Max_Contact_Iter
        ...
        if(i_NR_P==1)then 
            ...
            call Cal_Contact_Contact_State_Gauss_3D(isub,1,1,i_NR_P,Contact_DISP_0,Yes_Contact,Elem_Conta_Sta, CT_State_Gauss)
        ...
        all_local_0,diag_precon_no_invert_0,diag_precon_no_invert_Updated, Kn,Kn_Gauss,Kt1_Gauss,Kt2_Gauss,CT_State_Gauss)
        ...
        do i_PCG =1,max_num_PCG
            ...
            case(0)
                ...
                do i_E=1,num_FEM_Elem
                    ...
                    MATMUL(storK_FEM(1:num_Loc_ESM,1:num_Loc_ESM,c_Ele_Location), p(local(1:num_Loc_ESM)))
                enddo
                !$omp end do 
                !$omp end parallel
...
case(6)
    ...
    call EBE_XFEM_PCG_3D_Modify_K_CS(isub,num_freeD,freeDOF,size_local_0, all_local_0,diag_precon_no_invert_Updated)
    ...
    do i_NR_P = 1,Max_Contact_Iter
        ...
        do i_PCG =1,max_num_PCG
            ...
            do i_E=1,num_XFEM_Elem
                ...
                c_Ele_Location = Elem_Location(c_Elem,1) 
                u_thread(local(1:num_Loc_ESM),c_thread)=u_thread(local(1:num_Loc_ESM),c_thread)+ &
                MATMUL(storK_XFEM_Updated(c_Ele_Location)%row(1:num_Loc_ESM,1:num_Loc_ESM), p(local(1:num_Loc_ESM)))
            enddo
            !$omp end do 
            !$omp end parallel
            DO i_Thread = 1,omp_get_max_threads()
