!-----------------------------------------------------------
! Brief: 3D XFEM element-by-element PCG solver with on-the-fly K assembly.
!
! Parameters:
...
!
! Notes:   Handles 3D crack-tip F-enrichment, multi-crack elements, and
!          contact surface penalty; uses EBE matrix-vector multiply.
!-----------------------------------------------------------
...
do i_PCG =1,max_num_PCG
    ...
    do i_E=1,num_XFEM_Elem
        ...
        cEle_Loc = Elem_Location(c_Elem,1) 
        u_thread(local(1:num_Loc_ESM),c_thread)=u_thread(local(1:num_Loc_ESM),c_thread)+ &
        MATMUL(storK_XFEM(cEle_Loc)%row(1:num_Loc_ESM,1:num_Loc_ESM), p(local(1:num_Loc_ESM)))
