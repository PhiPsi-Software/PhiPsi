!-----------------------------------------------------------
! Brief: Adjust crack coordinates to remove Heaviside singularity.
!
! Parameters:
!
! Notes:   Detects singularity via D3_Check_Crack_Heaviside_Singularity
!          and applies up to num_Try_1 translations plus num_Try_2
!          orientation adjustments. Driven by Key_Check_and_Adjust_
!          Cracks_3D. Parallelised with OpenMP.
!-----------------------------------------------------------
    ...
    case(5)
        ...
        do i_G = 1,Tem_Num_Gauss_3D
            ...
            if(Key_InPlane_Growth== 0 ) then
                call D3_Get_Signed_Dis_to_Crack_Mesh(Global_coor_Gauss,i_C,Check_Ball_R,c_Distance(i_G), c_Signed_Dis_v2, &
            ...
            elseif(Key_InPlane_Growth== 1) then
                call D3_Get_Signed_Dis_to_Crack_Mesh_for_InPlane_Growth(Global_coor_Gauss,i_C, Check_Ball_R,c_Distance(i_G), &
                c_Yes_Node_PER_in_FS,c_PER_Node_to_FS(1:3), Yes_Found_Min_Signed_Dis,c_n_Vector)
