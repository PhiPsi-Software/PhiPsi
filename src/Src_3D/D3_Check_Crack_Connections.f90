!-----------------------------------------------------------
! Brief: Detect connectivity among 3D cracks and update crack type/status.
!
! Parameters:
!   Input:  i_WB   - wellbore / stage identifier
!           i_Stage - fracture stage index
!           i_Prop  - proppant identifier
!           isub    - load substep
!
! Notes:   Iterates over crack mesh triangles to find HF-NF intersections,
!          activates natural fractures when intersected (Key_NaCr_Active_
!          Scheme_3D), updates Crack_Type_Status_3D and NaCr3D_Status.
!-----------------------------------------------------------
...
case(3)
        ...
        if(Crack_Type_Status_3D(i_C,1) == 1) then 
            ...
            do j_C=1,num_Rand_Na_Crack
                ...
                if(NaCr3D_Status(j_C,1) /=1)then
                    ...
                    Ele_Num_Cache = 1
                    call Cal_Ele_Num_by_Coors_3D(HF_NF_Inter_Point(1),HF_NF_Inter_Point(2),&
                         HF_NF_Inter_Point(3), Ele_Num_Cache, c_OUT_Elem)
                    ...
                    call Cal_Any_Point_Str_KesiYita_3D(c_OUT_Elem,0,1,1,c_Kesi,c_Yita,c_Zeta,1,&
                         DISP, c_Sxx,c_Syy,c_Szz,c_Sxy, c_Syz,c_Sxz)

                    ! Calculate the outward normal vector of natural fractures. 2024-02-22. IMPROV2024022201.
                    call Tool_Normal_vector_of_3D_Tri(Na_Crack3D_Coor(j_C,1,1:3), Na_Crack3D_Coor(j_C,2,1:3), &
    ...
    do i_C=1,num_Rand_Na_Crack
        if(NF_Flag_Active(i_C).eqv. .True.) then
            ...
            Ele_Num_Cache = 1
            call Cal_Ele_Num_by_Coors_3D(New_Crack_Center(1),New_Crack_Center(2),&
                 New_Crack_Center(3),Ele_Num_Cache,c_OUT_Elem)
            ...
            call Tool_Normal_vector_of_3D_Tri(Na_Crack3D_Coor(i_C,1,1:3), Na_Crack3D_Coor(i_C,2,1:3), &
