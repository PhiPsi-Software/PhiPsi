!-----------------------------------------------------------
! Brief: Compute fluid element calc points for 3D hydraulic fractures.
!
! Parameters:
!   Input:  isub - load step / substep index
!
! Notes:   Populates Cracks_FluidEle_*, Cracks_CalP_*, and LCS arrays
!          (centroids, areas, normal vectors, local coordinate frames)
!          for every fluid element of every crack. Used by linear HF
!          element assembly. Marks nodes near model boundary.
!-----------------------------------------------------------
...
real(kind=FT) :: vector_temp(3)
integer :: ncp
real(kind=FT), allocatable :: mat_tmp(:,:)
...
do i_C = 1,num_Crack
            ...
            if (num_Inter_Outline==2)then
                do i_P =1,4
                    ...
                    if(Yes_Exist .eqv. .False.)then
                        ...
                        ! Calculate the outward normal vector of the element
                        call Tool_Normal_Unit_vector_of_3D_Tri(c_Four_Points(1,:),c_Four_Points(2,:), &
                                                               c_Four_Points(3,:),Unit_Norm_Vector)             
                        Cracks_CalP_Orient_3D(i_C)%row(c_num_CalP,1:3) =Unit_Norm_Vector  
                    endif
                enddo
    ...
    !-----------------------------------------------------------------------------------
    ! Deduplicate calculation points for all elements again to ensure no duplicate
    ! coordinates exist within an element. 2026-04-26. IMPROV2026042601.
    !-----------------------------------------------------------------------------------
    if (Count_Eles_Has_CalP > 0) then
        do i_E = 1, Count_Eles_Has_CalP
            c_E = Eles_Has_CalP_List(i_E)
            if (c_ele_num_CalP(c_E) > 1) then
                ! Copy points of the current element to a temporary matrix for deduplication.
                Matrix(1:c_ele_num_CalP(c_E), 1:3) = tmp_ele_CalP(Eles_Has_CalP_Location(c_E), 1:c_ele_num_CalP(c_E), 1:3)
                call Matrix_Unique_Row_Dou(c_ele_num_CalP(c_E), 3, c_ele_num_CalP(c_E), Matrix(1:c_ele_num_CalP(c_E), 1:3), &
                Uniqued_Matrix(1:c_ele_num_CalP(c_E), 1:3), Uniqued_m, Uni_Mat_Count(1:c_ele_num_CalP(c_E)))
                ! If the number of points decreases after deduplication, update the count and 
                ! coordinates for this element.
                if (Uniqued_m < c_ele_num_CalP(c_E)) then
                    c_ele_num_CalP(c_E) = Uniqued_m
                    tmp_ele_CalP(Eles_Has_CalP_Location(c_E), 1:Uniqued_m, 1:3) = Uniqued_Matrix(1:Uniqued_m, 1:3)
                end if
            end if
    ...
    do i_E = 1,Num_Elem 
        if (c_ele_num_CalP(i_E) >=3)then
            ...
            do i_CalP=1,c_ele_num_CalP(i_E)   
                !IMPROV-2026040801. 
                vector_temp(1:3) = tmp_ele_CalP(Eles_Has_CalP_Location(i_E), i_CalP, 1:3)  
                ncp = Cracks_CalP_Num_3D(i_C)
                allocate(mat_tmp(ncp,3))
                mat_tmp = Cracks_CalP_Coors_3D(i_C)%row(1:ncp,1:3)

                call Vector_belongs_Matrix_Is_Dou(Cracks_CalP_Num_3D(i_C),3,mat_tmp, vector_temp,c_Location,c_Yes)     
