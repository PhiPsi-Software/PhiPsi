!-----------------------------------------------------------
! Brief: Compute signed distance to crack mesh for in-plane growth.
!
! Parameters:
!   Input:  Point                - query point (x,y,z)
!           i_C                  - crack index
!           Check_Ball_R         - detection-ball radius
!   Output: Signed_Dis          - signed distance to crack surface
!           c_Yes_Node_PER_in_FS - foot point inside surface?
!           c_PER_Node_to_FS     - foot-point coordinates
!           Yes_Found_Min_Signed_Dis - minimum found?
!           n_Vector             - outward normal at foot point
!
! Notes:   In-plane growth variant: triangles inside the detection
!          sphere are scanned; range prefilter is applied first.
!-----------------------------------------------------------
