!-----------------------------------------------------------
! Brief: Generate initial HF cracks for one wellbore stage.
!
! Parameters:
!   Input: i_Prop  - propagation step index (acts only when 1)
!          i_WB    - wellbore index
!          i_Stage - stage index within the wellbore
!
! Notes:   At the first propagation step, lays out the initial
!          segmented fractures for the stage and updates the
!          Crack_Type_Status_3D status flags of prior cracks.
!-----------------------------------------------------------
...
use Global_HF
use Global_Filename
use Global_Stress,only:InSitu_Strs_Gaus_xx, InSitu_Strs_Gaus_yy, InSitu_Strs_Gaus_zz, InSitu_Strs_Gaus_yz, &
InSitu_Strs_Gaus_xz, InSitu_Strs_Gaus_xy
...
do i_C = 1,num_Cr_Stage
    ...
    if (Key_Gen_Ini_Crack_Rule_Wellbores==2)then 
        ...
        elseif(Key_InSitu_Strategy /=0)then
            ...
            call Tool_Principal_Stresses_3D(InSitu_Strs_Gaus_xx(Center_Elem_num,1), InSitu_Strs_Gaus_yy(Center_Elem_num,1), &
            InSitu_Strs_Gaus_zz(Center_Elem_num,1), InSitu_Strs_Gaus_yz(Center_Elem_num,1), &
            InSitu_Strs_Gaus_xz(Center_Elem_num, &
            1), InSitu_Strs_Gaus_xy(Center_Elem_num,1), S_1,S_2,S_3, Vector_S1,Vector_S2,Vector_S3)
            n_Vector = Vector_S3
            call Vector_Normalize(3,n_Vector)
        endif
    endif
    ...
    elseif(Key_Gen_Ini_Crack_Wellbores==3)then
        call Tool_Get_3D_Polygon_by_n_Vector_and_Center( n_Vector(1:3),Num_Poly_Edges_PolyCr_WB, c_Crack_Center(1:3),Crack_L, &
