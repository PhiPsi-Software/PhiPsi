!-----------------------------------------------------------
! Brief: Generate the surface mesh for all initial 3D cracks.
!
! Parameters:
!   (none)
!
! Notes:   Handles rectangular (4-point), circular, elliptical,
!          and polygonal initial cracks. Populates the global
!          Crack3D_Meshed_Node / _Ele / _Outline arrays for
!          every crack. Does not handle HF in-plane growth.
!-----------------------------------------------------------
...
do i_C=1,num_Crack
    ...
    if(sum(abs(Crack3D_Coor(i_C,1:4,1:3)))>Tol_20 .and.Each_Cr_Poi_Num(i_C) == 4)then
        ...
        !NEWFTU-2026050501.
        if (User_Specified_Crack_Mesh_Size > ZR) then
            Mesh_Size = User_Specified_Crack_Mesh_Size
            write(*, '(A, ES10.3, A)') '     WARNING :: the mesh size is taken as the user-specified value:', Mesh_Size, ' m'
            if (User_Specified_Crack_Mesh_Size > Ten*Ave_Elem_L) then
                print *, '    WARNING :: the user-specified crack mesh size is too large!'
                print *, '               Please check keyword *User_Specified_Crack_Mesh_Size!'
                call Warning_Message('S',Keywords_Blank)
            endif
            if (User_Specified_Crack_Mesh_Size < ZPZ1*Ave_Elem_L) then
                print *, '    WARNING :: the user-specified crack mesh size is too small!'
                print *, '               Please check keyword *User_Specified_Crack_Mesh_Size!'
                call Warning_Message('S',Keywords_Blank)
            endif
        endif
    ...
    elseif(sum(abs(Crack3D_Cir_Coor(i_C,1:7)))>Tol_20)then 
        ...
        elseif (Key_3D_Circular_Crack_Meshing_Scheme==1) then
            ...
            call D3_Mesh_Circle_By_Rings( i_C, cir_center, cir_vec, radius, Cricle_Elem_L, Ele_Num_Cache)
        endif
