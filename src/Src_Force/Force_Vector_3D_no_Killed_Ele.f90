!-----------------------------------------------------------
! Brief: Build 3D load vector skipping killed elements.
!
! Parameters:
...
!
! Notes:   Simplified assembly used for initial stress field
!          computation; ignores initial-stress correction.
!-----------------------------------------------------------

subroutine Force_Vector_3D_no_Killed_Ele(c_Total_FD, isub,Lambda,globalF)
...
if(Key_Gravity==1) then
    do i_E = 1,Num_Elem
        ...
        do i_N = 1,8
            c_node = G_NN(i_N,i_E)
            globalF(3*c_node-2)  = globalF(3*c_node-2)- g_X_Y_Z(1)*density*Elem_Vol(i_E)/EIG
        end do
        do i_N = 1,8
            c_node = G_NN(i_N,i_E)
            globalF(3*c_node-1)  = globalF(3*c_node-1)- g_X_Y_Z(2)*density*Elem_Vol(i_E)/EIG
        end do
        do i_N = 1,8
            c_node = G_NN(i_N,i_E)
            globalF(3*c_node)  = globalF(3*c_node)- g_X_Y_Z(3)*density*Elem_Vol(i_E)/EIG
        end do
    end do
end if
