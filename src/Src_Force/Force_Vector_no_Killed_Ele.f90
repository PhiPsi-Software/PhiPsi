!-----------------------------------------------------------
! Brief: Build 2D load vector skipping killed elements.
!
! Parameters:
...
!
! Notes:   Simplified assembly for initial stress field; ignores
!          initial-stress correction and pore water pressure.
!-----------------------------------------------------------

subroutine Force_Vector_no_Killed_Ele(c_Total_FD, isub,Yes_Biot,Lambda,globalF)
!     Load vector (excluding active and inactive elements, and without considering the external load correction for initial stress from Zienkiewicz_7ed_P216; used for initial stress field calculation).
!     Note: The initial stress field calculation does not take pore water pressure into account, 2020-03-15.
...
real(kind=FT) Div_Points(Max_Num_Cr_CalP,2),Coor_Tip(2), r,Coor_AB(2,2),Orient_Tip,ele_Sign,c_Sign
...
if(Key_Thermal_Stress==1)then
    ...
    do i_E = 1,Num_Elem
        ...
        do i_G = 1,c_Num_Gauss_Point
            ...
            globalF_T(Location_ESM(1:num_Loc_ESM)) = globalF_T(Location_ESM(1:num_Loc_ESM))  + c_Inter_Force(1:num_Loc_ESM)* &
