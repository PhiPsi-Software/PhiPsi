!-----------------------------------------------------------
! Brief: Assemble the 2D global load (force) vector.
!
! Parameters:
!   Input:  c_Total_FD - total DOFs
!           isub       - substep index
!           Yes_Biot   - include Biot effective stress?
!           Lambda     - load factor
!   Output: globalF    - global force vector
!
! Notes:   Builds nodal forces from boundary tractions, gravity,
!          body forces, thermal loads, and crack-tip enrichments.
!-----------------------------------------------------------
...
logical, save :: Yes_Per_Step_Load_2D = .false.
logical, save :: Checked_Per_Step_2D = .false.
...
real(kind=FT) kesi_Enr_64(64), &
              yita_Enr_64(64), weight_Enr_64(64)
...
if (.not. Checked_Per_Step_2D) then
    Checked_Per_Step_2D = .true.
    write(iter_str, '(I0)') 1
    temp_name_ps = trim(Full_Pathname)//'.focx_'//trim(adjustl(iter_str))
    inquire(file=temp_name_ps, exist=alive_ps)
    Yes_Per_Step_Load_2D = alive_ps
end if

if (Yes_Per_Step_Load_2D) then
    ...
    if (alive_ps) then
        ...
        if (nlines_ps > 0) then
            ...
            do j_ps = 1, nlines_ps
                ...
                globalF(2*cur_Node-1) = Temp_DATA_ps(j_ps,2)
            end do
    ...
    if (alive_ps) then
        ...
        if (nlines_ps > 0) then
            ...
            do j_ps = 1, nlines_ps
                cur_Node = int(Temp_DATA_ps(j_ps,1))
                globalF(2*cur_Node) = Temp_DATA_ps(j_ps,2)
            end do
            deallocate(Temp_DATA_ps)
        end if
    end if
    ...
    do i_E = 1,Num_Elem
      ...
      c_density = Material_Para(mat_num,3);
        ...
        if(Key_EKILL==1 .and. num_Ele_Killed>=1) then
          ...
          do i_N = 1,4
            c_node = G_NN(i_N,i_E)
            globalF(2*c_node-1)  = globalF(2*c_node-1)- g_X_Y_Z(1)*c_density*th*Elem_Area(i_E)/FOU
        end do
          do i_N = 1,4
            c_node = G_NN(i_N,i_E)
            globalF(2*c_node)  = globalF(2*c_node)- g_X_Y_Z(2)*c_density*th*Elem_Area(i_E)/FOU
        end do
      end do
end if
if(Key_Thermal_Stress==1 .and. Yes_XFEM .eqv. .True.)then
    ...
    do i_E = 1,Num_Elem
        ...
        if(num_Crack/=0)then
          do i_C =1,num_Crack
            call Location_Element_Stiff_Matrix(i_E,i_C,c_POS(:,i_C),Location_ESM_C_Crack,&
                num_Loc_ESM_C_Crack, &
        ...
        if(num_Hole/=0)then
          do i_H =1,num_Hole
            call Location_Element_Stiff_Matrix_Hl(i_E,i_H,c_POS_Hl(:,i_H),&
        ...
        if(num_Inclusion/=0)then
          do i_Incl =1,num_Inclusion
            call Location_Element_Stiff_Matrix_Incl(i_E,i_Incl,c_POS_Incl(:,i_Incl),&
        ...
        do i_G = 1,c_Num_Gauss_Point
            ...
            c_Inter_Force(1:num_Loc_ESM) = MATMUL(transpose(B(1:3,1:num_Loc_ESM)), c_TStress)
            globalF_T(Location_ESM(1:num_Loc_ESM)) = globalF_T(Location_ESM(1:num_Loc_ESM)) &
...
if((Key_Thermal_Stress==1) .and. (Yes_XFEM .eqv. .False.))then
    ...
    do i_E = 1,Num_Elem
        ...
        do i_G = 1,Num_Gauss_P_FEM
            ...
            call Cal_Ele_B_N4(i_E,c_X_NODES,c_Y_NODES, c_kesi,c_yita,c_B)
            c_TStress(1:3) =ZR
            if(Key_Type_2D==1)then
                c_TStress = c_T_Alpha*Thermal_Str_Temper(Elem_Mat(i_E))* MATMUL(c_D,[ONE,ONE,ZR])
            elseif(Key_Type_2D==2)then
            ...
            globalF(local) =  globalF(local) +c_temp(1:8)*c_thick*weight(i_G)*detJ
        end do
    enddo
endif
...
if(Key_InSitu_Strategy==2)then
    if(abs(InSitu_x) <0.0001D6 .and. abs(InSitu_y) < 0.0001D6)then
        print *,'    #--#--#--#--#--#--#--#--#--#--#--#--#--#'
        print *,'    Warning :: cannot find InSitu_x and InSitu_y (warning in Force_Vector.f)!'
        print *,'    #--#--#--#--#--#--#--#--#--#--#--#--#--#'
        goto 199
    endif
    call Cal_Gauss_Points_QUAD(Num_Gauss_P_FEM,kesi,yita,weight)
    ...
    do i_E = 1,Num_Elem
        ...
        do i_G = 1,Num_Gauss_P_FEM
            ...
            call Cal_detJ(c_kesi,c_yita,c_X_NODES,c_Y_NODES,detJ)
            call Cal_Ele_B_N4(i_E,c_X_NODES,c_Y_NODES, c_kesi,c_yita,c_B)
            c_Stress = [InSitu_Strs_Gaus_xx(i_E,i_G), InSitu_Strs_Gaus_yy(i_E,i_G), InSitu_Strs_Gaus_xy(i_E,i_G)]
            c_temp(1:8) = MATMUL(transpose(c_B(1:3,1:8)),c_Stress)
            globalF(local) = globalF(local)  - c_temp(1:8)*c_thick*weight(i_G)*detJ
        end do
    enddo

    if(Key_Analysis_Type==1 .and. num_Crack/=0 .and. Key_Force_Control /= 5)then
      num_CT_Gauss = 2
      ...
      do i_C=1,num_Crack
        ...
        do i_CT_Elem = 1,Num_Div_Points - 1
            ...
            do i_CT_GAUSS  = 1,num_CT_Gauss
                ...
                globalF(Local_W(1:2*num_Local_W)) = globalF(Local_W(1:2*num_Local_W))  + &
                          MATMUL(transpose(N_W(1:2,1:2*num_Local_W)), c_PC_Gauss)* &
