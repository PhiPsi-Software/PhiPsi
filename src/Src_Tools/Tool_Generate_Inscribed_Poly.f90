!-----------------------------------------------------------
! Brief: Randomly generate regular inscribed polygon inclusions within a model domain.
!
! Parameters:
!   Input:  M_x_min,M_x_max,M_y_min,M_y_max - model coordinate bounds
!   Input:  c_Ave_R - average circumradius of the polygon circles
!   Input:  c_Delta_R - range of the circle radius variation
!   Input:  num_Rand_Poly_Incl - total number of polygons to generate
!   Input:  num_Vert_Poly_Incl - number of sides per polygon
!   Input:  check_R - minimum separation radius between circle centers
!   Input:  Max_Dis_to_Edge - minimum allowed distance to model boundary
!   Output: Poly_Incl_Coor_x - x coordinates of the generated polygon vertices
!   Output: Poly_Incl_Coor_y - y coordinates of the generated polygon vertices
!
! Notes:   Uses random placement while enforcing non-overlap between
!   generated circles and keeping centers away from domain edges.
!-----------------------------------------------------------

subroutine Tool_Generate_Inscribed_Poly( M_x_min,M_x_max,M_y_min,M_y_max, c_Ave_R,c_Delta_R, &
num_Rand_Poly_Incl,num_Vert_Poly_Incl, &
check_R, &
Max_Dis_to_Edge, Poly_Incl_Coor_x, Poly_Incl_Coor_y)
!     Randomly generate a circle, then generate an inscribed polygon within the circle
!     Algorithm: Generate one by one, and the midpoint of the circle generated later should not fall within the radius of the midpoints of other cracks.
...
real(kind=FT),intent(out)::  Poly_Incl_Coor_x(num_Rand_Poly_Incl, num_Vert_Poly_Incl)
real(kind=FT),intent(out)::  Poly_Incl_Coor_y(num_Rand_Poly_Incl, num_Vert_Poly_Incl)
...
real(kind=FT) c_Center_Cir(num_Rand_Poly_Incl,2), c_R_Cir(num_Rand_Poly_Incl)
...
do i_Try = 1,50000000
    ...
    if(Yes_In_Circle .eqv. .False.)then
        ...
        if (Try_x+try_R<M_x_max-Max_Dis_to_Edge .and. Try_x-try_R > M_x_min+Max_Dis_to_Edge) then
            Yes_In_Model_x = .True.
        endif
        if (Try_y+try_R<M_y_max -Max_Dis_to_Edge .and. Try_y-try_R > M_y_min +Max_Dis_to_Edge) then
            Yes_In_Model_y = .True.
        endif
...
do i_Incl =1,num_Rand_Poly_Incl
    ...
    do i_Point =1,num_Vert_Poly_Incl
        Poly_Incl_Coor_x(i_Incl,i_Point) = c_Center_Cir(i_Incl,1) + c_R_Cir(i_Incl)*cos(Start_Theta+delta_Theta*(i_Point-1))
        Poly_Incl_Coor_y(i_Incl,i_Point) = c_Center_Cir(i_Incl,2) + c_R_Cir(i_Incl)*sin(Start_Theta+delta_Theta*(i_Point-1))
    enddo
enddo
