...
if ( logger_initialized ) then
    ...
    if ( append_real ) then
        open (log_fileunit, FILE= log_file , ACTION='WRITE', STATUS='UNKNOWN', POSITION ='APPEND')
    else
...
if (  unit == -1 ) then
    ...
    close( unit )
    open( unit, FILE=filename, ACTION='WRITE', STATUS='UNKNOWN', POSITION ='APPEND')
endif
...
case default
    write (message,"(A,A,A,l5,A)") "Unknown option ", option, " for value ", value, " in log_configure_logical"
...
case ( "logfileunit" )
    log_fileunit = value
case default
    write (message,"(A,A,A,I5,A)") "Unknown option ", option, " for value ", value, " in log_configure_integer"
    call log_error ( message )
end select
...
case ( "level_string_subsection" )
    log_level_string_chapter = value
case default
    write (message,"(A,A,A,A,A)") "Unknown option ", option, " for value ", value, " in log_configure_character"
    call log_error ( message )
end select
...
case default
    write (message,"(A,l5,A)") "Unknown option ", option, " in log_cget_logical"
    call log_error ( message )
end select
...
case ( "logfileunit" )
    value = log_fileunit
case default
    write (message,"(A,I5,A)") "Unknown option ", option, " in log_cget_integer"
    call log_error ( message )
end select
...
case ( "level_string_subsection" )
    value = log_level_string_chapter
case default
    write (message,"(A,A,A)") "Unknown option ", option, " in log_cget_character"
    call log_error ( message )
end select
