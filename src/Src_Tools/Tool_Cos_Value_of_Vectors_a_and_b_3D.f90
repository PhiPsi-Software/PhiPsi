!-----------------------------------------------------------
! Brief: Computes the cosine of the included angle between two 3D vectors.
!
! Parameters:
!   Input:  a, b     - 3D vectors
!   Output: cos_value - cosine of the included angle
!
! Notes:   Returns -1 exactly for opposite vectors and otherwise applies the
!          standard dot-product over product-of-norms formula.
!-----------------------------------------------------------
...
if(abs(x1+x2)< Tol_11  .and. abs(y1+y2)< Tol_11  .and. abs(z1+z2)< Tol_11) then
    cos_value = -ONE
    goto 100
endif
