!-----------------------------------------------------------
! Brief: Tests whether a 3D point lies inside or on a spatial tetrahedron.
!
! Parameters:
!   Input:  Point(3) - XYZ coordinates of the test point
!   Input:  A,B,C,D(3) - the four tetrahedron vertex coordinates
!   Output: Yes_in - .true. if the point is strictly inside
!   Output: Yes_on - .true. if the point lies on the boundary
!
! Notes:   Uses the four signed-distance barycentric ratios alpha/beta/gamma/
!          delta with respect to each face, pre-screening on bounding box and
!          on coincident vertices; uses signed distances only and exits early
!          on first failed condition.
!-----------------------------------------------------------

subroutine Tool_Yes_Point_in_3D_Tetrahedron(Point, A,B,C,D, Yes_in,Yes_on)
