!-----------------------------------------------------------
! Brief: Compute 3D distance from point P to line/segment AB
!
! Parameters:
!   Input:  P(3)       - query point coordinates
!   Input:  A(3)       - first endpoint of the line
!   Input:  B(3)       - second endpoint of the line
!   Input:  Key        - 1 for infinite line, 2 for line segment
!   Output: Dis        - normal/perpendicular distance
!   Output: foot_point(3) - perpendicular foot coordinates
!
! Notes:   Uses cross-product formula; warns if AB is degenerate.
!-----------------------------------------------------------

subroutine Tool_Dis_Point_to_Line_AB_3D(P,A,B,Dis,foot_point,Key)
! Calculate the normal distance from point P to the spatial line segment AB (output the foot of the perpendicular coordinates)
! Key =1 Treat AB as a straight line; Key =2 Treat AB as a line segment
! 2020-01-04
! Reference: http://mathworld.wolfram.com/Point-LineDistance3-Dimensional.html or
! 	  theoretical_documents\006 Distance from a Point to a Straight Line in Space-2020-01-04.pdf
