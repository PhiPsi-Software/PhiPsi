!-----------------------------------------------------------
! Brief: Tests whether a set of 3D points is coplanar.
!
! Parameters:
!   Input:  num_points - number of points (must be >= 4)
!   Input:  Points     - num_points x 3 array of coordinates
!   Output: Yes_on     - true if all points lie on the same plane
!
! Notes:   Builds a reference triangle from the first three non-collinear
!          points and measures the distance of every remaining point to it.
!-----------------------------------------------------------

subroutine Tool_3D_Yes_Points_on_Same_Plane(num_points, Points,Yes_on)
...
call Tool_Dis_Point_to_3D_Tri(Points(i_Point+3,1:3), Tri_P1,Tri_P2,Tri_P3, c_Distance,c_PER(1:3), &
