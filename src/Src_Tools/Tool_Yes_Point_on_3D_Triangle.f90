!-----------------------------------------------------------
! Brief: Tests whether a 3D point lies on the plane of a triangle (within tolerance).
!
! Parameters:
!   Input:  P(3),A(3),B(3),C(3) - test point and three triangle vertices
!   Output: Yes_on - .true. if P is coplanar with the triangle
!
! Notes:   Uses the area-decomposition test: if P lies on the plane the sum
!          of sub-triangle areas ABP+ACP+BCP equals the area of ABC.
!-----------------------------------------------------------
...
!     Algorithm: If point p is inside the triangle, then the original triangle ABC is divided into three triangles: ABP, ACP, and BCP.
