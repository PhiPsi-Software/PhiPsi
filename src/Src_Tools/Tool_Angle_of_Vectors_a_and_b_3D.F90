!-----------------------------------------------------------
! Brief: Computes the included angle between two 3D vectors, with method-dependent range.
!
! Parameters:
!   Input:  a, b       - 3D vectors
!   Input:  Key_Method - selects algorithm (1 = 0-360 via cross product direction)
!   Output: angle      - included angle in radians
!
! Notes:   Handles degenerate cases of opposite, identical, or near-zero vectors
!          and uses the cross product orientation to extend the angle to 0-2*pi.
!-----------------------------------------------------------

subroutine Tool_Angle_of_Vectors_a_and_b_3D(a,b,angle,Key_Method)
! Calculates the included angle of vector a and b (3D).
! Bug fix on 2021-01-01.
...
abs(y1+y2)< Tol_8  .and. abs(z1+z2)< Tol_8) then
angle = pi
return
endif
...
if(Key_Method==1) then
    ...
    call Vector_Cross_Product_3(a_Normalized, b_Normalized,Direction)
    ...
    tem_check = Direction(1)*Direction_Flag(1) + Direction(2)*Direction_Flag(2) + Direction(3)*Direction_Flag(3)
...
if(Key_Method==2) then
    ...
    call Vector_Cross_Product_3(a_Normalized,b_Normalized, Cross_Vector_a_b)
    norm_Cross_Vector_a_b = sqrt(Cross_Vector_a_b(1)**2+ Cross_Vector_a_b(2)**2+ Cross_Vector_a_b(3)**2)
...
if (isnan(angle)) then
    print *, '    Error :: NAN in Tool_Angle_of' //'_Vectors_a_and_b_3D.f'
endif
return
