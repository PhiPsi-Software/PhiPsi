!-----------------------------------------------------------
! Brief: Randomly generate irregular polygon inclusions with multiple size types.
!
! Parameters:
!   Input:  M_x_min,M_x_max,M_y_min,M_y_max - model coordinate bounds
!   Input:  c_num_Rand_Poly_Incl - total number of polygon inclusions
!   Input:  c_num_Vert_Poly_Incl - number of sides per polygon
!   Input:  c_num_Rand_Poly_Incl_for_Each_Type - counts per size class
!   Input:  c_Rand_Poly_Incl_R_Min_and_Max - radius range per size class
!   Input:  c_Irregular_R_Delta_Factor - radius variation factor
!   Input:  c_Irregular_Angle_Factor - angle variation factor
!   Input:  c_Irregular_Extension_Factor - elongation factor
!   Input:  c_Irregular_Inclination - orientation angle
!   Input:  check_R_Factor - center separation coefficient
!   Input:  check_Edge_Dis_Factor - boundary distance coefficient
!   Output: Poly_Incl_Coor_x,Poly_Incl_Coor_y - vertex coordinates
!
! Notes:   Produces non-strictly-inscribed irregular polygons;
!   uses the Zhu Huaxiang random block generation approach.
!-----------------------------------------------------------

subroutine Tool_Generate_Inscribed_Poly_Irregular( M_x_min,M_x_max,M_y_min,M_y_max, &
c_num_Rand_Poly_Incl, &
c_num_Vert_Poly_Incl, c_num_Rand_Poly_Incl_for_Each_Type, &
...
Poly_Incl_Coor_x, Poly_Incl_Coor_y)
!     Randomly generate a circle, and then generate a polygon inscribed in the circle (not a strictly inscribed polygon)
!     Algorithm: Generate them one by one, and the midpoint of the subsequently generated circle should not fall within 
!                a circle of radius r centered on the midpoint of any other crack.
!     Irregular Polygon Generation Algorithm, Reference: Hohai University, Zhu Huaxiang Master's Thesis: Study on the 
!     Mechanical Parameters of Granular Bodies Based on Random Block Models
...
real(kind=FT) c_Center_Cir(c_num_Rand_Poly_Incl,2), c_R_Cir(c_num_Rand_Poly_Incl)
integer :: i_point
...
integer i_Cir,c_Cir
real(kind=FT) Ran_n_Num(c_num_Vert_Poly_Incl), Ran_n_R(c_num_Vert_Poly_Incl), Ran_n_Angle(c_num_Vert_Poly_Incl)
    ...
    if (num_Poly_this_Type>=1)then
        do i_Try = 1,50000000
            ...
            if(Yes_In_Circle .eqv. .False.)then
                ...
                if (Try_x+try_R<M_x_max-check_Edge_Dis .and. Try_x-try_R > M_x_min+check_Edge_Dis) then
                    Yes_In_Model_x = .True.
                endif
                if (Try_y+try_R<M_y_max -check_Edge_Dis .and. Try_y-try_R > M_y_min +check_Edge_Dis) then
                    Yes_In_Model_y = .True.
                endif
...
do i_Type = 1,10
    ...
    if (num_Poly_this_Type>=1)then
        do i_Cir = 1,num_Poly_this_Type
            ...
            Ran_n_R(1:n_Vertex)= Ave_R_this_Type + (TWO*Ran_n_Num(1:n_Vertex)-ONE)* (max_R_this_Type -min_R_this_Type)* &
            c_Irregular_R_Delta_Factor
            if(key_Random==1 .or. key_Random==0)then
                call random_number (Ran_n_Num(1:n_Vertex)) 
            elseif(key_Random==2 .or. key_Random==3)then
            ...
            Ran_n_Angle(1:n_Vertex)= delta_Theta + (TWO*Ran_n_Num(1:n_Vertex)-ONE)* delta_Theta*c_Irregular_Angle_Factor
            sum_Angle =sum(Ran_n_Angle(1:n_Vertex))
            Ran_n_Angle(1:n_Vertex)= Ran_n_Angle(1:n_Vertex)*TWO*pi/sum_Angle
            c_Theta =  ZR
            do i_Point =1,c_num_Vert_Poly_Incl
                c_Theta = c_Theta + Ran_n_Angle(i_Point)
                tem1= Ran_n_R(i_Point)*cos(c_Theta)*sqrt(Elongation)
                tem2= Ran_n_R(i_Point)*sin(c_Theta)/sqrt(Elongation)
                c_angle = atan2(tem2,tem1)
                tem3 = sqrt(tem1**2+tem2**2)*cos(c_angle+Rotation)
                tem4 = sqrt(tem1**2+tem2**2)*sin(c_angle+Rotation)
                Poly_Incl_Coor_x(c_Cir,i_Point) = c_Center_Cir(c_Cir,1) + tem3
                Poly_Incl_Coor_y(c_Cir,i_Point) = c_Center_Cir(c_Cir,2) + tem4
            enddo
