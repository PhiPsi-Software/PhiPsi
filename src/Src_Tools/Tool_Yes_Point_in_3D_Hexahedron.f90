!-----------------------------------------------------------
! Brief: Test whether a 3D point lies inside (or on) a 3D hexahedron
!
! Parameters:
!   Input:  Point(3)        - test point coordinates
!   Input:  A(3)..H(3)      - eight hexahedron vertex coordinates
!   Output: Yes_in          - true if point is strictly inside the hexahedron
!   Output: Yes_on          - true if point lies on a hexahedron face/edge
!
! Notes:   Decomposes hexahedron into three 3D pyramids (H-BCGF, H-EABF, H-ABCD) and tests each.
!-----------------------------------------------------------
...
call Tool_Yes_Point_in_3D_Pyramid(Point, H,B,C,G,F, c_Yes_in_1,c_Yes_on_1)
if (c_Yes_in_1) then
    Yes_in = .True.
    return
endif
...
call Tool_Yes_Point_in_3D_Pyramid(Point, H,E,A,B,F, c_Yes_in_2,c_Yes_on_2)
if (c_Yes_in_2) then
    Yes_in = .True.
    return
endif
...
call Tool_Yes_Point_in_3D_Pyramid(Point, H,A,B,C,D, c_Yes_in_3,c_Yes_on_3)
if (c_Yes_in_3) then
    Yes_in = .True.
    return
endif
