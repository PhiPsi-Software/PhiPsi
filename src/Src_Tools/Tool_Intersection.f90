!-----------------------------------------------------------
! Brief: Compute the intersection point of two 2D line segments
!
! Parameters:
!   Input:  A, B     - Endpoints of the first line segment
!   Input:  C, D     - Endpoints of the second line segment
!   Output: X, Y     - Coordinates of the intersection point
!   Output: Yes_Cross - True if the segments cross within tolerance
!
! Notes:   Handles parallel, collinear, and vertical-segment cases;
!          returns the closest approach when segments do not strictly cross.
!-----------------------------------------------------------

subroutine Tool_Intersection(A,B,C,D,X,Y,Yes_Cross)
!     Calculate the intersection point of a line segment
!     A: Coordinates of point A of line segment AB;
!     B: Coordinates of point B of line segment AB;
!     C: Coordinates of point C of line segment CD;
!     D: Coordinates of point D of line segment CD.
...
if ((Yes_A1_eq_B1.eqv..False.) .and. (Yes_C1_eq_D1.eqv..False.)) then
    k1=(B(2)-A(2))/(B(1)-A(1))
    k2=(D(2)-C(2))/(D(1)-C(1))
    b1=A(2)-k1*A(1)
    b2=C(2)-k2*C(1)
