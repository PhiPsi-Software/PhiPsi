!-----------------------------------------------------------
! Brief: Randomly generate 3D natural rectangular cracks within a cuboid model.
!
! Parameters:
!   (no explicit arguments; uses module globals and project keywords)
!
! Notes:   Generates one crack at a time, ensuring midpoints are
!   separated by a sphere of radius r, and populates 3D crack
!   arrays with size, normal, and orientation data. Honors an
!   optional fracture-zone bounding box when defined.
!-----------------------------------------------------------
...
do i_Try = 1,30000000
    ...
    if(Yes_In_Circle .eqv. .False.)then
        ...
        if(Key_NaCr_Type_3D==4)then
            ...
            if(abs(NaCr_3D_Rect_Longside_Vector_Delta)<=Tol_10)then
                ...
                call Tool_Generate_Rand_Point_in_3D_Circle(c_Crack_Center(1:3), NaCr_3D_Rect_Longside_Vector(1:3),TWO,Rand_Point)
        ...
        call Tool_Generate_Rand_Point_in_3D_Circle(c_Crack_Center(1:3), NaCr_3D_n_Vector(1:3),TWO,Rand_Point)

        Rotate_Axis(1:3)  = Rand_Point - c_Crack_Center(1:3)
        call Vector_Normalize(3,Rotate_Axis(1:3))   


        call Tool_Vector_a_Rotate_around_Vector_b(NaCr_3D_n_Vector(1:3), Rotate_Axis,c_Rotate_Degree,Modified_Vector)
        c_n_Vector(1:3) = Modified_Vector(1:3)


        if(Key_NaCr_Type_3D==1)then
            call Tool_Get_3D_Rect_by_n_Vector_and_Center(c_n_Vector(1:3), c_Crack_Center(1:3),c_Crack_Size,c_Crack3D_Coor(1:4,1:3))
            Yes_Points_In_Model = .True.
            do  i_P = 1,4
                c_Point = c_Crack3D_Coor(i_P,1:3)
                if(c_Point(1) > M_x_min .and. c_Point(1) < M_x_max .and. c_Point(2) > M_y_min .and. c_Point(2) < M_y_max .and. &
    ...
    elseif(Key_NaCr_Type_3D==2)then
        ...
        call Tool_Get_3D_Polygon_by_n_Vector_and_Center(c_n_Vector(1:3), Circle_3D_Eqv_Polygon_Resolution, &
        c_Crack_Center(1:3),c_Crack_Size, c_Crack3D_Coor(1:Circle_3D_Eqv_Polygon_Resolution,1:3))
