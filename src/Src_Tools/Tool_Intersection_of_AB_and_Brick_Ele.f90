!-----------------------------------------------------------
! Brief: Find one intersection between a 3D segment and a hexahedral element
!
! Parameters:
!   Input:  A, B          - Endpoints of the 3D segment
!   Input:  c_El          - Element number (uses global node tables)
!   Output: Yes_Inter     - True if a triangle face reports an intersection
!   Output: InterSection_P - Coordinates of the first intersection found
!
! Notes:   Decomposes the hex into 12 triangles; returns at the first hit
!          (does not enumerate all possible segment-cube intersections).
!-----------------------------------------------------------

subroutine Tool_Intersection_of_AB_and_Brick_Ele(A,B,c_El, Yes_Inter,InterSection_P)
!     Calculate the intersection of line segment AB and the element with number 'Element' (2020-01-01)
!     Current functionality: There may be multiple intersections, but this program calculates only one and then stops.
