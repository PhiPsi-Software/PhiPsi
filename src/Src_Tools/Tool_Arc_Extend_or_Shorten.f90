!-----------------------------------------------------------
! Brief: Extends or shortens an arc by a signed arc length along its original orientation.
!
! Parameters:
!   Input:  delta_L_Arc - signed length change (positive extends, negative shortens)
!   Input:  Old_Arc_Cr  - description of the original arc (center, radius, direction, etc.)
!   Output: New_Arc_Cr  - description of the resulting arc
!   Output: Extend_Status - true if the operation succeeded
!
! Notes:   Validates that a shortening does not exceed the existing arc length and
!          delegates the geometry update to Tool_2_Points_of_Cir_give_Point_L.
!-----------------------------------------------------------

subroutine Tool_Arc_Extend_or_Shorten( delta_L_Arc, Old_Arc_Cr, New_Arc_Cr,Extend_Status)
! Extend or shorten the arc (along the original direction (clockwise or counterclockwise) delta_L_Arc).
! delta_L_Arc > 0 means extension; delta_L_Arc < 0 means shortening
...
call Tool_2_Points_of_Cir_give_Point_L( o_x,o_y,r, Point_I,delta_L_Arc,Point_Status, P1_P2)
if(Point_Status ==1 .and.(sum(P1_P2(2,1:2))>Tol_11)) then
    call Tool_Arc_r_and_Radian_Given_Coors(c_Arc_Direction, c_Arc_Start_Point, P1_P2(1,1:2),c_Arc_Center, c_Yes_feasible, &
    r,c_Arc_Radian_S_P1,c_Arc_Radian_E_P1, Arc_Radian_P1)
    call Tool_Arc_r_and_Radian_Given_Coors(c_Arc_Direction, c_Arc_Start_Point, P1_P2(2,1:2),c_Arc_Center, c_Yes_feasible, &
    r,c_Arc_Radian_S_P2,c_Arc_Radian_E_P2, Arc_Radian_P2)
    if(delta_L_Arc >= Tol_11)then
