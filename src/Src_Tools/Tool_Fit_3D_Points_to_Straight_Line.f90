!-----------------------------------------------------------
! Brief: Fit 3D points to a line via PCA and snap to it
!
! Parameters:
!   Input:  num_Point   - number of points to fit
!   Input:  In_Points   - input point coordinates
!   Output: Out_Points  - nearest points on the fitted line
!
! Notes:   Uses covariance-matrix eigenvector of largest eigenvalue.
!-----------------------------------------------------------

subroutine Tool_Fit_3D_Points_to_Straight_Line(num_Point, In_Points,Out_Points)
!     Fit the 3D spatial points into a straight line, and then output the points on the fitted line.
!     Theory ref: https://math.stackexchange.com/questions/1611308/best-fit-line-with-3d-points
...
do i_P = 1,num_Point
    ...
    cov_XY= cov_XY + (In_Points(i_P,1) -Ave_Location(1))* (In_Points(i_P,2) -Ave_Location(2))
    cov_YZ= cov_YZ + (In_Points(i_P,2) -Ave_Location(2))* (In_Points(i_P,3) -Ave_Location(3))
    cov_XZ= cov_XZ + (In_Points(i_P,1) -Ave_Location(1))* (In_Points(i_P,3) -Ave_Location(3))
enddo
...
abserr = Tol_9
call Matrix_Eigenvalues_and_Eigenvectors(cov_Matrix(1:3,1:3), Eigen_Vector,abserr,3)
...
do i_P = 1,num_Point
    call line_exp_point_near_3d(Line_P1,Line_P2, In_Points(i_P,1:3), Nearest_Point(1:3), c_dist, c_t)
