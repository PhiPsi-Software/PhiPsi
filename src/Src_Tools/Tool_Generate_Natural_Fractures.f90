!-----------------------------------------------------------
! Brief: Randomly generate 2D natural fractures inside a rectangular model.
!
! Parameters:
!   Input:  M_x_min,M_x_max,M_y_min,M_y_max - model coordinate bounds
!   Input:  Length_Cr - mean fracture length
!   Input:  Orien_Cr - mean fracture orientation in degrees
!   Input:  Length_Cr_Delta - length variation range
!   Input:  Orien_Cr_Delta - orientation variation range in degrees
!   Input:  c_num_Na_Cr - number of fractures to generate
!   Input:  c_R - minimum separation radius between midpoints
!   Output: c_Na_Cr - tip coordinates of the generated fractures
!
! Notes:   Places fractures one at a time, rejecting any whose
!   midpoint falls within R of an existing midpoint. Length is
!   validated against the model width plus height.
!-----------------------------------------------------------

subroutine Tool_Generate_Natural_Fractures( M_x_min,M_x_max,M_y_min,M_y_max, &
Length_Cr,Orien_Cr,Length_Cr_Delta,Orien_Cr_Delta, c_num_Na_Cr,c_R,c_Na_Cr)
! Randomly generate natural cracks, only applicable to rectangular models
! Algorithm: Generate one by one, and the midpoint of the subsequently generated crack should not fall within the radius of the midpoint of any other crack.
...
do i_Try = 1,30000000
    ...
    if(Yes_In_Circle .eqv. .False.)then
        ...
        if(c_Tip_1(1) > M_x_min .and. c_Tip_1(1)< M_x_max .and. c_Tip_1(2) > M_y_min .and. c_Tip_1(2)< M_y_max )then
            Yes_Tip1_In_Model = .True.
        endif
        if(c_Tip_2(1) > M_x_min .and. c_Tip_2(1)< M_x_max .and. c_Tip_2(2) > M_y_min .and. c_Tip_2(2)< M_y_max )then
