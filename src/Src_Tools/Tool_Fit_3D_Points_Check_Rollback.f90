!-----------------------------------------------------------
! Brief: Roll back smoothed points that fall into previous front polygon
!
! Parameters:
!   Input:     i_C            - crack index
!   Input:     num_Point      - number of front vertices
!   Input:     Pre_Fronts     - original front polygon points
!   Input:     Points_to_Smooth - pre-smoothing vertex coordinates
!   In/Out:    Points_Smoothed - smoothed output (rolled back if needed)
!
! Notes:   Blends with original point until the candidate lies outside the prior polygon.
!-----------------------------------------------------------
