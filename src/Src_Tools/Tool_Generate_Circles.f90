!-----------------------------------------------------------
! Brief: Randomly place non-overlapping circles in a rectangle
!
! Parameters:
!   Input:  M_x_min,M_x_max,M_y_min,M_y_max - model bounds
!   Input:  c_Ave_R_Ball   - mean circle radius
!   Input:  c_Delta_R_Ball - radius spread
!   Input:  c_num_Cir      - number of circles requested
!   Input:  check_R        - center-min-distance threshold
!   Input:  Max_Dis_to_Edge - min distance from rectangle edges
!   Output: c_Center_Cir(c_num_Cir,2) - generated circle centers
!   Output: c_R_Cir(c_num_Cir)        - generated circle radii
!
! Notes:   Rejection sampling; supports seeded or intrinsic RNG (key_Random).
!-----------------------------------------------------------

subroutine Tool_Generate_Circles( M_x_min,M_x_max,M_y_min,M_y_max, c_Ave_R_Ball,c_Delta_R_Ball, &
c_num_Cir,check_R,Max_Dis_to_Edge, c_Center_Cir,c_R_Cir)
!     Randomly generate circles, only applicable to rectangular models
