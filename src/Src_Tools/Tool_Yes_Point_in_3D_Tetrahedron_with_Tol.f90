!-----------------------------------------------------------
! Brief: Tests whether a 3D point lies inside or on a spatial tetrahedron, with a user-supplied tolerance.
!
! Parameters:
!   Input:  Point(3) - XYZ coordinates of the test point
!   Input:  A,B,C,D(3) - tetrahedron vertex coordinates
!   Input:  Tol - tolerance applied to barycentric ratio tests
!   Output: Yes_in - .true. if the point is strictly inside
!   Output: Yes_on - .true. if the point lies on the boundary
!
! Notes:   Same barycentric signed-distance approach as the non-tolerance
!          variant, but the alpha/beta/gamma/delta comparisons use Tol.
!-----------------------------------------------------------

subroutine Tool_Yes_Point_in_3D_Tetrahedron_with_Tol(Point, A,B,C,D,Tol, Yes_in,Yes_on)
!     Determine whether a point in space is inside a spatial tetrahedron. Reference: http://blog.csdn.net/bugrunner/article/details/7423727
!     Or My Notes V3-P176
!     With tolerance Tol, 2022-10-07.
...
implicit none
real(kind=FT),intent(in)::Point(3), A(3),B(3),C(3),D(3),Tol
