!-----------------------------------------------------------
! Brief: Test whether a 2D point lies on the line segment AB
!
! Parameters:
!   Input:  x,y     - point coordinates
!   Input:  A(2),B(2) - segment endpoints
!   Output: Yes_ON  - true if the point is on segment AB
!
! Notes:   Uses a length-additivity check with element-area-scaled tolerance.
!-----------------------------------------------------------

subroutine Tool_Yes_On_Line(x,y,A,B,Yes_ON)
!     Determine whether a point P(x, y) lies on the line segment AB
...
if ((abs((L_AP + L_BP)-L_AB)) .le. 1.0D-12*sqrt(Ave_Elem_Area)) then
    Yes_ON = .True. 
end if
