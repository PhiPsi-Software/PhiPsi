!-----------------------------------------------------------
! Brief: Test whether a 3D point is inside (or on) a hexahedron using a user-supplied tolerance
!
! Parameters:
!   Input:  Point(3)        - test point coordinates
!   Input:  A(3)..H(3)      - eight hexahedron vertex coordinates
!   Input:  Tol             - tolerance for face/edge on-boundary check
!   Output: Yes_in          - true if point is strictly inside
!   Output: Yes_on          - true if point lies on a face/edge within Tol
!
! Notes:   Same three-pyramid decomposition as the no-tolerance variant.
!-----------------------------------------------------------

subroutine Tool_Yes_Point_in_3D_Hexahedron_with_Tol(Point, A,B,C,D,E,F,G,H,Tol, Yes_in,Yes_on)
!     Determine whether a point in space is inside a spatial hexahedron (split into 3 spatial tetrahedrons for determination: H-BCGF, H-EABF, H-ABCD)
!     With tolerance.
...
call Tool_Yes_Point_in_3D_Pyramid_with_Tol(Point, H,B,C,G,F,Tol, c_Yes_in_1,c_Yes_on_1)
if (c_Yes_in_1) then
    Yes_in = .True.
    return
endif
...
call Tool_Yes_Point_in_3D_Pyramid_with_Tol(Point, H,E,A,B,F,Tol, c_Yes_in_2,c_Yes_on_2)
if (c_Yes_in_2) then
    Yes_in = .True.
    return
endif
...
call Tool_Yes_Point_in_3D_Pyramid_with_Tol(Point, H,A,B,C,D,Tol, c_Yes_in_3,c_Yes_on_3)
if (c_Yes_in_3) then
    Yes_in = .True.
    return
endif
