!-----------------------------------------------------------
! Brief: Compute the intersection point of two 3D line segments
!
! Parameters:
!   Input:  A, B       - Endpoints of the first 3D segment
!   Input:  C, D       - Endpoints of the second 3D segment
!   Output: Yes_Inter  - True if a unique intersection exists
!   Output: InterSection_P - Coordinates of the intersection point
!
! Notes:   Uses a 2x2 linear system in parameters t, s for the two
!          segment parameterizations, plus range overlap reject tests.
!-----------------------------------------------------------

subroutine Tool_Intersection_of_AB_and_CD_3D(A,B,C,D, Yes_Inter,InterSection_P)
!     Calculate the intersection point of 3D line segments AB and CD.
!     Fristly written on 2019-08-06.
!     https://math.stackexchange.com/questions/28503/how-to-find-intersection-of-two-lines-in-3d
!     ~~ or ~~
!     \theoretical_documents\003

!     Note: This code test failed! 2019-08-07
...
call Tool_Yes_Two_Ranges_Overlapped_Double( [min_x_AB,max_x_AB], [min_x_CD,max_x_CD],Logical_Yes_x)
if(Logical_Yes_x .eqv. .False.)then
    return
endif
call Tool_Yes_Two_Ranges_Overlapped_Double( [min_y_AB,max_y_AB], [min_y_CD,max_y_CD],Logical_Yes_y)
if(Logical_Yes_y .eqv. .False.)then
    return
endif
call Tool_Yes_Two_Ranges_Overlapped_Double( [min_z_AB,max_z_AB], [min_z_CD,max_z_CD],Logical_Yes_z)
...
if (det_K /= ZR) then
    ...
    if(check_value<=Tol_11) then
        ...
        call Tool_Yes_Point_on_Line_Segment_3D(A,B, InterSection_P,Yes_on_AB)
        call Tool_Yes_Point_on_Line_Segment_3D(C,D, InterSection_P,Yes_on_CD)
