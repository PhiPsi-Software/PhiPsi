!-----------------------------------------------------------
! Brief: Dispatch 3D-point fitting to a Python helper script
!
! Parameters:
!   Input:  Key_Twice  - 0 single pass, 1 averaged two-pass smoothing
!   Input:  num_Point  - number of points to fit
!   Input:  In_Points  - input point coordinates
!   Output: Out_Points - smoothed output coordinates
!
! Notes:   Selects cubic-spline (CSAPS) or B-spline (SciPy) based on Key_Smooth_Front.
!-----------------------------------------------------------

subroutine Tool_Fit_3D_Points_using_Python(Key_Twice, num_Point, In_Points,Out_Points)
! Key_Smooth_Front=3, cubic spline fitting.
! Call \Python_Tools\Smooth_3D_Points_CSAPS.py.
! Ref: https://pypi.org/project/csaps/.
!
! ---------or---------
!
! Key_Smooth_Front=4, B-spline fitting.
! Call \Python_Tools\Smooth_3D_Points_Scipy.py.
! Ref: https://stackoverflow.com/questions/18962175/spline-interpolation-coefficients-of-a-line-curve-in-3d-space.
! Ref: https://docs.scipy.org/doc/scipy/reference/generated/scipy.interpolate.splprep.html
!
!
! Firstly written on 2021-11-01.
...
if (Key_Twice==0) then
    call Fit_3D_Points_using_Python(num_Point, In_Points,Out_Points)
...
if (Key_Twice==1) then
    ...
    call Fit_3D_Points_using_Python(num_Point, In_Points_1,Out_Points_1)

    mid_num = int(num_Point/2)
    In_Points_2(1:mid_num,1:3)=In_Points(mid_num:1:-1,1:3)
    In_Points_2(mid_num+1:num_Point,1:3)= In_Points(num_Point:(mid_num-1):-1,1:3)
    call Fit_3D_Points_using_Python(num_Point, In_Points_2,Out_Points_2)

    New_Out_Points_2(1:mid_num,1:3)=Out_Points_2(mid_num:1:-1,1:3)
    New_Out_Points_2(mid_num+1:num_Point,1:3)= Out_Points_2(num_Point:(mid_num-1):-1,1:3)
...
subroutine Fit_3D_Points_using_Python(num_Point, In_Points,Out_Points)
use Global_Float_Type
use Global_Model
use Global_Common
...
pathname =  trim(PhiPsi_Current_Directory)//string_connector //'Python_Tools'//string_connector
...
if(Key_Smooth_Front==3) then
    call system ('cd .\Python_Tools\ && Smooth_3D_Points_CSAPS_v1.0.py')
elseif(Key_Smooth_Front==4) then
    call system ('cd .\Python_Tools\ && Smooth_3D_Points_Scipy_v1.0.py')
endif
