!-----------------------------------------------------------
! Brief: Solve a sparse real linear system using the Lis library.
!
! Parameters:
!   Input:  Key_Indent   - indentation width for log output
!           input_NZZ    - number of non-zeros in the CSR matrix
!           input_n      - matrix dimension (n x n)
!           K_Ptr        - CSR row pointer array (size n+1)
!           K_Index      - CSR column index array (size NZZ)
!           K_Value      - CSR non-zero values array (size NZZ)
!           input_F      - RHS vector (size n)
!   Output: output_D     - solution vector (size n)
!
! Notes:   Wrapper around Lis (lisf.h). Iterates up to the Lis
!          default 2000 iterations and reports the relative
!          residual norm on convergence.
!-----------------------------------------------------------
