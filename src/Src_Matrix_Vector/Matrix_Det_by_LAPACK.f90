!-----------------------------------------------------------
! Brief: Compute the determinant of a square matrix via LAPACK LU.
!
! Parameters:
!   Input:  n      - order of the matrix
!           Matrix - input square matrix (n,n)
!   Output: det    - determinant value
!           INFO   - LAPACK return code (0 success, >0 singular U)
!
! Notes:   Calls DGETRF to factor A = P*L*U, then combines diag(U)
!          with the sign of the pivot permutation to get det(A).
!-----------------------------------------------------------
