!-----------------------------------------------------------
! Brief: Compute the transpose of a real CSR sparse matrix.
!
! Parameters:
!   Input:  m, n, nnz      - matrix dimensions and non-zero count
!           values, col_ind, row_ptr - source CSR matrix
!   Output: values_t, col_ind_t, row_ptr_t - transposed CSR matrix
!
! Notes:   Built from column-counting then per-row fill, with the source
!          array used as scratch.
!-----------------------------------------------------------
