...
!        build the list of free DOFs for the model.
!
! Parameters:
!   Input:  c_Total_FD - total DOF count (2D)
!           isub       - current load step index
!   Output: freeDOF    - indices of free DOFs
!           c_num_FD   - number of free DOFs
!
! Notes:   Also treats nodes inside hole boundaries as
!          constrained, and supports a circular/elliptical
!          hole constraint for the first fracturing step.
!-----------------------------------------------------------

subroutine Boundary_Cond(c_Total_FD,isub, freeDOF,c_num_FD)
!     Boundary conditions.
! Total_FD: Total degrees of freedom.
! freeDOF:     Freedom of degrees of freedom.
! c_num_FD: Number of degrees of freedom after removing constraints.
...
integer c_index,fixedDOF(c_Total_FD)
