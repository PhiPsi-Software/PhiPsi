...
!     ------------------------------------------------- !
      ...
      SUBROUTINE Location_Element_Stiff_Matrix_Hl(i_E,i_H,POS_c_Hl,
     &                                       Location_ESM_C_Crack,
     &                                       num_Loc_ESM_C_Crack,
     &                                       Location_ESM_C_Cr_NoFEM,
     &                                       num_Loc_ESM_C_Cr_NoFEM)
c     The position of the element stiffness matrix in the global stiffness matrix.
      ...
      if(maxval(Enriched_Node_c_Hl(c_NN)).eq.0 .and.
     &   minval(Enriched_Node_c_Hl(c_NN)).eq.0) then
          if (i_H .eq. 1)then
