!-----------------------------------------------------------
! Brief: Assemble the full 2D XFEM stiffness matrix in
!        dense storage, including crack enrichments.
!
! Parameters:
...
!
! Notes:   Handles cracks, holes, inclusions, crosses;
!          uses sub-triangle, distributed, or sub-cell
!          integration depending on the keyword setting.
!-----------------------------------------------------------

SUBROUTINE Assemble_Stiffness_Matrix_XFEM(isub,c_globalK,c_Total_Freedom,Total_Num_G_P)
...
!option 1
!     do i=1,c_Total_Freedom
!         do j=1,c_Total_Freedom
!             if (abs(c_globalK(i,j))< = Tol_15) then
!                  c_globalK(i,j) = ZR 
!             endif
!         enddo
!     end do
...
if(Num_Boux_nonzero >0)then
    ...
    do i_Boux_nonzero =1,Num_Boux_nonzero
        ...
        c_globalK(c_node_DOF,c_node_DOF) =c_globalK(c_node_DOF,c_node_DOF)+penalty_k              
    enddo
endif
...
if (num_CP_x_nodes >0)then
    ...
    do i_CP_node =2,num_CP_x_nodes
        ...
        c_globalK(first_node_DOF,first_node_DOF) =c_globalK(first_node_DOF,first_node_DOF)+penalty_k
        c_globalK(c_node_DOF,c_node_DOF) =c_globalK(c_node_DOF,c_node_DOF)+penalty_k
        c_globalK(first_node_DOF,c_node_DOF) =c_globalK(first_node_DOF,c_node_DOF)-penalty_k
        c_globalK(c_node_DOF,first_node_DOF) =c_globalK(c_node_DOF,first_node_DOF)-penalty_k
    enddo
endif
