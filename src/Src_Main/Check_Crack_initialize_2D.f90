!-----------------------------------------------------------
! Brief: Check whether a new 2D crack should nucleate from
!        the most stressed element based on principal
!        stress or shear criteria.
!
! Parameters:
!   Input:  ifra - current fracturing step index
!           iter - current iteration index
!   Output: New_Crack_Flag - true if a new crack forms
!
! Notes:   Computes element-center stresses, finds the
!          element with the largest tensile/shear stress,
!          and seeds an initial crack if it exceeds the
!          material strength.
!-----------------------------------------------------------
...
do i_E=1,Num_Elem 
    ...
    if(Key_Analysis_Type==7) then
        ...
        U =  [DISP(c_NN(1)*2-1),DISP(c_NN(1)*2),DISP(c_NN(2)*2-1), DISP(c_NN(2)*2),DISP(c_NN(3)*2-1), &
