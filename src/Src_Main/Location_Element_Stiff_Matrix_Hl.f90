...
!
! Parameters:
!   Input:  i_E, i_H            - element and hole indices
!           POS_c_Hl(Num_Node)   - per-node DOF positions
!   Output: Location_ESM_C_Crack(80)   - FEM + hole DOF positions
!           num_Loc_ESM_C_Crack        - count
!           Location_ESM_C_Cr_NoFEM(60) - hole-only positions
!           num_Loc_ESM_C_Cr_NoFEM     - count
!
! Notes:   Only nodes with Enriched_Node_Type_Hl==1 contribute.
!-----------------------------------------------------------

subroutine Location_Element_Stiff_Matrix_Hl(i_E,i_H,POS_c_Hl, Location_ESM_C_Crack, num_Loc_ESM_C_Crack, &
...
integer,intent(in)::i_E,i_H,POS_c_Hl(Num_Node)
...
integer :: Enriched_Node_c_Hl(Num_Node)
integer :: num_hl
integer:: Location_XFEM(60)
integer :: tem

Enriched_Node_c_Hl = Enriched_Node_Type_Hl(:,i_H)
...
if(maxval(Enriched_Node_c_Hl(c_NN)).eq.0 .and. minval(Enriched_Node_c_Hl(c_NN)).eq.0) then
    if (i_H .eq. 1)then
    ...
    num_hl = count(Enriched_Node_c_Hl(c_NN).eq.1)
    tem = 2*(num_hl)   
    cnt = 0

    do i_N = 1,4
        if (Enriched_Node_c_Hl(c_NN(i_N)) .eq. 1)then
            cnt = cnt + 1
            Location_XFEM(2*cnt-1) = 2*POS_c_Hl(c_NN(i_N)) - 1
            Location_XFEM(2*cnt  ) = 2*POS_c_Hl(c_NN(i_N))
        end if
    end do   
    !-----------------------
    if (i_H .eq. 1)then
...
return
END subroutine Location_Element_Stiff_Matrix_Hl
