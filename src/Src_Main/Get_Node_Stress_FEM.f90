!-----------------------------------------------------------
! Brief: Compute 2D FEM stress at every node.
!
! Parameters:
!   Input:  isub - current load-step index
!
! Notes:   Stores results in the global Stress_xx_Node, Stress_yy_Node,
!          Stress_xy_Node, Stress_vm_Node arrays, and into TStress_*_Node
!          when Key_Thermal_Stress==1. Honours Weibull modulus and kill
!          element flags under OpenMP parallelisation.
!-----------------------------------------------------------
...
do i_E = 1,Num_Elem
    ...
    U = [DISP(c_NN(1)*2-1),DISP(c_NN(1)*2), DISP(c_NN(2)*2-1),DISP(c_NN(2)*2), DISP(c_NN(3)*2-1),DISP(c_NN(3)*2), &
    ...
    do i_N = 1,4 
        ...
        Stress_xx_Node(C_Node) = Stress_xx_Node(C_Node) + c_Stress(1)
        Stress_yy_Node(C_Node) = Stress_yy_Node(C_Node) + c_Stress(2)
        Stress_xy_Node(C_Node) = Stress_xy_Node(C_Node) + c_Stress(3)
        ...
        if(Key_Thermal_Stress==1)then
            ...
            Stress_xx_Node(C_Node) = Stress_xx_Node(C_Node) - c_TStress(1)
            Stress_yy_Node(C_Node) = Stress_yy_Node(C_Node) - c_TStress(2)
            Stress_xy_Node(C_Node) = Stress_xy_Node(C_Node) - c_TStress(3)
        endif
