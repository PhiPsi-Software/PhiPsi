!-----------------------------------------------------------
! Brief: Determine XFEM enriched nodes for 3D problems.
!
! Parameters:
!   Input:  ifra - fracture step index
!           isub - current substep index
!
! Notes:   Classifies each 3D element as tip, Heaviside,
!          fully cracked, or junction and assigns matching
!          enrichment types per node and crack.
!-----------------------------------------------------------
...
!                                         !               4--Junction elements
! Enriched_Node_Type_3D: =1, Tip enrichment; =2, Heaviside enrichment
! 
...
use module_INTERFACE_Local_L2_projection_for_inter_element
!
!...........................
...
!!.......................................................................
!!                                                                      .
!!          Save old enriched nodes for 3D explicit analysis.           .
!!          2026-05-11.                                                 .
!!                                                                      .
!!.......................................................................
!if(Key_Analysis_Type==6 .and. isub >=2) then
!    IF(ALLOCATED(Enriched_Node_Type_3D_Old)) DEALLOCATE(Enriched_Node_Type_3D_Old)   
!    ALLOCATE(Enriched_Node_Type_3D_Old(Num_Node,num_Crack))
!    Enriched_Node_Type_3D_Old = 0
!    Enriched_Node_Type_3D_Old(1:Num_Node,1:num_Crack) = Enriched_Node_Type_3D(1:Num_Node,1:num_Crack)
!endif
...
if (Key_Save_Old_Solid_El_Tip_BaseLine==0) then
...
!NEWFTU-2026051401. Save Old Solid_El_Tip_BaseLine_*.
if (Key_Save_Old_Solid_El_Tip_BaseLine==1) then
    if (isub >=2) then
        IF(ALLOCATED(Solid_El_Tip_BaseLine_OLD)) DEALLOCATE(Solid_El_Tip_BaseLine_OLD)    
        IF(ALLOCATED(Solid_El_Tip_BaseLine_x_Vec_OLD)) DEALLOCATE(Solid_El_Tip_BaseLine_x_Vec_OLD)    
        IF(ALLOCATED(Solid_El_Tip_BaseLine_y_Vec_OLD)) DEALLOCATE(Solid_El_Tip_BaseLine_y_Vec_OLD)   
        IF(ALLOCATED(Solid_El_Tip_BaseLine_z_Vec_OLD)) DEALLOCATE(Solid_El_Tip_BaseLine_z_Vec_OLD) 
        IF(ALLOCATED(Solid_El_Tip_BaseLine_T_Matrix_OLD)) DEALLOCATE(Solid_El_Tip_BaseLine_T_Matrix_OLD) 
        allocate(Solid_El_Tip_BaseLine_OLD(Num_Elem))
        allocate(Solid_El_Tip_BaseLine_x_Vec_OLD(Num_Elem))
        allocate(Solid_El_Tip_BaseLine_y_Vec_OLD(Num_Elem))
        allocate(Solid_El_Tip_BaseLine_z_Vec_OLD(Num_Elem))
        allocate(Solid_El_Tip_BaseLine_T_Matrix_OLD(Num_Elem))
        do i_E = 1, Num_Elem   
            if(allocated(Solid_El_Tip_BaseLine(i_E)%row))then
                if(.not. allocated(Solid_El_Tip_BaseLine_OLD(i_E)%row))then
                    allocate(Solid_El_Tip_BaseLine_OLD(i_E)%row(Solid_El_Max_num_Crs,2,3))
                    Solid_El_Tip_BaseLine_OLD(i_E)%row(1:Solid_El_Max_num_Crs,1:2,1:3) = &
                    Solid_El_Tip_BaseLine(i_E)%row(1:Solid_El_Max_num_Crs,1:2,1:3)
                endif
            endif
            if(allocated(Solid_El_Tip_BaseLine_x_Vec(i_E)%row))then
                if(.not. allocated(Solid_El_Tip_BaseLine_x_Vec_OLD(i_E)%row))then
                    allocate(Solid_El_Tip_BaseLine_x_Vec_OLD(i_E)%row(Solid_El_Max_num_Crs,3))
                    Solid_El_Tip_BaseLine_x_Vec_OLD(i_E)%row(1:Solid_El_Max_num_Crs,1:3) = &
                    Solid_El_Tip_BaseLine_x_Vec(i_E)%row(1:Solid_El_Max_num_Crs,1:3)
                endif
            endif
            if(allocated(Solid_El_Tip_BaseLine_y_Vec(i_E)%row))then
                if(.not. allocated(Solid_El_Tip_BaseLine_y_Vec_OLD(i_E)%row))then
                    allocate(Solid_El_Tip_BaseLine_y_Vec_OLD(i_E)%row(Solid_El_Max_num_Crs,3))
                    Solid_El_Tip_BaseLine_y_Vec_OLD(i_E)%row(1:Solid_El_Max_num_Crs,1:3) = &
                    Solid_El_Tip_BaseLine_y_Vec(i_E)%row(1:Solid_El_Max_num_Crs,1:3)
                endif
            endif
            if(allocated(Solid_El_Tip_BaseLine_z_Vec(i_E)%row))then
                if(.not. allocated(Solid_El_Tip_BaseLine_z_Vec_OLD(i_E)%row))then
                    allocate(Solid_El_Tip_BaseLine_z_Vec_OLD(i_E)%row(Solid_El_Max_num_Crs,3))
                    Solid_El_Tip_BaseLine_z_Vec_OLD(i_E)%row(1:Solid_El_Max_num_Crs,1:3) = &
                    Solid_El_Tip_BaseLine_z_Vec(i_E)%row(1:Solid_El_Max_num_Crs,1:3)
                endif
            endif
            if(allocated(Solid_El_Tip_BaseLine_T_Matrix(i_E)%row))then
                if(.not. allocated(Solid_El_Tip_BaseLine_T_Matrix_OLD(i_E)%row))then
                    allocate(Solid_El_Tip_BaseLine_T_Matrix_OLD(i_E)%row(Solid_El_Max_num_Crs,3,3))
                    Solid_El_Tip_BaseLine_T_Matrix_OLD(i_E)%row(1:Solid_El_Max_num_Crs,1:3,1:3) = &
                    Solid_El_Tip_BaseLine_T_Matrix(i_E)%row(1:Solid_El_Max_num_Crs,1:3,1:3)
                endif
            endif
        enddo
    endif
...
do i_E = 1,Num_Elem   
    ...
    do i_C=1,num_Crack
            ...
            elseif(Solid_El_Vertex_Num(i_E)%row(c_Cr_Location)==3) then
                ...
                Solid_El_Tip_BaseLine_Nor_Vec(i_E)%row(c_Cr_Location,1:3) =  &
                (Solid_El_Vertex_Nor_Vec(i_E)%row(c_Cr_Location,1,1:3) + &
                Solid_El_Vertex_Nor_Vec(i_E)%row(c_Cr_Location,2,1:3) + Solid_El_Vertex_Nor_Vec(i_E)%row(c_Cr_Location,3,1:3))/THR
                ...
                !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                Solid_El_Tip_BaseLine_x_Vec(i_E)%row(c_Cr_Location,1:3) = (Solid_El_Vertex_x_Vec(i_E)%row(c_Cr_Location,1,1:3)  + &
                Solid_El_Vertex_x_Vec(i_E)%row(c_Cr_Location,2,1:3)  + Solid_El_Vertex_x_Vec(i_E)%row(c_Cr_Location,3,1:3))/THR
                Solid_El_Tip_BaseLine_y_Vec(i_E)%row(c_Cr_Location,1:3) = (Solid_El_Vertex_y_Vec(i_E)%row(c_Cr_Location,1,1:3)  + &
                Solid_El_Vertex_y_Vec(i_E)%row(c_Cr_Location,2,1:3)   + Solid_El_Vertex_y_Vec(i_E)%row(c_Cr_Location,3,1:3))/THR
                Solid_El_Tip_BaseLine_z_Vec(i_E)%row(c_Cr_Location,1:3) = (Solid_El_Vertex_z_Vec(i_E)%row(c_Cr_Location,1,1:3)  + &
                Solid_El_Vertex_z_Vec(i_E)%row(c_Cr_Location,2,1:3)  + Solid_El_Vertex_z_Vec(i_E)%row(c_Cr_Location,3,1:3))/THR
                call Vector_Normalize(3,Solid_El_Tip_BaseLine_x_Vec(i_E)%row(c_Cr_Location,1:3))
                call Vector_Normalize(3,Solid_El_Tip_BaseLine_y_Vec(i_E)%row(c_Cr_Location,1:3))
                call Vector_Normalize(3,Solid_El_Tip_BaseLine_z_Vec(i_E)%row(c_Cr_Location,1:3))
                !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                ...
                ! satisfied.
                call Tool_ThetaX_ThetaY_ThetaZ_3D_rotation(1, &
                Solid_El_Tip_BaseLine_x_Vec(i_E)%row(c_Cr_Location,1:3), Solid_El_Tip_BaseLine_y_Vec(i_E)%row(c_Cr_Location,1:3), &
                Solid_El_Tip_BaseLine_z_Vec(i_E)%row(c_Cr_Location,1:3),  &
                Vector_Crack_x(1:3),Vector_Crack_y(1:3),Vector_Crack_z(1:3), &
                ThetaX,ThetaY,ThetaZ,T_Matrx(1:3,1:3))
                Solid_El_Tip_BaseLine_T_Matrix(i_E)%row(c_Cr_Location,1:3,1:3)= T_Matrx(1:3,1:3)
...
do i_Node = 1,num_Node
    ...
    do i_C=1,num_Crack
        ...
        elseif(Key_InPlane_Growth== 1) then
            call D3_Get_Signed_Dis_to_Crack_Mesh_for_InPlane_Growth(c_Node_Coor,i_C, Check_Ball_R,c_Dis_Node_to_FS , &
            ...
            if(Elem_Type_3D(i_E,i_C) ==0 .and. Tem_Elem_Type(i_E,i_C) ==0  )then
                ...
                if ((c_Yes_Inter .eqv. .True.) .and.  num_Inter_Outline==1)then
                    ...
                    if(c_Cr_Location==0 .or. Solid_El_num_Crs(i_E)==0)then
                        ...
                        Solid_El_Tip_BaseLine_y_Vec(i_E)%row(Solid_El_num_Crs(i_E),1:3), &
                        Solid_El_Tip_BaseLine_z_Vec(i_E)%row(Solid_El_num_Crs(i_E),1:3), &
                        Vector_Crack_x(1:3),Vector_Crack_y(1:3),Vector_Crack_z(1:3), ThetaX,ThetaY,ThetaZ,T_Matrx(1:3,1:3))
    ...
    if (c_Elem_num_Related_Cracks >= 2)then
        ...
        do i_C=1,num_Crack  
            ...
            if(Elem_Type_3D(i_E,i_C) == 2)then
                ...
                Heaviside_Enrich_Type_Int_1 = 10000000*n_Vector_Sign_Int_1(1) + 1000000*n_Vector_Sign_Int_1(2) + &
                100000*n_Vector_Sign_Int_1(3) + 10000*n_Vector_Sign_Int_1(4) + 1000*n_Vector_Sign_Int_1(5) + &
                100*n_Vector_Sign_Int_1(6) + 10*n_Vector_Sign_Int_1(7) + 1*n_Vector_Sign_Int_1(8)
                ...
                do j_C = i_C,num_Crack 
                    if(j_C /= i_C) then
                        ...
                        if (Elem_Type_3D(i_E,j_C) == 2) then
                            ...
                            Heaviside_Enrich_Type_Int_2 = 10000000*n_Vector_Sign_Int_2(1) + 1000000*n_Vector_Sign_Int_2(2) + &
                            100000*n_Vector_Sign_Int_2(3) + 10000*n_Vector_Sign_Int_2(4) + 1000*n_Vector_Sign_Int_2(5) + &
                            100*n_Vector_Sign_Int_2(6) + 10*n_Vector_Sign_Int_2(7) + 1*n_Vector_Sign_Int_2(8)
                            ...
                            if(Heaviside_Enrich_Type_Int_2==  Heaviside_Enrich_Type_Int_1) then
                                ...
                                do i_Node = 1,8
                                    ...
                                    Dis_Node_to_FS(c_NN(i_Node))%row(j_C),1, Elem_Ave_L(i_E)*0.05D0, &
                                    c_Logical_Dif)
...
if(Key_InPlane_Growth== 1) then
    ...
    do i_C=1,num_Crack
        ...
        Crack3D_Meshed_Node(i_C)%row(Crack3D_Meshed_Outline(i_C)%row(1:c_num_Points,1),1:3), Cracks_Outline_Area(i_C))
    enddo
    !$omp end parallel do 
endif
            ...
            do i_E = 1,c_Num_Eles
                ...
                do i_G = 1,Tem_Num_Gauss_3D
                    ...
                    if(Key_InPlane_Growth== 0 ) then
                        ...
                        call D3_Get_Signed_Dis_to_Crack_Mesh(Global_coor_Gauss,i_C,Check_Ball_R,c_Distance(i_E,i_G), &
                        c_Signed_Dis_v2,c_Yes_Node_PER_in_FS,c_PER_Node_to_FS(1:3), Yes_Found_Min_Signed_Dis,c_n_Vector)
                    ...
                    !BUGFIX2022092602.
                    if(c_Logical_Positive .and. c_Logical_Negative .and. c_Logical_Positive_Num>=Must_Gauss_Number_3D .and. &
                    ...
                    do j_Crack_Ele =1,Crack3D_Meshed_Ele_num(j_C)
                        ...
                        if((c_Yes_Side_on .eqv. .True.) .and. (c_Yes_Parallel .eqv. .False.))then
                            ...
                            if(Mesh_Node_1_on_Outline .and. Mesh_Node_2_on_Outline) then
                                ...
                                call Cal_Ele_Num_by_Coors_3D(c_Inter_Points(1,1),c_Inter_Points(1,2), &
                                c_Inter_Points(1,3),Ele_Num_Cache,c_Elem) 
                                ...
                                call Tool_Offset_Point_A_to_Point_B_3D(c_Inter_Points(1,1:3), &
                                Cracks_Coor_Centers(i_C,1:3),delta_L,new_A)   
    ...
    do i_C = 1, num_Crack    
        ...
        do i_Node = 1, 8
            ...
            if (enrich_type > 0) then
                Elem_Topology_Markers_3D(i_E) = Elem_Topology_Markers_3D(i_E) + (i_C * 1000000) + (primes(i_Node) ** enrich_type)
            endif
        enddo
    enddo
enddo
