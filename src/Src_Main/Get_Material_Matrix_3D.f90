!-----------------------------------------------------------
! Brief: Build the 3D 6x6 material constitutive matrix D for all materials.
!
! Parameters:
!   (none)
!
! Notes:   Allocates D, D_Comp, S, St, Sc, T_Alpha, KIc, E, v, density
!          and D_for_cylindrical global arrays. Supports isotropic and
!          transversely isotropic (composite) materials, and bypasses D
!          allocation when the Xin Ao (Key_XA==2) element is selected.
!-----------------------------------------------------------

subroutine Get_Material_Matrix_3D
!     Calculate the material matrix D (3D problem)
...
do i_mat =1,num_of_Material
    ...
    if (Material_Type(i_mat).eq. 1  )then      
        ...
        !-----------------------------------------------------------------------------------
        ! -- OPTION 1 --
        !             D_for_cylindrical(i_mat,1,:)=
        !    &                         [ONE,    -c_v,    -c_v,   ZR,   ZR,  ZR]
        !             D_for_cylindrical(i_mat,2,:)=
        !    &                         [-c_v,    ONE,    -c_v,   ZR,   ZR,  ZR]
        !             D_for_cylindrical(i_mat,3,:)=
        !    &                         [-c_v,    -c_v,    ONE,   ZR,   ZR,  ZR]
        !             D_for_cylindrical(i_mat,4,:)=
        !    &                         [ZR,    ZR,    ZR,TWO*(ONE+c_v), ZR,  ZR]
        !             D_for_cylindrical(i_mat,5,:)=
        !    &                         [ZR,    ZR,    ZR,ZR, TWO*(ONE+c_v),  ZR]
        !             D_for_cylindrical(i_mat,6,:)=
        !    &                         [ZR,    ZR,    ZR,ZR, ZR,  TWO*(ONE+c_v)]             
        !             D_for_cylindrical(i_mat,:,:) = 
        !    &                   D_for_cylindrical(i_mat,:,:)/c_E
        ! -- OPTION 2 (Calculate the inverse matrix of D, equivalent to OPTION 1)--
        call Matrix_Inverse(D(i_mat,1:6,1:6), D_for_cylindrical(i_mat,1:6,1:6),6)
