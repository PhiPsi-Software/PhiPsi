!-----------------------------------------------------------
! Brief: Iteratively determine 2D crack-surface contact.
!
! Parameters:
...
!
! Notes:   Drives Newton-Raphson contact iterations via
!          penalty, reduced-penalty, or LCP methods.
!-----------------------------------------------------------
...
!     This subroutine determines the contact status of the fracture surface through iterative calculation.
...
case(1)
    ...
    do i_NR_P = 1,Max_Contact_Iter
        ...
        if(i_NR_P==1)then 
            ...
            call Cal_Contact_Contact_State_Gauss( iter,ifra,Counter_Iter,i_NR_P, Contact_DISP,Yes_Contact,Elem_Conta_Sta, &
            CT_State_Gauss)
        ...
        call Cal_Contact_PN_and_PT(iter,ifra,Counter_Iter, i_NR_P,c_Total_FD,c_num_freeDOF, &
        Kn,Kn_Gauss,Kt_Gauss,fric_mu,NR_DISP,delta_u_a, CT_State_Gauss, Elem_Conta_Sta, PC_Gauss_x,PC_Gauss_y)
        ...
        call Cal_Contact_Jacobian( iter,ifra,Counter_Iter,i_NR_P, c_Total_FD,c_num_freeDOF,ori_globalK, &
        c_freeDOF,Kn,Kn_Gauss,Kt_Gauss, CT_State_Gauss,CT_Jacobian)
        ...
        !------------------------------------------------------------
        call Cal_Contact_Resid( iter,ifra,Counter_Iter,i_NR_P, c_Total_FD,c_num_freeDOF,c_F, &
        ...
        !-------------------------------------------------------------------------------
        !             call Cal_Contact_Resid_option2(
        ...
        delta_u_a(1:c_Total_FD)  =ZR
        call Matrix_Solve_LSOE(5,1,Key_SLOE, CT_Jacobian(c_freeDOF(1:c_num_freeDOF), c_freeDOF(1:c_num_freeDOF)), &
        ...
        call Cal_Contact_Conve_Factor( iter,ifra,Counter_Iter,i_NR_P,Conve_Tolerance, c_Total_FD,c_freeDOF,c_num_freeDOF, &
        c_F,R_PSI,Last_R_PSI, delta_u_a,NR_DISP,Contact_DISP_0, Yes_Conve,Conve_Factor)
...
case(2)
    ...
    allocate(U_all_0(c_Total_FD),U_all(c_Total_FD), U_all_last(c_Total_FD),delta_U_all(c_Total_FD))
    K_uu_Inv(1:n_freeDOF_FEM,1:n_freeDOF_FEM) = ori_globalK(freeDOF_FEM(1:n_freeDOF_FEM), freeDOF_FEM(1:n_freeDOF_FEM))
    print *,'      Get the inverse matrix of Kuu......'
    call Matrix_Inverse_Lapack(K_uu_Inv(1:n_freeDOF_FEM, 1:n_freeDOF_FEM), n_freeDOF_FEM)
    ! Check whether the calculation of the inverse matrix is correct
    print *,'      Get Kue and Kee......'
    K_ue(1:n_freeDOF_FEM,1:enrich_FD) = ori_globalK(freeDOF_FEM(1:n_freeDOF_FEM), usual_FD+1:c_Total_FD)
    K_ee(1:enrich_FD,1:enrich_FD) = ori_globalK(usual_FD+1:c_Total_FD,usual_FD+1:c_Total_FD)
    !Invariant Part1 and Part2.
    Part1 = MATMUL(transpose(K_ue),MATMUL(K_uu_Inv,K_ue))
    Part2 = MATMUL(MATMUL(transpose(K_ue),K_uu_Inv), c_F(freeDOF_FEM(1:n_freeDOF_FEM)))
    ...
    do i_NR_P = 1,Max_Contact_Iter
        ...
        if(i_NR_P==1)then 
            ori_K_ee = K_ee
            U_e(1:enrich_FD) = ZR
            U_e_0(1:enrich_FD) =  Contact_DISP(Usual_Freedom+1: c_Total_FD)
            ...
            Elem_Conta_Sta(1:Num_Elem,1:Max_Num_Cr) = 0 
            Kt_Gauss(1:num_Crack,1:Max_Num_Cr_CalP-1,1:2) = kt
            call Cal_Contact_Red_Contact_State_Gauss( iter,ifra,Counter_Iter,i_NR_P, U_e_0,Yes_Contact,Elem_Conta_Sta, &
            CT_State_Gauss)
        ...
        print *,'         Get contact force and update Kt.'
        call Cal_Contact_PN_and_PT(iter,ifra,Counter_Iter, i_NR_P,c_Total_FD,c_num_freeDOF, &
        Kn,Kn_Gauss,Kt_Gauss,fric_mu,U_all,delta_U_all, CT_State_Gauss, Elem_Conta_Sta, PC_Gauss_x,PC_Gauss_y)
        ...
        call Cal_Contact_Red_Resid( iter,ifra,Counter_Iter,i_NR_P, c_Total_FD,c_num_freeDOF,n_freeDOF_FEM,c_F, freeDOF_FEM, &
        U_e,U_all,c_freeDOF, Part1,Part2,K_ee,K_uu_Inv,K_ue, PC_Gauss_x,PC_Gauss_y,R_red)
        ...
        call Cal_Contact_Red_Update_Kee_and_Jacobian( iter,ifra,Counter_Iter,i_NR_P, c_Total_FD,c_num_freeDOF,ori_globalK, &
        c_freeDOF,Kn,Kt_Gauss,CT_State_Gauss, Part1,Part2, ori_K_ee,K_ee,J_red)
        ...
        print *,'         Get displacemnt increament.'
        delta_U_e(1:enrich_FD)  =ZR
        call Matrix_Solve_LSOE(7,1,Key_SLOE, J_red, R_red, tem_DISP_red,enrich_FD)
        ...
        U_u(1:c_Total_FD) = ZR
        ! Output displacement
        red_tem1 = c_F(freeDOF_FEM(1:n_freeDOF_FEM)) - MATMUL(K_ue,U_e)
        U_u(freeDOF_FEM(1:n_freeDOF_FEM)) = MATMUL(K_uu_Inv,red_tem1)
        ...
        !#####################################################################################
        print *,'         Check convergence.'
        call Cal_Contact_Red_Conve_Factor( iter,ifra,Counter_Iter,i_NR_P,Conve_Tolerance, c_Total_FD,c_F,R_red,Last_R_red, &
        U_all_last,U_all,U_all_0, Yes_Conve,Conve_Factor)
        write(*,4022) Conve_Factor   
        ! Exit the iteration loop if convergence is achieved.
        if(Yes_Conve)then
    ...
    !****************************************************
    CT_Jacobian = ori_globalK
    CT_Jacobian(usual_FD+1:c_Total_FD, usual_FD+1:c_Total_FD) = K_ee
    Contact_DISP = U_all
