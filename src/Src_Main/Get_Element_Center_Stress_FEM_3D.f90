!-----------------------------------------------------------
! Brief: Compute stress at the center of each 3D FEM hex element.
!
! Parameters:
...
!   Output: Stress_El_Center - per-element stress (xx,yy,zz,xy,yz,xz,vm)
!
! Notes:   Uses element material stiffness D (or XA variant) and the
!          element DOF vector; for Key_XA==2 the Xin Ao element form
!          is used instead.
!-----------------------------------------------------------

subroutine Get_Element_Center_Stress_FEM_3D(Yes_Add_Insitu, isub,c_DISP, Stress_El_Center)
!     Stress at the center of the computational unit (FEM)
!     2020-03-13
!     Stress_El_Center(num_Ele,1) - xx
!     Stress_El_Center(num_Ele,2) - yy
!     Stress_El_Center(num_Ele,3) - zz
!     Stress_El_Center(num_Ele,4) - xy
!     Stress_El_Center(num_Ele,5) - yz
!     Stress_El_Center(num_Ele,6) - xz
!     Stress_El_Center(num_Ele,7) - vm
...
do i_E = 1,Num_Elem
    ...
    c_DISP(c_NN(2)*3-2),c_DISP(c_NN(2)*3-1),c_DISP(c_NN(2)*3), c_DISP(c_NN(3)*3-2),c_DISP(c_NN(3)*3-1),c_DISP(c_NN(3)*3), &
    c_DISP(c_NN(4)*3-2),c_DISP(c_NN(4)*3-1),c_DISP(c_NN(4)*3), c_DISP(c_NN(5)*3-2),c_DISP(c_NN(5)*3-1),c_DISP(c_NN(5)*3), &
    c_DISP(c_NN(6)*3-2),c_DISP(c_NN(6)*3-1),c_DISP(c_NN(6)*3), c_DISP(c_NN(7)*3-2),c_DISP(c_NN(7)*3-1),c_DISP(c_NN(7)*3), &
    ...
    if(Key_Thermal_Stress==1)then
        ...
        Stress_El_Center(i_E,1)=Stress_El_Center(i_E,1)- c_TStress(1)
        Stress_El_Center(i_E,2)=Stress_El_Center(i_E,2)- c_TStress(2)
        Stress_El_Center(i_E,3)=Stress_El_Center(i_E,3)- c_TStress(3)
        Stress_El_Center(i_E,4)=Stress_El_Center(i_E,4)- c_TStress(4)
        Stress_El_Center(i_E,5)=Stress_El_Center(i_E,5)- c_TStress(5)
        Stress_El_Center(i_E,6)=Stress_El_Center(i_E,6)- c_TStress(6)
    endif
    ...
    if(Key_InSitu_Strategy==2 .and. Yes_Add_Insitu)then
        ...
        Stress_El_Center(i_E,1)=Stress_El_Center(i_E,1)+ Str_InSitu_El(1)
        Stress_El_Center(i_E,2)=Stress_El_Center(i_E,2)+ Str_InSitu_El(2)
        Stress_El_Center(i_E,3)=Stress_El_Center(i_E,3)+ Str_InSitu_El(3)
        Stress_El_Center(i_E,4)=Stress_El_Center(i_E,4)+ Str_InSitu_El(4)
        Stress_El_Center(i_E,5)=Stress_El_Center(i_E,5)+ Str_InSitu_El(5)
        Stress_El_Center(i_E,6)=Stress_El_Center(i_E,6)+ Str_InSitu_El(6)
    endif
    ...
    if(Key_InSitu_Strategy==4)then
        ...
        ! thermal stress.
        c_Stress = MATMUL(c_D,[ InSitu_Strain_Gaus_xx(i_E,1), InSitu_Strain_Gaus_yy(i_E,1), InSitu_Strain_Gaus_zz(i_E,1), &
        InSitu_Strain_Gaus_xy(i_E,1), InSitu_Strain_Gaus_yz(i_E,1), InSitu_Strain_Gaus_xz(i_E,1)])
        Stress_El_Center(i_E,1) = Stress_El_Center(i_E,1) - c_Stress(1)
        Stress_El_Center(i_E,2) = Stress_El_Center(i_E,2) - c_Stress(2)
        Stress_El_Center(i_E,3) = Stress_El_Center(i_E,3) - c_Stress(3)
        Stress_El_Center(i_E,4) = Stress_El_Center(i_E,4) - c_Stress(4)
    ...
    call Tool_von_Mises_3D(Stress_El_Center(i_E,1), Stress_El_Center(i_E,2), Stress_El_Center(i_E,3), &
    Stress_El_Center(i_E,4), Stress_El_Center(i_E,5), Stress_El_Center(i_E,6), Stress_El_Center(i_E,7))
end do  
