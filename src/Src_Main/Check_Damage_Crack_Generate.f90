!-----------------------------------------------------------
! Brief: Detect and generate cracks from elastic damage.
!
! Parameters:
!   Input:  ifra - fracture step index
!           iter - iteration index
!           Total_Num_G_P - total number of Gauss points
!   Output: Yes_Generated_out - true if a damage crack forms
!
! Notes:   For damage materials (Type 3): finds the maximum
!          damage Gauss point, orients the new crack normal
!          to the maximum principal tensile stress there.
!-----------------------------------------------------------

subroutine Check_Damage_Crack_Generate(ifra,iter,Total_Num_G_P, Yes_Generated_out)
!     Determine whether a crack has formed based on the damage factor, and determine the crack 
!     direction according to the direction of the perpendicular of the maximum principal tensile
!     stress at the location of maximum damage.
!     Applicable to, Analysis Type 1, with elastic damage material (Material Type 3)
...
real(kind=FT) Crack_P1(2),Crack_P2(2), Line_AB(2,2),new_Line_AB(2,2)
...
if (Crack_Gen_from_Damage == 0) then
    if(Max_Damage >= Material_Dam2Frac_Value)then
        ...
        call Cal_Ele_Num_by_Coors(Coor_X_Max_Dam,Coor_Y_Max_Dam, c_Elem_Num)
        !-------------------------------------------------------
        ...
        call Cal_Weighted_Ave_Stress_of_Point(Key_Ave_Stress, [Coor_X_Max_Dam,Coor_Y_Max_Dam], Search_R,a_Weight,l_Ordina, &
