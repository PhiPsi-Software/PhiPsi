!-----------------------------------------------------------
! Brief: Check whether new cracks initiate in a 3D problem.
!
! Parameters:
!   Input:  ifra - current fracture step index
!           iter - current iteration index
!   Output: New_Crack_Flag - true if a new crack has formed
!
! Notes:   Evaluates element-center stress using principal-
!          stress, shear, or combined criteria; inserts a
!          new circular or rectangular crack at the worst
!          element when strength is exceeded.
!-----------------------------------------------------------
...
if (Yes_XFEM .eqv. .False.) then
    ...
    do i_E=1,Num_Elem   
        ...
        call Cal_Any_Point_Str_KesiYita_3D(i_E,0,1,1,ZR, ZR,ZR,1,DISP, Stress_El_Center(i_E,1), Stress_El_Center(i_E,2), &
        Stress_El_Center(i_E,3), Stress_El_Center(i_E,4), Stress_El_Center(i_E,5), Stress_El_Center(i_E,6))
    enddo
    !$OMP END PARALLEL DO
endif
...
if(Key_Ini_Rule==1)then  
    ...
    do i_E=1,Num_Elem   
        ...
        call Tool_Principal_Stresses_3D(S_xx,S_yy,S_zz, S_yz,S_xz,S_xy, S1_El(i_E),S2_El(i_E),S3_El(i_E), &
        Vector_S1_El(i_E,1:3),Vector_S2_El(i_E,1:3), Vector_S3_El(i_E,1:3))
    enddo
    !---------------------------
