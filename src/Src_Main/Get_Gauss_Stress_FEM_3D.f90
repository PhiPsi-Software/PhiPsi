!-----------------------------------------------------------
! Brief: Compute 3D FEM stress at every Gauss point of every element.
!
! Parameters:
!   Input:  isub   - current load-step index
!           c_DISP - global nodal displacement vector
!
! Notes:   Stores all six Cartesian components and von Mises in the
!          Stress_*_Gauss global arrays. Loops over elements with OpenMP.
!-----------------------------------------------------------
...
do i_E = 1,Num_Elem
    ...
    do i_G = 1,Num_Gauss_P_FEM_3D
        ...
        !coordinate system.
        call Cal_Ele_Str_N8_3D(i_E,1,1,1, c_X_NODES,c_Y_NODES,c_Z_NODES, c_D,c_kesi,c_yita,c_zeta,U, c_Stress)
        ...
        Stress_xy_Gauss(G_Counter),Stress_yz_Gauss(G_Counter),Stress_xz_Gauss(G_Counter), Stress_vm_Gauss(G_Counter))
    ...
    if (allocated(Elem_Ave_Gauss_Stress)) then
        ...
        Elem_Ave_Gauss_Stress(i_E,3),Elem_Ave_Gauss_Stress(i_E,4), Elem_Ave_Gauss_Stress(i_E,5),Elem_Ave_Gauss_Stress(i_E,6), &
        c_S_1,c_S_2,c_S_3, Vector_S1,Vector_S2,Vector_S3)
