!-----------------------------------------------------------
! Brief: Assemble the 2D XFEM stiffness matrix in CSR
!        sparse storage, including crack enrichments.
!
! Parameters:
!   Input:  isub           - current load step index
!           freeDOF, fixedDOF, num_FreeD, num_FixedD
...
!
! Notes:   Handles cracks, inclusions, cross-junctions;
!          OpenMP parallelized with progress reporting.
!-----------------------------------------------------------
...
real(kind=FT) kesi_N_Enr(4), yita_N_Enr(4), &
weight_N_Enr(4)
        ...
        type( CLS_CMD_Progress ) ::Progress_Location_COO
            ...
            if (Scheme ==1) then
                ...
                do i_E = 1,Num_Elem
                    ...
                    elseif(Key_Integral_Sol.eq.2 .or. Key_Integral_Sol.eq.3)then
                        ...
                        if(num_Hole/=0)then
                            do i_H =1,num_Hole
                                call Location_Element_Stiff_Matrix_Hl(i_E,i_H, c_POS_Hl(1:Num_Node,i_H),&
                        ...
                        if(num_Cross/=0)then
                            do i_Cross =1,num_Cross
                                call Location_Element_Stiff_Matrix_Cross(i_E,i_Cross, c_POS_Cross(1:Num_Node,i_Cross),&
                                Location_ESM_C_Crack, num_Loc_ESM_C_Crack, Location_ESM_C_Cr_NoFEM, num_Loc_ESM_C_Cr_NoFEM)
                                ! Includes FEM degrees of freedom
                                Location_ESM(num_Loc_ESM+1:num_Loc_ESM+num_Loc_ESM_C_Crack) = &
                                Location_ESM_C_Crack(1:num_Loc_ESM_C_Crack)
                ...
                do i_E = 1,Num_Elem
                    ...
                    elseif(Key_Integral_Sol.eq.2 .or. Key_Integral_Sol.eq.3)then
                        ...
                        do i_G = 1,c_Num_Gauss_Point
                            ...
                            if(num_Crack/=0)then
                                do i_C =1,num_Crack
                                    call Cal_B_Matrix_Crack(kesi(i_G),yita(i_G), i_C,i_E,i_G, &
                            ...
                            if(num_Poly_Incl/=0)then
                                do i_Incl =1,num_Poly_Incl
                                    call Cal_B_Matrix_Poly_Incl(kesi(i_G), yita(i_G),i_Incl,i_E,i_G, &
                                    c_NN,c_X_NODES,c_Y_NODES, tem_B,num_tem_B)
                                    B(1:3,num_B+1:num_B+num_tem_B) =  tem_B(1:3,1:num_tem_B)
                                    num_B = num_B + num_tem_B
                                end do
                            ...
                            localK(1:num_B,1:num_B) = localK(1:num_B,1:num_B) + c_thick*weight(i_G)*detJ* &
                ...
                call Matrix_n_x_3_Combine_Same_i_j_Lines(K_Each_Count,K_Each(1:K_Each_Count,1:3),&
                                                         Combined_K_Each(1:K_Each_Count,1:3), num_rows_Combined_K)
            ...
            if (Scheme ==2) then
                ...
                if(Sparse_Store_Method==1)then
                    ...
                    ! efficiency~~~~~~~~~~~~~~~
                    if(isub<=1)then
                        Open(1001,File = c_File_name_2,Access = 'Direct', Form = 'Unformatted', status='unknown', &
                        RecL = T_Freedom*2)
                    else
                        Open(1001,File = c_File_name_2,Access = 'Direct', Form = 'Unformatted', status='old', &
                        RecL = T_Freedom*2)
                    endif
                    ...
                    ! efficiency~~~~~~~~~~~~~~~
                    if(isub<=1)then
                        Open(1002,File =c_File_name_1,Access='Direct', Form = 'Unformatted',status='unknown', &
                        RecL = 4*T_Freedom*2)
                    else
                        Open(1002,File =c_File_name_1,Access='Direct', Form = 'Unformatted',status='old', &
                        RecL = 4*T_Freedom*2)
                    endif
                ...
                !************************************************************
                call coocsr(num_FreeD,K_CSR_NNZ,K_COO_aa_try2(1:K_CSR_NNZ), K_COO_ia_try2(1:K_CSR_NNZ), K_COO_ja_try2(1:K_CSR_NNZ),&
                K_CSR_aa(1:K_CSR_NNZ), K_CSR_ja(1:K_CSR_NNZ), K_CSR_ia(1:num_FreeD+1))
                ...
                job = 3
                value2 = 1
                call clncsr(job,value2,num_FreeD,K_CSR_aa(1:K_CSR_NNZ), K_CSR_ja(1:K_CSR_NNZ), K_CSR_ia(1:num_FreeD+1), indu,iwk)

                !**************************************************************************
                ! Step 5: Sort the CSR sparse matrix (in ascending order by column number)
                !**************************************************************************
                ALLOCATE(iwork(2*K_CSR_NNZ))
                call csort(num_FreeD,K_CSR_aa(1:K_CSR_NNZ),K_CSR_ja(1:K_CSR_NNZ), K_CSR_ia(1:num_FreeD+1),iwork,values)
