!-----------------------------------------------------------
! Brief: Pre-check crack coords for XFEM enrichment.
!
! Parameters:
!   Input:  ifra - fracture step index
!           isub - current substep index
!
! Notes:   Prevents an element edge from being intersected
!          twice by the same crack segment; corrects crack
!          geometry before enriched-node detection.
!-----------------------------------------------------------

subroutine Determine_Enriched_Nodes_Pre_Check(ifra,isub)
!     (1) Correct the crack coordinates to prevent certain difficult-to-handle situations.
!     The main situation that needs to be prevented is: preventing a single element edge from having two intersections with a crack segment.
!     Corresponding notes can be found in: V5-P49
!     (2)...
...
real(kind=FT) point_side_1(2),point_side_2(2), point_seg_1(2),point_seg_2(2), X_NODES_L(5),Y_NODES_L(5)
...
do i_C=1,num_Crack
    ...
    do i_E = 1,Num_Elem
        ...
        do i_S = 1,Each_Cr_Poi_Num(i_C)-1
            ...
            if ((Each_Cr_Poi_Num(i_C)-1 >= 2) .and. (i_S  < (Each_Cr_Poi_Num(i_C)-1))) then
                point_seg2_1=[Crack_Coor(i_C,i_S+1,1), Crack_Coor(i_C,i_S+1,2)]
                point_seg2_2=[Crack_Coor(i_C,i_S+2,1), Crack_Coor(i_C,i_S+2,2)]
            end if
            ...
            do i_Side = 1,4
                ...
                call Tool_Intersection( point_seg_1,point_seg_2, point_side_1,point_side_2, point_A(1),point_A(2),c_Yes_Cross)
                ...
                if(c_Yes_Cross.eqv..True.)then
                    ...
                    if(i_S  < (Each_Cr_Poi_Num(i_C)-1))then
                        call Tool_Intersection( point_seg2_1,point_seg2_2,  &
                        point_side_1,point_side_2, point_B(1),point_B(2),c_Yes_Cross2)
                        ...
                        if(c_Yes_Cross2)then
                            ...
                            call Tool_Shorten_or_Extend_Line( Line_VM,delta_L, 'B',Line_VN,point_N)
                    ...
                    call Tool_Yes_In_Poly (point_seg_1(1),point_seg_1(2), X_NODES_L,Y_NODES_L,5,c_Flag_1)
                    !Check if point_seg_2 is inside the current element.
                    call Tool_Yes_In_Poly (point_seg_2(1),point_seg_2(2), X_NODES_L,Y_NODES_L,5,c_Flag_2)
                    ...
                    if (c_Flag_1 .and. c_Flag_2) then
                        if(Key_Analysis_Type/=6) then
                            print *,"    Error :: A complete crack" // " segment found in one element."
                            print *,"             Message produced in" // " Determine_Enriched_Nodes_Pre_Check."
                            ! Exit program
