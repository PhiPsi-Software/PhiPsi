!-----------------------------------------------------------
! Brief: Smooth the crack front curve for 3D cracks.
!
! Parameters:
!   Input:  ifra - fracture step index
!           iter - iteration index
!
! Notes:   Activated when Key_Smooth_Front is enabled.
!          Operates only on cracks whose propagation is
!          allowed; loop is OpenMP parallelized.
!-----------------------------------------------------------
...
do i_C =1,n_Smoothed_Cracks   
    ...
    !//////////////
    if(Key_Smooth_Front==1)then
        call Tool_Fit_3D_Points_to_Straight_Line(Num_Point_Smooth, Points_to_Smooth(1:Num_Point_Smooth,1:3), &
    ...
    elseif(Key_Smooth_Front==2)then
        call Tool_Fit_3D_Points_to_Circle(Num_Point_Smooth, Points_to_Smooth(1:Num_Point_Smooth,1:3), &
        Points_Smoothed(1:Num_Point_Smooth,1:3),Ave_Radius)
        
        !Save Ave_Radius to *.arad file in the working directory. 2026-07-01.
        block
            character(200) Filename_1
            Filename_1=trim(Full_Pathname)//'.arad'
            if (iter==1) then
                open(101,file=Filename_1,status='replace')
                write(101,'(A)') '#      iter       Ave_Radius'
                write(101,'(I10,1(2X,E20.12))') iter,Ave_Radius
                close(101)
            else
    ...
    elseif(Key_Smooth_Front==3)then
        call Tool_Fit_3D_Points_using_Python( Key_Smooth_Front_Twice, Num_Point_Smooth, &
        Points_to_Smooth(1:Num_Point_Smooth,1:3), Points_Smoothed(1:Num_Point_Smooth,1:3))
    ...
    elseif(Key_Smooth_Front==5)then
        call Tool_Fit_3D_Points_to_Ellipse(Num_Point_Smooth, Points_to_Smooth(1:Num_Point_Smooth,1:3), &
        Points_Smoothed(1:Num_Point_Smooth,1:3))
    ...
    do i_Crack_Ele =1,Crack3D_Meshed_Ele_num(c_C)
        ...
        Crack3D_Meshed_Ele_Attri(c_C)%row(i_Crack_Ele,1)  = Tool_Function_2Point_Dis_3D(Point1,Point2) + &
