...
!     ------------------------------------------------- !
      ...
      SUBROUTINE Location_Element_Stiff_Matrix_Cross(i_E,i_Cross,
     &                                       POS_c_Cross,
     &                                       Location_ESM_C_Crack,
     &                                       num_Loc_ESM_C_Crack,
     &                                       Location_ESM_C_Cr_NoFEM,
     &                                       num_Loc_ESM_C_Cr_NoFEM)
c     The position of the element stiffness matrix in the overall stiffness (corresponding to the intersection point of the cross, excluding the two cracks)
c     Nodes other than the Heaviside-enhanced nodes.
      ...
      if(maxval(Enriched_Node_c_Cross(c_NN)).eq.0 .and.
     &   minval(Enriched_Node_c_Cross(c_NN)).eq.0) then
          if (i_Cross .eq. 1)then
