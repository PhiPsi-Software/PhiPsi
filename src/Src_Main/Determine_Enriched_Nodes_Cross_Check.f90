!-----------------------------------------------------------
! Brief: Detect cross-shaped intersection cracks.
!
! Parameters:
!   Input:  ifra - fracture step index
!           isub - current substep index
!   Output: Yes_Cross_Ele - true if a cross element is found
!
! Notes:   Tests every pair of crack segments for crossing
!          and records the intersection point and crack ids
!          in the cross-point globals.
!-----------------------------------------------------------

subroutine Determine_Enriched_Nodes_Cross_Check(ifra,isub, Yes_Cross_Ele)
!     (1) Check for the presence of cross-shaped intersection cracks and cross cracks
!     (2)...
        ...
        do i_S = 1,Each_Cr_Poi_Num(i_C)-1
            ...
            do j_C =i_C+1,num_Crack
                do j_S = 1,Each_Cr_Poi_Num(j_C)-1
                    ...
                    S4 = [Crack_Coor(j_C,j_S+1,1), Crack_Coor(j_C,j_S+1,2)]
                    ! Determine whether two crack segments intersect
                    call Tool_Intersection( S1,S2,S3,S4, c_Inter_x,c_Inter_y,c_Yes_Cross)
                    ...
                    Yes_T_Crack = .False.
                    if( (abs(c_Inter_x-S1(1))<Tiny_V .and.abs(c_Inter_y-S1(2)) < Tiny_V) .or. (abs(c_Inter_x-S2(1)) < Tiny_V .and. &
                    abs(c_Inter_y-S2(2)) < Tiny_V)   .or. (abs(c_Inter_x-S3(1)) < Tiny_V .and. abs(c_Inter_y-S3(2)) < Tiny_V) .or. &
                    (abs(c_Inter_x-S4(1)) < Tiny_V .and. abs(c_Inter_y-S4(2)) < Tiny_V))then
                    Yes_T_Crack = .True.
                endif
                ! If intersecting
                if(c_Yes_Cross.and.(Yes_T_Crack.eqv..false.))then
                    ...
                    !*****************************************************
                    call Cal_Ele_Num_by_Coors( c_Inter_x,c_Inter_y,c_Elem)
                    !********************
                    ! Save relevant data
                    !********************
                    ...
                    Cross_Point_RABCD(num_Cross,1,1:2) = [c_Inter_x,c_Inter_y]
                    !*************************************************************************
                    ...
                    do i_Side = 1,4
                        ...
                        call Tool_Intersection( S1,S2, point_side_1,point_side_2, c_X,c_Y,c_Yes_Cross_2)
                    ...
                    dis_S1_A = Tool_Function_2Point_Dis( S1,c_Inters(1,1:2))
                    dis_S1_B = Tool_Function_2Point_Dis( S1,c_Inters(2,1:2))
                    if(dis_S1_A <=dis_S1_B) then
                        Cross_Point_RABCD(num_Cross,2,1:2) = c_Inters(1,1:2)
                        Cross_Point_RABCD(num_Cross,3,1:2) = c_Inters(2,1:2)
                    else
                        Cross_Point_RABCD(num_Cross,2,1:2) = c_Inters(2,1:2)
                        Cross_Point_RABCD(num_Cross,3,1:2) = c_Inters(1,1:2)
                    endif
                    ! First, calculate the intersection points of crack i_C with the four boundaries of the element.
                    c_num_Inters = 0
                    c_Inters(1:10,1:2) = ZR
                    do i_Side = 1,4
                        ...
                        point_side_2 = [Coor(n_node_2,1), Coor(n_node_2,2)]
                        !Check if they intersects and calculate intersection point.
                        call Tool_Intersection( S3,S4, point_side_1,point_side_2, c_X,c_Y,c_Yes_Cross_2)
                        if (c_Yes_Cross_2.eqv..True.) then
                            c_num_Inters = c_num_Inters + 1
                            c_Inters(c_num_Inters,1:2)= [c_X,c_Y]
                        end if
                    ...
                    ! Sequential detection and adjustment
                    dis_S3_A = Tool_Function_2Point_Dis( S3,c_Inters(1,1:2))
                    dis_S3_B = Tool_Function_2Point_Dis( S3,c_Inters(2,1:2))
                    if(dis_S3_A <=dis_S3_B) then
                        Cross_Point_RABCD(num_Cross,4,1:2) = c_Inters(1,1:2)
                        Cross_Point_RABCD(num_Cross,5,1:2) = c_Inters(2,1:2)
                    else
                        Cross_Point_RABCD(num_Cross,4,1:2) = c_Inters(2,1:2)
                        Cross_Point_RABCD(num_Cross,5,1:2) = c_Inters(1,1:2)
                    endif
                endif
            enddo
        end do
...
if (Yes_Cross_Ele) then
    print *, '    Error :: crossing cracks are not allowed ' // 'in 2D PhiPsi!'
