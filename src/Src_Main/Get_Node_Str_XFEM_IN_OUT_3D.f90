!-----------------------------------------------------------
! Brief: Compute 3D XFEM stress or strain at every node.
!
! Parameters:
...
!   Output: Str_xx_N .. Str_vm_N - per-node stress or strain arrays
!
! Notes:   Uses OpenMP and stores per-element nodal values first to
!          avoid write conflicts, then averages into the global nodal arrays.
!-----------------------------------------------------------
...
Str_xy_N(num_Node),Str_yz_N(num_Node),Str_xz_N(num_Node), Str_vm_N(num_Node)
...
Ele_Str_xy_N(Num_Elem,8),Ele_Str_yz_N(Num_Elem,8),Ele_Str_xz_N(Num_Elem,8), Ele_Str_vm_N(Num_Elem,8)
integer c_Elem
...
do i_E = 1,num_FEM_Elem
    ...
    do i_N = 1,8
        ...
        elseif  (Yes_Enrich_Node .eqv. .True.) then
            ...
            c_kesi,c_yita,c_zeta,1,DISP, c_Str(1),c_Str(2),c_Str(3), c_Str(4),c_Str(5),c_Str(6))
        endif
        ...
        if(Key_InSitu_Strategy==4 )then
            c_TStr = MATMUL(c_D,[ InSitu_Strain_Gaus_xx(c_Elem,1), InSitu_Strain_Gaus_yy(c_Elem,1), &
