!-----------------------------------------------------------
! Brief: Determine XFEM enriched nodes for 2D problems.
!
! Parameters:
!   Input:  ifra - fracture step index
!           isub - current substep index
!
! Notes:   Detects element crack-segments, classifies nodes
!          by tip / Heaviside / junction enrichment, and
!          handles holes, inclusions and cross-intersections.
!-----------------------------------------------------------

subroutine Determine_Enriched_Nodes(ifra,isub)
!     This subroutine is used to determine the enhancement nodes.
...
real(kind=FT) crack_p1(2),crack_p2(2), Outline_p1(2),Outline_p2(2)
integer EndByEnd_Outline(size(Outline,1)+1) 
real(kind=FT) c_X,c_Y,c_Line_AB(2,2),shorted_Line_AB(2,2), new_Point(2)
logical c_Yes_Cross,Flag_1,Flag_2
integer N1,N2,N3,N4,NN(4),NN_L(5),n_node_1,n_node_2,Num_Intersect
real(kind=FT) point_side_1(2),point_side_2(2), X_el_inter,Y_el_inter,crack_inter(5,2), point_seg_1(2),point_seg_2(2), &
X_NODES_L(5),Y_NODES_L(5)
integer :: point_seg_inElemnt_Flag(Num_Elem)
real(kind=FT) point_seg_inElemnt(Num_Elem,2)
integer i_inter,i_Segment,Crack_Seg_Num(5), Uniqued_Cr_Seg_Num(5),Uniqued_Num_Cr_Seg
...
integer DOMAIN_Outline(20,2),m_DOMAIN_Outline, Domain_El(10),n_Domain_El
real(kind=FT) tem_01,AB(2,2),CD(2,2),tem_02(2),tem_03(2), tem_point_seg_1(2),tem_point_seg_2(2)
...
!     ---------------------------------------------------------------------------------------
!     Initialize the matrix elements to 0. Note:
!     Some variables are only initialized in the first rupture step, while others need to be
!     initialized
!     in any rupture step.
!     ---------------------------------------------------------------------------------------
...
if(Key_Analysis_Type==6)then
    if(allocated(Coors_Element_Crack_OLD)) then
        Coors_Element_Crack_OLD = ZR
        Coors_Element_Crack_OLD = Coors_Element_Crack
    endif
endif
...
1101 format(5X,'Enrichment ratio is ',F8.4,'%')     
...
!     -----------------------------------------------------------------------------------------------
!     ---------------------         Step 0: Conduct Preliminary Check          ---------------------
!     -----------------------------------------------------------------------------------------------
...
if (i_S.eq.1) then
    ...
    Cr_First_Tip_Ori(i_C)=atan2(crack_p1(2)-crack_p2(2), crack_p1(1)-crack_p2(1))
end if
if(i_S.eq.Each_Cr_Poi_Num(i_C)-1) then
    ...
    ! Save to the global variable of dip angle for the fracture segment where the crack tip is located
    Cr_Second_Tip_Ori(i_C)=atan2(crack_p2(2)-crack_p1(2), crack_p2(1)-crack_p1(1))
end if
!***************************************************************************
...
!********************************************************************
!********************************************************************
Call Tool_Yes_In_Poly (crack_p1(1),crack_p1(2), Coor(EndByEnd_Outline,1),Coor(EndByEnd_Outline,2), &
size(EndByEnd_Outline),Yes_in_Outline1)
Call Tool_Yes_In_Poly (crack_p2(1),crack_p2(2), Coor(EndByEnd_Outline,1),Coor(EndByEnd_Outline,2), &
...
do i_O = 1,size(Outline,1)
    ...
    call Tool_Intersection(crack_p1,crack_p2, Outline_p1,Outline_p2, c_X,c_Y,c_Yes_Cross)
    !**********************************************************
    ...
    if(c_Yes_Cross.eqv..true.) then
        ...
        !##########################
        if ((i_S .eq. 1).and. (Yes_in_Outline1.eqv..False.))then
            ...
            call Tool_Shorten_or_Extend_Line (c_Line_AB,tem_01,'A', shorted_Line_AB,new_Point)
            ...
            do i_Side = 1,4
                ...
                call Tool_Intersection( shorted_Line_AB(1,:),shorted_Line_AB(2,:), point_side_1,point_side_2, c_X,c_Y,c_Yes_Cross)
                if (c_Yes_Cross.eqv..True.) then
                    X_el_inter = c_X
                    Y_el_inter = c_Y
                    exit
            ...
            Coors_Element_Crack(c_Edge_Elem,i_C,1:4) = [c_X,c_Y,X_el_inter,Y_el_inter]
            !#########################
            ! The last crack fragment
            !#########################
        elseif ((i_S .eq. Each_Cr_Poi_Num(i_C)-1).and. (Yes_in_Outline2.eqv..False.))then 
            ...
            call Tool_Shorten_or_Extend_Line (c_Line_AB,tem_01,'B', shorted_Line_AB,new_Point)
            ! Cracks for post-processing
            Edge_Disposed_Crack(i_C, Each_Cr_Poi_Num(i_C),:)=new_Point
            ! Calculate Coors_Element_Crack
            !Get the intersection point of the shorted first crack segment 
            !'shorted_Line_AB' and the 4 element sides.
            N1  = Elem_Node(c_Edge_Elem,1)                                         
...
point_seg_1=[Crack_Coor(i_C,i_S,1), Crack_Coor(i_C,i_S,2)]
point_seg_2=[Crack_Coor(i_C,i_S+1,1), Crack_Coor(i_C,i_S+1,2)]
...
do i_Side = 1,4
    ...
    if(c_Yes_Arc_Crack .eqv. .False.) then
        call Tool_Intersection( point_seg_1,point_seg_2, point_side_1,point_side_2, c_X,c_Y,c_Yes_Cross)
    elseif(c_Yes_Arc_Crack .eqv. .True.) then
        call Tool_Intersection_Line_and_Arc( point_side_1,point_side_2, &
        Arc_Crack_Coor(i_C,i_S,1:11), c_num_Inter_Line_Arc, c_Inter_Line_Arc, &
        c_Yes_Cross)
    ...
    if(c_Yes_Cross.eqv..True.)then
        ...
        elseif(c_Yes_Arc_Crack .eqv. .True.) then 
            crack_inter(Num_Intersect,1:2) = c_Inter_Line_Arc(1,1:2)
        endif
        !Check if point_seg_1 is inside the current element.
        call Tool_Yes_In_Poly (point_seg_1(1),point_seg_1(2), X_NODES_L,Y_NODES_L,5,Flag_1)
        !Check if point_seg_2 is inside the current element.
        call Tool_Yes_In_Poly (point_seg_2(1),point_seg_2(2), X_NODES_L,Y_NODES_L,5,Flag_2)
        ...
        point_seg_inElemnt(i_E,:) = point_seg_1*int_Flag_1 + point_seg_2*int_Flag_2
        if (Flag_1.eqv..True.) then
...
!###################################################
if((Num_Intersect.eq.2).and. (Flag_1.eqv..False.).and. (Flag_2.eqv..False.))then
    Elem_Type(i_E,i_C) = 2
    ...
    do i_inter=1,2
        ...
        do i_Segment = 1,Each_Cr_Poi_Num(i_C)-1
            ...
            ! Is the current crack segment an arc segment (2017-07-16)
            c_Yes_Arc_Crack = .False.
            if(sum(abs(Arc_Crack_Coor( i_C,i_Segment,1:11)))> ZR)then
                c_Yes_Arc_Crack = .True.
            endif
            if(c_Yes_Arc_Crack .eqv. .False.) then
                call Tool_Yes_On_Line(x_inter,y_inter, point_seg_1,point_seg_2,Yes_on_Segment)
            elseif(c_Yes_Arc_Crack)then
                call Tool_Yes_On_Arc(x_inter,y_inter, Arc_Crack_Coor(i_C,i_Segment,1:11), Yes_on_Segment)
            endif
    ...
    call Vector_Unique_Int(5,tem_i_1,Crack_Seg_Num, Uniqued_Cr_Seg_Num,Uniqued_Num_Cr_Seg)
    ...
    if(dot_product(tem_02,tem_03) .ge. ZR) then
        do iii = 1,num_Crack
            Coors_Element_Crack(i_E,iii,1:4)= [crack_inter(1,1),crack_inter(1,2), crack_inter(2,1),crack_inter(2,2)]
        enddo
    else
        !Change the order of "crack_inter" to make sure that the order are the same as the crack.
        do iii = 1,num_Crack
            Coors_Element_Crack(i_E,iii,1:4)= [crack_inter(2,1),crack_inter(2,2), crack_inter(1,1),crack_inter(1,2)]
        enddo
    end if
end if
!
...
if(Num_Intersect .eq. 1)then 
    ...
    do j_C=1,num_Crack
        if(j_C.ne.i_C) then
            ...
            do j_S = 1,Each_Cr_Poi_Num(j_C)-1
                ...
                call Tool_Yes_On_Line (Coors_Tip(i_E,1),Coors_Tip(i_E,2), tem_point_seg_1,tem_point_seg_2, Yes_On_Line)
                !&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
                ! If yes, then: Junction enhancement element
                !&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
                if(Yes_On_Line)then
                    Elem_Type(i_E,i_C) = 4
                    Coors_Junction(i_E,i_C,1:2)  = crack_inter(1,:)
                    Coors_Junction(i_E,i_C,3:4)  = [Coors_Tip(i_E,1),Coors_Tip(i_E,2)]
                    Jun_Ele_Negative_Cr_Num(i_E,i_C)=j_C
                    !Determine the type of the crack tip.
                    if (point_seg_inElemnt_Flag(i_E).eq.1) then
                        Crack_Tip_Type(i_C,1) = 1
                        Crack_Jun_CrNum(i_C,1)= j_C
                        Crack_Jun_Elem(i_C,1) = i_E
                    elseif ( point_seg_inElemnt_Flag(i_E).eq.2) then
    ...
    ! Tip splitting unit.
    !&&&&&&&&&&&&&&&
    if ((Elem_Type(i_E,i_C) .ne. 4) .and. (Key_TipEnrich .ne. 0)) then
        Elem_Type(i_E,i_C) = 1
        do iii = 1,num_Crack
            Coors_Element_Crack(i_E,iii,1:2)= crack_inter(1,1:2)
        enddo
        cc_tip_x = Coors_Tip(i_E,1);
        cc_tip_y = Coors_Tip(i_E,2);
        Coors_Element_Crack(i_E,i_C,3:4)= [cc_tip_x,cc_tip_y]
...
if (Yes_Arc_Crack)then 
    ...
    do i_P = 1,num_points_crack
        ...
        if(i_P==1) then
            ...
            call Cal_Ele_Num_by_Coors(crack_p1(1),crack_p1(2), c_OUT_Elem)
            crack_inter(1,1) = Coors_Element_Crack(c_OUT_Elem,i_C,1)
            crack_inter(1,2) = Coors_Element_Crack(c_OUT_Elem,i_C,2)
            ...
            if(sum(abs(Arc_Crack_Coor(i_C,1,1:11)))>Tol_11) then
                ...
                do i_El_Check=1,num_Ele_Eles(c_OUT_Elem)
                    ...
                    if(c_Check_El/=c_OUT_Elem)then
                        ...
                        c_Point_1 = [Coors_Element_Crack(c_Check_El,i_C,1), Coors_Element_Crack(c_Check_El,i_C,2)]
                        c_Point_2 = [Coors_Element_Crack(c_Check_El,i_C,3), Coors_Element_Crack(c_Check_El,i_C,4)]
                        call Tool_Yes_On_Arc( c_Point_1(1),c_Point_1(2), Arc_Crack_Coor(i_C,1,1:11),Yes_P1_ON)
                        call Tool_Yes_On_Arc( c_Point_2(1),c_Point_2(2), Arc_Crack_Coor(i_C,1,1:11),Yes_P2_ON)
                        if(Yes_P1_ON .and. Yes_P2_ON)then 
                            ...
                            ! with point B.
                            call Tool_Yes_Point_Equal( crack_inter(1,1:2),c_Point_1, Yes_P_Equal_1)
                            call Tool_Yes_Point_Equal( crack_inter(1,1:2),c_Point_2, Yes_P_Equal_2)
                            if(Yes_P_Equal_1)then
                                ! Coordinates of point C
                                Crack_Arc_Tip_A_B_C_x(i_C,1,3) = c_Point_2(1)
                                Crack_Arc_Tip_A_B_C_y(i_C,1,3) = c_Point_2(2)
                            endif
                            if(Yes_P_Equal_2)then
                                ! Coordinates of point C
                                Crack_Arc_Tip_A_B_C_x(i_C,1,3) = c_Point_1(1)
                                Crack_Arc_Tip_A_B_C_y(i_C,1,3) = c_Point_1(2)
                            endif
                            exit
                        endif
                    endif
        ...
        elseif(i_P==num_points_crack) then
            ...
            crack_inter(1,2) = Coors_Element_Crack(c_OUT_Elem,i_C,2)
            ! Coordinates of point B
            Crack_Arc_Tip_A_B_C_x(i_C,2,2) = crack_inter(1,1)
            Crack_Arc_Tip_A_B_C_y(i_C,2,2) = crack_inter(1,2)
            !||||||||||||||||||||||||||||||||||
            ! Next, the coordinates of point C
            !||||||||||||||||||||||||||||||||||
            ! Determine whether the current crack segment is curved; if not, there is no need to calculate it.
            if(sum(abs(Arc_Crack_Coor(i_C, num_points_crack-1,1:11)))>Tol_11)then
                ! First, find the cell number of the adjacent cell through which the curved crack passes.
                do i_El_Check=1,num_Ele_Eles(c_OUT_Elem)
                    c_Check_El=Ele_Elements(c_OUT_Elem,i_El_Check)
                    ! It cannot be the current cell itself
                    if(c_Check_El/=c_OUT_Elem)then
                        ...
                        c_Point_1 = [Coors_Element_Crack(c_Check_El,i_C,1), Coors_Element_Crack(c_Check_El,i_C,2)]
                        c_Point_2 = [Coors_Element_Crack(c_Check_El,i_C,3), Coors_Element_Crack(c_Check_El,i_C,4)]
                        call Tool_Yes_On_Arc( c_Point_1(1),c_Point_1(2), Arc_Crack_Coor(i_C,num_points_crack-1, 1:11),Yes_P1_ON)
                        call Tool_Yes_On_Arc( c_Point_2(1),c_Point_2(2), Arc_Crack_Coor(i_C,num_points_crack-1, 1:11),Yes_P2_ON)
                        if(Yes_P1_ON .and. Yes_P2_ON)then 
                            ...
                            if(Yes_P_Equal_1)then
                                ! Coordinates of point C
                                Crack_Arc_Tip_A_B_C_x(i_C,2,3) = c_Point_2(1)
                                Crack_Arc_Tip_A_B_C_y(i_C,2,3) = c_Point_2(2)
                            endif
                            if(Yes_P_Equal_2)then
                                ! Coordinates of point C
                                Crack_Arc_Tip_A_B_C_x(i_C,2,3) = c_Point_1(1)
                                Crack_Arc_Tip_A_B_C_y(i_C,2,3) = c_Point_1(2)
                            endif
                            exit
                        endif
                    endif
...
do i_H=1,num_Circ_Hole
    ...
    do i_Tip = 1,2
        ...
        c_Dis = Tool_Function_2Point_Dis(Coor_Tip, [x0_Hole,y0_Hole])
        if(abs(c_Dis - R0_Hole) <= Tol_10) then
            Crack_Tip_Type(i_C,i_Tip) = 1
            Crack_Jun_HoleNum(i_C,i_Tip)= i_H
            call Cal_Ele_Num_by_Coors(Coor_Tip(1),Coor_Tip(2), c_Jun_Ele)
            ...
            !..........................
            if(i_Tip==1)then
                crack_p1 = [Crack_Coor(i_C,1,1), Crack_Coor(i_C,1,2)]
                crack_p2 = [Crack_Coor(i_C,2,1), Crack_Coor(i_C,2,2)]
            elseif(i_Tip==2)then
                crack_p1 = [Crack_Coor(i_C,Each_Cr_Poi_Num(i_C),1), Crack_Coor(i_C,Each_Cr_Poi_Num(i_C),2)]
                crack_p2 = [Crack_Coor(i_C,Each_Cr_Poi_Num(i_C)-1,1), Crack_Coor(i_C,Each_Cr_Poi_Num(i_C)-1,2)]
            endif
            ! Loop through the four edges of the current element
            ...
            do i_Side = 1,4
                ...
                point_side_2 =[Coor(n_node_2,1), Coor(n_node_2,2)]
                !Check if they intersects and calculate intersection point.
                call Tool_Intersection( crack_p1,crack_p2, point_side_1,point_side_2, c_X,c_Y,c_Yes_Cross)
                if(c_Yes_Cross.eqv..True.)then
                    Coors_Junction(c_Jun_Ele,i_C,1:2) = [c_X,c_Y]
                    Coors_Junction(c_Jun_Ele,i_C,3:4) = Coor_Tip
                    exit
                endif
            enddo
            !...............................
            ...
            do i_Side = 1,4
                ...
                point_side_2 =[Coor(n_node_2,1), Coor(n_node_2,2)]
                !Check if they intersects and calculate intersection point.
                call Tool_Intersection_Line_and_Circle(x0_Hole,y0_Hole,R0_Hole,point_side_1,point_side_2,c_num_Inter_CH,c_State, &
                c_Inter)
                if (c_State==2 .and. c_num_Inter_CH ==1) then
                    c_num_Inter = c_num_Inter +1
                    Coors_Element_Crack( c_Jun_Ele,i_C,c_num_Inter*2-1) = c_Inter(1,1)
                    Coors_Element_Crack( c_Jun_Ele,i_C,c_num_Inter*2)   = c_Inter(1,2)
                endif
            enddo
        endif
    enddo
...
do i_H=1,num_Ellip_Hole
    ...
    do i_Tip = 1,2
        ...
        call Tool_Yes_Point_in_Oblique_Ellipse(Coor_Tip, x0_Hole,y0_Hole,a_Hole,b_Hole,theta_Hole, &
        Return_Statu,Tol_10)
...
!     -----------------------------------------------------------------------------------
!     ---------------------   Step 2: determine the enriched nodes  --------------------
!     -----------------------------------------------------------------------------------
loop1: do i_C=1,num_Crack
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ! Cycle between each element
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    loop2: do i_E = 1,Num_Elem
        N1  = Elem_Node(i_E,1)                              
        N2  = Elem_Node(i_E,2)                          
        N3  = Elem_Node(i_E,3)                          
        N4  = Elem_Node(i_E,4)  
        ...
        elseif (Elem_Type(i_E,i_C) .eq. 2) then
            ...
            do i_N = 1,4
                ...
                if (Enriched_Node_Type(NN(i_N),i_C).eq. 0) then
                    ...
                    if (Key_Analysis_Type/=6) then
                        call Cal_Support_Domain_of_Node(NN(i_N), DOMAIN_Outline, m_DOMAIN_Outline, Domain_El, n_Domain_El)
                        !Get the A+ and A- Domain.
                        call Cal_Ap_and_Am(Crack_Coor(i_C,:,:), Each_Cr_Poi_Num(i_C), DOMAIN_Outline(1:m_DOMAIN_Outline,:), &
                        m_DOMAIN_Outline, &
                        Domain_El(1:n_Domain_El),n_Domain_El, i_E,NN(i_N), Yes_Cutthrough, Yes_Ap_has_GP,Yes_Am_has_GP)
                        !------------------------------------------------------------                        
                        ...
                        !For explicit dynamic analysis, do not check Ap and Am. 2026-06-22.     
                    else
                        Enriched_Node_Type(NN(i_N),i_C) = 2
                    endif
                end if
        ...
        elseif (Elem_Type(i_E,i_C) .eq. 4) then
            ...
            loop3: do i_Su_Ele = 1,num_Ele_Eles(i_E)
                ...
                do i_Chk_Node = 1,4
                    ...
                    !Get the support domain of the current node.
                    call Cal_Support_Domain_of_Node(c_Chk_N, DOMAIN_Outline,m_DOMAIN_Outline, Domain_El,n_Domain_El)
                    ! Convert support domain boundaries (node numbers) into a closed polygon
                    Clo_Supp_Domain(1:20,1:2) = ZR
                    Clo_Supp_Domain(1:m_DOMAIN_Outline,1:2) = Coor(DOMAIN_Outline(1:m_DOMAIN_Outline,1), 1:2)
                    Clo_Supp_Domain(m_DOMAIN_Outline+1,1:2) = Coor(DOMAIN_Outline(1,1),1:2)
                    ...
                    call Tool_Yes_Crack_Polygon_Intersects( Positive_Cr, &
                    Clo_Supp_Domain(1:m_DOMAIN_Outline+1,1:2) ,m_DOMAIN_Outline+1,Yes_Posit_Cross)
                    ! Check if the passive cracks intersect with the support zone_Yes_Negat_Cross
                    call Tool_Yes_Crack_Polygon_Intersects( Negative_Cr, &
                    Clo_Supp_Domain(1:m_DOMAIN_Outline+1,1:2) ,m_DOMAIN_Outline+1,Yes_Negti_Cross)
        ...
        elseif (Elem_Type(i_E,i_C) .eq. 6) then
            ...
            call Tool_Yes_Cricle_Polygon_Intersects( x0_Hole,y0_Hole,R0_Hole, &
            Clo_Supp_Domain(1:m_DOMAIN_Outline+1,1:2) ,m_DOMAIN_Outline+1,Yes_Hole_Cross)
            ...
            if(Yes_Posit_Cross .and. Yes_Hole_Cross)then
                ...
                Node_Jun_elem(c_Chk_N,i_C) = i_E
                Node_Jun_Hole(c_Chk_N,i_C) = Ele_Jun_Hole(i_E,i_C)
            endif
            ! Mark this node as detected
            tem_Checked_Nodes(c_Chk_N) = 1
...
!     -------------------------------------------------------------------------------------------
!     ----------------------------   Step 3: Check Enhanced Node Type  -------------------------
!     -------------------------------------------------------------------------------------------
            ...
            do j_C=1,num_Crack
                if(j_C /= i_C)then
                    if(Enriched_Node_Type(i_N,j_C)==1) then
                        print *, '    WARNING :: Junction enrichment and' //' tip enrichment should not coexist for a node!'
...
!     ------------------------------------------------------------------------------------------
!     ------------------------ Step 4.1: Identify circular hole reinforcement elements and
!     reinforcement nodes -------------
!     ------------------------ Circular holes and elliptical holes do not coexist -------------
!     ------------------------------------------------------------------------------------------
...
!     ------------------------------------------------------------------------------------------
!     ------------------------ Step 4.2: Identify Hole Reinforcement Elements and Reinforcement
!     Nodes
!     ------------------------------------------------------------------------------------------
!     ------------------------ Circular holes and elliptical holes do not coexist -------------
!     ------------------------             2020-08-09                      -------------
...
!     -----------------------------------------------------------------------------------------
!     Step 5: Determine the reinforcement elements and reinforcement nodes at the cross-shaped
!     intersection
!     -----------------------------------------------------------------------------------------
if (num_Cross /= 0) then   
    do i_Cross = 1,num_Cross
        ...
        Node_Cross_elem(Elem_Node(c_Cross_Ele,1:4) ,i_Cross) = c_Cross_Ele
        !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
!     -----------------------------------------------------------------------------------------
! ---------------------------- Step 6: Determine the Circular Inclusion Reinforcement Unit and
! Reinforcement Node -----------------
!     -----------------------------------------------------------------------------------------
! Execute only on the first rupture step
...
!     ---------------------------------------------------------------------------------
!     Step 7: Determine the polygon inclusion enhancement element and enhancement node
!     ---------------------------------------------------------------------------------
...
do i_Incl = 1,num_Poly_Incl
    ...
    ! Closed-form polygon inclusion
    Poly_Incl_Coor_x_Cl(i_Incl,1:n_Vertex)= Poly_Incl_Coor_x(i_Incl,1:n_Vertex)
    Poly_Incl_Coor_x_Cl(i_Incl,n_Vertex+1)= Poly_Incl_Coor_x(i_Incl,1)
    Poly_Incl_Coor_y_Cl(i_Incl,1:n_Vertex)= Poly_Incl_Coor_y(i_Incl,1:n_Vertex)
    Poly_Incl_Coor_y_Cl(i_Incl,n_Vertex+1)= Poly_Incl_Coor_y(i_Incl,1)
enddo
! Execute only on the first rupture step
if (num_Poly_Incl /= 0 .and. ifra==1) then   
    ...
    do i_E = 1,Num_Elem
        ...
        do i_Incl = 1,num_Poly_Incl
            ...
            Nodes_4_in(1:4) = .False.
            do i_N = 1,4
                call Tool_Yes_In_Poly( Coor(NN(i_N),1),Coor(NN(i_N),2), Poly_Incl_Coor_x_Cl(i_Incl,1:n_Vertex+1), &
                Poly_Incl_Coor_y_Cl(i_Incl,1:n_Vertex+1), n_Vertex+1, Nodes_4_in(i_N))
            enddo
            num_in_Nodes = count(Nodes_4_in(1:4))
            if(num_in_Nodes == 4)then
                !no nothing!
...
!     -------------------------------------------------------------
!     Step 9: Determine the average cell area of the enhanced unit   
!     -------------------------------------------------------------
! Cycle between elements
Area_Enrich = ZR
iEl_contour = 0
do i_E = 1,Num_Elem
    if (maxval(Elem_Type(i_E,1:num_crack)) /=0 .or. minval(Elem_Type(i_E,1:num_crack)) /=0) then
...
!     ---------------------------------------------------------------------------------
!      Step 10: If using the multi-tip enrichment node algorithm Key_Multi_TipEnrNode=1
!     It is necessary to re-determine the crack tip enhancement elements and nodes            
!     ---------------------------------------------------------------------------------
if (Key_TipEnrich /= 0 .and. Key_Multi_TipEnrNode==1 .and. num_Crack>=1) then
    do i_C=1,num_Crack
        do i_Tip = 1,2
            ...
            if (Crack_Tip_Type(i_C,i_Tip) ==0) then
                ...
                do i_Node =1,Num_Node
                    ...
                    c_dis =Tool_Function_2Point_Dis([c_tip_x,c_tip_y], Coor(i_Node,1:2))
                    ...
                    if(c_dis<=check_L)then
                        ...
                        if(Enriched_Node_Type(i_Node,i_C)==0)then
                            ...
                            ! element number of the crack tip
                            call Cal_Ele_Num_by_Coors(c_tip_x,c_tip_y, c_ELe_Num)
                            ! The element number corresponding to the crack tip at the crack-tip enhancement node
                            Ele_Num_Tip_Enriched_Node(i_C,i_Node)= c_ELe_Num
                        endif
                    endif
                enddo
            endif
...
!     Elem_Type(531:541,1)=1
!     Elem_Type(583:592,1)=1
!     Elem_Type(481:490,1)=1

!     ----------------------------------------------------------------------
!      The adjacent elements of each crack tip (those crossed by the crack)
!      TipEle_Adjacent_Ele(num_crack,2)
!     ----------------------------------------------------------------------
do i_E = 1,Num_Elem
    do i_C=1,num_Crack
        if (Elem_Type(i_E,i_C)==1) then
            ...
            do i_N=1,4
                ...
                do j_E =1,num_Node_Elements(c_N)
                    ...
                    if(Elem_Type(c_Adj_Ele,i_C)==2 .or. Elem_Type(c_Adj_Ele,i_C)==3 .or. Elem_Type(c_Adj_Ele,i_C)==4 .or. &
...
! fracture.
!***************************************************************************************
if (Key_Symm_HF==0 .and.(Key_Analysis_Type==3 .or. Key_Analysis_Type==4 .or. Key_Analysis_Type==5))then
    if((Crack_Tip_Type(Inject_Crack_Num,1)==-2) .or. &
    (Crack_Tip_Type(Inject_Crack_Num,2)==-2))    then
    print *, '    Error :: injection crack cannot be an edge' //' crack for the full HF model!'
    call Warning_Message('S',Keywords_Blank) 
endif
endif
...
!********************************************************************************************
if (Key_Symm_HF==1 .and.(Key_Analysis_Type==3 .or. Key_Analysis_Type==4 .or. Key_Analysis_Type==5))then
    if(Crack_Tip_Type(Inject_Crack_Num,1)/=-2)    then
        print *, '    Error :: injection crack must be an edge' //' crack for the symmetric HF model!'
        call Warning_Message('S',Keywords_Blank) 
    endif
endif

!Enriched_Node_Type(:,1) = 2

!2026-05-24.
!if(Set_All_Nodes_as_Both_Heaviside_and_Tip==1) then
!    Enriched_Node_Type(:,:)  = 11
!endif
...
do i_E = 1,Num_Elem
    c_NN    = G_NN(:,i_E)
    if(sum(abs(Enriched_Node_Type(c_NN,1:num_Crack))).gt.0)then               
...
if(Temp_Show_Message)then
    ...
    write(*,1101) dble(num_XFEM_Elem)/dble(num_Elem)*dble(100)   
endif
