!-----------------------------------------------------------
! Brief: Assemble the full 2D FEM global stiffness matrix
!        for 4-node quadrilateral elements.
!
! Parameters:
!   Input:  isub          - current load step index
!           T_Freedom     - total DOF count
!   Output: globalK       - assembled stiffness matrix
!           Total_Num_G_P - total number of Gauss points
!
! Notes:   Supports Weibull random fields, killed elements,
!          damage-induced softening, and contact penalties;
!          parallelized with OpenMP.
!-----------------------------------------------------------

subroutine Assemble_Stiffness_Matrix_FEM(isub,globalK, T_Freedom,Total_Num_G_P)
...
do i_E = 1,Num_Elem
    ...
    do i_row = 1,8
        do i_col = 1,8
            globalK(local(i_row),local(i_col)) = globalK(local(i_row),local(i_col)) + localK(i_row,i_col)
        end do
    end do
end do
!$omp end parallel do
...
!     do i=1,T_Freedom
!         do j=1,T_Freedom
!             if (abs(globalK(i,j))< = 1.0D-8) then
!                  globalK(i,j) = ZR 
!             endif
!         enddo
!     end do
...
if(Num_Boux_nonzero >0)then
    ...
    do i_Boux_nonzero =1,Num_Boux_nonzero
        ...
        globalK(c_node_DOF,c_node_DOF) = globalK(c_node_DOF,c_node_DOF)+penalty_k
    enddo
endif
...
if (num_CP_x_nodes>0)then
    ...
    do i_CP_node =2,num_CP_x_nodes
        ...
        globalK(first_node_DOF,first_node_DOF) = globalK(first_node_DOF,first_node_DOF)+penalty_k
        globalK(c_node_DOF,c_node_DOF) = globalK(c_node_DOF,c_node_DOF)+penalty_k
        globalK(first_node_DOF,c_node_DOF) = globalK(first_node_DOF,c_node_DOF)-penalty_k
        globalK(c_node_DOF,first_node_DOF) = globalK(c_node_DOF,first_node_DOF)-penalty_k
    enddo
endif
