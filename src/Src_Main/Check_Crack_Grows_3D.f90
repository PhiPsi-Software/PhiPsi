!-----------------------------------------------------------
! Brief: Check 3D crack growth using the weighted-average
!        maximum principal stress criterion, updating the
!        discrete crack mesh nodes and elements.
!
! Parameters:
!   Input:  ifra - current fracturing step index
!           iter - current iteration index
!   Output: Yes_Grow - per-3D-crack growth flag
!
! Notes:   Computes weighted-average stress over a search
!          region around each tip, chooses a candidate
!          propagation direction, and meshes the new tip.
!-----------------------------------------------------------
...
real(kind=FT) Dynamic_Time
real(kind=FT) :: DY_Propagation_Time, DY_Propagation_Speed, DY_Propagation_L
logical Falg_DY_Crack_Propagaton_at_Specified_Time_and_Speed
real(kind=FT) temp_vector(3)
...
1062 FORMAT(5X,'-- ATTENTION :: crack ',I4,' will not propagate!')      
...
2001 FORMAT(5X,'-- ATTENTION :: Crack growth at specified time and speed for', F16.6,' mm.')    
...
! Dynamic Analysis. 2026-05-07. NEWFTU-2026050703.
Falg_DY_Crack_Propagaton_at_Specified_Time_and_Speed = .False.
if(Key_Analysis_Type==6) then
    Dynamic_Time = delt_time_Explicit*iter
    ! Allow crack growth at Specified Time and Speed. NEWFTU-2026050703.
    if (sum(EDY_Crack_Propagaton_at_Specified_Time_and_Speed) >=ZR) then
        ...
        if (Dynamic_Time >= DY_Propagation_Time) then
            Falg_DY_Crack_Propagaton_at_Specified_Time_and_Speed = .True.
            DY_Propagation_L = DY_Propagation_Speed*Delt_Time_Explicit
            delta_L = DY_Propagation_L
            if (Temp_Show_Message) then
                write(*,2001) DY_Propagation_L*1.0D3
            endif
        endif
    endif
endif
...
do i_C =1,num_Crack 
    ...
    !/                                                                        /
    !/                                                                        /
    !/ CFCP=1, Maximum circumferential tensile stress criterion.              /
    !/         2026-07-04. NEWFTU-2026070401.                                 /
    !/                                                                        /
    !/                                                                        /
    !//////////////////////////////////////////////////////////////////////////
    case(1)
        !delta_angle(i_Tip) = TWO*atan(-2*KII(i_C,i_Tip)/tem_value_k_I/ (ONE+sqrt(ONE+EIG*(KII(i_C,i_Tip)/tem_value_k_I)**2))) !BUGFIX-2026062503.
        ...
        do i_Out_Node = 1,Num_CrMesh_Outlines
            ...
            if(Key_NaCr_Active_Scheme_3D == 3)then
                ...
                if (Crack_Type_Status_3D(i_C,6) >=1) then
                    ...
                    call Vector_Location_Int(Num_List_Elements, Na_Crack3D_Ele_List(Natural_Crack)%row(1:Num_List_Elements), &
                    in_Ele_Num, c_Ele_location,c_Yes_Ele_In)
            ...
            !.......................................................................
            ! Find c_Theta such that the circumferential stress sigma_theta_theta
            ! is maximized (Maximum Circumferential Tensile Stress criterion, MTS).
            ! Erdogan & Sih (1963). KIII is not considered.
            !.......................................................................
            do i_Check_Theta =1,num_of_check+1
                ...
                ! MTS criterion: maximize circumferential stress sigma_theta_theta only.   !MTS-CHANGE
                ! KIII is not considered (Tao_Theta term removed).                         !MTS-CHANGE
                ! Stress_Theta = sigma_theta_theta = cos(theta/2)/sqrt(2*pi*r)             !MTS-CHANGE
                !                * [KI*cos^2(theta/2) - (3/2)*KII*sin(theta)]              !MTS-CHANGE
                Spe_Pri_Stress(i_Check_Theta) = Stress_Theta
            enddo 
            ...
            !2D MTS analytical solution (equivalent to MTS, without KIII). 2026-02-06.
            !Theta_All(i_Out_Node) = TWO*atan((c_KI_3D - sqrt(c_KI_3D**2 + EIG*c_KII_3D**2)) / (FOU*c_KII_3D))
            !c_Theta = Theta_All(i_Out_Node)
            ...
            ! MTS criterion: KI_eq = cos(theta/2) * [KI*cos^2(theta/2) - (3/2)*KII*sin(theta)]
            ! Reference: sigma_theta_theta_max * sqrt(2*pi*r) = KI_eq
            !........................................
            cc_Part1 = c_KI_3D*(cos(c_Theta/TWO))**2
            cc_Part2 = THR/TWO*c_KII_3D*sin(c_Theta)
            ! MTS criterion: KI_eq without KIII term.                                      !MTS-CHANGE
            KI_eq(i_Out_Node) = cos(c_Theta/TWO)*(cc_Part1 - cc_Part2)
        ...
        !NEWFTU-2026050703.
        if(Falg_DY_Crack_Propagaton_at_Specified_Time_and_Speed) then
            Flag_Propagation_this_Crack = .True.
        endif
        if(Key_Analysis_Type==6) then
            if (sum(EDY_Crack_Propagaton_at_Specified_Time_and_Speed) >=ZR) then
        ...
        do i_Out_Node =1,Num_CrMesh_Outlines
            ...
            if(sum(abs(c_poss_Vector(i_Out_Node,1:3)))<=Tol_10)then
                if (Boundary_Cracks(i_C) .and. Key_Analysis_Type==6) then
                    !do nothing
                elseif (Boundary_Cracks(i_C) .and. Key_Save_Old_Solid_El_Tip_BaseLine==1) then
                    !do nothing
                else
                    print *,'    Error-2026051101 :: c_poss_Vector!'
    ...
    case(2)
        ...
        do i_Out_Node = 1,Num_CrMesh_Outlines
            ...
            Line_AB(2,1:3) = [c_x_old+5.0D0*Ave_Elem_L*c_Vec_x(1), c_y_old+5.0D0*Ave_Elem_L*c_Vec_x(2), &
            ...
            if (in_Ele_Num_new /=0) then
                ...
                elseif(Key_Ave_Stress == 3)  then
                    ...
                    cc_kesi,cc_yita,cc_zeta,i_G,DISP, Ave_Sxx_GP,Ave_Syy_GP,Ave_Szz_GP,Ave_Sxy_GP,Ave_Syz_GP,Ave_Sxz_GP)
        ...
        if (Boundary_Cracks(i_C) .eqv. .True.) logical_close = .False.
            ...
            if(mod(i_Out_Node,1)==0)then
                write(*,1035) i_C, maxval(S1_all,mask=(S1_all>-Con_Big_15))/Cn_M, minval(S1_all,mask=(S1_all>-Con_Big_15))/Cn_M, &
        ...
        case(3)        
            ...
            ! finite element Methods for three-dimensional crack growth simulations, Equation 3.12.
            !.......................................................................................
            if (Key_Schollmann_Twist == 1)then
                ...
                Global_kw(Num_CrMesh_Outlines,Num_CrMesh_Outlines) = &
                Global_kw(Num_CrMesh_Outlines,Num_CrMesh_Outlines)+ Penalty_Value          
            ...
            !...........................................................................................
            ! Calculate the crack front propagation vector, theoretical Pereira_2010_Generalized finite
            ! element methods for three-dimensional crack growth simulations, P122, Figure 3.15
            !...........................................................................................
            do i_Out_Node =1,Num_CrMesh_Outlines
                c_Mesh_Node =Crack3D_Meshed_Outline(i_C)%row(i_Out_Node,1)
                in_Ele_Num  =Cr3D_Meshed_Node_in_Ele_Num(i_C)%row(c_Mesh_Node)
            ...
            !............................................................................................
            ! Calculate the points after the crack front expansion, theoretical Pereira_2010_Generalized
            ! finite element methods for three-dimensional crack growth simulations, P122, Figure 3.15
            !   Modified on 2021-09-04.
            !   BUGFIX2021090401
            !............................................................................................
            Ele_Num_Cache = 1
        ...
        if(Key_Smooth_Front==6)then
            ...
            if(Num_CrMesh_Outlines>=12)then
                ...
                do i_Out_Node =1,Num_CrMesh_Outlines
                    call Cal_Ele_Num_by_Coors_3D(Points_to_Smooth(i_Out_Node,1), Points_to_Smooth(i_Out_Node,2), &
                    Points_to_Smooth(i_Out_Node,3), Ele_Num_Cache,c_OUT_Elem)
                ...
                if (Boundary_Cracks(i_C) .eqv. .True.) then
                    call Tool_Fit_3D_Points_Taubin_Non_Closed(i_C,Num_CrMesh_Outlines,Points_to_Smooth(1:Num_CrMesh_Outlines,1:3), &
                    Points_to_Smooth_Flag_Inside(1:Num_CrMesh_Outlines), Points_Smoothed(1:Num_CrMesh_Outlines,1:3))
                else
                    call Tool_Fit_3D_Points_Taubin(i_C,Num_CrMesh_Outlines, Points_to_Smooth(1:Num_CrMesh_Outlines,1:3), &
                    Points_to_Smooth_Flag_Inside(1:Num_CrMesh_Outlines), Points_Smoothed(1:Num_CrMesh_Outlines,1:3))
                endif
                ...
                call Tool_Fit_3D_Points_Check_Rollback(i_C,Num_CrMesh_Outlines, Pre_Fronts(1:Num_CrMesh_Outlines,1:3), &
                Points_to_Smooth(1:Num_CrMesh_Outlines,1:3), Points_Smoothed(1:Num_CrMesh_Outlines,1:3))
        ...
        Crack3D_Meshed_Outline(i_C)%row(1:Crack3D_Meshed_Outline_num(i_C),1:2), Cros_Product_Vector_Old)
        ...
        !-------------------------------------------------------------------------------------------
        call D3_Get_Crack_Mesh_Outline_Clockwise(i_C,Crack3D_Meshed_Outline_num(i_C), &
        Crack3D_Meshed_Outline(i_C)%row(1:Crack3D_Meshed_Outline_num(i_C),1:2), Cros_Product_Vector_New)
    ...
    do i_C =1,num_Crack_Do   
        ...
        do i_outline =1,c_Outlines_num
            ...
            do j_outline = (i_outline+2),(c_Outlines_num-1)
                ...
                call Tool_Intersection_of_AB_and_CD_3D(c_P1_Coor_i,c_P2_Coor_i,c_P1_Coor_j,c_P2_Coor_j,c_Yes_Inter,c_InterSection_P)
    ...
    !$OMP          Point1,Point2,Point3,Vector_V1,Vector_V2,Vector_Normal,in_Elem_num)  
    do i_C =1,num_Crack    
        do i_Crack_Ele =1,Crack3D_Meshed_Ele_num(i_C)
            ...
            Tool_Function_2Point_Dis_3D(Point2,Point3) + Tool_Function_2Point_Dis_3D(Point1,Point3)
            ...
            if(sum(abs(Vector_Normal))<=Tol_10)then
                if (Boundary_Cracks(i_C) .and. Key_Analysis_Type==6) then
                    ...
                    print *,'    Warning-2023081601 :: illegal Vector_Normal!'
    ...
    !$OMP               Crack_Node1,Crack_Node2,Crack_Node3,c_Max_N_Node_3D,temp_vector)   
    do i_C =1,num_Crack   
        c_Max_N_Node_3D = size(Crack3D_Meshed_Node(i_C)%row,1)
        ...
        do i_Crack_Node =1,Crack3D_Meshed_Node_num(i_C) 
            ...
            !call Vector_Normalize(3,Crack3D_Meshed_Node_Nor_Vector(i_C)%row(i_Crack_Node,1:3)) ! IMPROV2023081301, Regularization.


            !2026-05-17.
            temp_vector(1:3) = c_All_Nor_Vector/c_count_Ele
            call Vector_Normalize(3,temp_vector(1:3))
            Crack3D_Meshed_Node_Nor_Vector(i_C)%row(i_Crack_Node,1:3)  = temp_vector(1:3)
