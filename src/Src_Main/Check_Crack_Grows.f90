!-----------------------------------------------------------
! Brief: Check 2D crack growth using the maximum
!        circumferential tensile stress or weighted-average
!        maximum principal stress criterion.
!
! Parameters:
!   Input:  ifra - current fracturing step index
!           iter - current iteration index
!   Output: Yes_Grow - per-crack, per-tip growth flags
!
! Notes:   Determines propagation angle, new tip position,
!          crack-coalescence handling with natural
!          fractures, and validity checks for new tips.
!-----------------------------------------------------------
...
real(kind=FT) cr_Tip(2,2),omega_Tip(2),delta_angle(2), theta(2),tem1,K_tem,KI_Critical,new_tip(2,2), &
delta_L_Growth,tem_Crack_Coor(Max_Num_Cr_P,2)
integer EndByEnd_Outline(size(Outline,1)+1) 
logical Yes_In_Model
real(kind=FT) New_Seg_A(2),New_Seg_B(2),c_Seg_C(2),c_Seg_D(2), Intersec_x,Intersec_y,Signed_Dis,c_Seg_CD(2,2), &
Check_Distance,tried_New_Tip(2,2), Tri_New_Seg_A(2),Tri_New_Seg_B(2), Tri_Intersec_x,Tri_Intersec_y
...
real(kind=FT) Dynamic_Time,DY_Propagation_Time,DY_Propagation_Speed
real(kind=FT) theta_c,theta_c_tem1,theta_c_tem2,theta_c_tem3,theta_c_tem4
real(kind=FT) Keq_dyn
real(kind=FT) tem_vector_1(2),tem_vector_2(2)
...
if (Propagation_Length> Tol_10)then   
    ...
    if (delta_L_Growth < 0.5D0*Ave_Elem_L_Enrich)then
        print *, '    WARNING :: *Propagation_Length is too small!'
        print *, '                Please check *Propagation_Length!'  
        print *, '                WARNING in Check_Crack_Grows.f' 
        !call Warning_Message('S',Keywords_Blank)
    endif
    if (delta_L_Growth > 10.0D0*Ave_Elem_L_Enrich)then
        print *, '    WARNING :: *Propagation_Length is too large!'
        print *, '                Please check *Propagation_Length!'     
        print *, '                WARNING in Check_Crack_Grows.f'                 
        !call Warning_Message('S',Keywords_Blank)
    endif          
...
loop_i_C:do i_C =1,num_Crack    
    ...
    loop_i_Tip:do i_Tip = 1,2
        ...
        !~~~~~~~~~~~~~~~~~~~~~~~~~
        if(CFCP==1)then
            tem_value_k_I = KI(i_C,i_Tip)
            if (abs(KI(i_C,i_Tip)) < Tol_20)then
                tem_value_k_I = sign(Tol_20, KI(i_C,i_Tip))
            endif
            !delta_angle(i_Tip) = TWO*atan(-2*KII(i_C,i_Tip)/KI(i_C,i_Tip)/ (ONE+sqrt(ONE+EIG*(KII(i_C,i_Tip)/KI(i_C,i_Tip))**2)))
            delta_angle(i_Tip) = TWO*atan(-2*KII(i_C,i_Tip)/tem_value_k_I/ (ONE+sqrt(ONE+EIG*(KII(i_C,i_Tip)/tem_value_k_I)**2)))
        ...
        if(CFCP==1)then
            ...
            K_tem = cos(tem1)*(KI(i_C,i_Tip)*cos(tem1)**2 - 1.5D0*KII(i_C,i_Tip)*sin(delta_angle(i_Tip)))
        elseif(CFCP==2)then
            if (Material_Type(mat_num) .eq. 1  )then
        ...
        if(Key_Analysis_Type==2)then
            ...
            tem_value_k_I = k_I
            if (abs(k_I) < Tol_20)then
                tem_value_k_I = sign(Tol_20, k_I)
            endif
            ...
            K_tem1 = cos(tem3)*(k_I*cos(tem3)**2 -1.5D0*k_II*sin(delta_angle_second))

            ! The denominator cannot be 0. BUGFIX2024091451.
            if (K_tem1==ZR) K_tem1=Tol_20
            tem_delta_L = delt_time_NewMark*Velocity_Rayl*(ONE-(c_KIc/K_tem1)**2)*DY_Propagation_L_Factor
        ...
        !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ! Explicit dynamic analysis: how to determine the extension step size. 2026-06-03.
        !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        if(Key_Analysis_Type==6)then
            ...
            Velocity_long = sqrt((Lambda+TWO*mui)/c_density)
            Velocity_tran = sqrt(mui/c_density)
            Velocity_Rayl = Velocity_tran*(0.875D0-0.2D0*c_v-0.05D0*(c_v+0.25D0)**3)

            ! Crack Tip Stress Intensity Factor
            k_I = KI(i_C,i_Tip)
            k_II = KII(i_C,i_Tip)

            if (abs(k_II) <= Tol_20) then
                theta_c = ZR
            else
                theta_c_tem1 = k_I/k_II
                theta_c_tem2 = sqrt(theta_c_tem1**2 + 8.0D0)
                if (k_II >= ZR) then
                    theta_c_tem3 = theta_c_tem2
                else
                    theta_c_tem3 = -theta_c_tem2
                endif
                theta_c = TWO*atan(0.25D0*(theta_c_tem1 - theta_c_tem3))
            endif

            if(k_I > ZR) then
                Keq_dyn = cos(theta_c/TWO)**3*k_I - 1.5D0*cos(theta_c/TWO)*sin(theta_c)*k_II
            else
                Keq_dyn = - 1.5D0*cos(theta_c/TWO)*sin(theta_c)*k_II
            endif

            !if(Keq_dyn >= c_KIc .and. k_I > ZR)then
            if(Keq_dyn >= c_KIc)then
                Yes_Tip_Growth =.True.

                ! Adjust the angle to prevent it from being too large (<=Prop_Angle_Allowed). 2026-06-23.
                if(theta_c > Prop_Angle_Alowed*pi_180) then  
                    theta_c =  Prop_Angle_Alowed*pi_180
                elseif(theta_c < -Prop_Angle_Alowed*pi_180) then
                    theta_c = -Prop_Angle_Alowed*pi_180
                end if

                theta(i_Tip)   = omega_Tip(i_Tip) + theta_c  
                ! The denominator cannot be 0. BUGFIX2024091451.
                if (abs(Keq_dyn)<=Tol_20) Keq_dyn=Tol_20
                tem_delta_L = delt_time_Explicit*Velocity_Rayl*(ONE-c_KIc/Keq_dyn)*DY_Propagation_L_Factor
                delta_L_Growth = tem_delta_L
                
            
                if (Temp_Show_Message) then
                    write(*,1012) i_Tip,i_C,delta_L_Growth
                endif
                Crack_Prop_Velocity(i_C,i_Tip) = Velocity_Rayl*(ONE-c_KIc/Keq_dyn)
            else
                Crack_Prop_Velocity(i_C,i_Tip) = ZR
            endif
            
            !2026-06-24. NEWFTU-2026062401.
            if(DY_Time_to_Active_Crack_Propagation > ZR) then
                Dynamic_Time   = delt_time_Explicit*iter
                if(Dynamic_Time < DY_Time_to_Active_Crack_Propagation) then
                    Yes_Tip_Growth = .False. 
                endif
            endif
        endif
        ...
        !2026-05-20. NEWFTU-2026050703.
        if(Key_Analysis_Type==2 .or. Key_Analysis_Type==6) then
            if (sum(EDY_Crack_Propagaton_at_Specified_Time_and_Speed) >=ZR) then
                DY_Propagation_Time   = EDY_Crack_Propagaton_at_Specified_Time_and_Speed(1)
                DY_Propagation_Speed  = EDY_Crack_Propagaton_at_Specified_Time_and_Speed(2)

                if(Key_Analysis_Type==6) then
                    Dynamic_Time   = delt_time_Explicit*iter
                    if(Dynamic_Time >= DY_Propagation_Time)then
                        delta_L_Growth = DY_Propagation_Speed*Delt_Time_Explicit
                        Yes_Tip_Growth = .True. 
                        theta(i_Tip)   = ZR
                        Crack_Prop_Velocity(i_C,i_Tip) = DY_Propagation_Speed
                    else
                        Yes_Tip_Growth = .False. 
                    endif
                elseif(Key_Analysis_Type==2) then
                    Dynamic_Time   = delt_time_Implicit*iter
                    if(Dynamic_Time >= DY_Propagation_Time)then
                        delta_L_Growth = DY_Propagation_Speed*delt_time_Implicit
                        Yes_Tip_Growth = .True. 
                        theta(i_Tip)   = ZR
                        Crack_Prop_Velocity(i_C,i_Tip) = DY_Propagation_Speed
                    else
        ...
        if (Yes_Tip_Growth.eqv..True.) then
            ...
            !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            if((Key_Analysis_Type==3 .or.Key_Analysis_Type==4) .and. num_Na_Crack>=1 .and. Key_Na_Crack_Type==1 )then
                ...
                !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            elseif((Key_Analysis_Type==3 .or. Key_Analysis_Type==4) .and. num_Na_Crack>=1 .and. Key_Na_Crack_Type==2)then
                ! If the current crack tip of the crack has already completed parasitism, the crack extension
                ! direction is no longer restricted.
                if(Cracks_NF_JiS_Stat(i_C,i_Tip)/=-1)then
                    new_tip(i_Tip,1) = cr_Tip(i_Tip,1) + delta_L_Growth*cos(theta(i_Tip))
                ...
                ! Same):
                !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            elseif((Key_Analysis_Type==3 .or. Key_Analysis_Type==4) .and. num_Na_Crack>=1 .and. Key_Na_Crack_Type==3 )then
                ! If the current crack tip of the crack has already completed parasitism, the crack extension
                ! direction is no longer restricted.
                if(Cracks_NF_JiS_Stat(i_C,i_Tip)/=-1)then
                    new_tip(i_Tip,1) = cr_Tip(i_Tip,1) + delta_L_Growth*cos(theta(i_Tip))
        ...
        if (num_Poly_Incl/=0)then
            ...
            if (Crack_Tip_Type(i_C,i_Tip)==0)then
                ...
                do i_Incl = 1,num_Poly_Incl
                    ...
                    call Tool_Yes_In_Poly(new_tip(i_Tip,1),new_tip(i_Tip,2), Poly_Incl_Coor_x_Cl(i_Incl,1:n_Vertex+1), &
            ...
            elseif (Crack_Tip_Type(i_C,i_Tip)==-9)then
                ...
                call Cal_Weighted_Ave_Stress_of_Point(Key_Ave_Stress, Possoble_P_1,Search_R,&
                a_Weight,l_Ordina, WA_S_xx,WA_S_yy,WA_S_xy)
                ...
                call Tool_Normal_and_Shear_Stresses_2D(WA_S_xx,WA_S_yy,WA_S_xy,c_Alpha, WA_S_normal,WA_S_shear)
                ...
                do i_Incl = 1,num_Poly_Incl
                    ...
                    Poly_Incl_Coor_x_Cl(i_Incl,1:n_Vertex+1),Poly_Incl_Coor_y_Cl(i_Incl,1:n_Vertex+1), n_Vertex+1,Yes_in_Poly_Inc)
                    if (Yes_in_Poly_Inc) then
        ...
        if (Key_Fracture_Zone==0) then
            Call Tool_Yes_In_Poly(new_tip(i_Tip,1),new_tip(i_Tip,2), Coor(EndByEnd_Outline,1),Coor(EndByEnd_Outline,2), &
        ...
        do i_Node = 1,Num_Node
            ...
            tem_vector_1(1:2) = cr_Tip(i_Tip,1:2)
            tem_vector_2(1:2) = new_tip(i_Tip,1:2)
            call Tool_Yes_On_Line(c_Node_x,c_Node_y,tem_vector_1(1:2),tem_vector_2(1:2),Yes_Node_OnSeg)
            ...
            if (Crack_Tip_Type(i_C,i_Tip)==0)then
                ...
                do i_Incl =1,num_Circ_Incl
                    ...
                    if(c_Dis<=c_Incl_r)then
                        ...
                        call Tool_Arc_r_and_Radian_Given_Coors(c_Arc_Direction, c_Inter_P(1,1:2),Want_Point,[c_Incl_x,c_Incl_y], &
                        ...
                        if (c_Arc_Radian > Con_180)then
                            ...
                            [c_Incl_x,c_Incl_y],c_Yes_feasible,c_Arc_r,c_Arc_Radian_S, c_Arc_Radian_E,c_Arc_Radian)
                        endif
                ...
                if(CFCP==1)then
                    if(K_tem >= Material_Interface(2))then
                        if (abs(delta_angle(i_Tip))>Con_30)then
                            ...
                            ! crack).
                            call Tool_Yes_Point_in_Inclusions(new_tip(i_Tip,1), new_tip(i_Tip,2),c_Yes_in_Incl,c_Incl_Num)
        ...
        !Key_Propa_Type = 2  ! New crack points are only added when the length of the crack expansion reaches a certain value.
        !Key_Propa_Type = 1  ! New crack points are added in each crack propagation step.
        ! Directly adjust the original crack tip position to the new position; if not included, add a new
        ! crack coordinate point.
            ...
            elseif(Key_Propa_Type==2)then
                ...
                if (i_Tip==1) then
                    ...
                    elseif(Yes_Update_Arc_Cr.eqv. .True.) then
                        ...
                        elseif((Yes_In_OneEl .eqv..False.) .and. (c_L < 0.99D0*Ori_delta_L_Growth))then
                            !Modify the original tip location.
                            Crack_Coor(i_C,1,1:2) = new_tip(i_Tip,1:2)   
        ...
        if((Key_Analysis_Type==3 .or.Key_Analysis_Type==4) .and. num_Na_Crack>=1 .and. Key_Na_Crack_Type==1)then
            ...
            if(Yes_HF_met_NF .eqv. .True.)then    
                ...
                ! Tip1 point of the new fracture generated from the HF and NF intersection along the Tip1 direction
                call Tool_Point_Giv_2P_and_L( &
                tem_Intersec,NF_Tip1,HLF*L_New_Crack, New_Crack(num_New_Crack,1,1:2), Statu_Tip1,L_C_Tip1)
                ! Tip2 point of a new fracture generated from the HF and NF intersection along the Tip2 direction
                call Tool_Point_Giv_2P_and_L( &
                tem_Intersec,NF_Tip2,HLF*L_New_Crack, New_Crack(num_New_Crack,2,1:2), Statu_Tip2,L_C_Tip2)
        ...
        elseif((Key_Analysis_Type==3 .or.Key_Analysis_Type==4) .and. num_Na_Crack>=1 .and. Key_Na_Crack_Type==3) then
                    ...
                    do jPt = 2,nPt
                        ...
                        elseif (Yes_Cross .eqv..False.) then 
                            ...
                            if (abs(Signed_Dis) <= Check_Distance) then
                                ...
                                Tri_New_Seg_A(1:2)=tried_New_Tip(i_Tip,1:2)
                                Tri_New_Seg_B(1:2)=cr_Tip(i_Tip,1:2)  
                                call Tool_Intersection(Tri_New_Seg_A,Tri_New_Seg_B, c_Seg_C,&
                                c_Seg_D,Tri_Intersec_x,Tri_Intersec_y,Tri_Yes_Cross)
                                !if intersect, then: 
                                if(Tri_Yes_Cross.eqv..True.)then  
            ...
            if (Yes_HF_met_NF .eqv. .True.) then
                if (Cracks_QinS_Stat(Real_Cr_NF)/= 1) then    
                    ...
                    ! Tip1 point of the new fracture generated from the HF and NF intersection along the Tip1 direction
                    call Tool_Point_Giv_2P_and_L( &
                    tem_Intersec,NF_Tip1,HLF*L_New_Crack, New_Crack_Tip_1(1:2),Statu_Tip1,L_C_Tip1)
                    ! Tip2 point of the new fracture generated from the HF and NF intersection along the Tip2 direction
                    call Tool_Point_Giv_2P_and_L( &
                    tem_Intersec,NF_Tip2,HLF*L_New_Crack, New_Crack_Tip_2(1:2),Statu_Tip2,L_C_Tip2)
        ...
        ! Accumulate growth length for the current crack (sum of both tips). 2026-06-29.
        if (Yes_Grow(i_C,i_Tip) .eqv. .True.) then
            if (i_Tip == 1) then
                Cracks_Growth_Length_Last_Step(i_C) = Cracks_Growth_Length_Last_Step(i_C) + &
                    Tool_Function_2Point_Dis(Crack_Coor(i_C,1,1:2), cr_Tip(i_Tip,1:2))
            else
                Cracks_Growth_Length_Last_Step(i_C) = Cracks_Growth_Length_Last_Step(i_C) + &
                    Tool_Function_2Point_Dis(Crack_Coor(i_C,Each_Cr_Poi_Num(i_C),1:2), cr_Tip(i_Tip,1:2))
            endif
        endif
