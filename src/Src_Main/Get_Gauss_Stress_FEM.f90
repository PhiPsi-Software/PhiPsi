!-----------------------------------------------------------
! Brief: Compute 2D FEM stress at every Gauss point.
!
! Parameters:
!   Input:  isub - current load-step index
!
! Notes:   Stores the six-plane-stress components plus von Mises in
!          Stress_xx_Gauss / Stress_yy_Gauss / Stress_xy_Gauss /
!          Stress_vm_Gauss. Honours kill-element and Weibull-modulus flags.
!-----------------------------------------------------------

subroutine Get_Gauss_Stress_FEM(isub)
!     Computational Gauss Stress for 2D.
!     Store in global variables: Stress_xx_Gauss, Stress_yy_Gauss, Stress_xy_Gauss, Stress_vm_Gauss
...
do i_E = 1,Num_Elem
    ...
    do i_G = 1,Num_Gauss_P_FEM
        ...
        if(Key_InSitu_Strategy==2)then
            Stress_xx_Gauss(G_Counter)=Stress_xx_Gauss(G_Counter) + InSitu_Strs_Gaus_xx(i_E,i_G)
            Stress_yy_Gauss(G_Counter)=Stress_yy_Gauss(G_Counter) + InSitu_Strs_Gaus_yy(i_E,i_G)
            Stress_xy_Gauss(G_Counter)=Stress_xy_Gauss(G_Counter) + InSitu_Strs_Gaus_xy(i_E,i_G)
            ! Accumulation of stresses at each Gauss point of the current element (used to calculate the average
            ! Gauss stress of each element)
            c_Ele_Gauss(1) = c_Ele_Gauss(1) + InSitu_Strs_Gaus_xx(i_E,i_G)
            c_Ele_Gauss(2) = c_Ele_Gauss(2) + InSitu_Strs_Gaus_yy(i_E,i_G)
            c_Ele_Gauss(3) = c_Ele_Gauss(3) + InSitu_Strs_Gaus_xy(i_E,i_G)
        endif
    ...
    Elem_Ave_Gauss_Stress(i_E,1:3) = c_Ele_Gauss(1:3)/ Num_Gauss_P_FEM
