!-----------------------------------------------------------
! Brief: Define the working floating-point kind and a large set of
!        numeric constants (zero, one, two, pi, tolerances, time
!        units) used project-wide.
!
! Notes:   Switches between single and double precision via the
!          SP preprocessor symbol; double precision is the default.
!-----------------------------------------------------------
...
!  integer,PARAMETER::FT  = 2                           !single=1;double=2.
!  integer,PARAMETER::LIT = 4                           !Long Int Type (64bit).
!#endif 
...
!-----------------------------------------------------------
! Brief: Define ragged (jagged) real-array derived types for 1D,
!        2D, 3D, and 4D collections of arrays with varying row
!        lengths.
!
! Notes:   Used to store per-crack / per-element data where the
!          per-row size is not known a priori.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Define ragged (jagged) integer-array derived types for
!        1D, 2D, 3D, and 4D collections of variable-length rows.
!
! Notes:   Mirror of the real raggers for integer payloads.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold solver-wide control flags, key integers (crack
!        propagation, enrichment scheme, BC strategy, integration
!        order) and the floating-point tolerance / conversion
!        constants referenced by every other module.
!
! Notes:   The largest single module of project-wide state.
!-----------------------------------------------------------
...
! for a 3D rectangular initial crack, it refers to the side length; for a 2D crack, it refers to the length)
...
! element boundary length > Prp_Bisec_Factor_3D * Ave_Elem_L_Enrich, then bisect.
...
! the ill-conditioning of the system of equations and reducing computational effort; currently applicable to 3D.
...
! of contact iterations (the larger it is, the easier it is to converge; default: 1.0D-4*Ave_Elem_L_Enrich)
...
! calculation bugs, such as V5-P50)
...
! the first iteration and remains unchanged thereafter (default);
...
character(len=1)  Keywords_Blank
parameter (Keywords_Blank  = ' ')
character(len=4) Space_4
parameter  (Space_4  = '    ')
character(len=8) Space_8
parameter  (Space_8  = '        ')
...
! average of a total of 2n+1 values from the previous and next moments.
integer Smooth_Vertex_n
integer Key_Smooth_Vertex_Value2
!using the moving average method, averaging a total of 2n+1 values from the previous and following time points.
...
! averaging a total of 2n + 1 values from the previous and next time steps.
integer Smooth_Theta_n
integer Key_Smooth_Theta_Value2
! using the moving average method, averaging a total of 2n+1 values from the previous and next time points.
...
logical Temp_Show_Message
integer Key_Cal_Energy
real(kind=FT) Elastic_Strain_Energy,Fracture_Energy,External_Work,Residual_Energy,Normalized_Residual_Energy
real(kind=FT) HF_Pres_Initial_Guess
end module Global_Common

!-----------------------------------------------------------
! Brief: Hold model data: nodal coordinates, element connectivities,
!        boundary-condition lists, material tags, the global DOF
!        counters, and rupture-zone bounds.
!
! Notes:   The central geometric model module.
!-----------------------------------------------------------
...
!(the maximum node number difference of the element - 1)
...
! corresponding to all degrees of freedom. 2024-11-07. IMPROV2024110703.
...
!1: Block element number; 2: Integration point number; 3: Local coordinates and weights of the integration point.
...
!-----------------------------------------------------------
! Brief: Track which elements are FEM-only versus XFEM-enriched,
!        the per-step XFEM flags, the rollback list, and the 3D
!        Must_Gauss_Number_3D parameter.
!
! Notes:   Used by 3D XFEM assembly to decide which element
!          stiffness to compute and how to color elements.
!-----------------------------------------------------------
...
!There should be at least Must_Gauss_Number_3D Gauss points on each side of the crack surface; 
!otherwise, the Heaviside enriched nodes will be removed.
...
!-----------------------------------------------------------
! Brief: Hold the input filename, working directory, full path,
!        Python directory, and the PhiPsi installation directory.
!
! Notes:   Tiny module; populated from command-line -i / cwd.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold dynamic-analysis state: initial velocity/accel/disp
!        arrays, time-step sizes, mass scheme, Newmark beta, lumped
!        mass, and earthquake/sine excitation data.
!
! Notes:   Also stores crack propagation velocity and explicit-DY
!          energy / aperture diagnostics.
!-----------------------------------------------------------
...
real(kind=FT),ALLOCATABLE::Idp_x(:,:)
real(kind=FT),ALLOCATABLE::Idp_y(:,:)
real(kind=FT),ALLOCATABLE::Idp_z(:,:)
...
real(kind=FT) Delt_Time_Implicit
integer Key_Mass_Lumped
real(kind=FT) Explicit_time_inc
real(kind=FT),ALLOCATABLE::Dy_DISP(:),Dy_VELC(:),Dy_ACCL(:)
real(kind=FT) Crack_Prop_Velocity(100,2)
integer EDY_Data_Save_Interval_Timesteps
integer EDY_Info_Show_Interval_Timesteps
real(kind=FT) EDY_Total_Time,EDY_Force_Time
integer EDY_Key_Cal_Crack_Aperture
integer EDY_Key_Cal_Energy
real(kind=FT) EDY_Kinetic_Energy,EDY_Internal_Energy,EDY_Total_Energy
integer EDY_Internal_Force_Scheme
integer EQ_Ac_nodes_list_method     
integer Key_EDY_Save_Nodal_Velocity_Accel
integer Key_EDY_Lumped_Mass_Scheme       
integer Key_EDY_Save_Lumped_Mass
integer Key_EDY_Contact_Penalty_Parameter_Scheme
integer EDY_num_Crack_Gauss_Points_3D
real(kind=FT) EDY_Contact_Aperture_Tol
real(kind=FT) EDY_Save_SIFs_at_Specified_Point(3)
real(kind=FT) EDY_Save_SIFs_at_Specified_Plane(7)
real(kind=FT) EDY_Save_Aperture_at_Specified_Point(7)
real(kind=FT) EDY_Crack_Propagaton_at_Specified_Time_and_Speed(2)
real(kind=FT),ALLOCATABLE::LastDISP(:),LastVELC(:),LastACCL(:)
real(kind=FT),ALLOCATABLE::LastLastDISP(:)
integer DY_Save_SIFs_at_Specified_Tip(2)
integer DY_Save_Propagation_Speed_at_Specified_Tip(2)
integer Key_Map_Tip_to_H,Key_Map_Tip_to_H_Last_Last
integer Data_Map_Tip_to_H(1000,4),Data_Map_Tip_to_H_Last_Last(1000,4)
integer DY_Keep_Initial_Velocity
integer DY_Save_Crack_Length_2D
real(kind=FT) Initial_Crack_Length
real(kind=FT) DY_Propagation_L_Factor
real(kind=FT) DY_Time_to_Active_Crack_Propagation
integer DY_SIFs_IIM_Enable_f1_f2,DY_SIFs_IIM_Enable_Velocity_Coupling
end module Global_Dynamic

!-----------------------------------------------------------
! Brief: Hold 2D crack state: crack coordinates, tip positions,
!        enrichment type per node, junction elements, hole-related
!        crack enrichment, and crack-tip / vertex coordinate arrays.
!
! Notes:   The 2D crack counterpart of Global_Crack_3D.
!-----------------------------------------------------------
module Global_Crack
use Global_Float_Type
implicit none
...
!parameter (Max_Num_Cr = 150 ) ! For Simply Fortran, if Max_Num_Cr > 100 and fail, then use -fmax-stack-var-size=102400 to compile. 2026-01-15.
!parameter (Max_Num_Cr_P      = 2000 )       ! Each crack can have up to 2000 crack points
!parameter (Max_Num_Cr_CalP   = 300  )       ! Each crack can contain up to 300 calculation points (Note: it can't be larger, 
! otherwise there will be memory errors, but the reason is unknown)
...
!2026-06-22.
real(kind=FT),ALLOCATABLE::Crack_Coor(:,:,:)
real(kind=FT),ALLOCATABLE::Arc_Crack_Coor(:,:,:)
real(kind=FT),ALLOCATABLE::Na_Crack_Coor(:,:,:)
real(kind=FT),ALLOCATABLE::Edge_Disposed_Crack(:,:,:)

logical Boundary_Cracks(Max_Num_Cr)
logical Key_Cracks_InPlane_Growth(Max_Num_Cr)
!real(kind=FT) Crack_Coor(Max_Num_Cr,Max_Num_Cr_P,2)           ! Crack number, crack coordinate point, (x, y)
!real(kind=FT) Arc_Crack_Coor(Max_Num_Cr,Max_Num_Cr_P-1,11)    ! Define which segment of the crack is curved, and provide the corresponding center coordinates (
...
!real(kind=FT) Na_Crack_Coor(Max_Num_Cr,Max_Num_Cr_P,2)        ! Natural fracture number, crack coordinate point number, (x, y)
...
integer,ALLOCATABLE:: Ref_Elem_Maping_Tip(:)
...
real(kind=FT),ALLOCATABLE::  Coors_Element_Crack_OLD(:,:,:)
...
!real(kind=FT) Edge_Disposed_Crack(Max_Num_Cr,Max_Num_Cr_P,2)! Used for post-processing edge cracks
...
!----------
integer,ALLOCATABLE::Cracks_CalP_Type(:,:,:)
!integer Cracks_CalP_Type(Max_Num_Cr,Max_Num_Cr_CalP,2) ! The type of each crack calculation point and its associated crack number
...
integer,ALLOCATABLE::Cracks_MidJunCalpNum(:,:)
!integer Cracks_MidJunCalpNum(Max_Num_Cr,Max_Num_Cr_CalP) ! The calculation point number corresponding to the junction point of &
!each crack that is not a crack tip (the current crack's calculation point number)
...
! coupling.
integer,ALLOCATABLE::Cracks_GloNumCalP(:,:)
integer,ALLOCATABLE::Cracks_LocalNumCalP(:,:)
!integer Cracks_GloNumCalP(Max_Num_Cr,Max_Num_Cr_CalP)        ! Global numbering of each calculation point for each crack
!integer Cracks_LocalNumCalP(Max_Num_Cr*Max_Num_Cr_CalP,2)    ! Crack number corresponding to the global calculation point and local calculation point number
! 1: Crack Number
! 2: Local Calculation Point Number
! It includes both water-containing cracks and dry cracks that do not participate in fluid-solid
! coupling.
integer,ALLOCATABLE::Cracks_GloNumCalP_W(:,:)
integer,ALLOCATABLE::Cracks_LocalNumCalP_W(:,:)
!integer Cracks_GloNumCalP_W(Max_Num_Cr,Max_Num_Cr_CalP)      ! Global numbering of each calculation point of each water-containing crack
!integer Cracks_LocalNumCalP_W(Max_Num_Cr*Max_Num_Cr_CalP,2)  ! Crack number corresponding to the global calculation point and local calculation point number
! 1: Crack Number
! 2: Local Calculation Point Number
!----------
integer Num_JunPair
...
!**************************************
real(kind=FT),ALLOCATABLE::Cracks_HF_Ele_L(:,:)
!real(kind=FT) Cracks_HF_Ele_L(Max_Num_Cr,Max_Num_Cr_CalP-1)         ! The length of each hydraulic fracture unit
!***************************************************************
...
!*************************
integer,ALLOCATABLE::Cracks_Coh_Ele_Type(:,:)
!integer  Cracks_Coh_Ele_Type(Max_Num_Cr,Max_Num_Cr_CalP-1)    ! The type of each crack cohesive element: =1 indicates a cohesive element, =0 indicates a regular element.
...
real(kind=FT) IIM_Radius_Factor 
real(kind=FT) Initial_Cracks_Length(Max_Num_Cr)
real(kind=FT) Cracks_Growth_Length_Last_Step(Max_Num_Cr)

end module Global_Crack

!-----------------------------------------------------------
! Brief: Hold shared 2D crack counts, per-step state (number of
!        cracks, HF state, proppant, propagation flags, friction),
!        fluid-driven flags, and crack-calculation-point lists.
!
! Notes:   Differs from Global_Crack in that it stores per-step
!          state rather than crack coordinates.
!-----------------------------------------------------------
...
! (i.e., corresponding to hydraulic fracture-driven points)
integer num_Tol_CalP_Water_ConP
! calculation (i.e., corresponding to the common water-induced fractures)
...
!used when Key_NaCr_Active_Scheme_3D = 3. 2023-01-11.
...
!-----------------------------------------------------------
! Brief: Hold 3D crack state: per-crack mesh node coordinates,
!        natural-fracture properties, SIF arrays (ragged), signed
!        distances, fluid-element / fluid-node sizes, and crack-tip
!        baseline data structures.
!
! Notes:   The most complex crack module; uses ragged arrays to
!          accommodate variable per-crack size.
!-----------------------------------------------------------
...
!If this is insufficient, each fracture automatically expands. IMPROV2023081306.
...
!If insufficient, each fracture will automatically expand. IMPROV2023081305.
...
! fluid element number, and the fluid node numbers of the corresponding fluid elements, 2022-06-04. &
! type(Ragged_Int_Array_2D)::Cracks_FluidEle_CalP_3D(Max_Num_Cr_3D)       !Using ragged array, 2022-09-04.
...
type(Ragged_Array_3D),allocatable::Solid_El_Tip_BaseLine_OLD(:)
type(Ragged_Array_2D),allocatable::Solid_El_Tip_BaseLine_x_Vec_OLD(:)
type(Ragged_Array_2D),allocatable::Solid_El_Tip_BaseLine_y_Vec_OLD(:)
type(Ragged_Array_2D),allocatable::Solid_El_Tip_BaseLine_z_Vec_OLD(:)
type(Ragged_Array_3D),allocatable::Solid_El_Tip_BaseLine_T_Matrix_OLD(:)

!--------------------
integer Num_Check_T_Matrix
!(default is 180; smaller values compute faster)
...
!integer,ALLOCATABLE:: Enriched_Node_Type_3D_Old(:,:)   !2026-05-11.
!integer,ALLOCATABLE:: Enriched_Node_Type_3D_New(:,:)   !2026-05-11.
...
!Subsequent cracks will not be reinforced.
...
real(kind=FT),ALLOCATABLE:: Contact_Stress(:,:,:)
real(kind=FT) User_Specified_Crack_Mesh_Size

integer :: Prev_Enrich_Slot_Total = 0          
integer, allocatable :: c_POS_3D_Prev(:,:)   
integer, allocatable :: Enriched_Node_Type_3D_Prev(:,:) 

integer Key_Save_Old_Solid_El_Tip_BaseLine
end module Global_Crack_3D

!-----------------------------------------------------------
! Brief: Hold data for crack-intersection ('cross') enrichment:
!        element types, enriched node types, per-cross node
!        positions, the intersecting element number, and the
!        per-intersection ABCD corner coordinates.
!
! Notes:   Used by Location_Element_Stiff_Matrix_Cross.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold inclusion definitions (circular, elliptical, polygonal,
!        random / irregular) and the per-inclusion enrichment type,
!        element type, and DOF position arrays.
!
! Notes:   Also tracks inclusion-randomization switches and size
!          / inclination factors.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold aggregated element geometric statistics: min/max/avg
!        area, length, and volume, plus enriched-element versions
!        of these quantities.
!
! Notes:   Used to size integration tolerances and the local
!          refinement logic.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold per-material constitutive data: D / S matrices,
!        thermal expansion, fracture toughness, Young's modulus,
!        Poisson ratio, density, thickness, Lame constants,
!        Mohr-Coulomb parameters, and Weibull statistics.
!
! Notes:   Also carries St_KIc_Conversion for the strength-to-
...
!-----------------------------------------------------------
! Brief: Hold the global displacement vector and the 2D/3D
!        Gauss-point nodal displacement arrays used by the
!        post-processing routines.
!
! Notes:   DISP_Cylinder stores displacements in the cylindrical
!          frame for the theta-rotation post-processing.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold hydraulic-fracturing controls: viscosity model, leak-
!        off, proppant, time-increment rule, stage-fracturing data,
!        injection rate / pressure / concentration time histories,
!        and the Newton-Raphson / Picard / AAP / MNR iteration
!        parameters and tolerances.
!
! Notes:   The most heavily parameterized HF module.
!-----------------------------------------------------------
...
! the initial fractures may not necessarily be at the endpoints)
...
!-----------------------------------------------------------
! Brief: Hold the crack-surface contact state per element and per
!        crack for the current and previous contact iteration,
!        and the proppant coordinates on contact elements.
!
! Notes:   Used by the 2D and 3D contact-iteration drivers.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold all node- and Gauss-point stress arrays: 2D/3D total
!        and thermal stress, in-situ stress, cylindrical-coords
!        stress, and the non-uniform initial stress data.
!
! Notes:   The single largest stress-storage module.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold all node- and Gauss-point strain arrays: 2D/3D total
!        and thermal strain, cylindrical-coords strain, and the
!        in-situ initial strain field.
!
! Notes:   Counterpart of Global_Stress for strain data.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold the post-processing switches: save Gauss coords,
!        displacements, stresses, element Gauss count, fracture
!        permeability, and the 3D IIM integration cylinder flag.
!
! Notes:   Controls what gets written to the *_i.vtk sidecars.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold rigid-ball model parameters: bounding box,
!        average / min / max radius, and total ball count.
!
! Notes:   Used by the 2D rigid-balls contact example.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold the field-problem (heat / pore-pressure) data: fixed
!        and nonzero boundary values, initial values, source terms,
!        Biot consolidation matrices, gas-production data, and
!        XFEM enrichment state for the field DOFs.
!
! Notes:   The most heavily parameterized field-problem module.
!-----------------------------------------------------------
...
! and the wpnp file must be obtained.
...
!-----------------------------------------------------------
! Brief: Hold the molecular-dynamics run parameters: molecule count,
!        mass, total time steps, time step, bounding-box size, and
!
! Notes:   Read by the PhiPsi_Molecular_Dynamics driver.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold plasticity state per Gauss point: elastic and plastic
!        D matrices, state variables, and the last incremental
!        nodal displacement for return mapping.
!
! Notes:   Used by the non-linear 2D driver.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold cohesive-zone model parameters: constitutive type,
!        critical openings, peak traction, tangential traction data,
!        and the per-element cohesive-crack state.
!
! Notes:   Used by the 2D cohesive driver.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold peridynamics state: point count, neighbor data,
!        bond damage, time step, and the displacement / velocity /
!        acceleration / damage arrays for every material point.
!
! Notes:   Used by PhiPsi_Peridynamic.f.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold the non-linear solver settings: load-time table,
!        N-R maximum iterations, convergence tolerances, allowed
!        load increment count, and the global displacement-
!        increment scratch vector.
!
! Notes:   Used by the 2D non-linear static driver.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold surface-load data: per-load filenames, element-and-
!        node lists, element areas, and the outward normal vectors,
!        plus the load pressure magnitudes.
!
! Notes:   Surface loads are generated by an external ANSYS macro.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold the 3D hydraulic-fracturing experimental parameters:
!        number of surface loads, initial injection rate, hole
!        material number, initial try pressure, and the pressure
!        load increment size.
!
! Notes:   Read by the 3D HF experiment driver.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold the parsed 3D natural-fracture coordinate scratch
!        arrays: polygon coordinates, circular-fracture coordinates,
!        and per-fracture point counts.
!
! Notes:   A staging area used while reading a .kpp file.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold a single integer (ITPACK_NNZ) used to interface with
!        the ITPACK iterative solver routines.
!
! Notes:   Lightweight compatibility shim.
!-----------------------------------------------------------
...
!-----------------------------------------------------------
! Brief: Hold the element-coloring state used by the explicit
!        dynamic (EDY) parallel assembly: number of colors,
!        per-element color, CSR row pointer for color groups,
!        and the color-sorted element index list.
!
! Notes:   Built by a greedy graph-coloring algorithm.
!-----------------------------------------------------------

MODULE Global_EDY_Color
use Global_Float_Type
implicit none
save

!> Total number of colors produced by the greedy algorithm
integer :: EDY_Num_Colors = 0

!> Color index of each element: EDY_Elem_Color(i_E) in [1, EDY_Num_Colors]
integer, allocatable :: EDY_Elem_Color(:)

!> CSR row-pointer for color groups (size EDY_Num_Colors+1)
integer, allocatable :: EDY_Color_Start(:)

!> Element indices sorted by color group (size Num_Elem)
integer, allocatable :: EDY_Color_Elem_List(:)

END MODULE Global_EDY_Color
