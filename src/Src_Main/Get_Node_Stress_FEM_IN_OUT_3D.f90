!-----------------------------------------------------------
! Brief: Compute 3D FEM stress at every node and return via arguments.
!
! Parameters:
...
!   Output: Stress_xx_N .. Stress_vm_N - per-node stress arrays
!
! Notes:   Uses per-thread accumulators under OpenMP to avoid data
!          conflicts. Honours Key_Simple_Post fast-exit and Weibull modulus.
!-----------------------------------------------------------

SUBROUTINE Get_Node_Stress_FEM_IN_OUT_3D(Yes_Add_Insitu, isub,c_DISP, Stress_xx_N,Stress_yy_N,Stress_zz_N, &
...
real(kind=FT), intent(out)::Stress_xx_N(num_Node), Stress_yy_N(num_Node), Stress_zz_N(num_Node), Stress_xy_N(num_Node), &
Stress_yz_N(num_Node), Stress_xz_N(num_Node), Stress_vm_N(num_Node)
real(kind=FT) c_T_Alpha,c_TStress(6)     
integer i_E
real(kind=FT) c_D(6,6),U(24)
real(kind=FT) c_v   
real(kind=FT) c_X_NODES(8),c_Y_NODES(8),c_Z_NODES(8), T_Kesi(8),T_Yita(8),T_Zeta(8), c_kesi,c_yita,c_zeta,c_Stress(6)
...
real(kind=FT),ALLOCATABLE::Stress_xx_N_Threads(:,:), Stress_yy_N_Threads(:,:), Stress_zz_N_Threads(:,:), &
Stress_xy_N_Threads(:,:), Stress_yz_N_Threads(:,:), Stress_xz_N_Threads(:,:), Stress_vm_N_Threads(:,:)
...
do i_E = 1,Num_Elem
    ...
    do i_N = 1,8
        ...
        if(Key_InSitu_Strategy==4)then
            ...
            ! thermal stress.
            c_Stress = MATMUL(c_D,[ InSitu_Strain_Gaus_xx(i_E,1), InSitu_Strain_Gaus_yy(i_E,1), &
            InSitu_Strain_Gaus_zz(i_E,1), InSitu_Strain_Gaus_xy(i_E,1), InSitu_Strain_Gaus_yz(i_E,1), InSitu_Strain_Gaus_xz(i_E,1)])
            Stress_xx_N_Threads(C_Node,i_Thread) = Stress_xx_N_Threads(C_Node,i_Thread)- c_Stress(1)
            Stress_yy_N_Threads(C_Node,i_Thread) = Stress_yy_N_Threads(C_Node,i_Thread)- c_Stress(2)
            Stress_zz_N_Threads(C_Node,i_Thread) = Stress_zz_N_Threads(C_Node,i_Thread)- c_Stress(3)     
            Stress_xy_N_Threads(C_Node,i_Thread) = Stress_xy_N_Threads(C_Node,i_Thread)- c_Stress(4)
...
do i_N=1,num_Node 
    ...
    Stress_xy_N(i_N),Stress_yz_N(i_N),Stress_xz_N(i_N), Stress_vm_N(i_N))
end do
!$omp end parallel do 
