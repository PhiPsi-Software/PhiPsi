!-----------------------------------------------------------
! Brief: Compute 2D XFEM stress at every node.
!
! Parameters:
!   Input:  isub   - current load-step index
!           c_DISP - global displacement vector
!
! Notes:   Stores the xx, yy, xy, and von Mises components into the
!          global Stress_xx_Node, Stress_yy_Node, Stress_xy_Node, and
!          Stress_vm_Node arrays. Handles enriched, partially enriched,
!          and plain-FEM elements.
!-----------------------------------------------------------

subroutine Get_Node_Stress_XFEM(isub,c_DISP)
!     Computational Node Stress
!     Store into global variables: Stress_xx_Node, Stress_yy_Node, Stress_xy_Node, Stress_vm_Node
...
real(kind=FT) :: c_v
real(kind=FT) c_X_NODES(4),c_Y_NODES(4),T_Kesi(4),T_Yita(4), c_kesi,c_yita,c_Stress(3),B(3,80), tem_B(3,80)
...
do i_E = 1,Num_Elem
    ...
    if(num_Hole/=0)then
        do i_H =1,num_Hole 
            call Location_Element_Stiff_Matrix_Hl(i_E,i_H, c_POS_Hl(:,i_H), Location_ESM_C_Crack, num_Loc_ESM_C_Crack, &
            Location_ESM_C_Cr_NoFEM, num_Loc_ESM_C_Cr_NoFEM)
    ...
    if(num_Cross/=0)then
        do i_Cross =1,num_Cross 
            call Location_Element_Stiff_Matrix_Cross(i_E,i_Cross,c_POS_Cross(:,i_Cross), Location_ESM_C_Crack, num_Loc_ESM_C_Crack,&
            Location_ESM_C_Cr_NoFEM, num_Loc_ESM_C_Cr_NoFEM)
            ! Includes FEM degrees of freedom
            Location_ESM(num_Loc_ESM+1: num_Loc_ESM+num_Loc_ESM_C_Crack) = Location_ESM_C_Crack(1:num_Loc_ESM_C_Crack)
            num_Loc_ESM  =  num_Loc_ESM + num_Loc_ESM_C_Crack
    ...
    if(num_Inclusion/=0)then
        do i_Incl =1,num_Inclusion
            call Location_Element_Stiff_Matrix_Incl(i_E,i_Incl, c_POS_Incl(:,i_Incl), Location_ESM_C_Crack, num_Loc_ESM_C_Crack, &
            Location_ESM_C_Cr_NoFEM, num_Loc_ESM_C_Cr_NoFEM)
            ! Includes FEM degrees of freedom
            Location_ESM(num_Loc_ESM+1: num_Loc_ESM+num_Loc_ESM_C_Crack) = Location_ESM_C_Crack(1:num_Loc_ESM_C_Crack)
            num_Loc_ESM  =  num_Loc_ESM + num_Loc_ESM_C_Crack
    ...
    do i_N = 1,4
        ...
        !Calculate the B Matrix, Loop through each crack.
        do i_C =1,num_Crack 
            call Cal_B_Matrix_Crack(c_kesi,c_yita, i_C,i_E,i_N, c_NN,c_X_NODES,c_Y_NODES, tem_B,num_tem_B)
        ...
        if(num_Hole/=0)then
            do i_H =1,num_Hole 
                call Cal_B_Matrix_Hl(c_kesi,c_yita, i_H,i_E,i_N, c_NN,c_X_NODES,c_Y_NODES, tem_B,num_tem_B)
                B(1:3,num_B+1:num_B+num_tem_B) = tem_B(1:3,1:num_tem_B)
                num_B = num_B + num_tem_B
            end do
        endif
        ...
        if(num_Cross/=0)then
            do i_Cross =1,num_Cross 
                call Cal_B_Matrix_Cross(c_kesi,c_yita, i_Cross,i_E,i_N, c_NN,c_X_NODES,c_Y_NODES, tem_B,num_tem_B)
                B(1:3,num_B+1:num_B+num_tem_B) = tem_B(1:3,1:num_tem_B)
                num_B = num_B + num_tem_B
            end do
        endif
        ...
        if(num_Circ_Incl/=0)then
            do i_Incl =1,num_Circ_Incl 
                call Cal_B_Matrix_Circ_Incl(c_kesi,c_yita, i_Incl,i_E,i_N, c_NN,c_X_NODES,c_Y_NODES, tem_B,num_tem_B)
                B(1:3,num_B+1:num_B+num_tem_B) = tem_B(1:3,1:num_tem_B)
                num_B = num_B + num_tem_B
            end do
            ! Determine whether the current node is within a certain mixture
            call Tool_Yes_Point_in_Inclusions( c_X_NODES(i_N),c_Y_NODES(i_N), Yes_Gauss_in_Incl,c_Incl_Num)
        ...
        if(num_Poly_Incl/=0)then
            do i_Incl =1,num_Poly_Incl 
                call Cal_B_Matrix_Poly_Incl(c_kesi,c_yita, i_Incl,i_E,i_N, c_NN,c_X_NODES,c_Y_NODES, tem_B,num_tem_B)
                B(1:3,num_B+1:num_B+num_tem_B) = tem_B(1:3,1:num_tem_B)
                num_B = num_B + num_tem_B
            end do
            ! Determine whether the current node is within a certain mixture
        ...
        c_Stress=MATMUL(MATMUL(c_D,B(1:3,1:num_Loc_ESM)), U(1:num_Loc_ESM))
        ! Thermal stress
        c_TStress(1:3) =ZR
        if(Key_Thermal_Stress==1)then
            if(Key_Type_2D==1)then
                c_TStress = c_T_Alpha* Thermal_Str_Temper(Elem_Mat(i_E))* MATMUL(c_D,[ONE,ONE,ZR])
            elseif(Key_Type_2D==2)then
                c_TStress = (ONE+c_v)*c_T_Alpha* Thermal_Str_Temper(Elem_Mat(i_E))* MATMUL(c_D,[ONE,ONE,ZR])
            endif

            TStress_xx_Node(C_Node) = TStress_xx_Node(C_Node) + c_TStress(1)
            TStress_yy_Node(C_Node) = TStress_yy_Node(C_Node) + c_TStress(2)
            TStress_xy_Node(C_Node) = TStress_xy_Node(C_Node) + c_TStress(3)
        endif
        c_Count(C_Node) = c_Count(C_Node) + 1

        ! Subtract thermal stress
        Stress_xx_Node(C_Node) = Stress_xx_Node(C_Node) + c_Stress(1)-c_TStress(1)
        Stress_yy_Node(C_Node) = Stress_yy_Node(C_Node) + c_Stress(2)-c_TStress(2)
        Stress_xy_Node(C_Node) = Stress_xy_Node(C_Node) + c_Stress(3)-c_TStress(3)
...
do i_N=1,num_Node   
    ...
    call Tool_von_Mises(Stress_xx_Node(i_N), Stress_yy_Node(i_N), Stress_xy_Node(i_N), c_v_all(i_N),Stress_vm_Node(i_N))
    if(Key_Thermal_Stress==1)then
        ...
        TStress_vm_Node(i_N) = sqrt(TStress_xx_Node(i_N)**2+ TStress_yy_Node(i_N)**2- TStress_xx_Node(i_N)* &
        TStress_yy_Node(i_N)   + THR*TStress_xy_Node(i_N)**2)
        call Tool_von_Mises(TStress_xx_Node(i_N), TStress_yy_Node(i_N), TStress_xy_Node(i_N), c_v_all(i_N),TStress_vm_Node(i_N))
    endif
end do
