!-----------------------------------------------------------
! Brief: Compute 2D XFEM stress at every Gauss point.
!
! Parameters:
!   Input:  isub   - current load-step index
!           c_DISP - global displacement vector
!
! Notes:   Stores xx, yy, xy, and von Mises into Stress_xx_Node,
!          Stress_yy_Node, Stress_xy_Node, Stress_vm_Node. Handles
!          enriched, partially enriched, and standard-FEM elements.
!-----------------------------------------------------------
...
real(kind=FT) c_X_NODES(4),c_Y_NODES(4), c_kesi,c_yita,c_Stress(3),B(3,80), tem_B(3,80)
...
do i_E = 1,Num_Elem
    ...
    do i_G = 1,c_Num_Gauss_Point
        ...
        if(num_Crack/=0)then
            do i_C =1,num_Crack 
                call Cal_B_Matrix_Crack(c_kesi,c_yita,i_C,i_E,i_G, c_NN,c_X_NODES,c_Y_NODES, tem_B,num_tem_B)
        ...
        if(Key_Thermal_Stress==1)then
            ...
            TStress_vm_Gauss(G_Counter) = sqrt(c_TStress(1)**2+ c_TStress(2)**2- c_TStress(1)*c_TStress(2)+ THR*c_TStress(3)**2)
        endif
        ! Stress
        ...
        if(Key_InSitu_Strategy==2)then
            Stress_xx_Gauss(G_Counter)=Stress_xx_Gauss(G_Counter) + sum(InSitu_Strs_Gaus_xx(i_E,1:Num_Gauss_P_FEM))/ Num_Gauss_P_FEM
            Stress_yy_Gauss(G_Counter)=Stress_yy_Gauss(G_Counter) + sum(InSitu_Strs_Gaus_yy(i_E,1:Num_Gauss_P_FEM))/ Num_Gauss_P_FEM
            Stress_xy_Gauss(G_Counter)=Stress_xy_Gauss(G_Counter) + sum(InSitu_Strs_Gaus_xy(i_E,1:Num_Gauss_P_FEM))/ Num_Gauss_P_FEM
            ! Accumulation of stresses at each Gauss point of the current element (used to calculate the average
            ! Gauss stress of each element)
            c_Ele_Gauss(1) = c_Ele_Gauss(1)+ sum(InSitu_Strs_Gaus_xx(i_E,1:Num_Gauss_P_FEM))/ Num_Gauss_P_FEM
            c_Ele_Gauss(2) = c_Ele_Gauss(2) + sum(InSitu_Strs_Gaus_yy(i_E,1:Num_Gauss_P_FEM))/ Num_Gauss_P_FEM
            c_Ele_Gauss(3) = c_Ele_Gauss(3) + sum(InSitu_Strs_Gaus_xy(i_E,1:Num_Gauss_P_FEM))/ Num_Gauss_P_FEM
        endif
        ...
        call Tool_von_Mises(Stress_xx_Gauss(G_Counter), Stress_yy_Gauss(G_Counter), Stress_xy_Gauss(G_Counter), &
        c_v,Stress_vm_Gauss(G_Counter))
        !if (Stress_vm_Gauss(G_Counter)>1000) then
        !endif
    end do
    ! Average Gauss stress of each element
    Elem_Ave_Gauss_Stress(i_E,1:3) = c_Ele_Gauss(1:3)/ c_Num_Gauss_Point
    ! Calculate the average principal stress for each element
    call Tool_Principal_Stresses_2D(Elem_Ave_Gauss_Stress(i_E,1), Elem_Ave_Gauss_Stress(i_E,2), &
    Elem_Ave_Gauss_Stress(i_E,3), c_S_1,c_S_3,c_theta_stress)
