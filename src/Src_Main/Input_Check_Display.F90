!-----------------------------------------------------------
! Brief: Validate, cross-check, and echo the contents of the input
!        keyword, model, crack, hole, inclusion, HF and dynamic
!        decks, reporting any inconsistencies before the analysis.
!
! Parameters:
!   Input:  (none - reads all global model/crack data)
!   Output: (none - prints diagnostics to stdout and the log)
!
! Notes:   The single largest routine in the project; it performs
!          dozens of independent checks (duplicates, cross-cracks,
!          injection point inside cracks, etc.).
!-----------------------------------------------------------
...
if (Key_Rand_Poly_Incl ==1 .and.Key_Rand_Poly_Incl_Irregular  == 1)then
    print *, '    Warning: *Key_Rand_Poly_Incl and *Key_Rand_Poly_Incl_Irregular cannot be' //' set to 1 at the same time!'
...
if(Key_Dimension==2) then
    ...
    elseif(Key_TipEnrich==4) then
        ...
        !NEWFTU-2026052601.
    elseif(Key_TipEnrich==5) then
        print *, '    Tip enrichment method:     Asymptotic H function'
        call log_msg('Tip enrichment method:     Asymptotic H function')
        !2026-05-28.
        if(Num_Gauss_Points<900) then
            Num_Gauss_Points=900
            print *, '    WARNING :: Num_Gauss_Points has been set to 900 for Asymptotic H function!'
        endif
    else
        call Warning_Message('E','Key_TipEnrich')
    end if
...
if(Key_Smooth_Front >=1) then
    ...
    if (Key_Analysis_Type /= 1 .and. Key_Analysis_Type /= 4 .and. Key_Analysis_Type /= 6 .and. Key_Analysis_Type /= 61)then
        print *, '    Error :: Crack front smooth for analysis type 1, 4, 6, 61 only!'
        call Warning_Message('S',Keywords_Blank)
    endif
endif
...
elseif(Key_Post_CS_N_Disp_Cylindr ==1) then
    ...
    if (abs(Post_cylinder_Coor_Vector_x(1)) > Con_Big_15 .or. abs(Post_cylinder_Coor_Vector_y(1)) > Con_Big_15 .or. &
...
elseif (Key_Integral_Sol == 3)then
    ...
    if (Key_Dimension == 2)then
        ...
        (Num_Sub_Quads == 14*14).or.(Num_Sub_Quads == 15*15).or.(Num_Sub_Quads == 16*16).or. (Num_Sub_Quads == 17*17)) then
...
if (Key_Analysis_Type ==6)then
    if(EDy_Num_Iteras ==0 .and. EDY_Total_Time <ZR)then
        print *,'    Error :: *EDy_Num_Iteras or *EDY_Total_Time not defined! Please define at least one!'
        call Warning_Message('S',Keywords_Blank)
    endif
    ...
    !2026-04-21.
    if(Key_EDY_Lumped_Mass_Scheme ==1)then
        print *,'    Mass lumping scheme :: Conventional Row-Sum scheme'
    elseif (Key_EDY_Lumped_Mass_Scheme ==2)then
        print *,'    Mass lumping scheme :: Conventional Row-Sum scheme ((FEM-XFEM DOFs decoupling)'
    elseif (Key_EDY_Lumped_Mass_Scheme ==3)then
        print *,'    Mass lumping scheme :: New scheme'
    else
        call Warning_Message('E','Key_EDY_Lumped_Mass_Scheme')
    endif
endif
...
if (num_Crack >0) then
    ...
    elseif(Key_Dimension==3)then
        ...
        do i_C = 1,num_Crack
            if(sum(abs(Crack3D_Coor(i_C,:,1:3)))<=Tol_20 .and. sum(abs(Crack3D_Cir_Coor(i_C,1:7)))<=Tol_20 .and. &
        ...
        do i_C=1,num_Crack
            ...
            if(sum(abs(Crack3D_Coor(i_C,:,1:3)))>=Tol_20)then
                ...
                do i_P = 1,Each_Cr_Poi_Num(i_C)
                    ...
                    ! model.
                    if(c_P_x < Max_X_Coor .and. c_P_x > Min_X_Coor .and. c_P_y < Max_Y_Coor .and. c_P_y > Min_Y_Coor .and. &
...
do i=1,num_of_Material
    ...
    Material_Type(i) .eq. 4 .or. Material_Type(i) .eq. 6  ) then
        WRITE (tem_char,"(i2)") i
        write(*,1315) i
        write(*,1316)
        ...
        if(Key_Analysis_Type /=7 .and. Key_Analysis_Type /=8)then
            if (Key_Damage ==1) then
                ...
                if(Key_Analysis_Type ==1 .and.Key_Type_2D       /=2) then
                    print *,'    Error ::  Damage material is avaliable only for plane strain model ' //'when *Key_Analysis_Type=1!'
                    call Warning_Message('S',Keywords_Blank)
                endif
            else
...
!2026-03-24.
if (Key_Analysis_Type==6) then
    if (edy_num_force_itr > edy_num_iteras) then
        print *,'    Error :: edy_num_force_itr must <= edy_num_iteras!'
        print *,'             edy_num_iteras:   ',edy_num_iteras
        print *,'             edy_num_force_itr:',edy_num_force_itr
        call Warning_Message('S',Keywords_Blank)
    endif
    if (EDY_Force_Time > EDY_Total_Time) then
        print *,'    Error :: EDY_Force_Time must <= EDY_Total_Time!'
        print *,'             EDY_Force_Time:',EDY_Force_Time
        print *,'             EDY_Total_Time:',EDY_Total_Time
        call Warning_Message('S',Keywords_Blank)
    endif

    !2026-04-06.
    if (EDY_Internal_Force_Scheme==1) then
        if (Key_Contact /=0) then
            print *,'    ERROR :: For explit dynamic analysis, EDY_Internal_Force_Scheme=1 is invalid for contact!'
            print *,'             Set *EDY_Internal_Force_Scheme>=2 or Key_Contact=0 and try again.'
            call Warning_Message('S',Keywords_Blank)
        endif
    endif    
endif
